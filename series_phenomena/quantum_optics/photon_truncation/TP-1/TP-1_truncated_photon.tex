...
{\Large \textbf{Version 0.2 --- 20 June 2026}}}
...
``can be arbitrarily high.'' CPP's specific content is to identify that formal cutoff with
the physical lattice scale $\wP=1/\tP$. Two regimes follow. For a realistic shutter (removal
time $T\gg1/\wg$) the mechanical switching already gives $\Nexp\ll1$ and the lattice cutoff
is dormant ($\tP$ is $\sim\!28$ orders below the optical period); CPP and the continuum
agree. The lattice acts only on the idealized instantaneous shutter, where the continuum has
a genuine logarithmic divergence: CPP forbids $T<\tP$, caps $\wcut\le\wP$, and yields
$\Nexp_{\max}=C\,\ln(\wP/\wg)\approx 63\,C$ for an optical photon --- order tens, finite,
with $C$ an $O(1)$ coefficient set by the photon amplitude at the cut. The honest status is
...
ultraviolet (and, as \S\ref{sec:class} shows, logarithmic), and identifying the lattice
cutoff with RGS's own formal one removes the continuum's instantaneous-shutter divergence.
...
remains open. It draws load-bearing structure from QM-5
...
\paragraph{The class is logarithmic.} In the RGS construction the forward/backward spectra
that enter the created-photon number are
\begin{equation}
\xi_\pm(\omega)=E(\omega)\int_{-\infty}^{\infty}dx\,\theta(\pm x)\,\xi(x)\,e^{-i\omega x},
\qquad E(\omega)=K\sqrt{\omega},
\end{equation}
and the expected number created from the initial vacuum is the mode sum
$\Nexp=\sum_\xi\big(\|\xi^-_-\|^2+\|\xi^-_+\|^2\big)$. For any photon wavefunction with
$\xi(x)\neq0$ at the cut point $x=0$, the Heaviside truncation $\theta(\pm x)\xi(x)$ carries
a step discontinuity, so its Fourier transform decays as $1/\omega$ at large $\omega$.
With the weighting $E(\omega)\propto\sqrt\omega$ this gives
...
\label{eq:logclass}
\end{equation}
a \emph{logarithmic} divergence. This is exactly the conclusion RGS state for the
instantaneous shutter (``the Heaviside function leads to a Fourier transform with asymptotic
behavior $\propto1/\omega$ \ldots together with $E(\omega)\propto\sqrt\omega$, the norms
diverge logarithmically''). The class is therefore derived, not posited; the coefficient
$C=O(1)$ is set by the photon amplitude at the cut $|\xi(0)|^2$ and the mode-sum measure.
...
governs. The framework-specific content of this paper lives entirely in Regime~B: the
continuum's instantaneous-shutter infinity is a cutoff artifact, and CPP supplies the finite
value the idealization should have carried.
...
one Absolute Moment $\tP$. Then $\Nexp$ is finite for every physical shutter, and is bounded
over the set of \emph{all} physical shutters by $\Nexp_{\max}=C\,\ln(\wP/\wg)$.
\end{proposition}
...
$\Nexp$ as shutter $\to$ instantaneous & $\to\infty$ logarithmically (formal cutoff, ``arbitrarily high'') & finite; formal cutoff identified with $\wP=1/\tP$ (Prop.~\ref{prop:reg}) \\
\bottomrule
\end{tabular}
\end{table}
...
identifying that cutoff with the physical lattice scale $\wP=1/\tP$. The truncated-photon
...
shutter (Regime~A) the lattice is dormant and CPP and the continuum agree. The result is a
foundational consistency statement --- the substrate supplies the finite value the
continuum's instantaneous-shutter idealization lacks --- not an experimental prediction.
