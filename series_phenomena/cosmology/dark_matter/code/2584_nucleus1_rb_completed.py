...
DTS=(1/100,1/50,1/25)
print("="*78); print("PATCH 2584 -- N1 R-B: THE COMPLETED-INTERACTION LADDER"); print("="*78)
print(f"E_qq = {EQQ:.2f} MeV at d = {D} fm [1812/2433]; Morse width union beta*d in {{2,4}}")
trunc_mode='DIST'
# CTRL-N1
P0=np.array([[0.,0.,0.]]); C0=np.array([+1.]); S0=['q']
r=n1_rb(P0,C0,S0,1/50,'atH',4.0)
print(f"CTRL-N1 singleton: drift={np.linalg.norm(r['Rw']):.1e}  PASS" )
# CTRL-N2 pinned regression, completed interaction
print("CTRL-N2 pinned (coupling=0), completed interaction:")
for rg in (1,2,3):
    H0,C,SP=rung(rg)
    res=n1_rb(H0,C,SP,1/50,'atH',4.0,coupling=0.0)
    print(f"  RUNG-{rg}: {classify(res)} (Rw/R0={res['Rw']/res['R0']:.3f})")
# CTRL-N3' pair as MEASUREMENT
print("CTRL-N3' pair fate (MEASUREMENT, per 2583 S3):")
Hp,Cp,Sp=pair_config()
for member in ('atH','atP'):
    for bd in (2.0,4.0):
        row=[classify(n1_rb(Hp,Cp,Sp,dtf,member,bd)) for dtf in DTS]
        print(f"  {member} bd={bd}: {row}")
# THE LADDER
for member in ('atH','atP'):
    for bd in (2.0,4.0):
        ...
        for rg in (1,2,3):
            ...
            row=[]
            for dtf in DTS:
                res=n1_rb(H0,C,SP,dtf,member,bd)
