#!/usr/bin/env python3
"""S4-X X6 (Patch 2735) under s4x_charter.md stage X6 -> PR5, sharpened
by the X1 paradox into the lead discriminator.

PREREGISTRATION (frozen HERE before any result):
MATCHED-FUNCTIONAL BATTERY -- both instruments (S4-E simulation chains;
the committed 2721 HNC solver) evaluated with IDENTICAL extraction
functionals:
  F1: real-space slope of ln|r h(r)| on the SIM window (2a_s, 3/kD).
  F2: real-space slope on the FAR window [0.40, 0.88] fm (0.88 = the
      sim data's rmax; the HNC-record window [0.40,1.00] is truncated
      to the common domain -- frozen now, disclosed). F2 is ADVISORY
      if the sim far-window fit error exceeds 10%.
  F3: k-space pole: linear fit of k^2/S_zz vs k^2 over the MAIN box's
      shells n^2 in {1,2,3} (k = 2.77, 3.91, 4.79 /fm), intercept =
      kappa_pole^2 -- HNC S_zz evaluated AT THE SAME k values.
DIAGNOSTIC: HNC window-sensitivity map, kappa_extract vs window start
r0 in {0.08, 0.20, 0.40} (end 0.88), at a_s = 0.04.
SELF-VALIDATION GATE: the local hd-exposing solver copy must reproduce
the committed 2721 values (1.0206 / 1.0042 at a_s = 0.04 / 0.02 on
[0.40,1.00]) to 3 decimals in the ratio, else STOP.
CLASSIFICATION UPDATE RULE (frozen): the X1 classification updates to
EXTRACTION-SYSTEMATIC (PR1-satisfiable: the instruments measured
different functionals of an O(Gamma)-shaped profile) iff F1 AND F3
matched comparisons at a_s = 0.04 agree within 2 sigma each;
otherwise CLOSURE-ERROR CANDIDATE / UNRESOLVED stands and X3/X4
carry. Sim errors: X1-corrected (block/tau-max). HNC uncertainty:
the +/-1.5% method floor per extraction."""
...
# --- sim data ---
def simprof(label):
    d=json.load(open(f"/tmp/s4e_{label}.json")) if False else json.load(
        __import__('gzip').open(f"../data/s4e_chains/s4e_{label}.json.gz"))
