"""
1870 -- Gold-standard check on the derived floor shape S(v): full rigid-body rod-rod
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:                     # single-velocity worker mode
        ...
        omc, Emax = run_batch(v / C_KMS, nt, rng)
        area = np.pi * R_SAMP ** 2
        d = json.load(open(store)) if os.path.exists(store) else {}
        key = str(v) if seed_off == 0 and len(sys.argv) <= 3 else "{}_s{}_dt{}".format(v, seed_off, DTV)
        d[key] = [area * float(np.mean(omc)), area * float(np.std(omc) / np.sqrt(nt)), Emax]
        json.dump(d, open(store, 'w'))
        print("{} done: sigma_T={:.2f}+/-{:.2f} fm^2, Emax={:.1e}".format(
            key, d[key][0], d[key][1], Emax))
        sys.exit(0)
