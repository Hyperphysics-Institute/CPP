#!/usr/bin/env python3
"""RV-2714 EXECUTION (Patch 2761) under the FROZEN 2760 prereg.

The five S4-E chains re-run with the FIXED A path. Machinery is the
committed 2714 script VERBATIM with exactly the changes the prereg
licenses: (1) the one-line defect fix `mn[i]=False` (2754/2755
lineage); (2) fresh reserved seeds 20260798-802; (3) gate v2
(CONV-005, BLOCKING) before any production sweep; (4) an S-drift
fresh-summation cross-check at every chunk boundary (X5b audit
spirit); (5) output paths under rv2714. The analyzer is 2714's
analyze() verbatim (frozen 2713 criteria C1-C5). RV-3 re-runs the
2735 matched-functional battery verbatim against the RV jsons.

Usage:
  gate            -> run gate v2 for all five geometries (BLOCKING)
  chunk LABEL NS  -> advance LABEL by up to NS sweeps (checkpointed)
  analyze         -> frozen 2713 criteria on completed RV jsons
  rv3             -> frozen 2735 matched-functional battery (X6)
  archive         -> gzip jsons into data/rv2714/
"""
...
RUNS={"RV-MAIN-A":(686,0.04,20260798,600,2400),"RV-MAIN-B":(686,0.04,20260799,600,2400),
      "RV-SIZE-S":(432,0.04,20260800,400,1600),"RV-SIZE-L":(1024,0.04,20260801,400,1600),
      "RV-CORE":(432,0.02,20260802,400,1600)}
CKDIR="/tmp/rv2714"; os.makedirs(CKDIR,exist_ok=True)
...
def gate_v2():
    """CONV-005 blocking gate: five seed-1 trial moves per geometry,
    fixed-A increments vs B total differences, threshold 1e-8."""
    worst_all=0.0
    for label in RUNS:
        ...
        for t in range(5):
            ...
            dA,_=dE_A_fixed(i,newp,pos,z,L,alpha,rc,a_s,kv,wk,pref_k,S)
            save=pos[i].copy(); pos[i]=newp
            dB=total_E(pos,z,L,alpha,rc,a_s,kv,wk,pref_k)-E0
            pos[i]=save
            rel=abs(dA-dB)/max(abs(dB),1e-12)
            worst=max(worst,rel)
        ok=worst<=1e-8; worst_all=max(worst_all,worst)
        print(f"GATE {label}: worst rel disc = {worst:.3e} -> {'PASS' if ok else 'FAIL'}")
        if not ok:
            print("GATE v2 FAIL -> PRODUCTION BLOCKED (CONV-005). Report the defect.")
            sys.exit(1)
    print(f"GATE v2: ALL FIVE GEOMETRIES PASS (worst {worst_all:.3e} <= 1e-8). Production licensed.")
...
def run_chunk(label,max_sweeps_this_call):
    ...
    if os.path.exists(ck):
        ...
        S=(z[:,None]*np.exp(1j*(pos@kv.T))).sum(0)
        done=0; profs=[]; szz=np.zeros(len(k2)); ns=0; acc=0; tot=0
