#!/usr/bin/env python3
"""
PATCH 2624 -- N2B-DISC-1-2 EXECUTION under n2b_disc1_2_prereg.md (2623) ONLY
(which carries the 2620 spec verbatim + the two 2622-designed corrections).
...
t0=time.time()
print("="*78); print(f"PATCH 2621 -- N2B-DISC-1 (prereg 2620; verdicts read there)  stage={stage} [DISC-1-2]")
print("="*78)
...
if stage=='controls':
    ...
    # C2a Sea machine-zero + C2b convergence gate (corrected per 2622/2623)
    ...
    print(f"[C2b] Edrift(eta=0) dt 1/100,1/200,1/400 = {Ed[1/100]:.3f},{Ed[1/200]:.3f},{Ed[1/400]:.3f} "
    ...
    for v in (0.10,0.95):
        for dtf in (1/50,1/100,1/200,1/400):
            ...
            s1,sc,dB,f,dc=reads(r)
            A[(v,dtf)]=(s1,dB)
            exp='BASELINE-EXPOSED' if s1<3*r['Edrift'] else 'clear'
            print(f"  v={v} dt=1/{int(1/dtf)}: S1p={s1:.2f} D^B={dB:.2f} "
                  f"|DB-S1p|/S1p={abs(dB-s1)/max(s1,1e-9):.3f} Scum={sc:.1f} "
                  f"Edrift={r['Edrift']:.1f} [{exp}] cls={classify(r)}")
    ...
    for v in (0.10,0.40,0.95):
        ...
        exp='BASELINE-EXPOSED' if s1<3*r['Edrift'] else 'clear'
        print(f"  physical grid v={v} dt=1/200: S1p={s1:.2f} Edrift={r['Edrift']:.1f} [{exp}]")
...
print("\nDone. Verdicts are read in n2b_disc1_2_record.md against the prereg.")
