...
def n1_atc(H0,C,SP,dtf,betad,coupling=1.0,TC=60):
    """atC [2587 S3]: electric force on homes = trailing Moment-cycle average of the
    CP-position-evaluated force (per-cycle held); strong instantaneous at homes;
    H-C-prime per-step Newtonian integrator at pinned masses."""
    NS=len(H0); A=amat(SP); qw=qw_of(SP,C); QW=np.outer(qw,qw); A2=A*A
    isE=np.array([s=='e' for s in SP]); kap=np.where(isE,KE,KQ); m=np.where(isE,KE,KQ)
    dt=TAUC*dtf; nst=int(TC*TAUC/dt); spc=max(1,int(round(1.0/dtf)))
    H=H0.copy(); P=H.copy()
    ...
    tgt=np.array([oppr[i][np.argmin(np.linalg.norm(H[oppr[i]]-H[i],axis=1))] for i in range(NS)])
    last=-np.ones(NS,int); out=np.ones(NS,bool); v=np.zeros(NS)
    Vh=np.zeros((NS,3)); accFe=np.zeros((NS,3)); FeC=np.zeros((NS,3))
    Rr=[]; Dm=[]; rel_ok=True
    for st in range(nst):
        ddp=P[:,None,:]-P[None,:,:]; r2p=(ddp*ddp).sum(axis=2); np.fill_diagonal(r2p,np.inf)
        FeP=(-((QW/(r2p+A2)**1.5))[:,:,None]*ddp*(-AHC)).sum(axis=1)
        accFe+=FeP*dt
        if (st+1)%spc==0:
            FeC=accFe/(spc*dt); accFe[:]=0.0     # perception updates per cycle
        Fs,_=strong_FU(H,SP,betad)
        if coupling>0:
            Vh=Vh+((FeC+Fs)/m[:,None])*dt
            if np.linalg.norm(Vh,axis=1).max()>=0.9: rel_ok=False
        H=H+Vh*dt
        # CP layer verbatim
        F=(-FeP)*0+((QW/(r2p+A2)**1.5))[:,:,None]*ddp*(-AHC)
        F=F.sum(axis=1) if F.ndim==3 else F
        Fm=np.linalg.norm(F,axis=1)
        v=v*eA+np.minimum(mu*Fm,1.0)*(1-eA); v=np.minimum(v,1.0)
        idx=np.arange(NS); o=idx[out]; r=np.sqrt(r2p)
        if len(o):
            rij=r[o,tgt[o]]; hit=rij<A[o,tgt[o]]
            for k,i in enumerate(o):
                if hit[k]: continue
        ...
        Rr.append(np.sqrt((dc*dc).mean())); Dm.append(dc.max())
    Rr=np.array(Rr); Dm=np.array(Dm); n=len(Rr); wsl=slice(int(0.75*n),n)
    return {'R0':Rr[0],'Dmax0':Dm[0],'Rw':Rr[wsl].mean(),'Dmaxw':Dm[wsl].max(),'hb_ok':rel_ok,'Vh':Vh}

import time as _t; t0=_t.time(); DTS=(1/200,1/100,1/50)
trunc_mode='DIST'
print("="*78); print("PATCH 2588 -- atC MEMBER: G-SP GATE, THEN THE LADDER"); print("="*78)
def Utot_pair(r,bd):
    e=np.exp(-(bd/D)*(r-D))
    return -ALPHA_S*AHC/np.sqrt(r*r+A_QQ*A_QQ)+EQQ*((1-e)**2-1)
print("[G-SP under atC] (must HOLD primary, every dt, both widths):")
gsp=True
for bd in (2.0,4.0):
    ...
    for dtf in DTS:
        ...
        res=n1_atc(H0,C0,S0,dtf,bd)
        cl=classify(res); row.append(cl+f"[R={res['Rw']/res['R0']:.2f}]")
        if cl!='HOLD': gsp=False
    print(f"  bd={bd} (r_eq={req:.3f}): "+" ".join(row))
print(f"  G-SP {'PASS' if gsp else 'FAIL -> Branch I on the atC member'}")
if gsp:
    for bd in (2.0,4.0):
        print(f"\n===== atC, beta*d={bd} =====")
        for rg in (1,2,3):
