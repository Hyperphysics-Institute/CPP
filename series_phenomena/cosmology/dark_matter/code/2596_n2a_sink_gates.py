...
def n1_sink(H0,C,SP,dtf,betad,eta,coupling=1.0,TC=60,V0=None):
    ...
    Rr=np.array(Rr); Dm=np.array(Dm); n=len(Rr); wsl=slice(int(0.75*n),n)
    return {'R0':Rr[0],'Dmax0':Dm[0],'Rw':Rr[wsl].mean(),'Dmaxw':Dm[wsl].max(),
            'hb_ok':rel_ok,'Edrift':Edrift,'Sea':Sea,'Vh':Vh,'m':m,'Rend':Rr[-1]}
...
t0=_t.time(); DTS=(1/200,1/100,1/50); trunc_mode='DIST'
H4,C4,S4=rung(1)
print("="*78); print("PATCH 2596 -- N2-A: SINK ADMISSION GATES (eta band edges {0.5, 1.0})"); print("="*78)
def Upair(r,bd):
    e=np.exp(-(bd/D)*(r-D)); return -ALPHA_S*AHC/np.sqrt(r*r+A_QQ*A_QQ)+EQQ*((1-e)**2-1)
for eta in (0.5,1.0):
    print(f"\n--- eta = {eta} ---")
    # GATE 1: strong-pair WITH sink (at r_eq: hold; displaced +10%: settle toward r_eq)
    ok=True
    for bd in (2.0,4.0):
        rs=np.linspace(0.4,2.5,8401); req=rs[int(np.argmin([Upair(r,bd) for r in rs]))]
        res=n1_sink([[0,0,0],[0,0,req]],[+1,-1],['q','q'],1/100,bd,eta)
        resd=n1_sink([[0,0,0],[0,0,1.1*req]],[+1,-1],['q','q'],1/100,bd,eta,TC=120)
        sep_end=resd['Rend']*2  # 2-body: RMS-from-centroid*2 = separation
        settled=abs(sep_end-req)<0.05*req
        print(f"  G1 bd={bd}: at-eq {classify(res)} Edrift={res['Edrift']:.3f} | "
              f"displaced: sep_end={sep_end:.3f} vs r_eq={req:.3f} "
              f"{'SETTLED' if settled else 'NOT settled'} Sea={resd['Sea']:.2f} MeV")
        ok&=(classify(res)=='HOLD') and settled and res['Edrift']<FLOOR
    # GATE 2: x2.0 expansion must CONVERGE with balanced ledger
    cen=H4.mean(axis=0); H0=cen+(H4-cen)*2.0
    for bd in (2.0,4.0):
        res=n1_sink(H0,C4,S4,1/100,bd,eta,TC=120)
        conv=(0.5*0.575<=res['Rw']<=2.0*0.575)  # reference-anchored (R0_ref of the square)
        print(f"  G2 x2.0 bd={bd}: Rw={res['Rw']:.3f} (ref 0.575) "
              f"{'CONVERGED' if conv else 'NOT converged'} Sea={res['Sea']:.1f} MeV "
              f"Edrift={res['Edrift']:.3f}")
        ok&=conv
    # GATE 3: the 5-cell completes sub-relativistically, energy closed
    h=A_Q/2
    H5=np.vstack([H4,[+h,+h,D]]); C5=np.append(C4,-1); S5=S4+['q']
    for bd in (2.0,4.0):
        res=n1_sink(H5,C5,S5,1/100,bd,eta,TC=120)
        print(f"  G3 5-cell bd={bd}: {'sub-rel OK' if res['hb_ok'] else 'REL ASSERT'} "
              f"Rw={res['Rw']:.3f} Sea={res['Sea']:.1f} Edrift={res['Edrift']:.3f}")
        ok&=res['hb_ok']
    # GATE 4: C-SR boost -- deposits must match rest frame; drift preserved
    for bd in (4.0,):
        r0=n1_sink(H4,C4,S4,1/100,bd,eta)
        Vb=np.tile(np.array([0,0,0.3]),(4,1))
        rb=n1_sink(H4,C4,S4,1/100,bd,eta,V0=Vb)
        vdrift=np.linalg.norm(rb['Vh'].mean(axis=0))
        dsea=abs(rb['Sea']-r0['Sea'])
        print(f"  G4 boost 0.3c bd={bd}: Sea(rest)={r0['Sea']:.3f} Sea(boost)={rb['Sea']:.3f} "
              f"d={dsea:.4f} | drift_end={vdrift:.4f}c "
              f"{'PASS' if dsea<FLOOR and abs(vdrift-0.3)<0.01 else 'FAIL'}")
        ok&=(dsea<FLOOR and abs(vdrift-0.3)<0.01)
    # GATE 5b: Edrift dt-convergence on the violent G2 cell (the prereg letter:
    # 'within dt-convergent tolerance' -- convergence IS the criterion during violence)
    edr=[n1_sink(H0,C4,S4,dtf,4.0,eta,TC=60)['Edrift'] for dtf in DTS]
    print(f"  G5b Edrift(x2.0) dt-conv: {edr[0]:.2f} {edr[1]:.2f} {edr[2]:.2f} "
          f"{'CONV' if edr[0]<edr[1]<edr[2] else 'NOT CONVERGENT'}")
    ok&=(edr[0]<edr[1]<edr[2])
    # GATE 5: dt convergence on G1-displaced Sea deposit
    deps=[n1_sink([[0,0,0],[0,0,1.1*1.131]],[+1,-1],['q','q'],dtf,4.0,eta,TC=60)['Sea'] for dtf in DTS]
    print(f"  G5 dt-conv Sea: {deps[0]:.3f} {deps[1]:.3f} {deps[2]:.3f} "
          f"{'CONV' if abs(deps[0]-deps[1])<abs(deps[1]-deps[2])+0.5 else 'CHECK'}")
    print(f"  eta={eta}: {'ALL GATES PASS' if ok else 'GATE FAILURE'}")
print(f"\n[{_t.time()-t0:.0f}s]")
