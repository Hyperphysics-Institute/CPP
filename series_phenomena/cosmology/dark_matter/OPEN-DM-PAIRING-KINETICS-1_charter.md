...
**Status update, Patch 3511:** fork resolved (graph §4). Founder: anti-quarks form symmetrically, annihilation follows, asymmetry dynamical and unmechanised; corpus: n₊ = n₋ exactly (neutrality grounding) closes the p = 1 branch. D1 unchanged; **D1′ added** (R₊/R₋ composition-dependence); **D4 now targets the re-derived S3-M1 sinks** (2520 §2 superseded in part). Next: the residue computation for both signs.

**Status update, Patch 3512:** the residue computation PERFORMED (`kinetics1_residue_computation.md`, verify 10/10). **D1′ read: R₊/R₋ = 1 exactly, composition-independent** (polarity-blind bias; n₊ = n₋). **D1 not readable to a number from the registered inputs:** constant H ⇒ p = 1 exactly (all sub-linearity is ε = −d ln H/dN at the window); the count law ends at one CP per GP, i.e. λ_q = ½ — inside the pair window — with the engine's H → 0, so on the engine alone the pool pairs down and p → 0 (outside); p runs from 0 through the band (dilution stopping at n̄ within ~1% of 1) to 1 as a function of **where the count law's dilution terminates — not registered.** Per §5 D1's band rule, **C-5 stays conditional**; not passed (no corpus termination inside the band), not killed (the engine's stall is its end, not a dilution law past it — D-6). One physics-picture question returned to the founder (note §4). D2–D4 not attempted; all inherit the terminal condition.

**Status update, Patch 3513:** founder's reply registered verbatim (`founders_voice/3513_planck_epoch_dispersal.md`; graph §5). Branch (i) selected in mechanism — the **repulsive era** (like-charge repulsion) carries dilution past n̄ = 1 — **rate not given, so D1 remains unreadable**; a third CP is **captured as a trio**, not a spoiler (kernel K2-direction, third bound); the naked-qCP residue is the **knock-off yield from trios**, not the never-paired pool (3510 cutoff row refined); pairing does not go to completion; annihilation confirmed. Landing statistics alone give the trio inventory ∝ n_q² (flag, `code/3513_trio_landing_flag.py`, not a reading). **Next founder question: the repulsive era's rate — Moments per e-fold of thinning at the freeze, and whether the interior thins.** Cross-lane flag to the EU lane (reheat budget / 3805 cancellation).

**Status update, Patches 3516–3517:** the founder's contraction worry answered from the corpus (3516, not fatal; the repulsive-era driver is in tension with 3805). **3517: a candidate mechanism checked — a THERMAL freeze** (pair ⇌ two lone charges held open by kT, the walk-and-talk's own 'does not go to completion'): toy Boltzmann gives p = ½ for fast expansion, drifting through the band over two to three decades of slow expansion — the band becomes natural instead of a knife-edge, and the engine's stall is irrelevant because the freeze is at kT ≈ E_b, not Planck kT. **This contradicts Q7 (cutoff by dilution at Planck kT); registered as a fork for the founder, not adopted.** If ruled in, u and D2 need the qDP/third binding energy E_b and the pairing rate; the framework's inputs change.
