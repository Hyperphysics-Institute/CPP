...
lattice shells with a declared resolution-robustness axis.

## Deliverables (verbatim from PR4, 2726 §5 — binding)

For the uniform Sea: (i) stationary marginal energy-only? (ii)
Gibbsian to screening-relevant accuracy? (iii) stationary
probability currents? (iv) additional slow variables? (v)
directional susceptibility? (vi) observable-dependent θ_eff?
Metropolis/HNC concordance cannot substitute. Execution prereg
(sizes, Moments, observables, pass bands) freezes in its own patch
before any run; adverse outcomes report in full; no promotion from
inside. Queued behind or parallel to S4-X per resources; 79.5%
untouchable.
