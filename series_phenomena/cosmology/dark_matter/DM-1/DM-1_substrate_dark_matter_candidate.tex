...
region); (c) Clause 1's four founder-attested exits (Patch 2358; frames and
...
% ============================================================
\noindent\textbf{[v1.5, 9 July 2026: dual-frame status of the falsifiers (Gate-1/B1; per the founder-ratified slate).} At the registered (v1) evaluation frame the capture channel is \emph{killed} (Patch 2333; six-flank closure, panel round, never regraded) --- the capture-mechanism falsifiers in the v1.1 block below are retained as the v1 branch's protocol and are dead with it at that frame. The live falsifiers are now: (i) the \textbf{branch-invariant set} --- the group-scale falsifier (branch-invariant since Patch 2324), elastic scattering (F5; astrophysical systematics computed and folded in the companion DM-3 v1.1), and np consistency (F2); (ii) the \textbf{population falsifiers} of \S\ref{sec:population} --- the $S(N)$ derivation rent (OPEN-SS-43) at the frame-required steepness, the formation-kinetics rent, the two named completion audits, and the dimer-regime $\approx 2.8$~GeV direct-detection discriminant (F-DM3-4 in DM-3; Clause 1 exit (d)'s paper face); (iii) \textbf{Clause 1's four founder-attested exits} (Patch 2358), with frames and readings fixed at ratification and no post-adverse softening. The paper gains falsifiers in the exchange.]\par\medskip
