...
%       charge-neutral mode) and ratified by the AI review panel incl. the original dissenter;
...
%   CONFIRM from all panel members on the paper and CONFIRM from all panel members on the v1.0 promotion (Grok, Gemini,
...
%   RATIFY-WITH-CHANGES; no RESTATE/REFUTE; attribution anomaly RESOLVED 5 Jul:
%   founder confirms slot-3 was Gemini self-misidentifying -- verdict stands
%   unqualified). All five requested changes folded:
...
%   disqualifier. OPEN-SS-43 de-novo gap derivation
%   (m_s = chi*hbarc/r_c) remains the promotion gate beyond Layer-C.
%   Returns: review/reviews_v1.2_panel_returns.md.
% v1.3-DRAFT (Patch 1882, 5 July 2026) --- BARYON SECTOR (CONFRONT-3, patches
%   1878-1881): RETRACTS the v1.2 np-selection claim (item ii). XQC partial-wave
%   recomputation excludes the J4-additive coupling x20-30 (1879); the composed
%   S_c ladder has exactly one non-sterile survival island [~0.005, 0.05] (LZ
%   un-shielding kills the naive dipole escape); RULING D5-A' adopted at the
%   island-center natural scale S_c = R_N/R_s = 0.035 (first-multipole
%   color-dipole), DISCLOSED as survival-conditional model selection with the
%   derivation debt attached: the multipole order must derive to FIRST power
%   (zeroth = XQC-dead, second = LZ-dead). J12 pins: Dewar/NFM18 not
%   constraining at 25.3 GeV; DAMIC-shielded at ruling point; Earth heating
%   negligible. New falsifiers F5 (XQC-class flight, x11), F6 (deep-Earth
%   thermalized population), F3' (multipole derivation). Two promotion-gate
%   derivation debts now explicit (m_s gap; coupling order). Halo sector of
%   v1.2 untouched. DRAFT: panel re-review REQUIRED (ratified claim retracted);
%   OSF HELD.
% v1.3 (Patch 1884, 5 July 2026) --- SHIPPED on 5/5 panel ratification (2nd
%   five-member cycle; unanimous on the retraction, J12 accounting, falsifiers;
%   ChatGPT RESTATE on D + 3x RWC, all folded: (1) D5-A' stated PROVISIONAL /
%   non-upgradable-until-derived / overturnable-by-derivation; (2) exclusion
%   wording tempered, independent reproduction invited; (3) shielding-criterion
%   coarseness + island-existence-vs-edges + dipole-squared clarification
%   in-notice; (4) governance: promotion needs both debts cleared AND a
%   stability cycle. Attribution anomaly RESOLVED 6 Jul: founder confirms
%   slot-5 was DeepSeek self-misidentifying (same failure mode as Gemini at
%   v1.2); 5/5 stands unqualified. Returns: review/reviews_v1.3_panel_returns
%   .md. OSF decision: founder has ruled DERIVATIONS-FIRST -- both debts to be
%   discharged (and a stability cycle passed) before public release.
% v1.4-DRAFT (Patch 1889, 6 July 2026) --- CONV-004 METHODOLOGY + SUBSTRATE
%   INVERSION (patches 1886-1888): founder ruling (verbatim archived) adopts
%   the measured-coefficient/Galilean-layer discipline in this sector; the
%   v1.3 "derivation debt / non-upgradable-until-derived" governance language
%   is SUPERSEDED -- m_s and S_c re-read as the campaign's first two substrate
%   MEASUREMENTS, with F3' kill-content retained undiminished (zeroth-order
%   derivation = XQC-dead; second = LZ-dead). SI-1/SI-2 folded: 8-unknown/
%   9-target ledgers, 7 registered-structure maps, honest counting (6 hard/8),
%   three pre-scan inferences (alpha_e/alpha_q ~ 6e-3; C_r ~ 2.4e-4 measured
%   cancellation; D_st ~ O(0.1-1) static singlet moment); 2M-sample existence
%   scan NON-EMPTY (913; alpha_q ~ 0.9; f_occ ~ 0.1 sparse Sea); F7 corner
%   exists (E_z ~ 16 keV => f_occ ~ 4e-3, S_p ~ 0.04). Falsifiers updated:
%   F5 dual-form (46/x11 ruling point; 2-28 region-weighted); DAMIC pin
%   adjudicates 40% of the region; F6 and group standing. Governance revised:
%   promotion = overdetermination discipline + stability cycle + derivation
%   target open with kill-content. DRAFT: short panel re-look required
%   (ratified governance sentence changes); OSF after ratification +
%   stability cycle.
% v1.4 (Patch 1890, 6 July 2026) --- SHIPPED on 5/5 panel ratification (3rd
%   five-member cycle; Grok "SHIP"; unanimous on SI discipline + falsifiers;
%   the invited unfalsifiability attack on CONV-004 returned in the
%   programme's favor by every reviewer). Folded: (1) provisional-empirical-
%   anchors sentence + anti-semantic-drift guard (ChatGPT); (2) governance
%   hard rules -- no MEASURED->DERIVED promotion without an independent
%   derivation preserving the confrontation ledger; Layer-C promotion needs
%   one full no-supersession stability cycle + independent-channel
%   overdetermination (ChatGPT+Copilot convergent). F5 D_st-prior sensitivity
%   check queued (SI follow-up). No attribution anomaly this cycle.
%   STABILITY-CYCLE CLOCK STARTS HERE; OSF on its completion.
...
% v1.5 (Patch 2360, 9 July 2026) --- THE POPULATION FINDING + DUAL-FRAME
%   REWRITE under the founder-ratified slate (2358: F3 dual-frame adopted;
%   pin band-reporting [50,72], steep edge live; Clause 1 population form
%   attested, exits (a)-(d); R2 split release). (a) NEW sec:population
%   (inserted from the 2357 draft, both slate slots resolved): provenance;
%   dual-frame statement (v1 kill STRENGTHENED by formation kinetics x1.189;
%   audited-extended natural pass margin x2.18; audited-central strained
%   alpha~3-6 / p>=13, envelope-split 0.3%); per-pin-reading two-strand
%   table (steep edge isothermal-only w/ demand-internal collision; Elbert
%   64 both-strands at alpha=100/p=12.3; band-top 70 both-strands
%   near-natural alpha=0.01/p=3.7; steep edge LIVE, no point selected);
%   the population's ledger (S(N) rent per frame, alpha inverse-
%   coefficients, two completion audits, shared perturbation residual);
%   the dimer discriminant feeding sec:falsify. (b) Abstract + sec:falsify
%   gain v1.5 dual-frame notices: the capture falsifiers are retained as
%   the killed v1 branch's record, dead with it at the registered frame;
%   live set = branch-invariant (F1 group-scale, F5 elastic, F2 np) +
%   the population falsifiers + Clause 1's attested exits. The paper
%   GAINS falsifiers in the exchange. The kill is never regraded; the
%   frames are stated side by side, never averaged. NOT YET RE-SHIPPED:
%   joint DM-1/DM-3 panel round pending per R2.
% v1.5-panel-repairs (Patch 2362, 9 July 2026) --- JOINT-ROUND REPAIRS
%   (adjudication: conv001_2026-07_dm1_dm3_returns_adjudication.md; five
%   returns, one NO-REVIEW abstention; seat mapping pending founder --
%   two self-IDs collided on DeepSeek, 2355/2356 precedent). Executed:
%   (1) TITLE FACE corrected v1.1-capture -> v1.5 dual-frame (the round's
%   top accepted finding; supersedes the title-untouched house pattern BY
%   PANEL FINDING); (2) abstract notice: kill BOLDED + anti-skim sentence
%   (candidate dead unregraded / population = different hypothesis under
%   Clause 1); (3) both-strands rows priced in prose after the table,
%   neither fiducial (J-C SHARPEN); (4) F-DM3-4 references made CONDITIONAL
%   on the overburden/rate computation (J-D BREAK accepted + discharged by
%   execution; mirrored in DM-3); (5) falsify notice: capture-conditional
%   vocabulary aligned to DM-3 taxonomy, Clause 1 exits (a)-(d) enumerated
%   in-line, the falsifier exchange COUNTED not asserted; (6) ledger (iv):
%   perturbation-residual propagation into the dimer mass fraction stated
%   as not-yet-bounded (the honest core of an A1 RWC; the requested
%   quantification REJECTED as not-in-record). Still NOT-YET-RE-SHIPPED:
%   release posture in the adjudication; the call is the founder's.
% v1.5-stage1 (Patch 2365, 9 July 2026) --- F-DM3-4 CONDITIONALITY NARROWED
%   (mirrors DM-3 v1.1-stage1): overburden ceilings computed (Stage 1,
%   verify 3/3, >= six-decade window every site class, outcome (i)); the
%   population-section discriminant item and the falsify-notice tag now
%   carry the narrowed form -- sigma_dimer (Stage 2) the sole remaining gate.
% KILL RECORDED (Patch 2369, 9 July 2026, founder-attested) --- the
%   population branch killed at the audited frames (XQC-2007, 2366-2368,
%   panel-verified); paper marked NOT-FOR-RELEASE, retained as record,
%   NOT rewritten (revision deferred behind OPEN-DM-DSPH-1 by founder
%   decision). Abstract carries the kill notice.
% Version history: documentation_suite/changelog-DM-1.md
% ============================================================
% Version 1.8 --- 17 August 2026 (Patch 3214):
...
% Version 1.7 --- 17 August 2026 (Patch 3213):
...
% Version 1.6 --- 17 August 2026 (Patch 3212):
...
{\Large \textbf{Version 1.5 --- Dual-Frame: the Kill and the Population Finding --- 9 July 2026}}}
...
\small OSF DOI: pending deposit; see \texttt{documentation\_suite/osf-deposit-DM-1.md} (parent project \texttt{10.17605/OSF.IO/JXE8D})\\[2pt]
{\small Version~1.6 --- 17 August 2026 (Patch 3212: added the generated plain-language appendix glossing the internal programme identifiers used in this paper (founder jargon ruling, 16 Aug 2026). No claim, equation, number or section changed.)}\\[2pt]
{\small Version~1.7 --- 17 August 2026 (Patch 3213: extended the generated identifier appendix to all five identifier families (THEO-, FI-, PRED-, CONJ-, PH- as well as OPEN-), and replaced review-convergence scores with AI review panel wording where present. No claim, equation, number or section changed.)}\\[2pt]
{\small Version~1.8 --- 17 August 2026 (Patch 3214: added a How-we-review methodology note stating plainly that AI panel review is a self-applied verification method and not journal peer review; twelve chirality papers retitled so the descriptive title leads and the identifier is demoted to a subtitle. No claim, equation, number or section changed.)}}
...
\noindent\textbf{[v1.0, 28 June 2026 (shipped on AI review panel convergence): the headline result is a \emph{velocity-dependent} self-interaction --- the Cross-Rod fragments at cluster velocities (collisionless) and stays intact at dwarf velocities (cores), so $\smm$ \emph{falls with} $v$, the SIDM-preferred pattern and the opposite of the $v^{-4}$ light-mediator behaviour. The corona dilution risk is retired \emph{unconditionally} (OPEN-COSMO-DM-3 derived at Layer B, ratified by the AI review panel), and the $\smm = 0.11\,N$ normalization is verified at the cube-core element level (the $4\times$ element mass cancelling via the additive London-polarizability cross-section). The lone open item is the \emph{single} $\smm$ number, flagged as the external SF-2/SF-5 edge-bond SSV make-or-break with a pre-registered falsifiable window. This remains a \emph{framework-internal} candidate: its conclusions are conditional on the correctness of the CPP inputs it inherits. The as-shipped velocity-\emph{independent} abstract below is superseded by this notice. See \S\ref{sec:xsec}.]}\par\medskip
...
\noindent\textbf{[KILL RECORDED --- v1.5 PREDATES IT (Patch 2369, 9 July 2026, founder-attested):} the population finding this version reports was subsequently KILLED at the audited frames --- the required $\approx 2.8$~GeV dimer (0.94--0.99 mass fraction at every passing point) is EXCLUDED-class against the 2007 XQC spectrum at every registered island coupling (Patches 2366--2368, five-seat flash verification, V1--V5). Both candidate branches are now dead: capture at the registered frame (2333), population at the audited frames (2369). This version is retained as record, NOT-FOR-RELEASE, not rewritten --- the revision is deferred behind the reopened missing-physics arc (OPEN-DM-DSPH-1) by founder decision.]\par\medskip
\noindent\textbf{[v1.5, 9 July 2026: Gate-1/B1 verdict + the population finding, reported dual-frame per the founder-ratified slate (Patch 2358). The capture channel of this candidate was \textbf{killed} at the registered evaluation frame (Patch 2333; closed on six flanks, panel-reviewed, never regraded) --- the v1.1 capture notices below are retained as the killed branch's record. \textbf{The capture candidate itself is dead at the registered frame and is not regraded; what survived is a different hypothesis} --- a formation-generated \emph{population} of the same aggregates, carried prospectively under Clause 1's attested exits --- which at the literature-audited extended frame passes the full anchor suite at natural knobs (margin $\times 2.18$), at the audited-central frame passes with named strains, and at licensed readings of the pin anchor's own velocity band ($\langle v \rangle \in [50,72]$, band-reported, steep edge live) satisfies \emph{both} published dwarf-scale strands jointly. The frames are stated side by side, never averaged; the kill is never regraded. See the new \S\ref{sec:population} and the restated falsifiers at \S\ref{sec:falsify}. NOT YET RE-SHIPPED: panel round pending per the R2 split release.]}\par\medskip
...
\begin{mdframed}[linewidth=1.2pt, linecolor=brown, backgroundcolor=brown!6, innertopmargin=8pt, innerbottommargin=8pt]
\textbf{REVISION NOTICE (v1.4, 6 July 2026) --- methodology and the substrate inversion: the measured-coefficient (CONV-004) framing supersedes the v1.3 ``derivation-debt'' governance language, and the first data-drawn portrait of the DP Sea enters the record; covers the governance items of the v1.3 notice below and adds \S(ii)--(iii). Campaign record: \texttt{OPEN-SS-43\_Rs\_derivation.md} \S\S25--27; \texttt{SI-1\_unknowns\_and\_forward\_maps.md}; reasoning/code 1886--1888; founder ruling verbatim in \texttt{founders\_voice/founder\_ruling\_measured\_coefficients\_2026-07-06.md}. \emph{Panel-ratified 5/5 (third five-member cycle: unanimous on the SI discipline and the falsifiers; the invited unfalsifiability attack on CONV-004 itself was returned in the programme's favor by every reviewer; 2$\times$ RATIFY-WITH-CHANGES folded --- Patch 1890; returns in \texttt{review/reviews\_v1.4\_panel\_returns.md}).}}

\textbf{(i) The framing supersession (CONV-004).} By founder methodological ruling (6 July 2026, archived verbatim), the programme adopts in this sector the \emph{measured-coefficient discipline}: where the substrate physics beneath a result is genuinely unresolved, the STRUCTURE is claimed and the DATA fix the coefficients that encode the unresolved depth --- Galileo's law before Newton's mechanism. Every number carries a ledger tag (\textsc{measured} / \textsc{derived} / \textsc{conjectured}) that travels with it; a measured coefficient is honest only under \emph{overdetermination} (it must keep surviving channels it was not fitted to, pre-registered, no refit); the discipline applies only where derivation is premature (the programme's zero-parameter \textsc{derived} corpus is not re-framed); and the axiomatic derivation layer remains the standing goal, to be confirmed \emph{against} the tag ledger --- numbers it demonstrably did not fit. \textbf{Accordingly, the v1.3 language ``derivation debt / non-upgradable until derived'' is superseded: the two quantities are re-read as the campaign's first two substrate \textsc{measurements}} --- $m_s = \chi\,\hbar c/r_c = 7.764$~MeV [\textsc{measured}: halo ladder] and $S_c \in [0.005, 0.05] \sim R_N/R_s$ [\textsc{measured}: XQC/LZ/shielding ladder]. \emph{The falsification content of v1.3's F3$'$ is retained undiminished:} if the eventual derivation of the colour-singlet coupling yields zeroth multipole order the candidate is XQC-dead, and if second order, LZ-dead --- the reframing changes what the number \emph{is} (a measurement awaiting explanation, not a debt blocking upgrade), not what can kill the model. \textbf{Per panel (v1.4 cycle): measured coefficients are PROVISIONAL EMPIRICAL ANCHORS, not derivations; a future derivation may overturn them --- and ``measured and surviving'' must never shade into ``established by theory.''}

\textbf{(ii) The substrate inversion (SI-1/SI-2) --- what the dark-matter data reveal about the DP Sea.} The full measured/pinned set was inverted through seven forward maps built from registered structure only (Debye-form screening; contact bonds at the pitch; the coherence-vs-cancellation residual; the singlet moment; all $\mathcal{O}(1)$ geometric constants scanned in $[1/3,3]$ and every modeling choice J-tagged), against eight substrate unknowns --- with the counting stated honestly in the foundation document: \emph{six hard equations, eight unknowns; the data pin combinations, and the arc grows by importing more pinned corpus numbers as targets, never by adding unknowns.} Three inferences precede any scan: the channel ladder $\alpha_e/\alpha_q \approx 6\times10^{-3}$; \textbf{the cancellation $C_r \approx 2.4\times10^{-4}$ ($0.6$--$8.7\times10^{-4}$) --- the quiescent Sea cancels the colour channel to a few parts in ten thousand, now a measured statement}; and $D_{st} = \mathcal{O}(0.1$--$1)$ --- a confined colour singlet presents a substantially \emph{static} leading residual moment. The existence scan ($2\times10^6$ samples, all hard targets simultaneously): \textbf{NON-EMPTY} --- 913 accepted, a physically sensible region: $\alpha_q = 0.89$ ($0.35$--$2.3$), \textbf{occupancy $f_{occ} \approx 0.10$ --- a sparse Sea}, with $n$, $S_p$, $E_z$ individually prior-shaped (flagged as such) and the combination $n S_p/E_z$ pinned to a decade. The pre-registered kill-condition (an empty region = structural falsification) did not fire. A \textsc{conjectured} corner: if the Sea's ZBW scale equals the 16~keV formation temperature (F7; cap mechanism open), the portrait sharpens to $f_{occ} \sim 4\times10^{-3}$, $S_p \sim 0.04$ --- a very sparse, weakly coherent Sea whose internal oscillation scale is the dark matter's birth temperature.

\textbf{(iii) Updated falsifiers and governance.} \textbf{F5} (XQC-class reflight): 46 predicted events at the ruling point ($\times 11$ below current data); $2$--$28$ region-weighted (median margin $\times 98$) --- both carried. \textbf{F6}: deep-Earth thermalized population $\bar n \sim 2\times10^{13}$~cm$^{-3}$, $S_c$-independent. \textbf{The DAMIC edge is quantified: the future DAMIC-floor pin adjudicates $40\%$ of the accepted region outright} --- a pre-registered, high-value data action. The group-scale falsifier ($0.037$--$0.05$ at $\sim1150$~km/s, $2.3\sigma$ below the current mild detection) stands. \emph{Governance (revised, superseding the v1.3 sentence):} promotion beyond Layer-C requires (a) the overdetermination discipline maintained --- every \textsc{measured} coefficient surviving pre-registered channels it was not fitted to, with the measured-vs-unknown ledger visible; (b) a stability cycle with no load-bearing corrections; and (c) the standing derivation target remains open with its kill-content intact, its eventual success to be judged \emph{by overdetermination against the tag ledger}. \textbf{Hard rules (per panel, folded): no \textsc{measured} coefficient may be promoted to \textsc{derived} status without an independent derivation that also preserves the confrontation ledger; and promotion beyond Layer-C requires at least one full stability cycle with no major supersessions AND overdetermination of the measured coefficients by independent channels.} \textbf{Status: v1.4 --- panel-ratified 5/5; the stability-cycle clock starts at Patch 1890; OSF on its completion.} The v1.3 notice below is retained per house discipline, with its governance items superseded as stated.
\end{mdframed}

\begin{mdframed}[linewidth=1.2pt, linecolor=teal, backgroundcolor=teal!6, innertopmargin=8pt, innerbottommargin=8pt]
\textbf{REVISION NOTICE (v1.3, 5 July 2026) --- the baryon sector: a retraction, an exclusion survived, and a survival-conditional ruling with its debts attached; covers \S\ref{sec:xsec}--\S\ref{sec:falsify} and supersedes item (ii) of the v1.2 notice below. Campaign record: \texttt{OPEN-SS-43\_Rs\_derivation.md} \S\S20--24; reasoning/code 1878--1884. \emph{Panel-ratified 5/5 (second five-member cycle: unanimous on the retraction, the J12 residual accounting, and the falsifiers; one RESTATE and three RATIFY-WITH-CHANGES on wording, all folded --- Patch 1884; returns in \texttt{review/reviews\_v1.3\_panel\_returns.md}).}}

\textbf{(i) RETRACTION of the v1.2 np-selection claim.} Composing the same rod--nucleon coupling that the v1.2 CONFRONT-1 analysis used (\S(ii) below) with the full baryonic phenomenology it implies exposed a decisive confrontation the campaign had not run: the XQC sounding-rocket exposure. A partial-wave recomputation (Born is invalid here --- the coherent amplitude on Hg exceeds the mediator range; solver validated to $0.1\%$ against the exact finite-$k$ Born limit; full pinned Erickcek-2007 exposure model) predicts $10{,}000$--$15{,}000$ events against $527$ observed at the J4-additive coupling: \textbf{excluded $\times 20$--$30$, every energy bin, both potential signs} --- robust across all computed variants; independent reproduction is invited via the archived solver (reasoning/code 1879). The exclusion's only escape is suppression of the rod--\emph{nucleon} coupling, $S_c \lesssim 0.1$ --- which renders $\delta a_{np} \lesssim 3\times10^{-4}$~fm, invisible. \textbf{Therefore the v1.2 claim that ``the np scattering length selects $N \approx 15$--$20$'' is RETRACTED.} The $\Delta B$/absorption analysis and the np computation stand as upper-bound consistencies; the $N \approx 15$--$20$ corner now rests on the floor ceiling, the dwarf window, and tail pruning --- convergent, less sharp.

\textbf{(ii) The $S_c$ landscape and the ruling (D5-A$'$), recorded honestly.} Suppressing the coupling also \emph{un-shields} the underground detectors; the fully composed ladder (XQC partial-wave; rock-overburden shielding at $4300$~m.w.e.; LZ SI limit $9.2\times10^{-48}$~cm$^2$ at 36~GeV, arXiv:2207.03764, if unshielded; np; CMB drag) as a function of $S_c$ has \textbf{exactly one non-sterile survival island, $S_c \in [\sim0.005, 0.05]$}: XQC passes (46 predicted vs 527), the rock shields LZ ($\sim10^3$ slowing collisions), np is invisible, CMB is trivial. The naive second-power dipole estimate $(R_N/R_s)^2 = 1.3\times10^{-3}$ lands in an LZ dead zone; full decoupling ($S_c \lesssim 10^{-9}$) survives sterilely. \textbf{The first-power color-dipole scale $S_c = R_N/R_s = 0.035$ sits at the island's center, and is adopted as the provisional ruling: color-neutral confined nucleons couple to the Sea's unipolar residual at first multipole order} --- the rod's coherent cage sources the channel at full strength; a nucleon, lacking cage coherence, couples through its leading residual moment only. \emph{Disclosure: this ruling was reached by computing where survival lives (founder-directed) and adopting the natural scale found there --- survival-conditional model selection, admissible because the derivation debt below makes it maximally falsifiable from inside the theory} (reasoning/code 1880). \textbf{Per panel restatement (v1.3 cycle): this is a PROVISIONAL model-selection decision --- a natural-scale choice pending derivation --- adopted because it survives current confrontations, not because it has been theoretically established. It is NON-UPGRADABLE until the multipole order is derived, and that future derivation has full authority to overturn it.} The shielding criterion used in the landscape ($\geq 10$ slowing collisions) is a coarse transport threshold: the island's \emph{existence} is more robust than its numerical edges, which may shift under detailed transport modelling; ``second power'' above denotes the dipole-moment-squared scaling of the coupling relative to the coherent cage source.

\textbf{(iii) The baryon sector that survives.} Mass-matched atmospheric thermalization ($\sim10^3$ collisions at $\sim45\%$ energy loss) shields all deep-underground detectors structurally, not by tuning --- the strongly-interacting-DM regime (arXiv:2209.04387). CMB drag and gas-cloud bounds pass $\times20$ and better. The Dewar/NFM18/NBN19 ladder is source-stated to reach only $m_X \sim 0.5$--$10$~GeV and is not constraining at $25.3$~GeV (barometric collapse of the surface density; arXiv:2112.00707, full-text pin); DAMIC-shallow is shielded at the ruling point; Earth heating is $0.16$~GW against a $44{,}000$~GW budget (reasoning/code 1881, J12 pins).

\textbf{(iv) New falsifiers (add to \S\ref{sec:falsify}).} \textbf{F5:} an XQC-class flight with modern thresholds sits a factor $\sim11$ above the predicted near-threshold event count --- a direct, near-future test. \textbf{F6:} a center-concentrated, thermalized deep-Earth rod population ($\bar n \sim 2\times10^{13}$~cm$^{-3}$) --- exotic-isotope and borehole phenomenology. \textbf{F3$'$ (the decisive debt):} the multipole order of the color-neutral coupling must be \emph{derived} from substrate mechanics and must come out FIRST power: zeroth order is XQC-dead, second order is LZ-dead. The candidate's survival is staked on a derivable yes/no.

\textbf{(v) Residuals recorded (J12$'$).} The island's bottom edge ($S_c \lesssim 0.01$) awaits a DAMIC-floor pin; the top edge carries the XQC thermalization-efficiency caveat; no published high-mass surface-run analysis exists. The ruling point clears every pinned channel.

\textbf{(vi) Grade.} Layer-C consistency, now carrying \textbf{two explicit derivation debts as promotion gates: the $m_s = \chi\,(\hbar c/r_c)$ gap and the first-multipole coupling order.} The halo-sector results of v1.2 (dwarf pin, LSB, cluster ladder, Bullet, group falsifier) are untouched by this notice. \textbf{Governance (per panel): promotion beyond Layer-C requires BOTH derivation debts cleared AND a subsequent stability cycle with no load-bearing corrections; the review-lessons record is maintained in the campaign file.} \textbf{Status: v1.3 --- panel-ratified 5/5; OSF release decision returned to the founder (debts open).} The v1.2 notice below is retained per house discipline, with its item (ii) superseded as stated.
\end{mdframed}
...
% ============================================================
% (Inserted Patch 2360; drafted Patch 2357; slate slots resolved per the
%  founder-ratified slate, Patch 2358: F3 + pin band-reporting + Clause 1.)
...
\subsection{Dual-frame statement (frame F3, founder-ratified)}
The dual-frame rule is founder-ratified (Patch 2358): the frames are as
delivered by the demand audit (2345), reported side by side, never averaged.
\begin{itemize}
...
\item \textbf{At the audited-extended frame:} the population (the post-kill hypothesis, evaluated prospectively under Clause 1) generated by the
...
\subsection{The two-strand result, per pin reading (band-reporting $\langle v\rangle \in [50,72]$, founder-ratified)}
...
convention. Per the ratified slate the band is reported whole --- no point is
selected:
\begin{center}
\begin{tabular}{lll}
\toprule
...
any origin distinguishes itself there. Both both-strands rows carry material
cost --- $\alpha = 100$, $p = 12.3$ (edge-riding) at 64; $p = 3.7$, above the
nucleation-only expectation, at 70 --- and neither is fiducial.
...
shared with DM-2; its propagation into the dimer-regime mass fraction
(0.94--0.99) is not yet bounded --- stated, not hidden.
...
carries; (b) the dimer-regime direct-detection channel, registered as
\textbf{F-DM3-4} in the companion DM-3 (a population \emph{requires} the light
species at 0.94--0.99 mass fraction at the nucleation-biased passing points; a
null at the required abundance kills the dimer-weighted corner of every passing
region --- conditionality \emph{narrowed by computation} at Patch 2365: the
overburden ceilings are computed under hurting-direction conventions and a
visibility window of $\geq$~six decades exists at every listed site class;
the one remaining owed number is $\sigma_{\rm dimer}$ from the Stage-2
model-side computation, and until it lands this remains a registered
conditional falsifier); (c) Clause 1's four founder-attested exits (Patch 2358; frames and
readings fixed at ratification, no post-adverse softening, add-only
amendments), registered in the inverse-arc file.

% ============================================================
\section{Falsifiability}\label{sec:falsify}
% ============================================================
\noindent\textbf{[v1.5, 9 July 2026: dual-frame status of the falsifiers (Gate-1/B1; per the founder-ratified slate).} At the registered (v1) evaluation frame the capture channel is \emph{killed} (Patch 2333; six-flank closure, panel round, never regraded) --- the capture-mechanism falsifiers in the v1.1 block below are retained as the v1 branch's protocol (capture-conditional, in the DM-3 taxonomy) and are dead with it at that frame. The live falsifiers are now: (i) the \textbf{branch-invariant set} --- the group-scale falsifier (branch-invariant since Patch 2324), elastic scattering (F5; astrophysical systematics computed and folded in the companion DM-3 v1.1), and np consistency (F2); (ii) the \textbf{population falsifiers} of \S\ref{sec:population} --- the $S(N)$ derivation rent (OPEN-SS-43) at the frame-required steepness, the formation-kinetics rent, the two named completion audits, and the dimer-regime $\approx 2.8$~GeV direct-detection discriminant (F-DM3-4 in DM-3; Clause 1 exit (d)'s paper face; conditionality narrowed at Patch 2365 --- ceilings computed, the model-side $\sigma_{\rm dimer}$ owed as Stage 2); (iii) \textbf{Clause 1's four founder-attested exits} (Patch 2358) --- (a) the $S(N)$ rent under OPEN-SS-43, (b) the formation-kinetics rent, (c) the two completion audits (per-source pin recomputation; nine-dwarf completion), (d) the dimer direct-detection null / F1 hit --- with frames and readings fixed at ratification and no post-adverse softening. The exchange is counted, not asserted: three capture-mechanism falsifiers retire to the record; three branch-invariant falsifiers carry forward; the population adds two derivation rents, two completion audits, one conditional direct-detection discriminant, and Clause 1's four attested exits --- the paper gains falsifiers.]\par\medskip
...
\begin{description}
  \item[\texttt{CONJ-COSMO-1}] \hfill \\ The conjecture that cosmological dark matter consists of charge-neutral substrate aggregates of quark-sector dipole pairs and hybrid tetrahedra.
  \item[\texttt{OPEN-COSMO-DM-3}] \hfill \\ The condition on which the corona-dilution risk is retired in the dark-matter cross-section analysis.
  \item[\texttt{OPEN-DM-DSPH-1}] \hfill \\ The reopened missing-physics arc in the dwarf-spheroidal dark-matter analysis, behind which that paper's revision is deferred.
  \item[\texttt{OPEN-SS-39}] \hfill \\ Derive the formation, growth, and decay kinetics of closed-hDP-loop glueballs — in particular the terminal (cocoon-arrest) radius set by eDP cocooning + local-resource depletion — and the branching ratio between glueball collapse and competing extended-aggregate channels.
  \item[\texttt{OPEN-SS-43}] \hfill \\ A de-novo derivation of the screening length as a function of rung count, on which the dwarf-core result is conditional.
\end{description}
