...
### §7.3 — FOUNDER CONFIRMATIONS + v4 (Patch 2455, 13 July 2026)
All four items answered (verbatim in reasoning/2455): (1) reach = {3 in-plane + 2 axial} qCP + 1 radial eCP — the in-plane diagonal partner is dynamically activated via (3); (2) plane geometry CONFIRMED (eCP–qCP–qCP–eCP on both diagonals) — **geometry #3 coded eCP placement (axes) corrected to diagonals, formal amendment with the CONV-001 packet**; (3) asymmetric preemption: eCP targets exclusive, qCP targets non-exclusive with strong-force pile-up continuation; (4) saturation DERIVED: a_qq = ħc/264 = 0.747, a_ee = ħc/553 = 0.357 (= the registered 1814 ƛ EXACTLY), a_qe = 0.516 fm; contact depth α_s·ħc/a_qq = 102 MeV inside the E_qq deep branch — two consistency locks. **v4 results: ring−straight −68.8 ± 6.5 MeV (10σ, fourth confirmation); ring modes: uniform tilt STABLE +294±109; tilt m=1/m=2 ZERO-CONSISTENT (+51±78 / −52±89 — 2454's marginals closed); ellipticity zero-consistent. Verdict ~75%, leaning-SURVIVES, UNPROMOTED. CONV-001 packet = 2450–2455; independent re-implementation from the verbatim spec invited; panel items: pile-up window, SSV proxy, kinematics, BO treatment, registry channel, marginal-mode statistics.**
