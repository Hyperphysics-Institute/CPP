...
def run(ds, n_side, seed, r_soft=1.0, drive=None, probe=False, fprobe=False, contact=False, cluster=False):
    ...
    for t in range(1, T):
        ...
        # Patch 3198 (D-CONTACT-1): inter-DP contact counting. Counts CP pairs
        # from DIFFERENT DPs within r_c = 1.0. No RNG, no dynamics touched.
        if contact and t >= 2*th:
            wc = X[:,None,:]-X[None,:,:]; wc -= L*np.round(wc/L)
            sc = np.sqrt(np.einsum('ijk,ijk->ij', wc, wc))
