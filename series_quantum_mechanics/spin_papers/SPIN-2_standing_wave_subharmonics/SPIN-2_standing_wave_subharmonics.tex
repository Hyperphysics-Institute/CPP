% %%TODO: v1.x REVISION REQUIRED BEFORE DEPOSIT (Patch 4289). thm:ratio anchors the pair at the mode-2 antinode
% r_th/3 and node 2 r_th/3; sec 'Scale Connection' then moves the pair to 35.27x those radii (11.8 and 23.5 r_th),
% beyond r_th, where the mode-2 wave has no node or antinode, so r_out = 2 r_in has no anchoring basis at that scale.
% At the anchoring radii themselves L = 0.168 hbar/2. See
% series_standard_model/axiom_maturation/4289_spin_papers_hbar_half_is_input.md.
...
\title{\textbf{SPIN-2: Derivation of the Standing-Wave Sub-Harmonic Condition\\
for Captured Dipole Particle Orbits in Conscious Point Physics}\\[6pt]
{\large Spin Companion II (permanent ID SPIN-2) ---
...
Version~2.1, 2026 (spin sub-arc, SPIN-1; in-arc name Spin companion~I).
