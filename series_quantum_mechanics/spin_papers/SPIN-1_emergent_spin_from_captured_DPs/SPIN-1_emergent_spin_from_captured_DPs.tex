...
% Version 2.1 — 16 August 2026 (Patch 3210): MATHEMATICS CORRECTED.
%   (a) eq:rin and its abstract twin stated r_in = aBohr/(4(1+sqrt2)^2)
%       = aBohr/(24+16 sqrt2). The two forms are not equal:
%       4(1+sqrt2)^2 = 23.3137, while 24+16 sqrt2 = 46.6274 — a factor
%       of 2. The first form is correct and is what the paper's own
%       numerics (23.31, 0.0429, 2.270e-12 m) use throughout. The
%       expanded form is corrected to 12+8 sqrt2.
%   (b) The lepton scaling claim read 'radii scale as 1/m_ell^2 through
%       aBohr ∝ 1/m', which is self-inconsistent. Rederivation from the
%       paper's own eq:force_balance and L = m w_out r_in^2 (2sqrt2+4)
%       with L = hbar/2 gives r_in = hbar^2/(4 e^2 k_e m (1+sqrt2)^2),
%       i.e. r_in ∝ 1/m exactly. Corrected to 1/m_ell.
%   No other result changes; every other occurrence already used the
%   correct 4(1+sqrt2)^2 form.
% Version 2 — Revised with glossary wording, open-problem clarification,
% and numerical-ratio column
% 16 March 2026
% ==================================================================

% %%TODO: v2.2 REVISION REQUIRED BEFORE DEPOSIT (Patch 4289). Theorem thm:spin solves r_in FROM L = hbar/2:
% Coulomb balance allows a circular pair at every r_in (L/(hbar/2) = 2(1+sqrt2) sqrt(r_in/a0)), so hbar/2 is the
% input, and the abstract's 'No free parameters' and the verification table's 'exact match' restate it. With both
% radii at SPIN-2's anchoring points (r_th/3, 2 r_th/3), L = 0.168 hbar/2. The magnetic moment of this carrier is
% g = 3 - 2 sqrt2 (Patch 4286). See series_standard_model/axiom_maturation/4289_spin_papers_hbar_half_is_input.md.
...
\title{\textbf{SPIN-1: Emergent Spin-$\tfrac{1}{2}$ from Captured Dipole Particle\\
...
\date{16 March 2026 --- Version~2\\[2pt]
{\small Version~2.1 --- 16 August 2026 (Patch 3210: corrected the
expanded denominator in Eq.~(\ref{eq:rin}) and the lepton radius
scaling exponent; see CHANGELOG.)}}
...
\begin{itemize}[noitemsep]
  \item \textbf{Muon, tau:} Same eDP capture mechanism with higher
    mass eigenvalue; the orbital radii scale as $1/m_\ell$ through
    $\aBohr \propto 1/m$ in Eq.~\eqref{eq:rin}.
