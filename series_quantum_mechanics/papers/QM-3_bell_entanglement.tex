...
% Version 1.3 — 8 August 2026 (Patch 3033, O-7 sweep: AP-2 content
%   enumeration in the anti-erasure note updated to the AP-4 payload,
%   ratified 3032; statelessness claim unchanged.)
...
{\large QM Series --- Paper 4 (Version 1.3)}}
...
CP aggregate carries a two-component pattern state --- its
Sea-polarization configuration --- whose phase, the ZBW helix angle
(companion~C4~\cite{abshier2026c4}), encodes the spin direction
(this is the QM-1 v2.0 FI-QMRG-1 identification specialised to the
spin qubit: the helix angle \emph{is} $\phi$).
When two such aggregates are created in a total-spin-zero state, their
joint pattern state is non-separable: it cannot be written as a product
...
The two-component pattern state is
\begin{equation}
...
%=====================================================================
\section{The Singlet as Non-Separable Joint Pattern State}
%=====================================================================

When two spin-$\tfrac12$ CP aggregates are created with total spin
zero, their joint pattern state is the singlet:

\emph{Anti-erasure note (v1.2).}  Prior versions named this object
the ``joint DI-bit state.''  The naming is retired (A1$'$/AP-2, enumeration superseded by AP-4 at
Patch 3032: DI-bits carry no state beyond the payload \{origin address,
E, S\} --- statelessness preserved across both formulations);
the object itself --- and every theorem below --- is unchanged.  The
state is a property of the aggregate pattern maintained in the
Grid-Point registers and, as this paper already stated in v1.x, of
the global Nexus ledger, not of any locally carried content.
\begin{equation}
...
\caption{EPR pair maintained by the Nexus.  Particles A (blue) and B
  ...
  The Nexus (dashed ellipse) enforces the non-separable joint
  pattern state globally at every tick.  Measurement at Detector~A along axis
...
(1)~the non-separable singlet pattern state (Theorem~\ref{thm:nonsep}),
