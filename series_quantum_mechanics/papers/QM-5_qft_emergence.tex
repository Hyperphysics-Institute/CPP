...
{\large QM Series --- Paper 6 (Version 1.2)}}
...
collective excitation modes of the complex Sea-polarization site
field on the 600-cell lattice.  The site field $\psi_i =
\sqrt{\rho_i}\,e^{i\phi_i}$ (QM-1 v2.0: count register
$\rho_i$, $\SSV_{\rm net}$-orientation phase $\phi_i$) is
expanded in the 120 orthonormal eigenmodes of the
...
collective excitations of the Sea-polarization site field on the
600-cell lattice.  No separate QFT postulates are required; the field operators,
...
%=====================================================================
\section{The Complex Site Field and Lattice Eigenmodes}
%=====================================================================

At each Grid Point $i$ the site field is $\psi_i =
\sqrt{\rho_i}\,e^{i\phi_i}$ (Paper~2~\cite{cpp2040a}, v2.0
pattern-level grounding).  (\emph{Anti-erasure note, v1.2:} prior
versions called this ``the DI-bit state''; retired per A1$'$/AP-2,
mathematics unchanged.)  The mode operators below act on the
count-like occupation content of the sites --- the AP-2-compliant
register --- and the coherent-mode amplitude--count relation this
expansion presupposes is exactly the B-QMRG-1 bridge registered at
QM-1 v2.0.  The dynamics
...
\begin{enumerate}
\item Field operators from eigenmode expansion of the complex
  site field (equations~\eqref{eq:ak}--\eqref{eq:field}).
