...
%                     of the GP's V_i directional content in the
...
($V_i$) in the distinguished ZBW plane (foundational input
...
multiplication by $-i$ is a quarter-turn of $V_i$ in the
...
\medskip\noindent
\textbf{Keywords:} Schrödinger equation, 600-cell lattice, Sea-polarization pattern, V_i phase register, graph Laplacian, tight-binding model, discrete quantum mechanics, Madelung decomposition, quantum pressure
...
count-like scalar $\SSV_{\rm abs}$ and the vector $V_i$
...
$V_i$ register (FI-QMRG-1, Patch 2996).
...
Grid Point's $V_i$ directional content in the
distinguished plane (FI-QMRG-1).  The microscopic contributors to
$V_i$ are the orientations of the underlying dipoles; the
...
$|V_{\perp}| \propto \sqrt{\rho}$ (the B-QMRG-1
...
\item \textbf{The A3$'$ register mapping.}  The Madelung content of
  ...
  scalar ($\SSV_{\rm abs}$, $l=0$) and vector ($V_i$,
...
$|V_{\perp}|^2 \propto \rho$.  This is the standard
...
$V_i$: a massive pattern executes ZBW at the Compton rate,
...
\caption{Gaussian Sea-polarization wave packet on the 600-cell lattice.
  ...
  the rotation of each site's $V_i$ orientation.
...
(iii)~\emph{Linearity:} $V_i$ is by definition the vector
...
$-i$ is a quarter-turn of $V_i$ in the distinguished
...
\emph{OPEN-QMRG-B1 (original registration)}  The bridge $|V_{\perp}|^2 \propto
...
Balance gives $|V_{\perp}|^2 \propto N\hbar/(\mu\omega)$
...
$V_i$ vectors summing with different orientations; it is
...
the orientation ($\phi \leftrightarrow V_i$ direction in
...
\begin{description}
  ...
  \item[\texttt{OPEN-QMRG-B1}] \hfill \\ The B1 dynamical energy-balance derivation, closed at proportionality grade rather than for every microscopic realisation.
  \item[\texttt{OPEN-QMRG-B1-CONST}] \hfill \\ The exact coefficient of the canonical normalisation in the B1 energy balance.
  \item[\texttt{OPEN-QMRG-ETA}] \hfill \\ Mode-independence and the geometric constant in the quantum-mechanics re-grounding arc.
  \item[\texttt{OPEN-QMRG-R4-MULTILINK}] \hfill \\ The multi-edge-correlated transport kernel question, spun off from the shipped component-diagonal transport class.
  \item[\texttt{OPEN-QMRG-UNIQ}] \hfill \\ Whether alternative register structures, such as higher-dimensional orientation or discrete cyclic counts, are excluded by the substrate axioms.
\end{description}
