...
{\large QM Series --- Paper 2 (Version 2.4)}}
...
linear-response regime.  Registered obligations \emph{(statuses per the CONV-015
adjudication, Patch 3004)}: \textbf{OPEN-QMRG-B1 --- CLOSED at
...
lacks); the exact coefficient of the canonical normalization awaits
\textbf{OPEN-QMRG-B1-CONST} (the turnover/participation constant;
blocking only for exact-normalization claims). The QM-1$\leftrightarrow$QM-5
...
correlated kernel question is spun off as
\textbf{OPEN-QMRG-R4-MULTILINK} (non-blocking for this corpus;
blocking for universal substrate-theorem claims). Citations under
