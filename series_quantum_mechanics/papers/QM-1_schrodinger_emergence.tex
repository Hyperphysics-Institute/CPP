...
{\large QM Series --- Paper 2 (Version 2.0)}}
...
content \{charge, type, origin address\} (A1$'$/AP-2); they carry no
phase --- the phase is a property of the pattern they collectively
refresh.  The pattern's phase advances at the Zitterbewegung rate
...
ratified ontology (A1$'$/AP-2, Patches 2982/2989), DI-bits are
stateless couriers with content \{charge, type, origin address\} that
reset at every hop --- they can carry neither a phase nor an
...
\item \textbf{U(1) compactness for free.}  A quantum phase must live
  ...
  require.  Of the registers the Grid Point holds, only the direction
  of a vector in a 2-plane natively supplies a compact periodic
  coordinate.  A scalar register would require the periodicity to be
  postulated by hand; the $\SSV_{\rm net}$-orientation reading obtains
  it from geometry.
...
mechanics as well: phase singularities occur precisely where the
register that carries the phase vanishes.
...
grade: clauses (i) and (ii) are corpus-level substrate facts; clause
(iii) consumes the bridging input B-QMRG-1 (coherent-mode
amplitude--count relation) and the small-signal linear-response
regime.  A publication-grade hardening would derive the linear
regime's validity bounds from the substrate response functions.  The
theorem of \S4 consumes only the \emph{form} of the update
(complex, linear, Hermitian-generated), which this proposition
supplies.
\end{remark}
