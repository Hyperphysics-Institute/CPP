...
| Capotauro (v1.0 SHIPPED Session 122 Patch 0415) | PRED-O-25 (parity-violation asymmetry $\Delta p_{LR} = \chi/6 = \phi^{-3}/6 \approx 0.0394$ on the K3-doublet; Capotauro mechanism primary empirical prediction CONFIRMED at structural-numerical level within 2% of empirical anchor $\sim 0.04$ from leptogenesis back-derivation; derived at theorem level via THEO-CAP-1 Composite Capotauro Wigner-Eckart Theorem registered Session 103 Patch 0397; conditional closure on FI-C-1 through FI-C-10 + 4 CPP axioms A1, A3, A4, A7); closes OPEN-SM-4 sub-claim (c). $\sin^2\theta_{13}$ derivation re-scoped to SF-2 v2.0+ at Session 101 Patch 0395 (linear-vs-quadratic scaling tension surfaced Session 100; wavefunction-level coupling hypothesis ruled out Session 101; candidate $\gamma$ structural observation $\sin^2\theta_{13} = b \cdot m_\perp \approx 0.0227$ matches observation within 1$\sigma$ but lacks rigorous derivation). $\delta_{CP}$ and $\eta_B$ are downstream observables from $\|M\| = \chi/6$ via PMNS perturbation machinery beyond Capotauro mechanism's direct scope at v1.0 |
