...
| 1.5 | `bootup.md` **§0.5** | **STANDING WORKER DISCIPLINE** — thirteen process rules (D-1–D-13; D-8 is stated in prose below the table), each with its recorded cost. Read before doing any work. | 3 min | **All of it.** Handovers are superseded every session; durable process rules live HERE, not in a handover. |
...
- **The canonical handover lives in `handovers/` at the repo root.** Open the folder and read the most recent file — that's the current "what's next" pointer for the programme. The folder is the single, sortable index for all handover documents per the migration that landed 17 May 2026, Patch 0422 (`templates/operating_system.md` §15 "Handover file location and naming convention"). Filename pattern: `YYYY-MM-DD_session_NNN_<scope>.md`; `ls handovers/` shows chronological order with the most recent file last — **except when several files share one `YYYY-MM-DD_session_NNN` prefix and carry no `pNNNN` patch token, in which case they sort alphabetically by scope, not by time (Patch 4132: Session 233's six handovers sorted the oldest last). Then run `git log --name-only --diff-filter=A --format= -- handovers/ | grep -v '^$' | head -3` and read the first file listed.** If the most recent file is paper-scoped or trajectory-scoped and doesn't match the work to be done in the new session, look back at the previous most-recent file with matching scope. See `handovers/README.md` for the folder-level convention and migration history.

> **TWO-STEP BOOT (Patch 4189).** The newest dated file in `handovers/` is a **BOOT CARD** (hard cap **4 KB**). Read `bootup.md` and the boot card, then **STOP and report** HEAD, next free ID, suite status and the recommended task — **do no work** until the founder replies. The founder checks the report (it carries a checksum fact); only then, in STEP 2, read the targeted section of the full handover the task needs. **Full handovers live in `handovers/detail/`** so they never win the newest-file sort. Per D-13, never read `todolist.md`, the OS or the maturation log wholesale.
...
- **Heartbeat check (ratified Patch 2111; activates when the Step-4 audit is built):** once the nightly extraction audit exists, confirm `Development/audit_log.md` has a dated line for the previous calendar day. A **missing** line is a LOUD, blocking flag — the audit may have failed to run; investigate before proceeding. (Until the audit is built, this file is scaffolded but no audit runs yet — no check needed.)
- **Transcript fragmentation hook:** if `Development/transcripts/` holds a session transcript with no matching `*.fragmented` marker, offer to fragment it before other work. The deterministic split (`@@FOUNDER` markers, `Registries_pending` deltas, heartbeat) is the macro's job; the **rich split is an AI pass** — read the transcript and emit categorized fragment patches (founder → `founders_vision` candidates; reasoning → paper-local `reasoning/`; scripts → `code/`; registry items → ledger), staged for TLA review. After fragmenting, write `<transcript>.fragmented` so it isn't reprocessed. This is the "morning command" for per-turn chat capture (helper: `scripts/cap.sh`).
...
| **D-5** | **Check `git log` and the id-block counter against `origin/main` BEFORE starting** — and **never `reset --hard origin/main` mid-session**: build every patch on your own HEAD, because your last patch is not on origin until Thomas pushes it (Session 227 lost 3522 and 3524 this way, twice, and rebuilt them from the patch files). Patches you generated but Thomas has not yet pushed are not in the repo. | Twice work was silently duplicated (3816/3817, then 3839/3840 redone worse and discarded). Twice new work was nearly built on an unpushed base (3818 failed to apply for exactly this reason; 3858 was caught only because the container had reset). |
...
| **D-9** | **A deferral is a write, in the same commit, gated.** Any patch that puts anything aside — *owed, deferred, flagged (not fixed here), at the next recompile, left to the X lane, carried, reconcile when…* — files that item in `todolist.md` **in the same `git am`**, under the lane that will act on it. Run `python3 code/deferral_gate.py` before `git format-patch`; it scans the commit for that vocabulary and fails if `todolist.md` is not in the diff (override only by the auditable token `NOTHING-DEFERRED` — **which since Patch 0975 must be an *assertion*, not a *mention*: clause-initial and followed by a dash or colon and your reason. A bare occurrence of the token in narrative prose no longer overrides, and the hits are printed under the override so the author sees what he is waving past. 0974 was waved through by the sentence "all PASS or NOTHING-DEFERRED asserted in the log"** ). **A deferral that is not in `todolist.md` is not a deferral; it is a drop.** Review files, sector-file status lines, dated registry bullets and handovers are *records*, not a queue — none of them is read by the lane that has to act. | **Three arcs, three times, each recovered only because the founder asked.** EU Sessions 169–223 put nothing in `todolist.md` in 55 sessions (TODO-3930-EU, registered late at 3930). The DM kinetics framework two lanes waited on was in no queue (TODO-3938-DM). Patches 0936/0937 left a paper corrigendum, a units flag and a cross-lane sign in a review file and a status line (TODO-0937-CHIR, 0938). The OS already said *"any session that identifies a new deferral adds it here"* — a session-scoped rule audited at a session close that, under D-8, rarely fires. The rule was aimed at the wrong moment; the moment is the patch. |
| **D-10** | **Before asserting that the corpus does NOT supply a premise — above all when contradicting the founder — grep `founders_vision.md` and the `reasoning/` records for a PRIOR CORRECTION on that premise.** D-1 and D-3 say *search the topic* and *locate the object*; D-10 says *search for the argument you are about to have again*. | **Three times in Session 234.** Patch **4138** cited a verify script's prose summary which inverted two rows of the table the same script printed. Patch **4161** withdrew an alarm that c03's own sentence refuted — a sentence quoted in my own Patch 4155, six patches earlier. Patch **4167** told the founder his shell count "needed a premise the corpus did not supply"; the corpus supplied it at **Patch 0733**, where a prior Opus made the identical l_P-as-lattice-spacing error and the founder *"pushed back across several rounds"* before it was written into `founders_vision.md`. **The corrected figure was wrong by ninety orders.** In all three the corpus held the answer and the failure was in SEARCH, not in reasoning. |
| **D-11** | **A document's prose must reproduce its script's classification table, not paraphrase it.** Any conclusion resting on a table a script computed pastes the rows verbatim, so the document and the script cannot silently disagree. | Patch **4125** set `T = {A:-1, V:-1, r:+1, v:-1}` and its own output read `A.v … -1 +1 chi_4 CAN source` — **T-even**. The prose two paragraphs below asserted T-odd, and a second row was likewise inverted. **Four patches and three sessions cited the prose.** TEST-A3G-2 stood recorded as PASSED on an argument its own verify script refuted, until Patch 4138 recomputed the table. |
| **D-12** | **After a context compaction, lost work is RE-DERIVED from the repo or explicitly marked UNVERIFIED — never reconstructed from the compacted summary's verdicts.** A summary records what was believed, not why; rebuilding from it produces documents that match conclusions rather than reach them, and nothing in the result shows the difference. | **Session 235 (discarded).** Its container reset to 4187 mid-session and its commits were lost; it rebuilt 4188–4190 from a summary that already stated *"A3G-6 PASSES"* and *"μ_p to 0.3%"*, and wrote documents to fit. Result: a 4189 derivation that never used the achirality it claimed (⟨V̂⟩ = 0 holds for any bound orbit), a 4190 justification that was **circular** (the 1.07 ratio was built from m_q = 313 MeV, the value being replaced), and a report of *"three clean commits"* after a failed push. Nothing reached the repo; all three were discarded. |
| **D-13** | **Never read `todolist.md`, `templates/operating_system.md`, or `chirality_axiom_maturation.md` WHOLESALE — grep them, or read line ranges.** They are ~115k, ~78k and ~25k tokens. A handover may name them only as *reference*, never as required reading, and must never tell a fresh window to read them "in order". | **Session 235 compacted mid-boot and confabulated.** Both Session 234 handovers (`p4180`, `p4187`) listed `chirality_axiom_maturation.md` **and** `todolist.md` as reading to do before any work — **~160k tokens of mandatory reading**, enough to force compaction before a single task began. Session 235 booted on `p4180`, compacted, lost what it had read, and rebuilt from a summary (D-12). **The handover caused it**, and Session 234 had itself grown `todolist.md` from 380 KB to 460 KB — +21% — with verbose inline-result entries. |

**The common remedy.** Before reasoning forward from a clause, an item, or another lane's
number, spend one `grep` on the thing it names. Twelve failures above, and in every case a
single search would have prevented it.

**D-4 has a second cost record, from Session 234, and it is worse than the first.** Patch 4167
§5 named its own risk exactly — *"the one thing I have not checked is whether that definition is
complete"* — and published anyway. The check was one `grep` and would have taken thirty seconds;
the definition was not complete, the founder said so, and 4167 was withdrawn entire at 4168.
**Naming a risk is not retiring it. A flagged assumption that could have been checked in thirty
seconds is a worse failure than an unflagged one**, because the patch demonstrates it knew where
to look.
...
>
> **SCOPE TEST, added Patch 0975b after the exemption was misapplied to ~21 of 39 patches in Session 228.** The exemption is decided by **content, not by wrapper**. A patch is bookkeeping only if it contains **no original derivation, no computation, and no finding**. It is *not* bookkeeping merely because it arrived inside a review cycle, an adjudication, a scoping session, or an answer to the founder. **If the patch ships a verify script, it is not bookkeeping** — the rider already implies this by requiring the script "if any computation was done", and the implication is now explicit because it was missed. *What the miss cost:* Session 228 saved reasoning for every patch that looked like physics and dropped it for every patch that looked like process — and **three of that session's substantive findings (the reversal-odd second harmonic, the superadditive joint corner, and the T-arrow identification) were made inside "process" patches.** When genuinely uncertain, capture: a short fragment on a bookkeeping patch costs a paragraph, and an uncaptured finding is unrecoverable once the window turns over.

> **DEFERRAL RIDER (registered Patch 0939; bootup §0.5 D-9).** Every patch whose text puts anything aside — owed, deferred, flagged-not-fixed, at-the-next-recompile, left-to-another-lane — bundles, in the SAME `git am`, the `todolist.md` entry that names the item and the lane that acts. `python3 code/deferral_gate.py` runs before `git format-patch` on every patch and is not advisory: it fails on the deferral vocabulary unless `todolist.md` is in the diff or the message **asserts** the auditable token `NOTHING-DEFERRED` — clause-initial, with a dash or colon and a stated reason (Patch 0975; a mention is not an assertion, and the overridden hits print in the log). A patch presented without clearing this gate is an incomplete deliverable, exactly like a patch presented without the apply macro. This rider is bound to the patch contract for the same reason the reasoning-capture rider is: the contract is the one habit honoured unasked, and the session-close audit (§15 Step E) fires too late and too rarely to catch a deferral made mid-window.
>
...
After committing locally in `/home/claude/CPP`, run the two gates, then export:

```bash
python3 code/next_id.py <lane>         # the ID you used is inside your block and was free
python3 code/deferral_gate.py          # anything put aside is in todolist.md (D-9)
git format-patch -N HEAD~N..HEAD -o /tmp/p_outdir/
```

If `deferral_gate.py` fails, file the items it lists in `todolist.md`, `git commit --amend`, and re-run. For multi-patch sessions run it once per commit (`python3 code/deferral_gate.py <sha>`).
