...
% V1.2 (Patch 3328, 21 Aug 2026): Settled notes on founder-ratified
%   CONV-032 (5/5): the ergoregion-instability / "Planck-core bomb"
%   item RESOLVED at derivation-conditional grade by the censorship
%   theorem (Patch 3320) — no exterior ergoregion at any spin or
%   reflectivity, conditional on census assumptions A1-A3
%   (OPEN-GR-RCORE-4 carries the substrate derivation; the panel's
%   weighted-norm counterexample recorded); spinning-geometry input
%   for the boson-cloud item DELIVERED (the derived saturation
%   surface, co-rotation committed under amended RCORE-3). V1.1 notes
%   retained verbatim, settled notes appended (anti-erasure).
% V1.1 (Patch 3308, 20 Aug 2026): status notes on Open Problems 1-3
%   + one F-R1/C* dependency-audit note. Occasion: OPEN-GR-RCORE-1
%   (Patch 3297, CONV-030-ratified) computed the Planck-core reflectivity
%   this paper's problems 1-3 all reduce to (the Session-152 W-B finding
%   of the shared bottleneck) — |R| = 1 exact, Dirichlet — and relocated
%   the surface OUTSIDE the never-formed horizon. Problems 1-3 receive
%   their delivered inputs honestly (delivered-input, not closed:
%   scalar/vector wall condition and Kerr surface remain open); problem 2's
%   "Planck-core bomb" is re-framed as the textbook ergoregion instability
%   of horizonless spinning perfect reflectors (OPEN-GR-RCORE-2(iv)).
%   Horizon-defined threshold flagged. Original text verbatim, notes
%   inline (anti-erasure). OP-4 (EM superradiance) untouched: genuinely
%   open, outside the RCORE arc; the threshold audit note covers its
%   horizon-defined condition by reference.
% ==================================================================
...
azimuthal $V_i$ broadcast rotating at the horizon angular
...
\textbf{Load-bearing:} the rotating census gradient supplies the
ergoregion; energy extraction is bookkeeping --- the wave leaves with
more census influence than it brought, paid from the hole's rotation,
with the ledger balanced Moment by Moment. \textbf{Smoothed:} the
amplification factors are GR's. \textbf{Macroscopic shadow:}
superradiance is the census refusing to let rotational stress hide:
extractable order, conserved throughout.
...
\emph{[Dependency-audit note added Patch 3308, 20 Aug 2026 (CONV-030
adoption 4).  The threshold $\omega < m\Omega_+$ and the co-rotation
argument are defined AT the outer horizon, which under the ratified
horizonless reading (GR-1c V2.3; CONV-030) never forms; the ratified
R-CSTAR-MAP additionally puts the local substrate wave speed at
$c_*(\bar r) \leq c$ in the evaluation region.  The exterior Teukolsky
agreement is unaffected as an exterior statement; the threshold's
restatement on the exclusion surface, and the c-vs-$c_*$ question, are
OPEN-GR-RCORE-2.]}

In CPP, the mechanism is transparent at the level of the SSV
broadcast.  The rotating black hole's azimuthal $V_i$
...
The azimuthal $V_i$ at the outer horizon rotates at $\Oplus$
...
\begin{enumerate}
  \item \textbf{Quantitative amplification factor.}
    ...
    \emph{[Status note added Patch 3308, 20 Aug 2026.  The key input
    is DELIVERED: OPEN-GR-RCORE-1 (Patch 3297; CONV-030 5--0) derives
    $|R_{\rm core}| = 1$ exactly with Dirichlet phase for the axial
    gravitational sector, and relocates the wall to areal
    $\tfrac{9}{8}r_S$ --- outside the never-formed horizon (GR-1c
    V2.3), so the boundary condition is at the exclusion surface, not
    at $r_+ + \lP$.  STILL OPEN as posed here: the Teukolsky solve
    with the derived wall on the (underived) Kerr exclusion surface,
    and the tensor-sector/polar wall condition
    (OPEN-GR-RCORE-2(iii)/(iv)).]}
  ...
  \item \textbf{Superradiance from the Planck core.}
    ...
    \emph{[Status note added Patch 3308, 20 Aug 2026.  The
    gravitational-sector reflectivity is now DERIVED ($|R| = 1$,
    GR-1d V3); the scalar/vector question this item asks remains open
    (OPEN-GR-RCORE-2(iii)).  RE-FRAMED: with the surface outside the
    never-formed horizon, the ``Planck-core bomb'' is the textbook
    \emph{ergoregion instability} of a horizonless spinning perfect
    reflector --- a sharper and better-studied problem, registered as
    OPEN-GR-RCORE-2(iv).  Its resolution is load-bearing for the
    viability of rapidly spinning CPP compact objects.]}
    \emph{[Settled note, Patch 3328, 21 Aug 2026 (founder-ratified
    CONV-032, 5/5).  RESOLVED at derivation-conditional grade: the
    ergoregion-censorship theorem (Patch 3320) shows that, given the
    three census assumptions A1--A3, the exclusion surface lies
    strictly outside the ergosphere at every spin and latitude ---
    no exterior ergoregion ever forms, so the trapped
    negative-energy-mode gain loop cannot be assembled at ANY spin
    or reflectivity.  The ``Planck-core bomb'' is structurally
    unassemblable under this conditionality.  Rapidly spinning CPP
    compact objects are therefore NOT in tension with observed
    black-hole spins on this channel.  The load now rests on the
    conditionality itself: A2's rotational normalization and A3's
    composition norm are underived assumptions (the panel's
    registered counterexample: a weighted quadrature $F = s^2 +
    \gamma v^2$, $\gamma < 1$, preserves the entire static sector
    yet can de-censor the ergosphere) --- OPEN-GR-RCORE-4 carries
    the substrate derivation that would upgrade this from a theorem
    of the census model to a physical theorem.  The scalar/vector
    wall condition this item originally asked for remains open
    (OPEN-GR-RCORE-2(iii)); the surviving milder channel is
    Zel'dovich SURFACE superradiance (rotating-reflector
    amplification without an ergoregion), with quantitative
    growth-time bounds committed under the amended
    OPEN-GR-RCORE-3.]}
  ...
  \item \textbf{Boson cloud saturation in CPP.}
    ...
    \emph{[Status note added Patch 3308, 20 Aug 2026.  The
    reflectivity input is DELIVERED for the gravitational sector
    ($|R| = 1$; scalar bosonic wall condition open,
    OPEN-GR-RCORE-2(iii)); the geometry input has CHANGED --- the
    relevant boundary is the exclusion surface at areal
    $\tfrac{9}{8}r_S$, not a near-horizon region.  The saturation
    problem is unposed against the amended geometry.]}
    \emph{[Settled note, Patch 3328, 21 Aug 2026 (founder-ratified
    CONV-032, 5/5).  The spinning geometry input is now DELIVERED at
    derivation-conditional grade: the Kerr-analog exclusion surface
    is the census saturation set $\{s^2 + v^2 = 1\}$ (Patch 3320,
    conditional on A1--A3), lying outside the ergosphere at every
    spin.  Any reposed saturation calculation should use this
    surface --- co-rotating at $\omega(r_{\rm surf})$, a committed
    template refinement under the amended OPEN-GR-RCORE-3 --- as the
    inner boundary, with the superradiant condition evaluated
    against the surface's angular velocity rather than a horizon's.
    The problem itself remains unposed; only its geometry is now
    available.]}
