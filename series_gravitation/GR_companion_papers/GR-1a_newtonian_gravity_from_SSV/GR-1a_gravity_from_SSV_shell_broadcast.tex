...
Newton's law of gravity --- masses attracting with a force that fades
as the square of the distance --- is here derived from counting.  Every
point of space sends out a fixed number of conserved messengers each
tick and tallies what arrives; matter biases the tally, and the tally's
influence spreads outward over ever-larger spheres, thinning exactly as
$1/r^2$.  The gravitational constant is not tuned: it is fixed by the
same bookkeeping convention used throughout the series.  This paper is
the arc's ground floor --- the same census, pushed harder, yields
Einstein's corrections and black holes in the companions.

\subsection*{CP/GP Signature at This Scale}
\textbf{Load-bearing:} the shell broadcast supplies the $1/r$ census
profile; the PSR response at LOWEST order supplies the attraction ---
Newton's law is the linear regime of the one formula, with $G$ fixed
by the registered normalisation, not tuned. \textbf{Smoothed:}
lattice discreteness invisible at $(l_P/r)^2$; superposition of
sources is EXACT at this order because the census is additive.
\textbf{Macroscopic shadow:} what Newton wrote as force-at-a-distance
is the gradient of a conserved messenger census.
