...
% PCD terminology corrected to canonical "Perceive--Compute--Displace"
%   (was the Session-146 drift "Polarize--Capture--Depolarize") — Patch 0803, 8 June 2026
% CORRIGENDUM (Patch 3262, 20 Aug 2026): Proposition field_eq restated with
%   the correct potential (log-lapse Form A; quasilinear Form B); the prior
%   F-term formula was defective at leading nonlinear order (O(u^2) vs the
%   required O(u); it failed against this paper's own exact Theorem-1
%   solution). Panel CONV-027 APPROVE-EITHER 5/5; founder-ratified.
%   Verification: series_gravitation/code/3259_fterm_reconciliation_verify.py
%   (8/8, exact-symbolic; independently SCRIPT-EXECUTED by two panel seats).
% CORRIGENDUM 4 (Patches 3389-3395, 2-3 Sep 2026, CONV-040 5/5): under the
%   founder-ratified log PSR law (second order calibrated by Mercury) the
%   floor l_P/2 is reached at lapse 1/2, u = 2/3: the saturation surface is
%   at areal 8mu/3 = 4/3 r_S, not the Buchdahl 9mu/4; Corrigendum 3's
%   extremality assumption is moot. New remark rem:core_surface_corrigendum.
%   No equation changes.
% CORRIGENDUM 3 (Patches 3366-3371, 1-2 Sep 2026, CONV-038 5/5): the CP
%   Exclusion Rule invoked by the proof of thm:core is RETIRED (founder
%   R-EXCL-RETIRED; never in c01). Bound PSR >= l_P/2 RETAINED with a
%   replaced derivation (Buchdahl + exact exterior; conditional on
%   RCORE-4); ATTAINMENT is an extremality assumption, not derived.
%   Admissible window 0.536 < u_max <= 1. New remark
%   rem:core_floor_corrigendum. No equation changes.
% CORRIGENDUM 2 (Patch 3301, 20 Aug 2026): Theorem thm:core RELABELED, no
%   equation changes. The saturation radius r_core = GM/c^2 is the ISOTROPIC
%   coordinate radius rbar = mu (this paper's own declared coordinates);
%   numerically equal to r_S/2 but NOT the areal radius r_S/2. Its areal
%   image is 9mu/4 = (9/8) r_S — OUTSIDE the would-be horizon, which never
%   forms (the horizon's isotropic image mu/2 lies inside the excluded
%   region). "Core of radius ~ l_P" corrected: l_P/2 is the saturated LOCAL
%   PSR scale; the saturated region's coordinate extent is macroscopic.
%   Panel CONV-030 Q5: Thm 1 SOUND 5-0; Thm 2 SOUND-WITH-RELABELING 5-0;
%   founder-ratified (founders_voice/ ruling 2026-08-20). Verification:
%   series_gravitation/code/3297_rcore_verify.py (9/9).
% ==================================================================
...
\date{17 March 2026 (corrigenda 20 August 2026) --- Version~2.3}
...
\textbf{Load-bearing:} the PSR formula taken EXACTLY (no truncation)
supplies the full Schwarzschild geometry in isotropic coordinates;
the rigid lattice supplies the coordinates themselves (lattice
$\equiv$ isotropic --- the lattice is flat; rulers and clocks
respond); the corrected field equation (V2.2) is the harmonicity of
the clock-rate logarithm. \textbf{Smoothed:} discreteness enters only
near the exclusion radius (Planck-adjacent). \textbf{Macroscopic
shadow:} the black hole is the census formula saturated, not geometry
bent.
...
the \textbf{Perceive--Compute--Displace (PCD) cycle}.  Each Absolute
Moment tick, every Grid Point (GP) in the 600-cell lattice perceives
the net SSV imposed on its resident Dipole Particles by neighboring
fields, computes the resulting displacement, and displaces --- enacting
that displacement and broadcasting it as a Lattice State Packet (LSP)
...
V_i contribution~\cite{c7}, giving the known isotropic
...
\begin{remark}[Corrigendum: coordinate labels in Theorem~\ref{thm:core}
--- Patch 3301, 20 August 2026]
\label{rem:core_relabel}
Two labels in Theorem~\ref{thm:core} and its proof are corrected here;
\emph{no equation changes}.  (i)~The saturation radius in
Eq.~\eqref{eq:r_core}, $r_{\rm core} = GM/c^2$, is the \emph{isotropic}
coordinate radius $\riso = \mu \equiv GM/c^2$ --- the coordinate this
paper declares throughout (Theorem~\ref{thm:schwarzschild}) and the
ratified lattice~$\equiv$~isotropic identification (T-1 CHARTER,
Patch~3262) makes mandatory.  It is \emph{numerically} equal to $r_S/2$,
but the gloss ``$= r_S/2$'' invites reading it as the \emph{areal}
radius $r_S/2$ (deep inside the horizon), which is incorrect: by this
paper's own exact map (Eq.~\eqref{eq:iso_coord}), the areal image of
$\riso = \mu$ is $r = \tfrac{9}{4}\mu = \tfrac{9}{8}\,r_S$ ---
\emph{outside} the would-be horizon.  The horizon's isotropic image,
$\riso = \mu/2$, lies inside the excluded region and never forms: on the
whole exterior $\riso \geq \mu$ one has $k\,\Delta|\SSV| \in (0,1]$.
The areal surface radius $9GM/(4c^2)$ is exactly the Buchdahl bound,
with surface lapse $1/3$ and redshift $z = 2$ (machine-verified,
\texttt{code/3297\_rcore\_verify.py}, 9/9).  (ii)~``Planck-density core
of radius $\sim \lP$'' conflates two scales: $\lP/2$ is the saturated
\emph{local} lattice spacing ($\PSR_{\rm eff}$ at the floor), while the
saturated region's \emph{coordinate extent} ($\riso \leq \mu$) is
macroscopic for astrophysical masses.  The correct statement: a
maximally compact body whose interior sits at the Exclusion floor
$\PSR_{\rm eff} = \lP/2$ throughout, with surface at $\riso = \mu$
(areal $\tfrac{9}{8}r_S$).  Downstream consequence, adjudicated
separately: the comparison table of
Section~\ref{subsec:comparison} carries a dated supersession note.
Panel record: CONV-030 Q5, Theorem~1 SOUND 5--0, Theorem~2
SOUND-WITH-RELABELING 5--0; founder-ratified 20 August 2026.  This
remark corrects the labels while preserving the original text above
verbatim (anti-erasure).
\end{remark}

\begin{remark}[Corrigendum 3: the premise of Theorem~\ref{thm:core} is
retired; the bound is retained with a replaced derivation --- CONV-038,
Patches 3366--3371, 1--2 September 2026]
\label{rem:core_floor_corrigendum}
The proof of Theorem~\ref{thm:core} invokes the \emph{CP Exclusion
Rule} (``no two Conscious Points may simultaneously occupy the same
Grid Point'').  That is the pre-demotion \emph{postulate}: on 13~April
2026 it was demoted to THEO-1 (CP Non-Persistent Co-Occupation,
\texttt{theorem-registry.md}), under which co-occupation is not
forbidden but lasts one Absolute Moment, and same-polarity co-occupation
can be forced by an external $V_i$ (founder rulings
R-EXCL-RETIRED, 1~September, and R-COOCCUPATION-FORCED, 2~September
2026; \texttt{axiom-registry.md} \S``Retired rules'').  THEO-1 constrains
co-occupation, not the PSR, and so delivers no floor.  The cited source,
the Absolute Moment companion~\cite{c1}, has never contained the rule in
any repository version.  The proof's reading of $\lP$
as the lattice spacing is also withdrawn (Patch~0733: $\lP$ is the
baseline PSR, enclosing ${\sim}10^{30}$ sub-Planck grid points).
\textbf{The bound $\PSR_{\rm eff} \geq \lP/2$ is retained as a
conditional lower bound with a replaced derivation.}  If the saturated
body is static and Einstein's equations hold in its interior
(OPEN-GR-RCORE-4), Buchdahl's theorem in the surface saturation
variable $u \equiv k\,\Delta|\SSV|$ reads $u^2 - 5u + 4 \geq 0$; the
root $u = 4$ lies inside the censored region, so the exterior branch
gives $u_{\rm surface} \leq 1$, hence $\PSR_{\rm surface} \geq \lP/2$
(\texttt{rcore\_derivation/3367\_psr\_floor\_from\_buchdahl.md};
verify 25/25; CONV-038, 5/5).  \textbf{Whether the register attains
this bound ($\PSR = \lP/2$ exactly) is an extremality assumption, not
a derived result.}  The founder has ruled the floor \emph{finite}
(R-FLOOR-FINITE) and has \emph{no argument for its magnitude}
(R-CELL-SIZE-OPEN, 2~September 2026); the relation to SR-1's register
cap $\text{SSV}_{\rm crit} = E_P/\lP^3$ is unresolved
(OPEN-GR-FLOOR-1).  The admissible window from observation (a
GW150914-class ringdown requires a light ring, so the saturation
surface lies inside areal $3\mu$) is $0.536 < u_{\rm max} \leq 1$,
i.e.\ $\lP/2 \leq \PSR_{\rm floor} < 0.651\,\lP$
(\texttt{rcore\_derivation/3370\_floor\_window\_from\_buchdahl\_and\_light\_ring.md}).
Every result of this series downstream of the floor carries the
standing banner: \emph{floor undetermined within a derived window;
Buchdahl conditional bound $\PSR \geq \lP/2$.}  The theorem statement
and its equations above are preserved verbatim (anti-erasure).
\end{remark}

\begin{remark}[Corrigendum 4: the saturation surface under the ratified
PSR law --- CONV-040, Patches 3389--3395, 2--3 September 2026]
\label{rem:core_surface_corrigendum}
The founder's clock mechanism (the lapse is the PSR shrink,
R-CLOCK-RATE-IS-DISPLACEMENT), tested at second order in the isotropic
PPN reading, fails Mercury's perihelion with the Pad\'e PSR law
$\lP/(1+u)$ ($\beta = 3/2$) and passes when the law's open
second-order coefficient is $\tfrac12$ --- Mercury \emph{calibrates}
it, and the founder has ratified the all-orders log form
$\PSR_{\rm eff}/\lP = (1 - u/2)/(1 + u/2)$ (R-PSR-LAW-LOG), whose cubic
and higher coefficients remain open.  Under that law the floor
$\PSR = \lP/2$ is reached at lapse $\tfrac12$, $u = 2/3$: the
saturation surface sits at isotropic $\bar r = 3\mu/2$, \textbf{areal
$8\mu/3 = \tfrac43\,r_S$}, redshift $z = 1$, inside the CONV-038
window and \emph{not} at the Buchdahl radius $9\mu/4$ used in
Corrigendum~2 and Patch~3297.  The extremality assumption of
Corrigendum~3 is thereby moot: saturation occurs inside the window, not
at its edge.  The surface is a free boundary (its even-sector wall law
is CONV-040's kinematic relation $(4 - 3u/2)H_2 + 2K = 0$).  Conditional
on the ratified completion of the PSR law; the census itself is
unchanged (Patch~3389: the shell-mean census gives $u = \mu/\bar r$
exactly).  Original text preserved verbatim (anti-erasure).
\end{remark}
...
\textbf{[Supersession note, Patch 3301, 20 August 2026:} the table and
the paragraph above are preserved verbatim (anti-erasure) but are
superseded by the corrigendum of Remark~\ref{rem:core_relabel} and the
CONV-030 adjudication.  Under the ratified isotropic reading the
exclusion surface sits at areal $\tfrac{9}{8}r_S$, \emph{outside} $r_S$:
no horizon forms (the row ``At $r = r_S$ \ldots horizon'' describes a
region that is never reached), and the divergence from GR begins at the
surface, in the \emph{observable} exterior --- which is precisely what
makes the echo prediction of companion GR-1d falsifiable.  See GR-1d
(amended) and \texttt{frontier\_sectors/GR.md}, OPEN-GR-RCORE-1.\textbf{]}
...
\begin{proposition}[CPP field equation --- corrected, Patch 3262]
\label{prop:field_eq}
In the continuum limit, the self-consistency
condition~\eqref{eq:fixed_point} is exactly the statement that the
\emph{logarithmic lapse}
\begin{equation}
  N \;\equiv\; \ln\sqrt{-g_{tt}/c^2}
  \;=\; \ln\!\frac{1-\varrho}{1+\varrho}
  \;=\; -2\,\mathrm{artanh}\!\bigl(\tfrac{k\,\Delta|\SSV|}{2}\bigr),
  \qquad \varrho = \tfrac{k\,\Delta|\SSV|}{2},
\end{equation}
is harmonic with respect to the effective metric, sourced by the
compressed-DP census:
\begin{equation}
  \boxed{\;\nabla_\lambda\nabla^\lambda N
  \;=\; -\,\frac{4\pi G}{c^2}\,\rho_{\rm mass}
  \;+\; \text{(higher multipole channels)}\,;
  \qquad \text{vacuum: } \nabla_\lambda\nabla^\lambda N = 0.\;}
  \label{eq:field_eq}
\end{equation}
\textbf{(Form A.)}  Equivalently, in the $\Delta|\SSV|$ variable
\textbf{(Form B)}:
\begin{equation}
  \nabla_\lambda\nabla^\lambda\bigl(\Delta|\SSV|\bigr)
  \;+\; \mathcal{F}_{\rm true} \;=\; \text{source},
  \qquad
  \mathcal{F}_{\rm true}
  = \frac{k^2\,\Delta|\SSV|/2}{1-(k\,\Delta|\SSV|/2)^2}\,
    \bigl|\nabla(\Delta|\SSV|)\bigr|^2_g .
  \label{eq:F_true}
\end{equation}
\textbf{Equivalence to the lattice frame.}  For the pointwise isotropic
dictionary $A(\Delta|\SSV|)$, $B(\Delta|\SSV|)$ of this paper, the
identity
\begin{equation}
  \nabla_\lambda\nabla^\lambda\,
  \mathrm{artanh}\!\bigl(\tfrac{k\,v}{2}\bigr)
  \;=\; \frac{32k}{(2-kv)(2+kv)^5}\;\nabla^2_{\rm flat}\,v
  \label{eq:equivalence}
\end{equation}
holds for \emph{generic} $v$ (no derivative terms survive; in full 3D
the cancellation of every $|\nabla v|^2$ term follows from
$f''/f' + \tfrac{d}{dv}\ln(\sqrt{A}\,B^{1/2}) = 0$, which holds
identically for $f = \mathrm{artanh}(kv/2)$).  The field equation is
therefore algebraically equivalent to the flat-lattice statement
$\nabla^2_{\rm flat}(\Delta|\SSV|) = \text{normalised source}$ --- the
form derived from the messenger census (T-1, Patch 3258).  In the
weak-field limit both forms reduce to
$\nabla^2\Phi = 4\pi G\rho$ and the linearised Einstein equations of
companion~7, unchanged.
\end{proposition}

\begin{remark}[Corrigendum note]
The Proposition as originally published stated the compensator as
$\mathcal{F} = [2k(\Delta|\SSV|)^2/(1+k\Delta|\SSV|)^2]\,
\Box\ln(1+k\Delta|\SSV|)$.  That formula is defective: on this paper's
own exact Theorem-1 solution it is $O(a^4)$ where the required
compensator is $O(a^3)$ (with the $O(a^2)$ term cancelling
identically), so it fails against the exact solution at leading
nonlinear order.  The defect was found by the T-1 census derivation's
HALT check (Patch 3258), diagnosed and repaired at Patch 3259, and the
correction was approved APPROVE-EITHER by all five seats of review
round CONV-027 and ratified by the founder (Patch 3262).  Machine
verification: \texttt{series\_gravitation/code/3259\_fterm\_%
reconciliation\_verify.py} (8/8, exact-symbolic; independently
SCRIPT-EXECUTED by two panel seats, outputs registered in
\texttt{series\_gravitation/review/reviews-CONV-027.md}).  Nothing
observable changes: Theorem~1's metric, the classical tests, and the
weak-field sector are identical under the defective and corrected
statements.
\end{remark}
...
The nonlinear structure arises from the metric dependence of the
d'Alembertian: writing the curved operator on the pointwise dictionary
$A(\Delta|\SSV|)$, $B(\Delta|\SSV|)$ and changing variable to the
logarithmic lapse $N = -2\,\mathrm{artanh}(k\Delta|\SSV|/2)$, every
gradient-squared term cancels identically
($f''/f' + \tfrac{d}{du}\ln(\sqrt{A}B^{1/2}) = 0$ for
$f = \mathrm{artanh}(ku/2)$), leaving the harmonicity statement of
Form~A --- equivalently the quasilinear Form~B with
$\mathcal{F}_{\rm true}$ as stated, and equivalently (via
Eq.~\eqref{eq:equivalence}) the flat-lattice census equation of the
T-1 derivation (Patch 3258).  The right-hand side follows from
...
Einstein field equations by construction.  Whether the corrected
equation (equivalently, the lattice-frame census equation with its
pointwise dictionary) produces the full nonlinear Ricci tensor
$G_{\mu\nu} = R_{\mu\nu} - \tfrac{1}{2}g_{\mu\nu}R$ in the
...
Setting $k\,\Delta|\SSV| = GM/rc^2 \ll 1$ (so $N \to -k\,\Delta|\SSV|$
and $\mathcal{F}_{\rm true} \to 0$, both forms coinciding at linear
order) in Eq.~\eqref{eq:field_eq}:
\begin{equation}
...
\begin{equation}
  (V_i)_\phi \;=\; \frac{J\sin^2\theta}{r^3}
  \cdot \xi_{\rm lattice},
...
\begin{equation}
  g_{t\phi} \;=\; -2k\,(V_i)_\phi \cdot r\sin^2\theta
...
\begin{enumerate}[noitemsep]
  \item \textbf{Zero rotation} ($a=0$, $J=0$): $(V_i)_\phi=0$,
...
\begin{itemize}[noitemsep]
  ...
  \item The \emph{LSP metric mapping} $|\SSV|_{\rm abs} \to g_{tt}$,
    $V_i \to g_{ij}$ is from companion~7~\cite{c7},
...
\begin{enumerate}
  \item \label{op:einstein}
    ...
    Proposition~\ref{prop:field_eq} (corrected, Patch 3262) gives the
    CPP field equation and
    proves its weak-field reduction to linearised GR.  Whether the
    corrected quasilinear structure in Eq.~\eqref{eq:field_eq}
  ...
  \item \label{op:kerr}
    ...
    postulates.
    \emph{[Status note added Patch 3305 (W-D), 20 Aug 2026.  DELIVERED
    at consistency level by companion GR-1f (18 Mar 2026): the
    Lense--Thirring term is exact in the weak field and the full
    Boyer--Lindquist form is shown \emph{consistent} with the mechanism
    at all orders --- but $\Sigma$ and $\Delta$ are not derived from
    the field equation (registered \texttt{op:allorders}, open).
    Companion GR-1g extends to Kerr--Newman.  Post-CONV-030 caveat: the
    exterior Kerr results stand, but the physical object is horizonless
    (this paper, Remark~\ref{rem:core_relabel}); the stability of a
    spinning horizonless perfect reflector against the ergoregion
    instability is OPEN-GR-RCORE-2(iv).]}
  ...
  \item \label{op:24cell}
    ...
    modes.
    \emph{[Status note added Patch 3305 (W-D), 20 Aug 2026.  The
    geometry this item states is RETIRED: founder ruling A1 (Patch
    3236) established that the 600-cell's dual is the 120-cell, so the
    lattice vertex's Voronoi domain is the \emph{regular dodecahedron},
    not the 24-cell.  The eigenvalue spectrum the item defers to was
    delivered by SPIN-3 (v1.0, formerly Spin~III) on the correct cell,
    together with the selection and protection theorems.  The
    discrete-to-continuum proof itself REMAINS OPEN, now to be posed
    against the ratified T-1 field equation (companion GR-1j); the
    quoted convergence-rate expectation is unverified.]}
  ...
  \item \label{op:hawking}
    ...
    \emph{[Status note added Patch 3305 (W-D), 20 Aug 2026.  Partially
    delivered, then honestly RE-OPENED in a new form.  Companion GR-1e
    (18 Mar 2026) supplied the geometric termination condition and the
    stable Planck-remnant endpoint.  Its V1.1 amendment (Patch 3303,
    after CONV-030) then applied the horizonless reading of this
    paper's Remark~\ref{rem:core_relabel}: no horizon forms, so the
    horizon-based emission mechanism does not apply as written, and the
    emission problem for the hard-surfaced body (surface temperature
    and spectrum at lapse $1/3$) is re-opened under OPEN-GR-RCORE-2.
    The remnant census logic and the stability Proposition stand.]}
  ...
  \item \label{op:echoes}
    ...
    \emph{[Status note added Patch 3305 (W-D), 20 Aug 2026.  DELIVERED
    by companion GR-1d (18 Mar 2026) and then CORRECTED by its V3
    amendment (Patch 3302): this item's own gloss ``$r_{\rm core} =
    r_S/2$'' and the quoted $\ln(r_S/\lP)$ delay are exactly what the
    V2.3 corrigendum of this paper (Remark~\ref{rem:core_relabel}) and
    CONV-030 corrected --- the reflecting surface sits at areal
    $\tfrac{9}{8}r_S$, outside the never-formed horizon, and the delay
    is $\Delta t = (3/2 + 8\ln 2)\,GM/c^3 \approx 2.15 \pm 0.14$~ms
    for GW150914, with $|R_{\rm core}| = 1$ now derived rather than
    assumed.  Panel record: CONV-030, 5--0 on every bottom line.]}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{CPP field equation} (rigorous,
    Proposition~\ref{prop:field_eq}, corrected Patch 3262): the
    self-consistency condition on the LSP distribution is the
    harmonicity of the logarithmic lapse,
    $\nabla_\lambda\nabla^\lambda N = \text{source}$ with
    $N = \ln\sqrt{-g_{tt}/c^2}$ --- algebraically equivalent to the
    flat-lattice census equation of T-1.  It reduces to the linearised
    Einstein equations in the weak-field limit.
  ...
  \item \textbf{Frame-dragging} (Proposition~\ref{prop:kerr}):
    ...
    $V_i$ curl; the Lense--Thirring limit is derived
