...
% CHANGELOG:
%   v3.7  2 Sep 2026 — (Patches 3366-3373, CONV-038 5/5) CP Exclusion Rule
%     RETIRED (founder R-EXCL-RETIRED; never in c01); new remark
%     rem:exclusion_corrigendum in Sec. Effective Horizon; horizon
%     identification superseded by GR-1c Corrigendum 2; no equation changes.
%   v3   15 Mar 2026 — prior shipped text.
%   v3.2  8 Aug 2026 — (Patch 3033, O-7 at-enactment sweep) note
%     status converted: AP-4 founder-ratified Patch 3032.
%   v3.1  8 Aug 2026 — (Patch 3030, F-SW-7 R-C fix pass) glossary
%     reconciliation note added to the LSP + DI-bit entries: payload =
%     {origin address, E, S} per AP-4a; stress magnitude/direction =
%     receiver-computed registers per AP-4d (panel-endorsed,
%     ratification pending); the four-quantity LSP statement reads
%     substantially true as written. No derivation touched.
...
\date{8 August 2026 (amended through 2 September 2026) --- Version~3.7\\[2pt]
{\small Version~3.3 --- 16 August 2026 (Patch 3204, F-SW-10 R-1 fix pass:
OBL-CAL-LABEL applied to the cosmological-constant future-direction item per
R-CC-CALIBRATE, Patch 3125; mathematics untouched)\\[2pt]
Version~3.4 --- 20 August 2026 (Patch 3293: figure rendering repaired --- the
three figure assets existed only as SVG and no \texttt{graphicspath} was set,
so the paper had never rendered them; PDFs committed and path added.
No technical content changed.)\\[2pt]
Version~3.5 --- 20 August 2026 (Patch 3294, OPEN-GR-PPP-1 W-D: dated status
notes added to Open Problems (1)--(3), which the arc's own later companions
had overtaken.  Original item text retained verbatim; notes name the
delivering companion and its limits.  No technical content changed.)\\[2pt]
Version~3.7 --- 2 September 2026 (Patches 3366--3373, CONV-038 5/5: the CP
Exclusion Rule named in Section~\ref{sec:BH} is the pre-demotion postulate
--- THEO-1 in the theorem registry is the live form; corrigendum remark
added; horizon identification superseded by GR-1c Corrigendum~2.  No
equation changed.)}}
...
vector $\mathbf{V}$ alongside the absolute SSV scalar
...
and $\mathbf{V}$ sources $g_{ij}$ (spatial curvature).
...
\textbf{Load-bearing:} the TWO-COMPONENT LSP broadcast (temporal +
spatial channels) supplies both weak-field potentials --- the
factor-of-two in light bending that separates this arc from any
scalar-only variant lives here; the PSR response at second order
supplies the Einstein corrections. \textbf{Smoothed:} discreteness
invisible; only the two-channel structure survives.
\textbf{Macroscopic shadow:} linearised Einstein gravity is the
two-channel census read at first order beyond Newton.
...
\item[Grid Point (GP):] A fixed, immovable node of the discrete
  ...
  at sub-Planck spacing (the fine scale of the canonical nested
  hierarchy of SR-1 \S Grid Resolution; the coarse $l_P$-scale motif
  is the per-Moment reach ceiling). Each GP receives CP State Registers from
...
\item[Space Stress Vector (SSV):] The energy density of CP
  ...
  $\mathbf{V}$ governs spatial curvature and CP
...
\item[Planck Sphere Radius (PSR):] The effective step size available
  ...
  (\emph{convention note, SR-1 Patches 2471/2474:} $\alpha$ cancels in the
  product $k\,\Delta|\SSV|_{\rm abs}$; the pair $(k,\Delta\text{SSV})$ is
  convention-matched, and every use of $k$ in this paper occurs inside that
  product, so no result here depends on the convention). All
...
\item[Lattice State Packet (LSP):] The four-component broadcast
  ...
  \[
    ...
    |\SSV|_{\rm abs},\; \mathbf{V}\bigr).
  \]
  The scalar $|\SSV|_{\rm abs}$ sources gravitational time dilation
  ($g_{tt}$); the vector $\mathbf{V}$ sources spatial
  curvature ($g_{ij}$). In flat space $\mathbf{V}$
...
\item[DI-bit (Distributed Information):] The predecessor broadcast
  ...
  \emph{Reconciliation note (v3.1, Patch 3030) for this and the LSP
  entry above.} Under the AP-4 payload respecification (panel-endorsed
  at CONV-013; founder-ratified Patch 3032), the DI-bit
  payload is \{origin address, $\mathbf{E}$, $\mathbf{S}$\} — a static
  snapshot imprinted by the origin GP — and the receiver extracts
  $V_i = \mathbf{E}+\mathbf{S}$ and computes
  $|\SSV|_{\rm abs}$ per AP-4b. The LSP's four components read
  accordingly: $(\mathbf{x}_{\rm GP}, t_{\rm abs})$ is AP-4a's origin
  address, and the stress magnitude and direction are the
  \emph{receiver-computed} registers (AP-4d), reconstructed each Moment
  from arriving payloads rather than riding as additional messenger
  content. In AP-4's own words, the payload is ``an LSP in miniature'':
  the four-quantity statement of this paper is substantially true as
  written, with the carriage mechanism now made precise. The
  ``predecessor carrying only the scalar'' phrasing above is the SR-era
  minimal-content reading that AP-4a supersedes. No derivation in this
  paper consumes the carriage mechanism; the weak-field results are
  unchanged.
...
$\mathbf{V}$ provides the missing component.
...
In flat space, $\mathbf{V}$ averages to zero over the
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{fig1_lsp_metric.pdf}
  ...
  dilation. The vector component $\mathbf{V}$ (green)
...
\begin{align}
  ...
  g_{ij}(\mathbf{x}) &\;=\; \delta_{ij}\bigl(1 +
    k\,|\nabla\mathbf{V}|_{ij}(\mathbf{x})\bigr),
  \label{eq:gij}
\end{align}
where $k = \lP^3/\EP$ is the CPP coupling constant
\citep[Eq.~1]{abshier2026sr} and
$|\nabla\mathbf{V}|_{ij}$ is the spatial gradient tensor
of $\mathbf{V}$.
\end{definition}
...
$|\nabla\mathbf{V}|_{rr}(r) = GM/(k\,r\,c^2)$
...
\begin{itemize}[noitemsep]
  \item $|\SSV|_{\rm abs}$ broadcast $\to$ $g_{tt}$ perturbation
    $\to$ $0.875$~arcsec.
  \item $\mathbf{V}$ broadcast $\to$ $g_{rr}$ perturbation
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{fig2_lensing.pdf}
  ...
  from $\mathbf{V}$ --- each contribute 0.875~arcsec,
...
pattern, with both $|\SSV|_{\rm abs}$ and $\mathbf{V}$
...
the $\mathbf{V}$ component of the LSP propagates at $c$
and carries energy.
...
\begin{remark}[Corrigendum: the CP Exclusion Rule is retired; the
horizon identification is superseded --- CONV-038, Patches 3366--3371,
1--2 September 2026]
\label{rem:exclusion_corrigendum}
The \emph{CP Exclusion Rule} named in this section and in the
definitions above is the pre-demotion \emph{postulate}: on 13~April
2026 it was demoted to a theorem, THEO-1 (CP Non-Persistent
Co-Occupation, \texttt{theorem-registry.md}; co-occupation is not
forbidden but lasts one Absolute Moment), which is what the founder
means by ``we used the ZBW effect instead'' (rulings R-EXCL-RETIRED,
1~September 2026, and R-COOCCUPATION-FORCED, 2~September 2026, verbatim
in \texttt{founders\_voice/}; ledger row in \texttt{axiom-registry.md}
\S``Retired rules'').  Its cited source~\citep[{\S}2]{abshier2026am}
has never contained it.  The sentence ``GPs become unavailable as
destinations for infalling CPs, which are deflected to neighboring
GPs'' is superseded by the founder's replacement mechanism: a CP
displaced onto an occupied GP superimposes for one Moment and is moved
per the local $V_i$ the next.  The identification
$k\,\Delta|\SSV|_{\rm abs} = 1$ at $r = r_s$ in
Eq.~\eqref{eq:horizon} is superseded by GR-1c Corrigendum~2
(saturation at isotropic $\bar r = \mu$, areal $9\mu/4$, outside the
would-be horizon, which never forms).  $\text{SSV}_{\rm crit}$
\citep[Eq.~1]{abshier2026sr} is retained as the register cap; the
founder has ruled the floor finite (R-FLOOR-FINITE) and has no
argument for the cap's magnitude (R-CELL-SIZE-OPEN, 2~September 2026);
the map from $\text{SSV}_{\rm crit}$ to the saturation variable is
OPEN-GR-FLOOR-1(c), with the admissible window $0.536 < u_{\rm max}
\leq 1$ (GR-1c Corrigendum~3).  The original text above is preserved
verbatim (anti-erasure).
\end{remark}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{fig3_psr_horizon.pdf}
...
\begin{itemize}[noitemsep]
  \item \emph{LSP}~\eqref{eq:LSP_explicit}: adds
    $\mathbf{V}$ to the broadcast; backward compatible
...
\emph{[Status note added Patch 3294 (W-D), 20 Aug 2026.  STILL OPEN as
stated, and substantially advanced.  The general CPP field equation was
derived from the messenger census (T-1, Patch 3258; ratified CONV-027,
consolidated in companion GR-1j), and the corrected form of the GR-1c
Proposition was proven algebraically equivalent to it.  What remains
untouched is exactly what this item names: whether that structure
produces the Ricci tensor of the exact Einstein equations.  Registered as
\texttt{op:einstein}.]}
...
\emph{[Status note added Patch 3294 (W-D), 20 Aug 2026.  DELIVERED by
companion GR-1c (17 Mar 2026), Theorem~1: the unexpanded nonlinear PSR
formula with the exact shell-broadcast source yields the isotropic
Schwarzschild metric without approximation.  The non-singular interior
this item asks about is GR-1c Theorem~2: CP Exclusion bounds
$\PSR_{\rm eff} \geq \lP/2$, giving a Planck-density core at
$r_{\rm core} = r_S/2$ in place of the singularity.  Note that neither
theorem has been externally reviewed --- CONV-027 examined GR-1c's
field-equation Proposition only.]}
\emph{[Addendum, Patch 3309, 20 Aug 2026.  Both sentences of the note
above were overtaken within the week.  (a)~Coverage: GR-1c Theorems~1
and~2 received their first external review at CONV-030 --- Theorem~1
SOUND 5--0, Theorem~2 SOUND-WITH-RELABELING 5--0.  (b)~The
``$r_{\rm core} = r_S/2$'' gloss was the relabeled defect: it is the
\emph{isotropic} radius, with areal image $\tfrac{9}{8}r_S$ ---
outside the would-be horizon, which never forms (GR-1c V2.3
corrigendum; founder-ratified).  The delivered result this item asked
for is thus stronger than the 3294 note could record: not a
non-singular interior behind a horizon, but a horizonless maximally
compact body at the Buchdahl radius.]}

\textbf{(3) Kerr metric.}  Rotating bodies require vorticity in
$\mathbf{V}$, encoding frame dragging as angular momentum
of the SSV field.
\emph{[Status note added Patch 3294 (W-D), 20 Aug 2026.  DELIVERED by
companion GR-1f (18 Mar 2026): the azimuthal component
$k(V_i)_\phi = GJ\sin^2\theta/(c^2r^3)$ gives the
Lense--Thirring term exactly in the weak field, and the Kerr bound
$J \leq GM^2/c$ follows from broadcast subluminality.  Companion GR-1g
extends to Kerr--Newman.  Caveat carried forward: GR-1f shows the full
Boyer--Lindquist form \emph{consistent} with the mechanism at all orders
but does not derive $\Sigma$ and $\Delta$ from the CPP field equation
(\texttt{op:allorders}, open).]}
...
\citep{abshier2026series}.  \emph{[Calibration label (Version~3.3, Patch
3204, F-SW-10 R-1 fix pass; OBL-CAL-LABEL under R-CC-CALIBRATE, Patch 3125,
CONV-020-endorsed).  This identifies a candidate \emph{mechanism} only.  The
\emph{magnitude} of $\Lambda$ is ruled NOT derivable from first principles at
current knowledge: the Sea spacing is a kinetic equilibrium whose
empirically-fixed value $d_s^{\rm emp} = 4.636$ Planck lengths is
\textbf{cosmology-calibrated, never predicted}, and is to be distinguished
from the separately measured floor $d_s^{*} = 2.450 \pm 0.091$.  The
floor-offset is a measured property that a future kinetics derivation must
reproduce.  No claim to bracket or close the orders-of-magnitude gap is made
or implied here; the earlier ``zero-parameter mechanism brackets the observed
$\Lambda$'' language was withdrawn at CONV-020.]}
...
\begin{enumerate}
  \item \textbf{LSP} \citep{abshier2026mass,abshier2026grav}
    (rigorous): $\mathbf{V}$ derived from
  ...
  \item \textbf{Weak-field Schwarzschild}
    ...
    $|\SSV|_{\rm abs}$, $g_{rr}$ from $\mathbf{V}$,
...
$\mathbf{V}$ alongside the scalar $|\SSV|_{\rm abs}$ ---
