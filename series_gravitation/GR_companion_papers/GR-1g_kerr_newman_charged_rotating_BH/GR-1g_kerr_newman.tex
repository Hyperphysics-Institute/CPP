...
% V1.1 (Patch 3307, 20 Aug 2026): W-D status notes on Open Problems 2-4
%   + dated F-R1/C* dependency-audit notes (CONV-030 adoption 4). Audit:
%   the KN bound and extremality remark consume horizon evaluation + the
%   c-vs-c_* ceiling (same two exposures as GR-1f's thm:kerr_bound);
%   horizon-merging language describes would-be surfaces. Exterior metric
%   structure and the RN limit unaffected. Original text verbatim, notes
%   inline (anti-erasure). Founder-ratified CONV-030 basis.
% ==================================================================
...
\textbf{Load-bearing:} the charged rotating census --- electromagnetic
SSV riding the same broadcast --- supplies the Kerr--Newman geometry;
charge and mass enter the one formula through the same
$\Delta|\mathrm{SSV}|$ bookkeeping. \textbf{Smoothed:} discreteness
invisible. \textbf{Macroscopic shadow:} the unified charged-rotating
solution is one census with two contributions, not two mechanisms.
...
charge $Q$ is:
\begin{equation}
  k\,(V_i)_r(r) \;=\; \frac{k_e Q_{\rm SI}^2 G}{c^4 r^2}
  \;\equiv\; \frac{\rQ^2}{r^2},
  \label{eq:ssv_charge}
\end{equation}
...
(companion~11).
\emph{[Dependency-audit note added Patch 3307, 20 Aug 2026 (CONV-030
adoption 4).  This remark and the KN bound inherit the two exposures
flagged at GR-1f's Kerr-bound theorem: the saturation condition is
evaluated AT the outer horizon, which under the ratified horizonless
reading (GR-1c V2.3; CONV-030) never forms, and the ceiling is taken
as $c$ where the ratified R-CSTAR-MAP gives local
$c_*(\bar r) \leq c$.  The horizon-merging statement
$r_+ = r_- = M$ describes would-be surfaces.  The exterior KN metric
structure, the RN limit, and the additive three-source pattern are
unaffected.  Bound status for the horizonless charged spinning body:
open, with OPEN-GR-RCORE-2(iv).]}
\end{remark}
...
\begin{itemize}[noitemsep]
  ...
  \item The \emph{radial charge SSV} $k(V_i)_r = \rQ^2/r^2$
...
\begin{enumerate}
  \item \label{op:allorders}
    ...
    |\Delta\SSV|_{\rm scalar}^2 + |(V_i)_\phi|^2 +
    |(V_i)_r|^2$ generates the exact $\Sigma$ and
  ...
  \item \textbf{Charged Hawking evaporation.}
    ...
    companion~10.
    \emph{[Status note added Patch 3307 (W-D), 20 Aug 2026.  The
    baseline this item defers to has been RE-OPENED: companion GR-1e's
    V1.1 amendment (Patch 3303, after CONV-030) applies the horizonless
    reading --- no horizon forms, and the emission mechanism for the
    hard-surfaced body is itself an open problem (OPEN-GR-RCORE-2)
    before its charged generalization can be posed.  STILL OPEN, now
    two layers deep.]}
  ...
  \item \textbf{Kerr-Newman echoes.}
    ...
    \emph{[Status note added Patch 3307 (W-D), 20 Aug 2026.  The
    uncharged baseline has CHANGED: GR-1d V3 (Patch 3302,
    CONV-030-ratified) relocates the reflecting surface to areal
    $\tfrac{9}{8}r_S$, replaces the delay with
    $(3/2 + 8\ln 2)\,GM/c^3$, and derives $|R| = 1$.  The KN extension
    remains open; the quoted $\rQ^2/r_S^2$ correction estimate is
    inherited from the superseded geometry and unverified against the
    amended baseline.]}
  ...
  \item \textbf{Kerr-Newman superradiance.}
    ...
    \emph{[Status note added Patch 3307 (W-D), 20 Aug 2026.  The
    uncharged mechanism was DELIVERED by companion GR-1h (18 Mar 2026,
    same batch).  Caveats carried: both $\Omega_+$ and $\Phi_+$ are
    defined at the outer horizon, which under the ratified horizonless
    reading never forms; the threshold's surface-form restatement and
    the ergoregion stability of the spinning perfect reflector
    ($|R| = 1$, GR-1d V3) are OPEN-GR-RCORE-2(iv).]}
\end{enumerate}
