...
% V1.2 (Patch 3327, 21 Aug 2026): Settled notes on founder-ratified
%   CONV-032 (5/5): Kerr exclusion surface derived (Patch 3320,
%   conditional on A1-A3); Kerr bound relocated to the derived surface;
%   ergosphere-row supersession (censorship: surface strictly outside
%   the ergosphere at every spin); Penrose/ergoregion-stability note
%   resolved by the censorship theorem. All settled content explicitly
%   conditional-on-A1-A3 (OPEN-GR-RCORE-4). V1.1 notes retained
%   verbatim, settled notes appended (anti-erasure).
% V1.1 (Patch 3306, 20 Aug 2026): W-D status notes on Open Problems 2-4
%   + dated F-R1/C* dependency-audit notes (CONV-030 adoption 4 executed:
%   audit for hidden horizon/interior assumptions BEFORE the notes).
%   Audit findings: thm:kerr_bound consumes (a) evaluation AT the outer
%   horizon and (b) the ceiling c-vs-c_*; sec:horizons tabulates surfaces
%   that never form under the ratified horizonless reading; the Penrose
%   discussion consumes "falling into the horizon". Exterior weak-field
%   results (Lense-Thirring, GP-B) unaffected. Original text verbatim,
%   notes inline (anti-erasure). Founder-ratified CONV-030 basis.
% ==================================================================
...
\date{18 March 2026 (status notes 20--21 August 2026) --- Version~1.2}
...
Boyer-Lindquist, V_i azimuthal component, Kerr bound, extremal
black hole.
...
direction, producing a net azimuthal $V_i$ component in
...
\textbf{Load-bearing:} rotational SSV --- the broadcast census of a
ROTATING compressed source --- supplies frame dragging and the Kerr
geometry; the same single PSR response, fed a rotating census.
\textbf{Smoothed:} discreteness invisible at astrophysical spins.
\textbf{Macroscopic shadow:} what GR writes as twisted spacetime is
the census of circulating stress, read by the same formula.
...
component to the $V_i$ broadcast.  The scalar $|\SSV|_{\rm abs}$
...
$V_i$.  A rotating source GP broadcasts azimuthal bias to
...
\emph{[Dependency-audit note added Patch 3306, 20 Aug 2026 (CONV-030
adoption 4).  This theorem's proof consumes two inputs now flagged:
(a)~the subluminality condition is evaluated AT the outer horizon
$r_+$, which under the ratified horizonless reading (GR-1c V2.3;
CONV-030) never forms --- the natural evaluation surface becomes the
exclusion surface, whose Kerr-analog location is underived; and
(b)~the ceiling is taken as $c$, while the ratified R-CSTAR-MAP gives
the local substrate wave speed $c_*(\bar r) = c/(1 + k\Delta|\SSV|)$,
equal to $c/2$ at the Schwarzschild surface --- whether broadcast
subluminality binds at $c$ or at local $c_*$ is exactly the question
NOTE-GR-CSTAR-STRONGFIELD raised, now subsumed under OPEN-GR-RCORE-2.
The weak-field results of this paper (Lense--Thirring, Gravity
Probe~B) are unaffected; the bound's status for the horizonless
spinning body is open alongside RCORE-2(iv).]}
\emph{[Settled note, Patch 3327, 21 Aug 2026 (founder-ratified
CONV-032, 5/5).  The Kerr-analog exclusion surface is now DERIVED
(Patch 3320), conditional on three stated census assumptions
A1--A3: the saturation surface $\{s^2 + v^2 = 1\}$ with
$s = 2(1-\alpha)/(1+\alpha)$ (scalar $\equiv$ ratified log-lapse
dictionary, exact at $a = 0$) and $v = \omega\varpi/\alpha$ (ZAMO
dragging speed).  The natural evaluation surface named in (a) above
therefore exists at derivation-conditional grade, and the bound
relocates there as a register-capacity statement: on the derived
surface the census is saturated by construction, so the
subluminality budget that this theorem evaluated at the never-formed
horizon is instead exhausted at the exclusion surface.  Question (b)
($c$ vs local $c_*$) is folded into the same conditionality.  The
census composition (unweighted quadrature) and the rotational
normalization are underived assumptions of that construction ---
OPEN-GR-RCORE-4 --- so this settled content is explicitly
conditional-on-A1--A3, per the panel's adopted prose law.]}
...
$V_i$ drag velocity equals $c$.  Inside the ergosphere,
...
\emph{[Status note added Patch 3306, 20 Aug 2026.  Under the ratified
horizonless reading (GR-1c V2.3 corrigendum; CONV-030), the table above
describes the \emph{would-be} GR surfaces: in CPP the exclusion
surface sits outside the outer horizon in the Schwarzschild case
(areal $\tfrac{9}{8}r_S$), so $r_\pm$ are never reached and the
Cauchy-horizon row describes a region that does not form.  The
ergosphere row survives --- $r_{\rm ergo}$ lies outside $r_+$ and,
pending the Kerr-analog surface location (open), outside the surface
as well.  The Kerr exclusion-surface geometry is underived; registered
under OPEN-GR-RCORE-2(iv).]}
\emph{[Settled note, Patch 3327, 21 Aug 2026 (founder-ratified
CONV-032, 5/5).  The geometry is now derived (Patch 3320,
conditional on census assumptions A1--A3): the exact Kerr identity
$g_{tt} = -\alpha^2(1 - v^2)$ makes the ergosphere the $v = 1$
surface identically, so the census there obeys $F = s^2 + 1 > 1$
strictly --- the exclusion surface lies STRICTLY OUTSIDE the
ergosphere at every spin and latitude (numeric scan minimum
clearance $0.25\,M$; even $\chi = 0.998$ keeps $0.258\,M$).  The
ergosphere row of this table therefore does NOT survive under the
derived surface: no exterior ergoregion exists at any spin,
conditional on A1--A3 (OPEN-GR-RCORE-4 carries the substrate
derivation of the census functional).  The prograde equatorial
photon ring is likewise buried for $\chi \gtrsim 0.55$.]}
...
\emph{[Dependency-audit note, Patch 3306: ``falling into the horizon''
consumes the horizon; under the horizonless reading the negative-energy
fragment is instead absorbed at --- or reflected by --- the exclusion
surface, and with $|R| = 1$ derived (GR-1d V3) the ergoregion-stability
question for a spinning perfect reflector becomes live:
OPEN-GR-RCORE-2(iv).]}
\emph{[Settled note, Patch 3327, 21 Aug 2026 (founder-ratified
CONV-032, 5/5).  Resolved by the ergoregion-censorship theorem
(Patch 3320): conditional on A1--A3, the reflecting surface sits
strictly outside the ergosphere at every spin, so no exterior
ergoregion exists, no negative-energy modes are trapped, and the
ergoregion instability cannot arise at any spin or reflectivity.
The Penrose process as described (requiring an exterior ergoregion)
is unavailable to CPP compact objects under the same conditionality;
rotational-energy extraction survives only through the milder
Zel'dovich surface-superradiance channel (bounded; growth-time
bounds committed under the amended OPEN-GR-RCORE-3).]}

In CPP, the mechanism is transparent.  Inside the ergosphere, the
azimuthal $V_i$ component carries rotational energy at
...
\begin{itemize}[noitemsep]
  ...
  \item The \emph{LSP off-diagonal metric mapping} $V_i
...
\begin{enumerate}
  \item \label{op:allorders}
    ...
    |(V_i)_\phi|^2$ generates the exact functions
  ...
  \item \textbf{Kerr--Newman extension.}
    ...
    is deferred.
    \emph{[Status note added Patch 3306 (W-D), 20 Aug 2026.  DELIVERED
    by companion GR-1g (18 Mar 2026, same batch --- the deferral and
    the delivery shipped the same week without a closing
    cross-reference pass, the W-B finding this note repairs).  GR-1g
    carries the charge-polarity extension explicitly; its own caveats
    are recorded in its W-D notes (Patch 3307).]}
  ...
  \item \textbf{Gravitational wave echoes from Kerr.}
    ...
    \emph{[Status note added Patch 3306 (W-D), 20 Aug 2026.  The
    Schwarzschild baseline this item defers from has CHANGED: GR-1d V3
    (Patch 3302, CONV-030-ratified) relocates the reflecting surface to
    areal $\tfrac{9}{8}r_S$ and replaces the delay with
    $(3/2 + 8\ln 2)\,GM/c^3$.  The Kerr extension remains open and now
    requires the Kerr exclusion-surface location first
    (OPEN-GR-RCORE-2(iv)); the quoted $(a/r_S)^2$ correction estimate
    is inherited from the superseded geometry and is unverified against
    the amended baseline.]}
  ...
  \item \textbf{Superradiance.}
    ...
    \emph{[Status note added Patch 3306 (W-D), 20 Aug 2026.  DELIVERED
    at mechanism level by companion GR-1h (18 Mar 2026, same batch).
    Caveats carried: the threshold $\omega < m\Omega_+$ is defined at
    the outer horizon, which under the ratified horizonless reading
    never forms --- the threshold's surface-form restatement, and the
    stability of a spinning \emph{perfect reflector} ($|R| = 1$,
    GR-1d V3) against the ergoregion instability, are
    OPEN-GR-RCORE-2(iv).]}
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Azimuthal SSV source} (exact, §\ref{sec:source}):
    a rotating mass $J$ sources $k(V_i)_\phi =
