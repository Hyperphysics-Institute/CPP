...
% AMENDMENT (Patch 3303, 20 Aug 2026 — Version 1.1): under the ratified
%   lattice ≡ isotropic identification and the GR-1c V2.3 corrigendum, the
%   core radius r_core = GM/c^2 is the ISOTROPIC radius rbar = mu; its
%   areal image is 9mu/4 = (9/8) r_S — OUTSIDE the would-be horizon, which
%   NEVER FORMS. Horizon-based emission language in this paper is
%   therefore superseded at the interpretive level: emission is from a
%   hard surface at the Buchdahl radius (lapse 1/3, z = 2), and the
%   re-derivation of the emission mechanism for the horizonless object is
%   registered OPEN (see the dated Amendment notice + Open Problems note).
%   UNAFFECTED and promoted: Proposition prop:stability (fixed-point
%   stability) — load-bearing input to the |R_core| = 1 derivation
%   (OPEN-GR-RCORE-1, CONV-030 SOUND-family 5-0). Founder-ratified
%   20 Aug 2026. Original text preserved verbatim (anti-erasure).
% ==================================================================
...
\date{18 March 2026 (amended 20 August 2026) --- Version~1.1}
...
\textbf{Load-bearing:} pair processes in the stressed Dipole Sea at
the horizon supply the Hawking flux; full-GP-occupancy saturation
supplies the terminal Planck remnant --- evaporation cannot destroy
conscious points, so it cannot complete. \textbf{Smoothed:} the
emitted spectrum is thermal at GR's temperature for macroscopic
holes. \textbf{Macroscopic shadow:} information-conserving
evaporation with a remnant endpoint --- the census cannot be
erased.

\subsection*{Amendment Notice (Patch 3303, 20 August 2026)}
This Version-1 text is preserved verbatim below; the following
corrections govern its reading, conditional on the PSR/W2 form and the
ratified lattice~$\equiv$~isotropic identification (T-1 CHARTER).
(i)~The companion-8 core radius $\rcore = GM/c^2$ is an
\emph{isotropic} coordinate radius; its areal image is
$\tfrac{9}{4}\,GM/c^2 = \tfrac{9}{8}\,\rS$ --- \emph{outside} the
would-be horizon, which never forms (GR-1c V2.3 corrigendum; CONV-030).
CPP compact objects are horizonless, hard-surfaced, maximally compact
bodies at the Buchdahl radius (surface lapse $1/3$, redshift $z = 2$).
(ii)~Consequently, this paper's horizon-based emission language (pair
production straddling a horizon; boundary condition at
$r_0 = \rS + \lP$) is superseded at the interpretive level: any
emission is from the hard surface, and the emission mechanism for the
horizonless object --- surface temperature, spectrum, and whether the
thermodynamic termination argument survives in surface form --- is
\emph{re-opened} rather than silently re-derived here
(OPEN-GR-RCORE-2; Open Problems note below).  (iii)~\emph{Unaffected
and promoted:} Proposition~\ref{prop:stability} (fixed-point
stability of the saturated configuration) does not depend on the
horizon reading and is now load-bearing beyond this paper --- it is
one of the two pillars of the $|R_{\mathrm{core}}| = 1$ derivation
(companion GR-1d V3, Amendment section; CONV-030 SOUND-family 5--0).
(iv)~The remnant-endpoint conclusion (evaporation, if any, cannot
destroy conscious points) is census logic, not horizon logic, and
stands.  Founder-ratified 20 August 2026; panel record CONV-030.
...
radiation at temperature $\THaw = \hbar c^3/(8\pi G M k_B)$.  This
...
$\rcore = \rS/2$ \textbf{[Amendment: isotropic radius; areal image
$\tfrac{9}{8}\rS$, outside the never-formed horizon --- see the
Amendment Notice]}.  This inner boundary modifies the Hawking calculation
...
\begin{equation}
  \THaw(M) \;=\; \frac{\hbar c^3}{8\pi G M k_B}
...
One might expect termination when $\THaw \to \TP$ (the Planck
temperature).  This does not occur first.  At $M = \mP$:
\begin{equation}
  \THaw(\mP) \;=\; \frac{\TP}{8\pi} \;\approx\; 0.040\,\TP.
  \label{eq:TH_mP}
\end{equation}
...
in the direction defined by the local $V_i$ vector — which
...
\begin{itemize}[noitemsep]
  ...
  \item The Planck remnant retains the quantum information encoded in
    ...
    $V_i$ vectors and CP positions within the $\sim 2\lP$
...
\begin{enumerate}
  \item \label{op:bogoliubov}
    ...
    \textbf{[Amended, Patch 3303:} the inner boundary is now the
    exclusion surface at areal $\tfrac{9}{8}\rS$ with the derived
    Dirichlet condition ($|R| = 1$, phase $\pi$; GR-1d V3), and the
    problem widens: the emission mechanism itself must be re-derived
    for a horizonless hard-surfaced body --- surface temperature and
    spectrum at lapse $1/3$, and whether Proposition
    prop:termination's geometric argument survives in surface form.
    Registered under OPEN-GR-RCORE-2.\textbf{]}
...
\begin{enumerate}
  ...
  \item \textbf{Geometric termination at $M \sim \mP$}
    ...
    not at a temperature divergence: $\THaw(\mP) = \TP/8\pi \approx 0.04\,\TP$.
...
The identification of the geometric breakdown ($N = 2$ at $M \sim \mP$)
...
stability were established on 18 March 2026.  Grok (xAI) independently
verified all three propositions of this paper and confirmed consistency
with companions 1, 5, 8, and 9.  The author thanks Claude (Anthropic)
for mathematical formulation, numerical verification, and LaTeX
preparation.
