...
\date{21 August 2026 --- Version~1.1}
...
\emph{(i) The comb does not survive the finer calculation.} At
...
$N_{\rm trapped} = 0$ for every exposed mode at $\chi = 0.68$ and
across $\chi \in [0.30, 0.98]$ --- the lengthening retrograde cavity
and the falling barrier top cancel (the integer census at
eikonal-WKB grade).  \emph{(ii) What replaces it} is a mode-resolved
...
the correspondence uncertainty.  \emph{(iv) Untested:} the static
Dirichlet wall has not been tested against surface co-rotation for
\emph{line structure}; the $N = 0$ phase-volume margin makes comb
restoration by co-rotation implausible but uncomputed.
\end{remark}
