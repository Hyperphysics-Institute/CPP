...
**Last updated:** 25 May 2026 (Session 143 Patch 0578 — **F.1 Dynamical Substrate Law v1.0 SHIPPED post-SHIP methods catalogue audit; outcome: no METH-DSL-N entries registered — scope-mismatch finding**). Five candidate METH-DSL-N entries from the F.1 v1.0 SHIP cycle (per `series_umbrella/series_substrate_chirality_arc/dynamical_substrate_law/phase_7B_content_pack.md` §4) were reviewed against the Purpose section discipline ("novel reasoning methods, mathematical techniques, and structural abstractions introduced through CPP **substantive derivation work**"). All five are workflow / publishing-protocol / reviewer-engagement / paper-composition patterns, NOT substantive-derivation methods, and are OUT OF SCOPE for this catalogue: (1) METH-DSL-1 Layer 3 promotion via dependency-graph anchor visualization — paper-composition discipline; (2) METH-DSL-2 `\date{}` line scope-framing pattern — publishing protocol / LaTeX formatting convention; (3) METH-DSL-3 v1.0 SHIP milestone Patch pattern (`.tex` metadata + frozen PDF + comprehensive CHANGELOG) — publishing protocol; (4) METH-DSL-4 PDF-upload-default reviewer engagement protocol — reviewer-interaction protocol; (5) METH-DSL-5 Recurring-pattern diagnostic resolution via upload-format check — reviewer debugging protocol. **Codification targets (workflow-pattern home)**: METH-DSL-4/5 patterns already tracked at OPEN-ORG-019 (Patch 0574 organizational_frontier.md registration) → `templates/operating_system.md` §17 reviewer-protocol codification; METH-DSL-1/2/3 patterns (paper-composition + publishing-protocol; not yet tracked) → propose folding into OPEN-ORG-019 expansion or registering new OPEN-ORG-021 for `templates/operating_system.md` §4 Phase 7 paper-completion discipline codification (deferred for handover consideration; not registered this patch to preserve surgical Phase 7B discipline). **F.1 substantive-derivation contributions to programme-level registries are theorem-level**, not method-level: three theorem candidates THEO-DSL-1 + THEO-DSL-2 + THEO-DSL-3 await `theorem-registry.md` registration at Patch 0581; five to six zero-parameter structural predictions PRED-O-NN await `predictions.md` registration at Patch 0580. **Subdir methods_catalogue (at `methods_catalogue/methods_catalogue.md`, 184 lines)** is a separate layered programme-wide catalogue (METH-L{1,2,3}-NNN identifiers; 19 entries 7 L1 + 8 L2 + 4 L3; explicit physics-derivation-only scope from Patch 0461). The dual-file existence is intentional and preserves distinct purposes (root: paper-of-origin paradigm with METH-CHIR-CONT-N + METH-DSL-N proposed; subdir: layered programme-wide L1/L2/L3) — no reconciliation needed at this patch. Earlier: 20 May 2026 (Session 137 Patch 0509 — **Chirality continuum joint paper v1.0 SHIPPED; METH-CHIR-CONT-1 through -4 paper-of-origin venue final consolidation**). All four METH-CHIR-CONT entries (METH-CHIR-CONT-1 sector-agnostic substrate Wigner-Eckart datum + METH-CHIR-CONT-2 continuum-limit projection map $\Phi$ via Wilson-Fisher block-spin renormalization + METH-CHIR-CONT-3 topological substrate quantity + METH-CHIR-CONT-4 topological-projection argument) have their paper-of-origin venue confirmed at `series_umbrella/series_substrate_chirality_arc/chirality_continuum/chirality_continuum.tex` v1.0 SHIPPED §3. METH-CHIR-CONT-3 (topological substrate quantity) and METH-CHIR-CONT-4 (topological-projection argument) carry **programme-level concept** status per theorem-registry entries — available for future cross-sector Layer 4 closures under OPEN-SD-CHIR-PRIMITIVE umbrella manifestations (iv) thermodynamic causal arrow + (v) cosmological-vacuum asymmetry via THEO-CHIR-CONT-4/-5 candidates and applicable to other substrate-handle inheritance closures programme-wide.
...
**Scope refinement registered Session 138 Patch 0523 (F.2/F.3 viability decision gate §8.3):** the topological-projection argument applies to **static-spatial-object substrate sectors only**. The three closed sectors at chirality continuum v1.0 SHIP (K3-doublet, W-bracelet, qDP/eDP) all involve static spatial substrate-objects — configurations on the 600-cell vertex graph at a fixed temporal slice with no temporal-evolution content. Manifestations (iv) thermodynamic causal arrow and (v) cosmological-vacuum asymmetry are the first cases involving **dynamical substrate-objects** (PCD-cycle-orientation pseudovector, cosmological-nucleation-event temporal structure). For dynamical-object substrate sectors, the substrate handle magnitude depends on a substrate-mechanism link (e.g., $\hat{n} \mapsto \vec{\omega}_{PCD}$ for manifestation (iv)) that is *substrate-dynamics-dependent* per Definition 3.4's exclusion clause for "dynamical degrees of freedom evolving in time"; the topological-substrate-quantity character is unverified absent F.1 dynamical-substrate-law gate closure. The argument therefore requires extension or replacement of METH-CHIR-CONT-4 to apply to dynamical-object sectors; the F.1 sub-question scoping sketch (`series_umbrella/series_substrate_chirality_arc/dynamical_substrate_law/sketches/F1_subquestion_pcd_orientation_link.md`) identifies the required substrate-mechanism work as load-bearing prerequisite. Until F.1 sub-question closes positive (Scenario A per decision gate §7.2 — link derived via substrate-mechanism), METH-CHIR-CONT-4's scope is bounded to static-spatial-object sectors. This scope refinement does not invalidate METH-CHIR-CONT-4's existing applications at chirality continuum §3–§6; it constrains *future* applications and surfaces the structural distinction between static-spatial-object and dynamical-object Layer 4 closures as a programme-level architectural feature.
