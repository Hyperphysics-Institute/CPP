...
**Status:** OPEN (candidate theorem) — registered 27 June 2026 (Patch 0877), from the DM extended-aggregate / 4-wide-cross goalpost campaign (patches 0865–0876; consolidation memo `series_phenomena/cosmology/dark_matter/extended_aggregate_cross_route_consolidation.md` v6). Panel-ratified as the **one residual gap** of the corona retirement (V5-Q: 3 CONFIRM + 1 RESTATE-with-fix; the RESTATE asked exactly this be named, not asserted). **[Patch 0882 — DERIVATION DRAFTED (Layer B), PENDING PANEL: all three RESTATE escape routes closed in the sub-threshold regime — (i) metastable single-particle states sub-critical (V₀/E_eDP~6e-4) → reversible, no real population; (ii) collective modes ungappable (matter gapped at E_eDP; weak well shifts ~V₀²/E_eDP~4e-7 of gap; the only gapless mode, the photon, is charge-sourced and decouples from the neutral Cross-Rod surface at k→0); (iii) any induced condensate BCS-suppressed ~10^−400…−1100 → non-extensive. The neutrality of the surface (genesis 0880) is the load-bearing step: it removes the only gapless channel, confining the response to the gapped matter sector where a sub-threshold well is perturbative. Doc: `OPEN-COSMO-DM-3_derivation.md`; bounds: `code/0882`. Residual (Layer B→A): full 600-cell surface-mode formalization (no anomalous sub-gap STRONG-coupling channel; self-limiting since V₀≪E_eDP is weak coupling). NOT CLOSED until panel ratifies; LEMMA-DM-CROSS-ROUTE-1 stays CONDITIONAL; no swarm/verdict change.]**
