...
### LEMMA-DM-CROSS-ROUTE-1: 4-wide-cross extended aggregate as the σ/m-viable DM morphology — finding-level, CONDITIONAL
**Status:** REGISTERED finding-level (frontier-tracked like **LEMMA-DM-CONSIST-1** / LEMMA-NS-HTHEOREM/BATH; **NO THEO, no `theorem-registry.md` row, no swarm-count change**) — registered 27 June 2026 (Patch 0877). **CONDITIONAL on OPEN-COSMO-DM-3** (ASM-DM-CORONA-LOCALITY). Does **NOT** move CONJ-COSMO-1 off NOT-confirmed; no verdict change.
...
**Scope / honesty:** Layer C; CONDITIONAL on OPEN-COSMO-DM-3. A **microphysics/morphology** finding, not a cosmological-derivation result (OPEN-COSMO-DM-1/DM-2 untouched). Supplies **no discriminating positive signal**; does not move CONJ-COSMO-1. The absolute σ/m for the clean cross spine against the 0.6–2 cm²/g dwarf-core window remains a scaling argument, not a hard figure (memo §9). If ever lifted to a hard, discriminating σ/m, re-scope (THEO candidacy + possible swarm entry).
