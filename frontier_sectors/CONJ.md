...
### OPEN-COSMO-DM-3: Sub-threshold Substrate-Locality (ASM-DM-CORONA-LOCALITY) — gates LEMMA-DM-CROSS-ROUTE-1
**Status:** **CLOSED — DERIVED (Layer B), PANEL-RATIFIED 4/4 (re-review, Patch 0885, 27 June 2026).** [History: OPEN, registered 27 June 2026 (Patch 0877), from the DM extended-aggregate / 4-wide-cross goalpost campaign (patches 0865–0876; consolidation memo `series_phenomena/cosmology/dark_matter/extended_aggregate_cross_route_consolidation.md` v6). Panel-ratified as the **one residual gap** of the corona retirement (V5-Q: 3 CONFIRM + 1 RESTATE-with-fix; the RESTATE asked exactly this be named, not asserted). **[Patch 0882 derivation drafted → Patch 0883 PANEL-REVIEWED & FIXES FOLDED (27 June 2026): four-model panel returned 3 CONFIRM-and-lift (Grok, Gemini, Copilot) + 1 RESTATE-with-fix (ChatGPT). The RESTATE is HONORED, not outvoted — it identifies a real unexcluded kill route the CONFIRMERS miss. Folded result: (i) sub-critical + no reservoir → no real population (SOLID); the photon half of (ii) — neutral surface decouples from the charge-sourced gapless photon at k→0 — SOLID; but (ii-matter) and (iii) reduce the whole remaining corona risk to ONE sharp residual: **does the 600-cell spine boundary host a near-zero-energy, charge-neutral collective surface mode?** (a near-zero mode evades the V₀²/E_eDP bound regardless of well depth; NOT excluded by V₀≪E_eDP). BCS step downgraded to heuristic (eDP sector is bosonic, not a Fermi-surface problem). RESULT: ASM-DM-CORONA-LOCALITY is now Layer-B–derived MODULO that residual; **LEMMA-DM-CROSS-ROUTE-1 stays CONDITIONAL — NOT lifted — but its condition is reduced from a broad assumption to one specific, generically-disfavored, checkable physical residual (strongly DE-RISKED).** Next work: the 600-cell spine-boundary surface-mode spectrum. Doc: `OPEN-COSMO-DM-3_derivation.md`; bounds `code/0882`; reasoning `0882`–`0883`. **[Patch 0884 — RESIDUAL ADDRESSED: the spine-boundary surface-mode spectrum (§6, code/0884) closes the near-zero-mode route — (A) no mass-sign domain wall (bound aggregate in the same vacuum); (B) no topological protection (gapped BOSONIC neutral sector; the only protected/gapless mode is the charge-sourced photon, decoupled from the neutral surface); (C) a weak neutral boundary (V₀≪Δ) binds only SHALLOW modes near the gap top — lowest charge-neutral surface mode ≈ E_eDP=88 MeV ≫ V₀~50 keV, a clean gap (near-zero would need deep binding ~Δ or a 1-in-1557 unforced fine-tuning). ASM-DM-CORONA-LOCALITY holds at the EFT+topology level; residual reduced to full 600-cell lattice numerics (a formality). LEMMA-DM-CROSS-ROUTE-1 lift now PENDING focused panel RE-REVIEW of §6 (esp. dissenter) — NOT lifted unilaterally.]** **[Patch 0885 — RE-REVIEW RETURNED 4/4 CONFIRM on §6 and 4/4 CONFIRM to lift, including the original dissenter (ChatGPT), who states §6 resolves the exact objection it raised. → OPEN-COSMO-DM-3 CLOSED (derived, Layer B); LEMMA-DM-CROSS-ROUTE-1 LIFTED to UNCONDITIONAL. SCOPE (honest, per the dissenter's caveat): the panel ratified the derivation's INTERNAL LOGIC within the CPP EFT / excitation-spectrum / topological-characterization assumptions — not the correctness of those assumptions, which is separate validation; and "the corona is dead" is precisely "within the CPP EFT, no mechanism supports persistent neutral low-energy surface modes," not an absolute impossibility. Full 600-cell lattice numerics remain optional Layer-A polish. Lemma stays finding-level (NO THEO, no swarm-count change); CONJ-COSMO-1 stays NOT-confirmed (σ/m-viability ≠ a discriminating DM identification).]** No further verdict change; CONJ-COSMO-1 untouched.]**
**The assumption (to derive):** A surface electric potential well shallower than the eDP creation energy (V₀_elec ≪ E_eDP = 88 MeV; here the eDP→spine electric van der Waals depth V₀_elec ~ 34–94 keV is ~1500× sub-threshold) elicits **only reversible, constituent-local vacuum polarization** — already absorbed into the dressed unit mass m_unit — and generates **no persistent real-eDP surface population** and **no extensive (surface-scaling) collective-mode or condensate mass**. Physically standard for a substrate with a mass gap (the Schwinger / supercritical-Z analogy a reviewer invoked); excludes the field-theory alternatives the V<<E argument alone does not (collective surface modes, induced condensates, metastable surface states).
**Why it matters:** it is the sole gap between "the 4-wide cross spine accretes no σ/m-diluting eDP corona" (the cross route's last load-bearing risk) and a closed result. Deriving it upgrades **LEMMA-DM-CROSS-ROUTE-1** from CONDITIONAL to unconditional.
**What a solution looks like:** a substrate theorem (from A1–A11 + PCD/DP-Sea dynamics) that a sub-creation-threshold local electric perturbation has only a reversible polarization response — no real-quantum population, no extensive surface mode — **or** a counterexample (a mechanism populating a sub-threshold well with persistent diluting mass, which would re-open the corona).
**Sector(s):** COSMO, SR (substrate / Sea-dynamics) · **Priority:** MEDIUM (gates the cross-route lemma; not a live framework threat).
**Cross-sector connections:** LEMMA-DM-CROSS-ROUTE-1 (the conditional result it gates); CONJ-COSMO-1 (DM microphysics); the eDP-Sea electric van der Waals (SF-2 electroweak / SF-5 strong, via the f_color/f_electric = (α_s/3α)² channel ratio).
**Files:** `series_phenomena/cosmology/dark_matter/extended_aggregate_cross_route_consolidation.md` (§7/§9), `reasoning/0874.md`–`0876.md`, `code/0874_Vsurf_SF_calculation.py`, `code/0875_rho_spine_rho_Sea_and_corona_reframe.py`.
**Registered:** 27 June 2026 (Patch 0877).
...
**Panel:** four-model review of the v6 consolidation memo — **V5-Q (corona reframing): 3 CONFIRM + 1 RESTATE-with-fix** (fix folded as OPEN-COSMO-DM-3); **beam stiffness / strand retirement / d_f criterion: 4/4 CONFIRM**.
**Scope / honesty:** Layer C morphology finding, now **corona-UNCONDITIONAL** (the gating OPEN-COSMO-DM-3 derived/panel-ratified at Layer B, Patch 0885 — within the CPP EFT/spectrum/topology assumptions, which are themselves separate validation). A **microphysics/morphology** finding, not a cosmological-derivation result (OPEN-COSMO-DM-1/DM-2 untouched). Supplies **no discriminating positive signal**; does not move CONJ-COSMO-1. The absolute σ/m for the clean cross spine against the 0.6–2 cm²/g dwarf-core window is bracketed (band reachable at N_dwarf~tens, velocity-dependent; memo §9/§13), not a single number — that needs the external pins (E_bond via SF-2/SF-5, or kT_form via the relic/epoch calc). If ever lifted to a hard, discriminating σ/m, re-scope (THEO candidacy + possible swarm entry).
**Files:** `series_phenomena/cosmology/dark_matter/extended_aggregate_cross_route_consolidation.md` (consolidation memo, v6+) + `reasoning/0865.md`–`0876.md` + `code/`.
**Registered:** 27 June 2026 (Patch 0877).
...
### CONJ-DPS-4: Mixed qDP–eDP Species Ladder as the Sea's qDP Reservoir (+ core-exclusion and confinement-enrollment sub-conjectures)
**Status:** CONJECTURE — registered 22 July 2026 (Patch 2782). Founder verbatim canonical in `founders_voice/founder_input_core_confirmation_species_ladder_2026-07-22.md`.
**Sector(s):** DPS, SS, DM/COSMO
**Priority:** MEDIUM (elevates if pursued toward the NB-F-1 dial)
**One-line statement:** The Sea's qDPs are held "in solution" by a ladder of mixed species — qDP, 2qDP (chain/square), 4qDP cube; qDP-eDP, 2qDP-2eDP (plane metastable / cube), 4qDP-4eDP (DM element) — with an eDP-coated qDP retaining an open strong edge (conjectured low/no activation barrier to 2qDP-2eDP formation), making 2qDP-2eDP a common reservoir species supplying string-breaking/meson-pair production and setting the DM-vs-baryon branching. Sub-conjectures: (a) 2qDP is excluded from the qCP core interior (only polar/eDP modes inside — consistent with the 2782 §1 core confirmation); (b) 2qDPs are enrolled during confinement to stiffen the string as a meson is pulled apart.
**Empirical anchor:** string breaking / meson-pair production is an experimental fact ⇒ the Sea demonstrably supplies qq̄ on demand; the conjecture names the carrier.
**What a solution looks like:** relative binding/activation ordering for {qDP+qDP, qDP+eDP, 2qDP+eDP→2qDP-2eDP, growth to 4-element}; lifetime/abundance estimate for 2qDP-2eDP; channel-competition kinetics; the branching ratio to DM:baryon at the formation epoch.
**Registered inputs already in corpus (nbf1t input audit):** E_qq map-strength window 40–170 MeV; E_ee inter-element edge banded [0.8 keV, 2 MeV] (η_screen·1.44 MeV; distinct from the 88 MeV eDP internal binding); ordering E_qq > E_ee sign-certain.
**Dependencies:** CONJ-DPS-1 (1:1 eDP:qDP lock supplies the inventory); CONJ-DPS-2 (freeze-out-ordering methodology; do not conflate thermal-equilibrium with kinetic freeze-out statements); c14/c15 (qDP chaining, color).
**Cross-sector connections:** **OPEN-DM-RELIC-1 REOPENING CONTRACT (Patch 2530): a declarative-strength registration of the relic-epoch Sea composition dial via the founder channel reopens RELIC-1 directly into the 2526/2527/2529 frozen apparatus — resolution is then one number (NB-F-1). This conjecture is that dial's microphysics.** Also: string-breaking scale (SS sector check); KINETIC-1 (core-interior response — sub-conjecture (a) testable there); Candidate (B)/Cross-Rod formation channels (competition).
**Falsification routes:** (a) a material activation barrier for 2qDP-2eDP formation (color/screening/geometric — the "no barrier" claim is unproven and must be estimated, not assumed); (b) 2qDP-2eDP short-lived/transient (no accumulating reservoir); (c) channel competition drains the reservoir into growth or baryons; (d) branching incompatible with the observed DM:baryon abundance; (e) sub-conjecture (a) fails under the KINETIC-1 core profile.
**Source of record:** founder verbatim (Patch 2782); assistant-dialogue evaluation preserved as secondary (the four-gap open-work list adopted).
**Registered:** 22 July 2026 (Patch 2782)

---

### CONJ-FP-1: Round-Trip Timing Asymmetry as the Inertial Mechanism (**MAJOR REVISION, Patch 2884 — see banner**)
**⚠ REVISED 2 Aug 2026 Patch 2884.** The founder has ruled the **round-trip propagation asymmetry** the *sole* mechanism, superseding the volume-transfer/slab account this entry originally carried. **ξ_arc is ELIMINATED** — the effect is pure kinematics (the CP moves during a finite-speed out-and-back) and assumes nothing about arc dynamics: no collapse asymmetry, no store-and-release lifecycle, no DP-Sea density. **One unmeasured continuous parameter is replaced by two BINARY structural conditions**, a strict gain in falsifiability. Round-trip times t_fwd = 2r/(c+v), t_aft = 2r/(c−v), asymmetry Δt = 4rv/(c²−v²) ≈ 4rβ — first order in v/c, growing linearly with distance (equal to one Moment at r = (1−β²)/(4β)). **Computed** at `flagship_papers/electromagnetism/code/2884_roundtrip_asymmetry.py`: drive **exactly linear in β** (drive/β constant to four significant figures across β = 0.01–0.10), confirming the founder's kinematics. **CONDITION A —** the drive's sign is the inverse of the Sea response's sign: attractive → drag, **repulsive → forward** of exactly the required form; the founder asserts repulsive, untested. **CONDITION B —** under Liénard–Wiechert propagation the drive is **exactly zero at every β** (10⁻¹⁷), since the LW field of a uniformly moving source points at its *instantaneous* position and retardation is exactly compensated; **the mechanism therefore requires the relay to NOT reproduce LW structure.** **ESCALATION:** if the relay *is* LW-like then SSV_net = 0 for uniform motion and the primitive gives d = 0 — **nothing could coast** — so Condition B is a question about the **substrate's viability**, not merely about inertia. **NEW EXECUTABLE ROUTE replacing the ξ_arc measurement: the AUTOMATON MOVING-SOURCE TEST** — the arc measured the *static* Coulomb profile to ±0.4% under two relays, but whether a *moving* source produces a field pointing at its instantaneous or retarded position has never been run; it is a defined simulation on the committed `2802_automaton2_engine.py`, needs no new physics, and settles Condition B. **Surviving unchanged:** the marginality condition C·PSR = SSV_abs, and stability of the coasting family (B1, no panel seat can execute it — the worker's own job). **Founder verbatim:** `founders_voice/founder_mechanism_roundtrip_timing_inertia_2026-08-02.md`. Full statement (with revision banner): `flagship_papers/electromagnetism/sketches/conj_fp_1_volume_transfer_inertia.md` — path retained deliberately, since renaming would break this registry pointer.

### CONJ-FP-1 (original registration, superseded in mechanism — retained for provenance): Volume-Transfer Mechanism of Inertial Maintenance (Position Plane slab)
ransfer)
**Status:** CONJECTURE — mechanism stated and internally coherent; three named links unproven, each with an executable route. Registered on the founder's own motion after the mechanism was traced to a dependence on an unmeasured substrate ratio.
**Sector(s):** FP (primary); DPS, SR, SM (cross-checks)
**Priority:** HIGH (supersedes the demoted F = κa account as the programme's candidate inertia mechanism; SF-6's tuned inertial-mass passage has no live replacement)
**One-line statement:** Constant velocity is maintained because each Moment a slab of space one displacement-increment thick crosses the Position Plane from the charging (fore) DP-arc volume to the discharging (aft) volume; that transferred slab is the entire net SSV drive, everything else cancelling. Newton I obtains iff **C·PSR = SSV_abs**, where C is (arc density)×(interacting cross-section)×(forward impulse per discharging arc).
**Free parameter:** **ξ_arc ≡ (fully-charged arc separation)/(inter-DP spacing)** — the conjecture's single unmeasured input. ξ_arc ≪ 1 kills the mechanism (symmetric collapse delivers zero net momentum, multipole suppression bites); ξ_arc ~ 1 clears it.
**What a solution looks like:** ξ_arc measured; C·PSR = SSV_abs shown forced (possibly by PSR's own definition, since PSR is *defined* as displacement at full coherence) rather than tuned; and the coasting family shown neutrally stable.
**Dependencies:** OPEN-FP-6-INERTIA-1 (supersedes its mechanism); OPEN-FSELF-CORRESPONDENCE-1; OPEN-CALIB-COUNT-1 (see cost below); AUTOMATON-2 density-dependence characterization (registered).
**Current best lead:** **Route 1** — AUTOMATON-2 measured ZBW turning radii at the lattice's √2 and 2√2 in a run of definite DP density, both in the same units, so **ξ_arc is computable from banked, L-4-exempt data**. *Indication only, NOT banked:* the pair correlation shows the gas is entirely unlike pairs at those radii with no like-charge contact structure, reading as ξ_arc ~ 1 — the regime in which the mechanism clears. This would be the ninth favourable turn of the arc and is treated accordingly. **Route 2** — test whether C·PSR = SSV_abs is forced; note arc density plausibly appears in both C and SSV_abs and **may cancel**, making the condition density-independent. **Route 3 (B1)** — linear stability analysis about the steady coasting solution; dispatched to the panel twice, returned 5–0 REASONED-UNVERIFIED both times, **no seat has an execution environment; it is the worker's job.**
**Cross-sector connections:** Follows the founder's 7 July no-carried-velocity ruling (velocity held holographically in the Sea, not carried by the particle) — inertia becomes a Sea property, not a particle property. **Pre-registered over-determination protocol: ξ_arc is fixed ONCE by Route 1 and must thereafter PREDICT the nominated cross-checks (barrier tunneling rates; Schrödinger compliance of orbital electrons; ZBW spectral signature at physical density) with nothing further adjustable.** If each phenomenon receives its own free parameter that is three fits, not triangulation; ξ_arc must be **over-determined**.
**Cost, registered not absorbed:** a postulate carrying a free parameter is a **second calibration input** unless ξ_arc is derived or shown to measure an already-declared scale. If promoted with ξ_arc fitted, the programme's single-calibration-input framing must be updated across `axiom-registry.md`, `predictions.md`, `theory-overview.md`, `programme_orientation.md` and every dependent zero-parameter claim.
**Falsification routes:** (a) ξ_arc ≪ 1 measured at physical density — symmetric collapse delivers no net momentum and the drive cannot exist; (b) the linear stability analysis showing the coasting family attracting (drag) or repelling (runaway) rather than neutral; (c) C·PSR ≠ SSV_abs when both are computed independently; (d) ξ_arc fixed by Route 1 failing to predict the nominated cross-checks.
**Source of record:** founder verbatim across `founders_voice/` inertia arc (Patches 2862–2870) and the 29 Jul – 1 Aug dialogue; full statement at `flagship_papers/electromagnetism/sketches/conj_fp_1_volume_transfer_inertia.md`. Registered on the founder's own motion to declare it a conjecture rather than press it to a theorem.
**Registered:** 1 August 2026 (Patch 2880)
