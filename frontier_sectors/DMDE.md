...
**Status: OPEN, not started. Priority: discretion (todolist.md TODO-3938-DM).** Formerly the unregistered blocker NB-S3a-1 (S3a, 17 Jul 2026), which existed only in handovers. **Statement:** construct and register rates (σv, rate equations vs expansion) for the founder's chain — qCPs evaporate from SCPs; some re-land on occupied GPs and re-stack (the re-formed SCP emits per GP, AP-4); escapees pair into qDPs; qDPs aggregate into heavier DP-entities — so that the populations at lock-in (unpaired +qCPs → baryons, S3-M1; paired feedstock → DM rings) are computed. **Consumers:** S3a Route α (this lane) and OPEN-EU-PHOTON-GENESIS-1 (EU lane), which needs the unpaired-inventory exponent p in [0.21, 0.48]. **Constraints on any result:** must reproduce S3-M1's retro-predictions (2520 §2: hDP-B excess 2n_b, clouds n_b, U_q(consumed) = 3n_b); must respect the H1 hazard (2520: equilibrium readings dead at e^(−30)–e^(−10⁴); only frozen-inventory kinetics survive); PD-007 (no rate is a free constant). **Founder pointer:** founders_voice (EU) `3934_photon_genesis_picture.md` §4; S3-M1 `relic1_s3_preregistration.md`.
