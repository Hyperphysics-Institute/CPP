...
**Last updated:** 30 May 2026 (Patch 0690 — **THEO-CHIR-TARROW-2 REGISTERED (v1.0, Layer 2.5, provisional) — a DERIVED substrate time-asymmetry mechanism; CANDIDATE W3→W1 upgrade (T-axis), conditional on Mechanism A; multi-AI review REQUIRED before the W-move is claimed. NO closed verdict move — V3 untouched, W3 STANDS pending review.** Artifact `chirality_derivations/theo_chir_tarrow_2.tex` (4-page PDF, clean 3-pass); reuses verify `code/verify_odelta3_kolmogorov_curl.py` (0689). **The mechanism half of TARROW-1:** TARROW-1 placed sign(δ) at W3 with 𝒞_T=No pinned on "no derived mechanism / F.2 physicalization unbuilt" and explicitly did NOT derive the mechanism; TARROW-2 supplies it — the O(δ³) non-reversibility of the Mechanism-A process (THEO-CHIR-TARROW-2 §4, from the 0689 computation: 420/1200 triangular faces carry nonzero per-face Kolmogorov content 2δ³·abc, all touching the 2nd shell ⇒ a steady-state probability current = a time-asymmetry). **Content:** Lemma (order-counting, odd-only, O(δ²)≡0 — corrects 0688); Lemma (per-face 2δ³abc via a+b+c=0 ⟹ a³+b³+c³=3abc); Theorem (420/1200 non-reversible, machine precision); Proposition (𝒞_T=Yes at the Mechanism-A level ⇒ candidate W3→W1, mechanism derivable + direction free per TARROW-1's axiom-level W2-exclusion — NOT W2). **Non-conflict with TARROW-1:** TARROW-1's "no T-odd object" was about static GEOMETRIC invariants; the current is a DYNAMICAL object (from the rates) = exactly the F.2 physicalization TARROW-1 flagged unbuilt — builds it, no contradiction. **Does NOT claim:** a spatial V-move (FI-C-9 stays V3); the arrow direction (W1, not W0); μ²<0 / RP-failure (P-side capacity separate; DB sufficient-not-necessary for RP; RP=OS reflection≠T-symmetry); a CLOSED W-move (v1.0 provisional; the W3→W1 claim is made at review-close, DG-3). Conditional on Mechanism A (axiom, OPEN-FP-F1-2); computed at O(δ³) (one nonzero face breaks reversibility ⇒ order-stable). Falsifiers T2-1..T2-4. **Next: multi-AI review cycle for TARROW-2 (the W3→W1 move claimed at close, per VW-1/2 + TARROW-1 pattern); then the μ²-sign computation on the reviewed foundation.** THEO-CHIR-CAPACITY-1 stays reserved (P-side/μ², untouched). Header count UNCHANGED (chirality sub-corpus changelog-style; no theorem-registry body-table row). Earlier: Patch 0689 — O(δ³) Kolmogorov computation, detailed balance VIOLATED; Patch 0688 — deep engine decomposed.)
