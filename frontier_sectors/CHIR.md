...
**Last updated:** 30 May 2026 (Patch 0680 — **THEO-CHIR-VW-1 DELIVERED** (`chirality_derivations/theo_chir_vw_1.tex`, v1.0, 7-page PDF; verify CHECK 1/2/3 pass): the V2-exclusion↔VW-no-go **unification** + VW-b (det-coset ℤ₂ vectorial) + VW-c (evasion audit) + VW-d (conditional verdict map) + VW-a first-pass. The substrate parity ℤ₂ is unbroken within the substrate axioms — explicit route closed unconditionally (STATUS-2), spontaneous route closed conditional on H1 (reflection positivity) — so the verdict-moving capacity question reduces to **one** question: is the DSL measure reflection-positive? **H1 NOT proved (open residual, may still touch §14.17); single-pass; multi-AI review REQUIRED before any verdict language changes (DG-3); NO verdict move — V3/W3 stand.** Earlier: Patch 0679 — sign(μ²) route SCOPED (VW + susceptibility backstop; THEO-CHIR-VW-1 reserved); Patch 0669 — B-ii SCOPED; Patch 0668 — B-iii capacity⟺sign(μ²) SCOPED; Patch 0665 — B-i review CLOSED 3/3, BRIDGE-1 v1.1; Patch 0663 — B-i DELIVERED; Patch 0662 — bridge SCOPED, CONJ-CHIR-1.)
