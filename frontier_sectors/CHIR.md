...
**Last updated:** 30 May 2026 (Patch 0679 — **B-iii `sign(μ²)` route SCOPED**: a Vafa–Witten reflection-positivity no-go (could FORCE `μ²>0` and settle the spontaneous-substrate-breaking branch by principle, without §14.17) + a chiral-susceptibility computational backstop (`sign(μ²)=sign(χ_η⁻¹)` via the THEO-DSL pipeline); reserved ID **THEO-CHIR-VW-1**; first artifact = the V2-exclusion↔VW-no-go unification. Reflection-positivity (H1) is the open hypothesis; **NO positivity proved, NO `sign(μ²)` computed, NO verdict move — V3/W3 stand**; any verdict-bearing outcome → multi-AI review first (DG-3). Earlier: Patch 0669 — B-ii SCOPED (χ φ⁻¹/φ⁻³ reconciled, B-ii-P shipped, B-ii-T signpost); Patch 0668 — B-iii capacity⟺sign(μ²) SCOPED (Landau); Patch 0670 — OPEN-SM-4 one-line corrected; Patch 0665 — B-i review cycle CLOSED 3/3, THEO-CHIR-BRIDGE-1 → v1.1; Patch 0663 — B-i DELIVERED; Patch 0662 — bridge SCOPED, CONJ-CHIR-1, co-owned with OPEN-SM-4. The bridge's structural skeleton is review-hardened and its three reachable faces are now mapped; the verdict moves only via B-iii.)
