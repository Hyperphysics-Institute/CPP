...
**Last updated:** 30 May 2026 (Patch 0685 — **THEO-CHIR-VW-2 REGISTERED — the δ=0 reflection-positivity anchor + the Osterwalder–Schrader reduction of H1 to VW-a-4; the VW route's reachable boundary marked with a theorem**: `chirality_derivations/theo_chir_vw_2.tex` (v1.0, clean three-pass, 4pp) + verify `code/verify_vw_2_delta0_rp_reduction.py` (CHECK 1/2/3 pass) + reasoning `reasoning/0685.md`. Layer-2.5 structural theorem (re-cut of the former 0684; 0684 ceded to SUSC). **Thm A (UNCONDITIONAL given THEO-DSL-3):** at δ=0 the substrate has outgoing–incoming detailed balance ⇒ a reversible measure is reflection-positive ⇒ **H1 holds at δ=0**, so H1 for the full measure is a persistence-under-δ-perturbation question. **Thm B (Layer 2.5):** given VW-a-1/2/3, all reflection-asymmetry is confined to S_I ⇒ **H1 ⟺ VW-a-4**, δ=0 base case by Thm A; VW-a-5 (RP under the Φ block-spin flow) the continuum lemma. **Coupling (honest):** VW-a-4's candidate sufficient condition = δ≠0 detailed-balance persistence, coupling to TARROW-1 — but RP=unitarity≠T-symmetry, so irreversibility removes the guarantee, NOT RP-violation. VW-a-4 = sign(μ²) is §14.17/F.2-gated. **Convergence with SUSC (Patch 0684):** the independent susceptibility route reaches the same terminus — VW-a-4 (principle) ≡ sign(χ_η⁻¹) (computation) are the same §14.17 object — confirming the capacity gate is structural, not a VW-framing artifact. THEO-CHIR-CAPACITY-1 stays reserved; changelog-style (no theorem-registry body-table row). **NO verdict move — V3/W3 stand.** Earlier: Patch 0684 — SUSC OPENED (chiral-susceptibility backstop; sign bit = mass channel = §14.17-gated; sign(μ²) route fully mapped, reachable capacity work complete); Patch 0683 — VW-a probed, H1 anchored + §14.17 stall; Patch 0682 — VW-1 review CLOSED 3/3 → v1.1; Patch 0680 — VW-1 DELIVERED; Patch 0679 — sign(μ²) route SCOPED.)
