...
**Last updated:** 30 May 2026 (Patch 0683 — **VW-a (H1) PROBED — reflection positivity sharpened into an Osterwalder–Schrader criterion; H1 anchored, residual found, §14.17 stall confirmed → SUSC triggered**: scope sketch `chirality_derivations/sketches/chir_vwa_h1_os_positivity_probe_scoping.md` + reasoning `reasoning/0683.md`. NO positivity proved, NO new theorem, NO verdict move. **Advance:** the substrate current vanishes at δ=0 (idealized H₄-symmetric substrate = outgoing–incoming detailed balance) ⇒ **RP holds at δ=0**, so H1 for the full measure is a *persistence-under-δ-perturbation* question, not from-scratch. OS decomposition (Osterwalder–Seiler S = S₊ + θS₊ + S_I): VW-a-1 (reflection θ = det-coset reflection, lattice symmetry — ✓), VW-a-2 (measure reality — ✓ first-pass, real ℚ[φ]), VW-a-3 (action θ-symmetry — ✓ geometric part), **VW-a-4 (cross-Π transfer-operator positivity — THE RESIDUAL, §14.17-gated)**, VW-a-5 (RP preserved under the Φ block-spin flow — continuum caveat). **The coupling:** VW-a-4's reachable candidate sufficient condition is PCD/substrate detailed balance *persisting* at δ≠0 (⇒ positive transfer operator ⇒ RP ⇒ μ²>0 ⇒ V3 by principle); this pins VW-a-4 to the substrate-arrow question (TARROW-1) — reversible→RP for free, irreversible→guarantee removed (but NOT RP-violation: RP=unitarity, not T-symmetry). The arrow is unbuilt (§14.17 / F.2; W3). **Verdict: VW-a ADVANCES but does NOT close** — a proof needs §14.17 or a δ≠0 reversibility resolution → **DG-2 stall signal → open SUSC** (compute sign(μ²)=sign(χ_η⁻¹) from the connected η–η two-point function via the THEO-DSL pipeline; finite-lattice suggestive, block-spin trend the honest continuum statement). **NO verdict move — V3/W3 stand.** Earlier: Patch 0682 — VW-1 review CLOSED 3/3 → v1.1; Patch 0680 — VW-1 DELIVERED; Patch 0679 — sign(μ²) route SCOPED; Patch 0668 — B-iii capacity⟺sign(μ²) SCOPED; Patch 0665 — B-i review CLOSED 3/3.)
