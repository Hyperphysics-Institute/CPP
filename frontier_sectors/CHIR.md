...
**Last updated:** 30 May 2026 (Patch 0688 — **DEEP ENGINE (1d-β-ii / §14.17 / OPEN-SM-4 (a)/(b)) DECOMPOSED — first reachable gate identified: the O(δ²) curl computation**: scope sketch `chirality_derivations/sketches/chir_open_1d_beta_ii_deep_engine_scoping.md`. NO measure built, NO μ² computed, NO verdict move. **The reframe:** §14.17's "missing measure" is not free — it is the **stationary measure of the Mechanism-A Markov process** (F.1 rate law r(ê)=r₀(1+δ ê·n̂)); the capacity bit sign(μ²) = whether that measure's η-curvature goes negative. **The gate:** equilibrium (detailed balance, RP, μ²>0, V3-by-principle) vs non-equilibrium steady state (probability current = the TARROW-1 arrow, RP-guarantee lifted, μ²<0 possible, V3→V1) is decided by the **Kolmogorov-cycle / discrete-curl content** of the rate field — one finite quantity deciding both the P-side capacity and the T-side arrow (CPT unification made computational). **The find:** the curl is ALREADY computed at first order and **VANISHES** (`dynamical_substrate_law/code/verify_b1q2_curl_content.py`, Patch 0535: zero circulation on all 30 first-shell side faces, I_h-spanning to the full 4D curl) ⇒ detailed balance / RP / μ²>0 at leading order — the leading-order case of VW-a-4's reachable sufficient condition, never connected to the verdict. **First reachable gate:** sign(μ²) lives at O(δ²) (η~δ near the symmetric point), so the verdict-determining order is the **O(δ²) curl** = the deferred OPEN-FP-F1-1 / op:delta-squared (the second-shell, 20-vertex dodecahedral extension; "substantive but methodologically analogous") — a bounded computation, the next-order replay of Patch 0535. Outcome: curl=0 ⇒ V3-by-principle (spontaneous branch closed); curl≠0 ⇒ V3→V1 opens + delivers the substrate arrow. Dictionary D1–D4 (curl⟺Kolmogorov⟺DB; DB⟹RP; RP⟹μ²>0; order-counting) stated for the computation to establish. Cross-sector OPEN-SM-4 half distinct: (b) Reading-C magnitude = P-face twin of Mechanism A, (a) nucleation = cosmological sign-selection, (c) χ/6 shipped; EWSB-identification (CONJ-CHIR-1) off the capacity critical path. **DG → Patch 0689: open the O(δ²) curl computation** (prerequisite: second-shell geometric identities; carries a verify script; the first verdict-moving computation in the arc). Conditional on Mechanism A (axiom, OPEN-FP-F1-2). **NO verdict move — V3/W3 stand.** Earlier: Patch 0687 — VW-2 review CLOSED 3/3 → v1.1 (Q1 restatement); Patch 0684 — SUSC OPENED; Patch 0680 — VW-1 DELIVERED.)
