...
- **OPEN-CHIR-1c:** capture/partnering handedness (audit E19) from a registered dynamical rule (shared with OPEN-CHIR-2d). **Scope sketch (Patch 0639):** `chirality_derivations/sketches/theo_chir_cap_1_scope.md`. Decomposition: capture handedness = **ζ-involution** (registered geometry — ζ^W = `p ↦ φn̂ − p`, ζ^qDP = host-CP inversion + n̂-flip + qCP-sign flip; built from n̂ + A2 + A1 charge-conjugation) **× σ_capture** (the handedness sign; an involution ζ²=1 carries no sign by itself — exact parallel to E20's `ω_PCD = σ_cycle·n̂`). Sub-gaps: **1c-α** (ground the involution in registered geometry, replacing Grok's unregistered `sgn(n̂·(disp×pol))` seed with a registered first-shell-vertex pseudoscalar; near-term, Layer 2/2.5; reserved **THEO-CHIR-CAP-1**) + **1c-β** (the sign σ_capture — the crux). Three live outcomes for 1c-β: **R1** σ_capture = the FI-C-9 frozen enantiomorph (→ E19 *emergent* from {ζ registered, FI-C-9 registered}; one substrate sign sources E19, E20/σ_cycle, n̂-orientation); **R2** σ_capture = σ_cycle (→ *merges* E19+E20); **R3** σ_capture independent (→ E19 a genuine *primitive*, register it). Decisive test: whether the SD-CHIR chirality-operator matrix-element sign is fixed by n̂/FI-C-9 alone. E19 stays **unregistered** until both sub-gaps resolve (no false reduction).
...
- **OPEN-CHIR-2d:** derive the capture handedness (audit E19; shared with OPEN-CHIR-1c) — the deepest unregistered entry, consumed by SD-CHIR-1/2 via ζ^W and ζ^qDP. **Scope sketch (Patch 0639):** `chirality_derivations/sketches/theo_chir_cap_1_scope.md` (see OPEN-CHIR-1c for the involution×sign decomposition and the R1/R2/R3 outcomes). The 1c/2d duplication encodes the open classification (1c = presume emergent; 2d = presume unregistered-primitive); it collapses onto 1c if 1c-β resolves R1/R2, onto 2d if R3.
