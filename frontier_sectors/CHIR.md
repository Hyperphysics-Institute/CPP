...
**Last updated:** 30 May 2026 (Patch 0691 — **THEO-CHIR-TARROW-2 multi-AI review cycle OPENED.** Self-contained review package `chirality_derivations/review/theo_chir_tarrow_2_review_package_v1.0.md` (theorem claims inline + verify script embedded in full + triage battery; self-containment run-verified — embedded script reproduces 420/1200, all checks pass). Triage foregrounds **Q1 (DEEPEST RISK): the capacity-bar inference** — TARROW-1's 𝒞_T=No was "no DERIVED mechanism"; TARROW-2 derives one CONDITIONAL ON MECHANISM A (itself an axiom, OPEN-FP-F1-2) — does "a mechanism conditional on an undischarged axiom" meet 𝒞_T=Yes (⇒ W1), or RELOCATE the No (⇒ conditional-W1 / W3-pending-MA)? + STATUS-1 falsifiable-constraint calibration + is "candidate W3→W1, conditional on Mechanism A" the right registration? (likely Proposition restatement driver). Supporting battery: Q2 (current⇒time-asymmetry; direction tied to sign(δ)⇒W1 not W0), Q3 (Lemma-B rigor: do the 1200 faces generate the 601-dim cycle space? rank-check), Q4 (order-stability), Q5 (TARROW-1 non-conflict — likely calibration: the current is the DYNAMICAL REALIZATION of sign(δ), not a new T-odd object), Q6 (run the script), Q7 (caps/no closed W-move). Reviewer-specific: ChatGPT→Q1+overclaim+registration-language; Grok→SCRIPT-EXECUTED+Q3 rank-check; Copilot→Q5+Lemma-B chain. §8 CONFIRM/RESTATEMENT/FALSIFIER criteria explicit; adjudication on merits not headcount (VW-2 precedent). **NO verdict move — V3/W3 stand; the W3→W1 move is claimed only at review-close (DG-3).** THEO-CHIR-CAPACITY-1 reserved/untouched. Header count UNCHANGED. Earlier: Patch 0690 — THEO-CHIR-TARROW-2 REGISTERED (v1.0); Patch 0689 — O(δ³) detailed balance VIOLATED; Patch 0688 — deep engine decomposed.)
