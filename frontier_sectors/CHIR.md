...
**Last updated:** 30 May 2026 (Patch 0687 — **THEO-CHIR-VW-2 review cycle CLOSED 3/3 (stands at Layer-2.5) → v1.1, with a Q1-driven RESTATEMENT**: reviews `review/reviews-CHIR-VW-2.md`; theorem `theo_chir_vw_2.tex` v1.0→v1.1 (clean three-pass, 5pp; verify CHECK 1/2/3 unchanged) + reasoning `reasoning/0687.md`. **Not a clean confirm:** on the foregrounded Q1 (reflection identity) the panel split — ChatGPT RESTATEMENT-NEEDED, Copilot SOUND-with-clarification, Grok SOUND-no-conflation. ChatGPT + Copilot independently identified the same structure (the OS positivity detailed balance delivers is the Euclidean **time**-reflection Θ_OS; θ = the **spatial** det-coset parity P_det is the *tested symmetry*), and on the merits the fix is restatement-grade (v1.0's Thm A *statement* claimed RP for the spatial parity — stronger than the detailed-balance engine proves). Grok's endorsement relied on an unproven update-time↔spatial-parity-hyperplane identification. **Adopted the restatement on the merits — a correct objection is not outvoted.** v1.1: Θ_OS (time, positivity engine) and P_det (spatial parity, tested symmetry) distinguished throughout; Thm A rescoped to a "δ=0 OS time-reflection positivity anchor"; the OS↔parity step now an explicit (standard-in-form) bridge; Q2 conditional-confinement + Q6 Nelson/OS calibrations applied. **Engine + reduction UNCHANGED; VW-1 not reopened; NO verdict move — V3/W3 stand.** With SUSC-0684 (VW-a-4 ≡ sign(χ_η⁻¹), same §14.17 terminus), the reachable sign(μ²) work is closed out; the remainder is the §14.17/F.2 fork. Earlier: Patch 0686 — VW-2 review OPENED; Patch 0685 — VW-2 REGISTERED; Patch 0684 — SUSC OPENED (sign(μ²) route fully mapped); Patch 0683 — VW-a probed; Patch 0682 — VW-1 review CLOSED 3/3 → v1.1.)
