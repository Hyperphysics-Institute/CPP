...
**Last updated:** 30 May 2026 (Patch 0684 — **SUSC OPENED — the chiral-susceptibility backstop set up; the sign bit is the mass channel = §14.17-gated; the sign(μ²) route is now FULLY MAPPED, both branches terminating at §14.17**: scope/channel-analysis sketch `chirality_derivations/sketches/chir_susc_chi_eta_susceptibility_scoping.md` + reasoning `reasoning/0684.md`. NO susceptibility computed, NO sign asserted, NO new theorem, NO verdict move. **Construction:** χ_η = Σ_x⟨η_xη_0⟩_c; sign(μ²)=sign(χ_η⁻¹) via the Landau identity χ_η⁻¹=V''(0)=2μ². **Channel analysis:** the THEO-DSL pipeline (operating on the fixed 600-cell) gives the vector-current channel (✓) and the kinetic/graph-Green's-function channel (✓ geometric), but sign(χ_η⁻¹)=sign(μ²) lives in the **mass/η² channel** (the even potential curvature — §14.17-gated), and a *connected* correlator needs the *measure* (§14.17-gated; no fluctuations on a fixed configuration). **SUSC-structural** (χ_η = single ℤ₂-even I_h-scalar + 600-cell Green's-function form) reachable; **SUSC-sign** gated; a finite-lattice answer would rest on the unjustified (H-SUSC) (geometric-proxy↔thermodynamic-curvature sign, the SUSC analog of (H5)). **Convergence finding:** VW-a-4 (principle) and SUSC-sign (computation) are the SAME §14.17 object reached by two independent routes — the gate on the capacity bit is *structural*, not a VW-framing artifact. Anti-erasure: refines the 0679 "single-bit target, pipeline computes it" optimism. **Disposition: the reachable CHIR-sector capacity work is complete** — capacity⟺sign(μ²) (0668), value-side axiom-excluded (STATUS-2 V2-exclusion), both sign-routes mapped to §14.17 (VW-1 review-hardened 3/3; VW-a + SUSC). **Actual emergence (V3→V1) genuinely waits on §14.17 / F.2 — the deep 1d-β-ii / OPEN-SM-4 (a)/(b) engine (the sole V3→V1 lever per STATUS-1/2 + the 0652 decomposition). The route did not move the verdict; it established that no Layer-2.5-reachable move can. NO verdict move — V3/W3 stand.** Earlier: Patch 0683 — VW-a probed, H1 anchored, §14.17 stall; Patch 0682 — VW-1 review CLOSED 3/3 → v1.1; Patch 0680 — VW-1 DELIVERED; Patch 0679 — sign(μ²) route SCOPED; Patch 0668 — B-iii capacity⟺sign(μ²) SCOPED.)
