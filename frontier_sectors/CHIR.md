...
**Last updated:** 30 May 2026 (Patch 0689 — **FIRST VERDICT-RELEVANT COMPUTATION OF THE DEEP ENGINE: detailed balance VIOLATED at O(δ³) — the curl-free "V3-by-principle" branch is RULED OUT; a substrate arrow candidate found. NO verdict move (V3/W3 stand).** Computation + finding `chirality_derivations/sketches/chir_odelta3_curl_finding.md` + verify `code/verify_odelta3_kolmogorov_curl.py` (CHECK 1/2 PASS; CHECK 3 reports curl content). **Order-counting CORRECTED (honest amendment to 0688):** the detailed-balance/Kolmogorov log-ratio L=log[(1+δc)/(1−δc)]=2(δc+δ³c³/3+…) is ODD in δ (forward/backward antisymmetry) ⇒ NO O(δ²) term; the first possible violation is **O(δ³)**, not O(δ²). **Per-face formula (derived):** oriented edge-projections satisfy a+b+c=0 (closed loop + uniform edge length 1/φ) ⇒ O(δ¹) vanishes for every face; O(δ³) curl = 2δ³·abc (triple product, via a³+b³+c³=3abc). **Computation (machine precision):** 600-cell (120 V / 720 E / 1200 triangular faces / degree 12); max|a+b+c|=2.2e-16 (DB at O(δ¹)); **max|abc|=0.25, 420 of 1200 faces have abc≠0 (values {1/8,1/4}), all 420 touch the 2nd shell** (consistent with first-shell cancellation). **RESULT: the Mechanism-A process is NON-reversible at O(δ³)** — it carries a probability current around 420 faces. **Physical picture:** the bias is a single-step gradient (O(δ¹) cycle-zero), but log r is nonlinear ⇒ the δ³ term ∝(ê·n̂)³ is non-conservative ⇒ the arrow is a nonlinear effect of the multiplicative rate bias, appearing at the 2nd shell. **Implications (framed, NO verdict move):** T-side — a steady probability current = a time-asymmetry ⇒ a concrete derived candidate mechanism for the **TARROW-1 arrow** (W3 → derived-mechanism candidate); P-side — DB fails ⇒ the VW-2 detailed-balance route to RP does NOT extend to δ≠0 ⇒ μ²>0 no longer forced by this route ⇒ the **V3→V1 route stays OPEN** (curl-free V3-by-principle branch RULED OUT). **CAUTION:** does NOT prove RP fails or μ²<0 — RP=OS reflection≠T-symmetry (VW-2 Θ_OS/P_det); DB sufficient-not-necessary for RP; μ²<0 now possible, not established. **V3/W3 STAND** — only the landscape changed (live target = the μ²-sign computation; the arrow result is theorem-assembly-ready, review next per DG-3). Conditional on Mechanism A (axiom, OPEN-FP-F1-2). NO theorem ID registered; header count UNCHANGED. Earlier: Patch 0688 — deep engine decomposed, first gate identified; Patch 0687 — VW-2 review CLOSED 3/3 → v1.1.)
