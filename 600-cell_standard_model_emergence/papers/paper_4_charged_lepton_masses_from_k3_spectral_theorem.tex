...
$K_3$.  This paper derives the three charged lepton masses from this
theorem.  The derivation requires one experimental input ($m_e = 0.511$~MeV)
and no additional free parameters: the Koide relation reduces three
masses to one.  Given $m_e$, the theorem predicts
$m_\mu = 105.65$~MeV and $m_\tau = 1776.9$~MeV, matching PDG values to
$0.01\%$ and $0.002\%$ respectively.  The Koide phase $\theta = 132.73°$
is calibrated from $m_e$ and is shown by a structural theorem (Session~E)
to be undetermined within the K3+SSV framework.  Its proximity to
$3\pi/4 = 135°$ (the zero-electron-mass critical point) is recorded as
Open Problem~OP-SM-7d.
\end{abstract}
...
\begin{theorem}[Three lepton masses from one]
\label{thm:masses}
The three charged lepton masses $\{m_e, m_\mu, m_\tau\}$ satisfy
the Koide relation $K = 2/3$ exactly (to the precision of current
PDG data, 11~ppm).  Given $m_e$ and $K = 2/3$, the other two masses
...
(Paper~3, K3 Spectral Theorem).  Given $m_e$ and $\theta$, the
amplitude $A$ is determined by~\eqref{eq:A}, and~\eqref{eq:mmu}--\eqref{eq:mtau}
follow immediately.  The only free input is $m_e$; $\theta$ is then the
unique angle satisfying~\eqref{eq:A} with $A > 0$.
\end{proof}

\begin{remark}[Counting free parameters]
The Koide parametrisation has three parameters: $A$, $\rho$, $\theta$.
\begin{itemize}
\item $\rho = \sqrt{2}$: derived (K3 Spectral Theorem).
\item $A$: determined by $m_e$ (one experimental calibration).
\item $\theta$: determined by $m_e$ and $\rho$ via~\eqref{eq:A}.
\end{itemize}
All three masses are encoded in a single number, $m_e$.
The K3 theorem reduces the lepton mass problem from three free
parameters to one.
\end{remark}
...
\begin{proposition}[Lepton mass predictions]
\label{prop:predictions}
With $m_e = 0.51099895$~MeV (CODATA~2018) and $K = 2/3$:
\begin{center}
...
\begin{remark}[Nature of the prediction]
This is a genuine two-from-one prediction: given the electron mass and
the geometrically derived $K = 2/3$, the muon and tau masses follow with
no fitted parameters.  The prediction would fail if the observed masses
did not lie on the Koide circle with $\rho = \sqrt{2}$ — they do, to
11~ppm.  This precision is not assumed; it is the empirical content of
the Koide formula, whose geometric origin is established by Paper~3.
\end{remark}
...
$m_\mu, m_\tau$ from $m_e$ & \textbf{Predicted} & Theorem~\ref{thm:masses} \\
$m_\mu$ error vs PDG & $0.004\%$ & Proposition~\ref{prop:predictions} \\
$m_\tau$ error vs PDG & $0.001\%$ & Proposition~\ref{prop:predictions} \\
...
predicts both $m_\mu$ and $m_\tau$ without fitted parameters.  The Koide
