...
\item \textbf{Open Problem~\ref{op:mq}} (quark mass formula):
  the formula $M_q \sim \sum_l E_{\rm DP}(l)\,\phig^{3(l-1)}$
  uses $k_{\rm SM}$ through the DP binding energy scale.
  The derivation of $k_{\rm SM}$ reduces OP-SS-1 from a two-unknown
  problem to a one-unknown problem (only the ZBW kernel exponent
