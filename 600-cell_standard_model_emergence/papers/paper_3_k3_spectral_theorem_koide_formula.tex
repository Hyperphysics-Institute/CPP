...
% VERSION 4 — P1 (H_ZBW = hbar omega_0 A_K3) derived from C3 symmetry + SSV hopping
%
% v3→v4 changes (Session C, 24 March 2026):
...
of the tetrahedral cage), given one postulate and two derived propositions.
The remaining postulate is (P3)~the $K_3$ eigenstates are thermally equipopulated
at high temperature (DP~Sea thermalisation). Postulate (P1) — the ZBW
Hamiltonian is proportional to $A_{K_3}$ — is now derived from the C3 cage
symmetry (Theorem~1) and the SSV hopping interaction
...
along the connecting edge.  The hopping matrix element between
nearest-neighbour vertices at separation $a = r_{\mathrm{conf}}/\varphi$
is set by the SSV interaction energy:
\begin{equation}
t = \frac{\mathrm{sea\_strength}\times\hbar c}{r_{\mathrm{conf}}}
\equiv \hbar\omega_0.
\end{equation}
This is the same mechanism that generates the uniform hopping amplitude
for the Gell-Mann operators in the strong sector (Theorem~1 of the strong
sector paper), restricted here to the $K_3$ base subgraph.
