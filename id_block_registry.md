...
| **EW** (electroweak) | **4000–4099** | **ACTIVE** — opened at 4000 (**G-EW-BLOCK-4000**, founder ruling 14 Sep 2026: *"begin the EW-t lane problem in a new EW series … in next series block available"*). Block selected by check, not by the table: 3900–3999 was named unallocated by this file's own last row but is occupied by EU to 3940, so the next genuinely free block is 4000–4099 (zero 40xx commits at time of opening). First use Patch 4000 (this registration); `code/next_id.py` BLOCKS taught in the same commit per Rule 6. Scope: `OPEN-EW-7` (W mass-breakdown species dependence) and the `OPEN-SM-4` EW development it gates. | `frontier_sectors/EW.md` |
