...
**Last updated:** 5 May 2026 (Session 16 Phase 4 close: anharmonic K$_3$ $\xi^4$ corrections in Gaussian expansion — first-order PT and all-orders Gaussian-average extension — RULED OUT via **sign theorem** + Rayleigh–Ritz: $f(s) \equiv (1+s)^{-1/2} - 1 + s/2 > 0$ for all $s > 0$ (one-line proof), hence $\Delta E_{\rm anharm}^{\rm all\text{-}orders} = -B_{\rm pair} f(s) < 0$ universally; true cluster ground state in full Gaussian Hamiltonian is provably *more* bound than harmonic estimate, never less. **Tenth programme-level negative result** in OPEN-SS-35 closure programme; **fifth in OPEN-SS-32 ↔ U-shape thread**. **Programme-level closure of Gaussian-K$_3$ framework at fixed cluster geometry**: Phase 3B-B (Session 15) closed harmonic-Hessian-belt-IRREP family at canonical $\sigma$; Phase 4 (this session) closes perturbative-correction family at canonical geometry. Together the entire Gaussian-K$_3$ framework at fixed cluster geometry cannot produce empirical U-shape — provably. **Cluster-geometry shift mechanisms beyond R1** (channels R3 = N-dependent boundary conditions on $R_\alpha$, R4 = cluster shape distortion) promoted to Session 17 Priority 1. Inelastic / out-of-framework channels promoted to Priority 2. Earlier Session 15 Phase 3B-B close: full C$_n$ IRREP decomposition with three belt-IRREP variants RULED OUT via n-vs-N structural argument; **R2 FORMALLY CLOSED**; ninth programme-level negative result; both registered candidates for OPEN-SS-35 sub-question (a) A-scaling closure (R1 + R2) now ruled out. OPEN-SS-35 closure programme has produced 10 programme-level negative results, 1 qualitative cross-paradigm consilience claim, 1 Decoupling Theorem, 1 formal R2 closure, and 1 sign-theorem closure of Gaussian-K$_3$ framework since Session 5).
...
**Purpose:** Track high-leverage moves that have been identified but deferred to keep individual sessions focused. Updated 5 May 2026 Session 16 Phase 4 close (Phase 4 anharmonic K$_3$ $\xi^4$ + all-orders Gaussian extension RULED OUT via sign theorem; **programme-level closure of Gaussian-K$_3$ framework at fixed cluster geometry** — combined with Phase 3B-B's R2 closure, the entire framework at canonical geometry is provably empty of viable U-shape mechanisms; **cluster-geometry shift mechanisms beyond R1 (channels R3, R4)** promoted to next-session priority; OPEN-SS-35 closure programme has produced 10 programme-level negative results, 1 qualitative cross-paradigm consilience claim, 1 Decoupling Theorem, 1 formal R2 closure, and 1 Gaussian-K$_3$ framework closure since Session 5).

### Active queue (next-session ready)

**(A.1) Cluster-geometry shift mechanisms beyond R1 — channels R3 (N-dependent boundary conditions on $R_\alpha$) and R4 (cluster shape distortion) (next-session priority).**
*Priority:* HIGH (the Gaussian-K$_3$ framework at fixed cluster geometry is now provably closed via Phase 3B-B's n-vs-N structural argument + Phase 4's sign theorem; the U-shape mechanism *must* live in either geometric-shift or out-of-framework channels). *Scope:* Single-session for each scoping investigation; multi-session by scope (3–5 sessions) per channel if scoping motivates full first-principles derivation. *Trigger:* Session 16 Phase 4 established the sign theorem and Rayleigh–Ritz variational closure of the Gaussian-K$_3$ framework at fixed canonical geometry. Combined with Phase 3B-B's R2 closure, the entire framework at canonical geometry is empty of viable U-shape mechanisms. R1 (Session 12) tested one specific geometric-shift form ($R_\alpha(A)$ surface-tension-motivated) and ruled it out for sign + U-shape pattern + Decoupling Theorem grounds. R3 and R4 are different geometric-shift forms not yet tested. *Mechanism (R3):* Cluster compression or expansion driven by N-dependent boundary conditions; each J-solid has different "surface" (number of edges per vertex, boundary topology), and equilibrium $R_\alpha$ may shift accordingly. Empirically would manifest as $R_{\alpha,{\rm eff}}(N)$ values close to but not equal to 2.37 fm. *Mechanism (R4):* Cluster shape distortion beyond rigid J-solid assumption; J-solid geometry minimizes pair-K$_3$ energy assuming uniform edge length, but with anisotropic perturbations the equilibrium shape may distort, breaking rigid-J-solid assumption. Could couple monotonically with $N$ via increasing complexity of J-solid edge graph. *Falsifier (each channel):* Parameterize the geometric shift, compute resulting K$_3$ binding shift, compare sign / magnitude / pattern against empirical. F1 (sign): does the geometric shift produce $\Delta E > 0$ in the J-solid range as empirical requires? F2 (magnitude): is the shift in the empirical 5–34% range? F3 (pattern): is the shift monotonic in $N$ across J-solid range, peaking at appropriate $N$? *Anti-priorities sharpened by Phase 4:* (i) no further perturbative anharmonic refinement (ξ⁶, ξ⁸, hybrid PT) within Gaussian-K$_3$ at fixed geometry — universally closed by sign theorem; (ii) no further belt-IRREP-projection variants — closed Phase 3B-B; (iii) no full point group $D_{nh}/D_{nd}$ extension — closed Phase 3B-B; (iv) no SS-9 v0.3 → v0.1 .tex conversion (OPEN-ORG-012) until §7 reformulated for Phase 4 closure layered on Phase 3B-B's R2 closure (§7 has now shifted SIX times in OPEN-SS-32 ↔ U-shape thread); (v) no further $V_{\rm SO}$ refinement within simple K$_3$ + HO + L·S framework. *Registered:* 5 May 2026 Session 16 Phase 4 close, OPEN-SS-35 paragraph in `Research_Frontier.md`. *Companion:* `series_strong/papers/SS-9/sketches/SS-9_OPEN-SS-32_Ushape_phase4_anharmonic_K3_xi4.md` §6.1.

**(A.2) Inelastic / out-of-framework channels for U-shape mechanism — Hoyle-state mixing, surface-energy shape dependence, Coulomb cluster-arrangement effects (parallel new priority).**
*Priority:* HIGH (the second of two remaining channel classes after geometric-shift; multi-session by scope but single-session scoping feasible to identify which channel matches empirical sign/pattern). *Scope:* Single-session for scoping (estimate channel-by-channel magnitudes); multi-session per channel if a candidate is identified. *Trigger:* Same as (A.1) — Phase 4 closure of Gaussian-K$_3$ framework at fixed geometry. The candidate channels outside the framework are: (b) coupling to inelastic excitations (alpha breathing modes, Hoyle-state $0^+_2$ mixing, other non-rigid-cluster physics); (c) physics outside K$_3$ entirely (Strutinsky-like shell corrections from surface-energy shape dependence, deformation-dependent surface area; Coulomb cluster-arrangement effects sensitive to specific geometric arrangement not captured by edge count alone; spin-orbit cluster corrections). *Falsifier (scoping):* Estimate magnitude of each channel for J-solid range $N$. Channels with magnitude << 5% of empirical or with wrong sign/pattern are ruled out at scoping level. Channels with consistent magnitude/sign/pattern get promoted to multi-session derivation. *Registered:* 5 May 2026 Session 16 Phase 4 close, OPEN-SS-35 paragraph in `Research_Frontier.md`. *Companion:* `series_strong/papers/SS-9/sketches/SS-9_OPEN-SS-32_Ushape_phase4_anharmonic_K3_xi4.md` §6.2.

**(A.3) OPEN-SS-35 closure programme — broader continuation.**
*Priority:* HIGH (largest cross-paradigm consilience target the programme has identified). *Scope:* Multi-session, possibly multi-paper. *State at Session 16 Phase 4 close:* First qualitative cross-paradigm consilience claim achieved Session 9 (empirical magic-number sequence reproduced from CPP first-principles via $\hbar\omega^*$ and $V_{\rm SO}$). Decoupling Theorem established Session 12 (A-scaling closure and gap-strength closure are independent open problems). **10 programme-level negative results** ruled out (Routes D, B-γ, 1b, Path (i), R1, Phase 2 model (a), Phase 3A naive full-Hessian, Phase 3B-A fixed-dim belt subspace, Phase 3B-B C$_n$ IRREP decomposition, Phase 4 anharmonic perturbative + all-orders Gaussian). **R2 FORMALLY CLOSED** (Session 15) and **Gaussian-K$_3$ framework at fixed cluster geometry FORMALLY CLOSED** (Session 16, sign theorem). Both registered candidates for sub-question (a) A-scaling closure (R1 and R2) ruled out; **and the entire Gaussian-K$_3$ framework at fixed geometry is now empty of viable closures** — needs new mechanism via geometric-shift channels (R3, R4) or out-of-framework physics ((b), (c)). Sub-question (b) layer 3 gap-strength closure INDEPENDENT of R2 / Phase 4 by Decoupling Theorem (Session 12), unaffected. *Best lead at registration:* CPP analog of spin-orbit coupling from 600-cell coordination + ZBW phase structure. Pattern-6 K$_3$ scale-recurrence at the nucleon-shell-organization scale. *Registered:* 2 May 2026 Session 4 follow-up 3rd sub-arc, OPEN-SS-35 entry in `Research_Frontier.md`. *Companion sketches:* SS-9 sketches OPEN-SS-35 sub-question (a) Ascaling and sub-question (b) Balpha layer1/2/3, plus OPEN-SS-32 ↔ U-shape unification phase 1/2/3a/3b-a/3b-b/4.
