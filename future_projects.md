...
**Last updated:** 5 May 2026 (Session 14 Phase 3B-A close: minimal fixed-dim belt-subspace projection RULED OUT — magnitude × 3 short, pattern-shape anti-correlated within axial polytopes; **eighth programme-level negative result**. R2 reduced to one untested realization — Phase 3B-B (full character-theory IRREP decomposition with belt-IRREP dimension scaling) is sole untested R2 realization with sharpened constraint: must produce non-monotonic-in-belt-size pattern within axial polytopes; small δ at N$_\alpha$=5, large at N$_\alpha$=10. N$_\alpha$=5 overshoot is structurally hardest constraint)
...
**Purpose:** Track high-leverage moves that have been identified but deferred to keep individual sessions focused. Updated 5 May 2026 Session 14 Phase 3B-A close (Phase 3B-A fixed-dim belt-subspace projection RULED OUT; Phase 3B-B promoted to next-session priority with sharpened constraint from N$_\alpha$=5 overshoot insight; OPEN-SS-35 closure programme has produced 8 programme-level negative results, 1 qualitative cross-paradigm consilience claim, and 1 Decoupling Theorem since Session 5).

### Active queue (next-session ready)

**(A.1) Phase 3B-B — full character-theory IRREP decomposition with belt-IRREP dimension scaling (next-session priority).**
*Priority:* HIGH (sharply constrained falsifier; sole untested R2 realization). *Scope:* Multi-session by scope (3–5 sessions); single-session-tractable for the first axial polytope's character-table-based projection construction. *Trigger:* Session 14 Phase 3B-A established that no fixed-dimension belt-subspace construction can produce the empirical pattern within axial polytopes — belt fraction monotonically decreases with N while empirical magnitude monotonically increases. Phase 3B-B must use a construction where belt-IRREP dimension scales appropriately with belt-vertex-count. *Sharpened constraint from Phase 3B-A:* the construction must produce a non-monotonic-in-belt-size pattern within axial polytopes; specifically *small* $\delta_{\rm belt}$ at $N_\alpha = 5$ (empirical $-12\%$) and *large* $\delta_{\rm belt}$ at $N_\alpha = 10$ (empirical $-34\%$). The N$_\alpha = 5$ small empirical softening is the structurally hardest constraint — any belt-IRREP construction that captures the 3-vertex belt's full radial-displacement subspace at $N_\alpha = 5$ overshoots. If structurally impossible (likely if the U-shape mechanism is purely belt-IRREP-projection of the K$_3$-Gaussian-Hessian), R2 is formally ruled out and U-shape mechanism must be sought outside the K$_3$-Gaussian-Hessian framework. *Anti-priorities sharpened by Phase 3B-A:* (i) no further fixed-dimension belt-subspace variants — anti-correlation rules them out collectively as a class; (ii) no higher-$m$ harmonics (m=3, m=4) as incremental enhancement of Phase 3B-A — cannot fix N$_\alpha = 5$ overshoot, only worsens it. *Registered:* 5 May 2026 Session 14 Phase 3B-A close, OPEN-SS-35 paragraph in `Research_Frontier.md`. *Companion:* `series_strong/papers/SS-9/sketches/SS-9_OPEN-SS-32_Ushape_unification_phase3b_a.md` §6.

**(A.2) OPEN-SS-35 closure programme — broader continuation.**
*Priority:* HIGH (largest cross-paradigm consilience target the programme has identified). *Scope:* Multi-session, possibly multi-paper. *State at Session 14 Phase 3B-A close:* First qualitative cross-paradigm consilience claim achieved Session 9 (empirical magic-number sequence reproduced from CPP first-principles via $\hbar\omega^*$ and $V_{\rm SO}$). Decoupling Theorem established Session 12 (A-scaling closure and gap-strength closure are independent open problems). 8 programme-level negative results ruled out (Routes D, B-γ, 1b, Path (i), R1, Phase 2 model (a), Phase 3A naive full-Hessian, Phase 3B-A fixed-dim belt subspace). R2 reduced to one untested realization — Phase 3B-B is the only remaining R2 closure realization untested, with sharpened constraint from Phase 3B-A's pattern-shape anti-correlation finding. *Best lead at registration:* CPP analog of spin-orbit coupling from 600-cell coordination + ZBW phase structure. Pattern-6 K$_3$ scale-recurrence at the nucleon-shell-organization scale. *Registered:* 2 May 2026 Session 4 follow-up 3rd sub-arc, OPEN-SS-35 entry in `Research_Frontier.md`. *Companion sketches:* SS-9 sketches OPEN-SS-35 sub-question (a) Ascaling and sub-question (b) Balpha layer1/2/3, plus the OPEN-SS-32 ↔ U-shape unification phase 1/2/3a/3b-a.
