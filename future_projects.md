...
**Last updated:** 5 May 2026 (Session 15 Phase 3B-B close: full C$_n$ IRREP decomposition with three belt-IRREP variants RULED OUT via **n-vs-N structural argument** (empirical magnitude monotonically increasing in $N$ while $n$ non-monotonic: $n = 3, 5, 2, 3, 4$ for $N = 5, 7, 8, 9, 10$); class-level argument extends ruling to entire belt-IRREP-projection family within K$_3$-Gaussian-Hessian framework. **Ninth programme-level negative result**. **R2 FORMALLY CLOSED — RULED OUT** (all four model-(b) realizations failed; unification hypothesis at canonical $\sigma_{K3}$ falsified). Both registered candidates for OPEN-SS-35 sub-question (a) A-scaling closure (R1 Session 12 + R2 Session 15) now ruled out. U-shape mechanism investigation redirected outside K$_3$-Gaussian-Hessian framework. Session 16 Priority 1 = anharmonic K$_3$ corrections at order $\xi^4$ in Gaussian expansion (most direct extension; scales monotonically with edge count; single-session-tractable))
...
**Purpose:** Track high-leverage moves that have been identified but deferred to keep individual sessions focused. Updated 5 May 2026 Session 15 Phase 3B-B close (Phase 3B-B full C$_n$ IRREP decomposition with three belt-IRREP variants RULED OUT via n-vs-N structural argument; **R2 FORMALLY CLOSED**; **anharmonic K$_3$ corrections at order $\xi^4$ in Gaussian expansion** promoted to next-session priority as most direct extension of framework with edge-count-monotonic scaling; OPEN-SS-35 closure programme has produced 9 programme-level negative results, 1 qualitative cross-paradigm consilience claim, 1 Decoupling Theorem, and now 1 formal R2 closure since Session 5).

### Active queue (next-session ready)

**(A.1) Anharmonic K$_3$ corrections at order $\xi^4$ in Gaussian expansion — U-shape mechanism investigation outside K$_3$-Gaussian-Hessian framework (next-session priority).**
*Priority:* HIGH (most direct extension of Phase 2/3A/3B framework after R2 formal closure; structurally compatible with empirical pattern; single-session-tractable). *Scope:* Single-session for the scoping computation across all eight canonical alpha-chain deltahedra; multi-session by scope (3–5 sessions) if the scoping result motivates a full first-principles derivation. *Trigger:* Session 15 Phase 3B-B established the n-vs-N structural argument that rules out the entire belt-IRREP-projection class within the K$_3$-Gaussian-Hessian framework, formally closing R2 and the unification hypothesis at canonical $\sigma_{K3}$. The U-shape mechanism must be sought outside the framework; the candidate mechanism with most direct extension and structurally-compatible scaling is anharmonic K$_3$ corrections at order $\xi^4 = (\delta r/\sigma_{K3})^4$ in the Gaussian expansion. *Mechanism:* The K$_3$ Gaussian pair-potential is harmonic only at second order in the displacement $\xi = \delta r/\sigma_{K3}$ around equilibrium. The fourth-order correction has form $-(B_{\rm pair}/24) \cdot \xi^4 \cdot \exp(-\xi^2/2)$ at small $\xi$; for an alpha-alpha contact at canonical $\sigma_{K3} = 1.68$ fm and $R_\alpha = 2.37$ fm, the natural amplitude is $\xi \sim \langle \delta r^2 \rangle^{1/2}/\sigma_{K3} \sim 0.5$, putting the cluster in a regime where anharmonic corrections are of order $5$–$10\%$ of harmonic — comparable to the empirical U-shape magnitude. *Scaling argument:* The anharmonic correction acts at every contact, so the total contribution scales with edge count $|E| = 3N - 6$, which IS monotonic in $N$ across the J-solid range (15, 18, 21, 24 for $N = 7, 8, 9, 10$). Structurally compatible with empirical monotonic-in-$N$ pattern, in contrast to belt-IRREP-projection mechanisms whose variance scales with non-monotonic-in-$N$ group structure. *Falsifier:* Compute perturbative anharmonic correction to per-edge zero-point variance using $\xi^4$ term; sum over edges; compare to empirical U-shape. If correction is too small (<10% of empirical magnitude) the mechanism is ruled out. If correction is right magnitude with right monotonic-in-$N$ pattern, scope multi-session first-principles derivation. *Anti-priorities sharpened by Phase 3B-B:* (i) no further belt-IRREP-projection variants within K$_3$-Gaussian-Hessian framework — n-vs-N argument rules out the class; (ii) no full point group D$_{nh}$/D$_{nd}$ extension with improper rotations — same structural argument applies; (iii) no energy-weighted IRREP filtering or higher-$m$ harmonics — same structural argument applies; (iv) no SS-9 v0.3 → v0.1 .tex conversion (OPEN-ORG-012) until §7 reformulated for ruled-out R2 (§7 has shifted FIVE times in OPEN-SS-32 ↔ U-shape thread). *Registered:* 5 May 2026 Session 15 Phase 3B-B close, OPEN-SS-35 paragraph in `Research_Frontier.md`. *Companion:* `series_strong/papers/SS-9/sketches/SS-9_OPEN-SS-32_Ushape_unification_phase3b_b.md` §6.

**(A.2) OPEN-SS-35 closure programme — broader continuation.**
*Priority:* HIGH (largest cross-paradigm consilience target the programme has identified). *Scope:* Multi-session, possibly multi-paper. *State at Session 15 Phase 3B-B close:* First qualitative cross-paradigm consilience claim achieved Session 9 (empirical magic-number sequence reproduced from CPP first-principles via $\hbar\omega^*$ and $V_{\rm SO}$). Decoupling Theorem established Session 12 (A-scaling closure and gap-strength closure are independent open problems). **9 programme-level negative results** ruled out (Routes D, B-γ, 1b, Path (i), R1, Phase 2 model (a), Phase 3A naive full-Hessian, Phase 3B-A fixed-dim belt subspace, Phase 3B-B C$_n$ IRREP decomposition). **R2 FORMALLY CLOSED** (all four model-(b) realizations failed; n-vs-N structural argument extends closure to entire belt-IRREP-projection class). Both registered candidates for sub-question (a) A-scaling closure (R1 and R2) ruled out; needs new mechanism outside K$_3$-Gaussian-Hessian framework. Sub-question (b) layer 3 gap-strength closure INDEPENDENT of R2 by Decoupling Theorem (Session 12), unaffected. *Best lead at registration:* CPP analog of spin-orbit coupling from 600-cell coordination + ZBW phase structure. Pattern-6 K$_3$ scale-recurrence at the nucleon-shell-organization scale. *Registered:* 2 May 2026 Session 4 follow-up 3rd sub-arc, OPEN-SS-35 entry in `Research_Frontier.md`. *Companion sketches:* SS-9 sketches OPEN-SS-35 sub-question (a) Ascaling and sub-question (b) Balpha layer1/2/3, plus the OPEN-SS-32 ↔ U-shape unification phase 1/2/3a/3b-a/3b-b.
