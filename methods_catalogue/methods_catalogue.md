...
**Patch 3807 addition (EU-lane session-close audit, catalog-first, sourced from the session's at-patch reasoning fragments `series_phenomena/cosmology/early_universe/reasoning/3805_ignition_handoff.md` and `3806_bath_depth.md`):** METH-L1-018 — **Direction-only dynamics under a saturated register (magnitude drops out; enumerate the discrete configuration space).** When AP-5's D1 clips the acted-on displacement to the floor, a stack's step has fixed length and only the *direction* of its net field matters; a many-body question then reduces to unit-vector kinematics with a pairing rule, and an unspecified discrete choice (which vertices carry which sign) is handled by running every configuration rather than picking one. First use 3805 (924 sign assignments, three field weightings). Pairs with METH-L2-022 — **Positive control for a null-result instrument.** An instrument that reports "no push / no signal" must first be shown to see a push when one is built in (3805: the twelve-like-sign control expands at ln 1.5 per Moment); a null without its positive control is not a result. Related: 3806's *serial-vs-parallel alternative recorded as excluded* — when a structural reading decides a 70-order question, the rejected reading's number is printed next to the adopted one so a reader sees the stakes.

**Patch 4239 addition (EW-lane session-237 close audit, catalog-first, sourced from `series_standard_model/axiom_maturation/4222_*.md`, `4223_*.md`, `4226_*.md`):** METH-L2-023 — **Symmetry classification by selection, not by sign products.** To test a discrete selection rule (argmax over a neighbour set) for C, P, T, apply the transformation to the register, re-run the selection, and check that the chosen element transforms covariantly over many random states; a rule is even under X iff selection commutes with X on every trial. Replaces multiplying the signs of a score's factors, which cannot see tie structure or non-linear scores. First used at 4222 (`code/4222_critic_4193_hop_selection.py`); confirmed 4193's table cell for cell. METH-L2-024 — **Birth-state test for a two-particle spin observable.** When a mechanism proposes an intermediate "mixing" between two departing spin-½ objects, first check whether the mixing operator commutes with total spin; if it does (any rotationally invariant coupling does), the singlet/triplet content is fixed at creation and the observable discriminates *birth states*, not mixings. Enumerate candidate birth states, evaluate each with the measurement rule, and compare to the full amplitude. First used at 4226 (`code/4226_birth_state_and_swapped_inheritance.py`); excluded the 4205 assignment and selected the swapped one.

**Patch 4257 addition (EW-lane session-238 close audit, sourced from `series_standard_model/axiom_maturation/4240_*.md`, `4242_*.md`, `4247_*.md`):** METH-L2-025 — **Joint-observable exclusion of a reduction family.** When a single structure (one spinor, one motion) feeds two observables, test candidate mechanisms on both at once: 4240/4242 excluded any one-quark current with a convection term by computing g_A and μ_p from the same state (μ_p −42% at g_A ≈ 1.25), leaving the family that reduces the spin projection without an orbital current. METH-L2-026 — **Let a downstream measurement decide an upstream picture.** When two readings of a mechanism differ only in a hidden occupancy or geometry, derive the observable's dependence on it and read the measurement back: 4247's d_iso = 1 − 2f with measured −1 forced f = 1 (the linear −eCP held), choosing between the founder's two recollections without a new ruling.

**Patch 4283 addition (EW-lane Session-239 close audit, sourced from `series_standard_model/axiom_maturation/4261_*.md`, `4269_*.md`, `4272_*.md`–`4276_*.md`, `4281_*.md`):**
- **METH-L2-027 — Seat-orbital Slater density.** When identity is tied to position (CPP colour = vertex), antisymmetrise the seat-attached states. The one-body density ρ = (1/n) Σ_ij (S⁻¹)_ji φ_i*φ_j, in momentum space for g_A and in position space for r_p, carries the Pauli term into both observables. Seat-occupancy rules decide which exchanges exist (4269–4270: none for the proton's u–d pair).
- **METH-L2-028 — A second observable against a free convention.** When a convention (here the breath frequency) moves one observable more than its residual, add an observable that depends on a different moment of the same state. g_A measures the momentum spread, r_p the position width. Where they disagree, look for a third input both depend on (4272–4276: the frame size). Do not fit both conventions to both data.
- **METH-L2-029 — g from the energy split of a circulating charge.** For a massless charge at c with conserved L in a central well at fixed total energy, μ = (q/2)L⟨1/|p|⟩ gives g = ⟨E_total/E_kin⟩ exactly (4281). This turns "what is g?" into "how is the rest energy split between motion and field?".
