...
% Version 2.2 --- 17 August 2026 (Patch 3212):
...
\date{13 March 2026 --- Version~2.1\\[2pt]
{\small Version~2.2 --- 17 August 2026 (Patch 3212: added the generated plain-language appendix glossing the internal programme identifiers used in this paper (founder jargon ruling, 16 Aug 2026). No claim, equation, number or section changed.)}\\[2pt]
...
\begin{equation}
  ...
  \label{eq:PSR}
\end{equation}
The coefficient $k$ encodes the elastic stiffness of the Dipole Sea against SSV
perturbation.  \emph{Correction note (SR-1 Patches 2471/2474):} Version~16's claim
that this value is ``fixed by dimensional necessity alone'' is withdrawn ---
dimensional analysis fixes dimensions, never a dimensionless prefactor --- and $k$'s
numerical value is now understood as a normalisation convention: $k = \alpha\,
\lP^3/\EP$, with $\alpha$ cancelling identically in the physical product
$k\cdot\Delta\SSV$ and $(k,\Delta\SSV)$ inherited as a matched pair.  This paper's
original question survives the correction and becomes sharper: what a referee may
reasonably ask is whether the microscopic Planck-scale interactions of the DP sea
actually produce the \emph{stiffness} $C$ --- a physical quantity, unlike $k$'s
convention-dependent numerical value --- or whether a different stiffness is equally
consistent with the CPP postulates.
...
This second-moment integral is evaluated on a \emph{single} 600-cell motif in
unit-circumradius coordinates---i.e.\ at the coarse, $l_P$-scale tile of the
nested 600-cell hierarchy made canonical in SR-1 (\S Grid Resolution). The
resulting stiffness, and hence $k = l_P^3/E_P$, is a coarse-motif property;
the sub-Planck fine nesting that sets the effective grid resolution is
self-similar ($R/a=\phi$ at every level) and leaves the integral and $k$
unchanged.
...
The factor $\alphageom \approx 0.5594$ is the Voronoi second-moment integral evaluated in circumradius units (\emph{convention note, SR-1 Patch 2474:} the same integral evaluates to $0.2444$ in $l_P$ units --- the number is exact but unit-dependent, so the earlier ``geometric packing efficiency $\approx 56\%$'' physical interpretation is withdrawn; only convention-invariant combinations carry physical content).  At any \emph{fixed} unit convention the integral's value still discriminates between polytopes: no other regular 4-polytope produces this value, its 12-fold coordination and pentagonal faces being unique to the 600-cell.  (The earlier appeal to the Version~16 Appendix~G dimensionality theorem as the selecting argument is retained only with the Patch-2474 scope note: Appendix~G's postulates include exact Lorentz covariance, which SR-1 now concedes is imported, and the theorem itself is flagged unaudited.)
...
formula~\eqref{eq:PSR}.  The stiffness $C$ is fixed by the lattice geometry; the product
$k\cdot\Delta\SSV$ is convention-invariant.  \emph{Correction note (SR-1
Patches 2471/2474):} the earlier text here claimed $\gamma = 1 + k\cdot\Delta\SSV$
is ``fully geometric --- no free parameter remains'' and cited a Monte-Carlo
verification routine.  Both claims are withdrawn: the $\gamma$ content enters
through the $\Delta\SSV$ normalisation (an input, per SR-1 App.~A.8.1), so the
equivalence with $\gamma_{\rm SR}$ holds by construction rather than by geometric
derivation; and the cited Monte-Carlo script as committed was a placeholder stub (one of four
unrecorded-verification citations withdrawn at Patch 2471 --- the run occurred in the
pre-protocol era but was never captured; the surviving verification is the stdlib
$\alpha$-cancellation battery, \texttt{code/2471\_*.py}, 31/31).  The earlier
inline expression $k = 1/(C\lP^3/\EP)$ was also dimensionally inconsistent with
the $\alpha=1$ value quoted in \S1 --- an instance of the matched-pair hazard this
correction removes (mixing $(k,\Delta\SSV)$ conventions rescales $\gamma-1$ by
exactly $\alpha$).  Whether this paper's Coulomb-constant chain
(Section~\ref{sec:implications}) is itself convention-invariant is registered as
OPEN-SR-C02-NORM.
...
\begin{description}
  \item[\texttt{OPEN-SR-C02-NORM}] \hfill \\ c02 derives the Coulomb constant from the stiffness C = α\_geom·SSV\_crit and the shell-broadcast geometry ("No separate electromagnetic coupling constant is introduced; k\_e is determined by C and Q"). α\_geom is unit-dependent (0.5594 per circumradius, 0.2444 per l\_P). If the k\_e chain uses C's numerical value in one unit convention while other lattice quantities enter in another, the result inherits the matched-pair defect (rescaling by α, up to \textasciitilde{}44\%); if the chain is built entirely from convention-invariant combinations, the result stands. Nobody has checked which.
\end{description}
