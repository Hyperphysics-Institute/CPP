...
%   v19     14 Jul 2026 — TRIAGE (patches 2471-2474), CONV-001 5-reviewer panel.
...
\begin{abstract}
Conscious Point Physics (CPP) \emph{represents} special relativistic effects---time dilation, length contraction, and the twin paradox---as consequences of stress-limited displacement in a discrete 4D lattice based on the 600-cell polychoron. Kinetic energy stored as excess Space Stress Vector ($\Delta$SSV) reduces effective Voronoi cell volumes, shrinking the Planck Sphere Radius (PSR) and limiting displacement per absolute Moment. We state plainly what this construction does and does not establish. \textbf{Established:} the 600-cell geometry is reproduced from the binary icosahedral group $2I$ (edge $a = 1/\phi$, coordination $z = 12$, cell volume $V_0$); a Hooke-like low-stress response together with the exact saturation boundary condition motivates the minimal rational constitutive relation $\text{PSR}_{\rm eff} = l_P/(1+k\cdot\Delta\text{SSV})$ within the lowest-order rational class; and---the principal positive result of this paper---the elimination theorem of Appendix~H.1 proves that \emph{no} purely geometric displacement model of the class examined (linear subtraction, corridor exclusion, exact 4D hyperspherical cap) recovers the $v^2/c^2$ scaling that $\gamma_{\rm SR}$ requires. \textbf{Not established:} the Lorentz factor is \emph{not derived here}. It enters as an \emph{input} through the energy-momentum identification of Appendix~A.8.1, which sets $\Delta\text{SSV} \propto (\gamma_{\rm SR}-1)mc^2$; exact Lorentz equivalence then follows by construction. Appendix~H.1 shows this input is unavoidable within the class examined, but proving that an external bridge is necessary does not convert that bridge into a derivation. The framework is consequently \emph{empirically equivalent to standard special relativity by construction} and yields no independent falsifiable predictions at present; the prediction set of earlier versions is withdrawn (Sec.~\ref{sec:empirical_status}). Nor is $k$ an independent prediction: any rescaling $k \to \alpha k$ absorbed into the $\Delta$SSV normalisation leaves $k\cdot\Delta\text{SSV}$---and hence every observable---unchanged, so $k = l_P^3/E_P \approx 2.16\times10^{-114}$~m$^3$/J records a \emph{normalisation convention} paired with the $\Delta$SSV definition, not a derived quantity (Appendix~A.5, Step~3). The functional form is a minimal ansatz within the rational class and is not shown to be unique to the 600-cell: Hooke-like response and saturation fix only the initial slope and the asymptote, and other saturating laws ($e^{-\varepsilon}$, $(1+2\varepsilon)^{-1/2}$, $(1+\varepsilon/n)^{-n}$) satisfy the same two conditions. Deriving the strain--kinematics map $\varepsilon(v)$ from substrate dynamics \emph{without} importing $\gamma_{\rm SR}$ is the outstanding problem of this line of work.
\end{abstract}
...
\textbf{This framework is empirically equivalent to standard special relativity by
construction, and yields no independent falsifiable predictions at present.}
Appendix~A.8.1 establishes $\gamma_{\rm CPP} = \gamma_{\rm SR}$ \emph{identically at all
velocities}. An identity admits no deviation: if the two Lorentz factors agree
everywhere, no measurement can distinguish the frameworks, and no residual remains to
be predicted.
...
(1+\varepsilon)$. The antecedent has not been established. The same conditional status
applies to the Casimir $(l_P/d)^4$ scaling of Appendix~E, which additionally requires an
independent derivation of the 4D spectral measure and boundary conditions before it can
be advanced as a prediction. Neither result inherits validity from the PSR constitutive
formula alone.

\subsection{The outstanding problem}

What must be derived, and has not been, is the \textbf{strain--kinematics map}
$\varepsilon(v)$ --- from Conscious Point dynamics, without importing $\gamma_{\rm SR}$
or an equivalent relativistic identity as input. A satisfactory closure would reproduce
at least the leading $v^2/2c^2$ behaviour and preferably the divergence as $v \to c$.
Appendix~H.1 shows this cannot be done by displacement geometry of the class examined;
it does not show that it cannot be done at all. Registered as
\texttt{OPEN-SR-EPSILON}.

Until that question is answered, the honest characterisation of this paper is: \emph{a
substrate representation of relativistic dilation, conditional on an imported
energy-momentum bridge, together with a rigorous negative theorem constraining what pure
displacement geometry can achieve.}

\section{Conclusion}

This paper presents a \emph{substrate representation} of special relativity within the
quasicrystalline 600-cell lattice of Conscious Point Physics, conditional on an imported
energy-momentum bridge. It is not a first-principles derivation of special relativity:
the Lorentz factor is supplied as an input (Appendix~A.8.1), and Appendix~H.1 proves that
the geometry cannot supply it unaided. By modeling space as an overlapping lattice of 600-cell motifs whose vertices
...
equivalent to standard special relativity \emph{by construction}, and therefore makes no
independent testable claim at present; the prediction set and muon constraint of earlier
versions are withdrawn (Sec.~\ref{sec:empirical_status}). What is established is
...
of this paper---Appendix~H.1 proves that no purely geometric displacement model of the
class examined can recover the $v^2/c^2$ scaling that $\gamma_{\rm SR}$ demands. That
negative theorem is not a limitation of the paper; it is the paper's most secure
contribution, and it sharply constrains any future substrate account of relativistic
kinematics---including CPP's own.

The programme's obligation is now explicit: derive $\varepsilon(v)$ from Conscious Point
dynamics without importing $\gamma_{\rm SR}$. Should that succeed, the representation
offered here becomes the derivation it was previously billed as. Should it fail in a
provable way, that too would be a result. Until then, Conscious Point Physics offers a
coherent mechanical interpretation of relativistic phenomena, conditional on standard
relativistic kinematics as input---not an explanation of their origin.
