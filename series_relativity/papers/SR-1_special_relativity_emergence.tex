...
Appendix~H.1 eliminates the three natural displacement models (exponents $n \in \{1,1,5/2\}$
against the required $n=2$); it does \emph{not} establish class coverage --- that claim
was withdrawn at Patch 2475 when the underlying cap expansion was found to be wrong. The
corrected exponents \emph{bracket} the target, so a displacement geometry with
$V_{\rm excl} \propto f^2$ remains a live candidate (\texttt{OPEN-SR-H1-CLASS}). Registered as
...
the Lorentz factor is supplied as an input (Appendix~A.8.1), and Appendix~H.1 shows that
the three natural displacement models cannot supply it unaided. By modeling space as an overlapping lattice of 600-cell motifs whose vertices
...
it, so the class question is genuinely open and an exclusion geometry with
$V_{\rm excl} \propto f^2$ remains a live candidate (\texttt{OPEN-SR-H1-CLASS}). Within
the three models examined the bridge is required; whether it is required \emph{in
...
is gone, and nothing now excludes an exclusion geometry whose excluded volume scales as
$V_{\rm excl} \propto f^{2}$ --- which by $V \propto r^4$ volume conservation would yield
$\varepsilon \propto f^2$, exactly what $\gamma_{\rm SR}$ requires. \emph{The erroneous
theorem closed a route that may in fact be open.} Registered as
\texttt{OPEN-SR-H1-CLASS}; see also \texttt{OPEN-SR-EPSILON}, for which this is now a
live candidate attack.
