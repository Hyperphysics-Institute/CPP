...
\begin{abstract}
Conscious Point Physics (CPP) derives special relativistic effects---time dilation, length contraction, and the twin paradox---from geometric constraints in a discrete 4D lattice based on the 600-cell polychoron. Kinetic energy stored as excess Space Stress Vector ($\Delta$SSV) reduces effective Voronoi cell volumes, shrinking the Planck Sphere Radius (PSR) and limiting displacement per absolute Moment. This produces relativistic phenomena as consequences of lattice saturation rather than postulated spacetime geometry. The framework reproduces standard SR at accessible energies while predicting deviations at accelerations $\gtrsim 10^{20}g$. We provide a first-principles derivation of the \emph{functional form} of the PSR formula from 600-cell packing geometry: Hooke-like response plus the exact saturation boundary condition fix $\text{PSR}_{\rm eff} = l_P/(1+k\cdot\Delta\text{SSV})$ as the unique lowest-order rational approximant. The coupling constant $k = l_P^3/E_P \approx 2.16 \times 10^{-114}$ m$^3$/J is \emph{not} an independent prediction: any rescaling $k \to \alpha k$ absorbed into the $\Delta$SSV normalisation leaves $k\cdot\Delta\text{SSV}$, and hence every observable, unchanged, so $k$'s numerical value records a normalisation convention rather than a derived quantity (Appendix~A.5, Step~3). Exact Lorentz equivalence follows from the energy-momentum bridge of Appendix~A.8.1, which is \emph{necessary}: the elimination theorem of Appendix~H.1 proves that no purely geometric displacement model recovers the required $v^2/c^2$ scaling. Existing muon storage-ring data at accelerations of order $10^{18}g$ are fully consistent with CPP, constraining any deviation from the Planck-derived value of $k$ by a factor of $10^{16}$ and confirming that the framework survives all current experimental tests.
\end{abstract}
...
Table~\ref{tab:predictions} summarises all five predictions. Each arises from the single expression $\text{PSR}_{\rm eff} = l_P/(1+k\cdot\Delta\text{SSV})$ with no adjustable parameters. The 600-cell geometry fixes the \emph{functional form} of this expression (Appendix~A.5); the coupling constant $k \approx 2.16\times10^{-114}$\,m$^3$/J is a normalisation convention paired with the $\Delta$SSV definition of Appendix~A.8.1, and the predictions depend only on the product $k\cdot\Delta\text{SSV}$, which is invariant under a common rescaling of the pair (Appendix~A.5, Step~3).
...
\caption{%
  ...
  with $k = l_P^3/E_P \approx 2.16\times10^{-114}$\,m$^3$/J, the convention paired
  with $\Delta\text{SSV} = E_{\rm kin}/l_P^3$; only the product $k\cdot\Delta\text{SSV}$
  enters, and it is rescaling-invariant. Fractional deviations scale linearly with acceleration
...
The derivation is supported by analytic expressions for the PSR formula and by a
numerical verification over all 120 vertices (Appendix~A.7) that reproduces the
600-cell geometric inputs ($a=1/\phi$, $z=12$, $V_0$, $\alpha_{\rm geom}$) and
confirms the rescaling-invariance of $k\cdot\Delta\text{SSV}$, together with
explicit clarification of the underlying displacement-reduction process. The framework has been checked against existing muon 
...
\]
so $\text{PSR}_{\rm eff}$, every prediction in Table~\ref{tab:predictions}, and the
muon bound are all independent of the prefactor. The quantity $k$ is not
...
(31/31 checks), which generates the 120 vertices from the binary icosahedral group
...
At Planck-scale accelerations (\(\Delta\text{SSV} \gtrsim \text{SSV}_{\rm crit}\)) the spectrum saturates nonlinearly as \(\text{PSR}_{\rm eff} \to 0\), providing a sharp ultraviolet cutoff that is a genuine, testable deviation from the standard Unruh prediction. The linear-regime result at laboratory accelerations and the saturation behaviour at extreme stress both follow analytically from the mode-counting derivation of Appendix~F. \emph{Correction note (Patch 2471):} an earlier version of this paragraph additionally reported that ``full numerical integration of the stressed 600-cell mode-counting integral (Monte-Carlo over the 120 vertices) recovers the linear result to machine precision''; that claim is \textbf{withdrawn}, as no such artifact exists in the repository and none is known to have been executed. The analytic derivation is unaffected; the numerical confirmation is registered as outstanding under \texttt{OPEN-SR-1-MC-VERIFY}.
