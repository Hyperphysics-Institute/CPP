...
3. **Read the source materials** in this priority order: the paper's mechanism-SS-N.md (substantive content), the paper's philosophy-SS-N.md (honest scoping framing), the paper's reviews-SS-N.md (what the reviewers cared about), the paper's lay-summary-SS-N.md if one exists (existing layperson translation as input, not as the chapter), the paper's .tex source if needed for specific technical claims.
