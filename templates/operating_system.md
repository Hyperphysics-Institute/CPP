...
| `todolist.md` | **Carried-over deferred items + hygiene gaps** (introduced 7 May 2026 Session 33). The "easy to lose" things — small enough to skip a `future_projects.md` entry but compound if not externalized. **The next paper does NOT start until this file's P1 — Must clear before next paper section is empty.** Maintenance: any patch that completes a TODO moves it to "Cleared items (history)" with date and patch number. **Any patch that identifies a new deferral adds it here IN THE SAME COMMIT, under the lane that will act — gated by `code/deferral_gate.py` before `format-patch` (bootup §0.5 D-9, §3 DEFERRAL RIDER, Patch 0939).** The earlier wording of this sentence ("any *session* that identifies a new deferral adds it here") was session-scoped and audited only at the §15 close; under D-8 that close fires at window turnover, so three arcs' owed items (TODO-3930-EU, TODO-3938-DM, TODO-0937-CHIR) were never filed until the founder asked. The obligation is per patch, at write time, mechanically checked. If P1 grows beyond ~10 items, promote items to `future_projects.md` as registered multi-session projects. |
...
- **`todolist.md`** — **verify, do not file**: run `python3 code/deferral_gate.py <sha>` over every patch of the window and confirm each passed (todolist.md touched, or `NOTHING-DEFERRED` asserted). Filing happens per patch under D-9; this line only catches a gate that was skipped. If it finds one, file the items now under the acting lane before Step H. (Added Patch 0939.)
...
- **`osf_deposit_queue.md` + `osf_deposit_manifest.json`** — **if any paper `.tex` changed this session (new paper, version bump, amendment, corrigendum, status notes, or relocation), re-run `code/build_osf_queue.py` and commit the regenerated queue + manifest in the closing patch.** The generator is carry-forward-safe for the human/pipeline-owned columns (APPROVED, RESERVED_DOI, PREPRINT_ID, ...), but its `WARNING: N carried row(s) no longer resolve` line must be VERIFIED, not waved through: list the orphaned paths, confirm each is a rename/relocation whose owned columns hold no irreplaceable data (a RESERVED_DOI or PREPRINT_ID on an orphan means STOP and remap by hand before accepting the regeneration). N/A only for sessions touching no `.tex`. (Adopted Patch 3319, after the Session-155 audit found the queue last generated 18 Aug and the ENTIRE gravitational series — 11 papers, including the CONV-030-amended GR-1c/GR-1d/GR-1e — absent from the 117-row queue: the artifact's own "re-run after any patch" header was not an OS-level order and demonstrably did not fire.)
...
- **Per-patch capture audit (§15.15):** ✓ / exceptions — every physics/derivation patch this session has (a) its `reasoning/<patch>.md` fragment, (b) its `code/` verify script if any computation was run, (c) a fragment that passes CONV-010's **stranger test** (reconstructible from the file alone — presence is not conformance), and (d) its `founders_voice/` verbatim capture per CONV-009 if the patch acted on founder physics. List any patch missing capture; capture the gap before the handover is complete. **In long-arc sessions this audit also fires at each §15.14 checkpoint, not only at close.** **Fires on any session-close or handover production, not only the literal `initiate handover protocol` command (§15.15 Trigger).**
...
**The check (extended Session 16x, 1 Aug 2026 Patch 2883 — see "Three defects found in use" below).** Walk the session's physics/derivation patches and confirm **four** things, not one:

1. **Fragment present** — `reasoning/<patch>.md` exists.
2. **Verify script present** — `code/<patch>*.py` exists wherever a computation was performed. **Running a computation and reporting its result without committing the script is a capture failure even if the result is correct.**
3. **CONFORMANCE, not merely presence (CONV-010).** Apply the stranger test to each fragment: *could a competent stranger reconstruct the result from this file alone, without the chat transcript?* A fragment that narrates having done the work, rather than showing it, is **Tier 3 content in Tier 4's location** and does **not** discharge the obligation. Tiers 1–3 derive *from* Tier 4, so **a fragment thinner than the documents citing it means the structure is inverted.**
4. **Founder verbatim captured (CONV-009).** Any patch acting on founder-supplied physics — a mechanism, ruling, correction, or scope decision — has its `founders_voice/` verbatim capture in the same `git am`. **The verbatim is the primary source; conjecture entries, frontier updates, paper passages and dispatches derived from it are secondary and must cite it by path.**

Mark ✓ only if all four hold. If any patch is missing capture, **capture the gap before the handover is complete** — write the missing fragment/script as a follow-on patch (carrying a `STATUS: reconstructed` header per the rider if it is now after the producing window, since post-window capture is reconstruction, not verbatim). Pure-bookkeeping/organizational/governance patches are exempt (as under the rider), though governance patches that made a non-trivial decision still warrant a reasoning fragment.
...
**Trigger, second amendment (Session 16x, 1 Aug 2026 Patch 2883) — the audit must not wait for the close.** The Session-158 amendment removed the audit's dependence on *command phrasing*; it left intact its dependence on *session close ever happening*. **In long-arc sessions (the §15.14 threshold: more than ~5 substantive patches in one window) the audit fires INCREMENTALLY — at every §15.14 checkpoint patch, i.e. roughly every 5 substantive patches — and again at close.**

**Empirical motivation, and it is a repeat.** §15.15's own founding motivation was three chirality-lane scripts that shipped with results-in-markdown but uncommitted code, *"caught only on Thomas's prompt."* At Session 16x (Patches 2873–2882, a ten-patch arc with no close) **the identical failure recurred**: two wake-sign computations were run, their negative result reported and cited in a registered conjecture, and **neither script was committed**; one reasoning fragment (2879) was missing entirely; and no `founders_voice/` capture was made across an entire session of founder physics. **All three were caught only on Thomas's prompt, two months after the audit designed to prevent exactly this was adopted.** The audit did not fail — **it had not yet fired**, because the session had not closed. A safety net that fires once per session cannot bound loss in a session of unbounded length, and §15.14 already established this reasoning for capture *cadence* without applying it to the *audit*. **This amendment applies §15.14's long-arc logic to §15.15.**

**Scope and intent.** This is a *check*, deliberately lightweight — one audit-table line, walked at each long-arc checkpoint and at session close. Its value is catching the rare missed capture (e.g. a long-arc session where a physics patch shipped without its fragment under context pressure) while the producing window is still close enough that reconstruction loss is bounded. It is the lowest-cost addition consistent with treating the per-patch physics reasoning + verify scripts as the programme's most important durable artifact. No separate frontier tracker is registered; the discipline is self-contained in the §15 Step H audit.
