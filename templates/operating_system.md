...
- **`methods_catalogue/methods_catalogue.md`** — audit whether the session used any new or adapted methods (Layer 1 mathematical techniques, Layer 2 methodological disciplines, or Layer 3 heuristic strategies). If yes, append METH-L{1,2,3}-NNN entry per the catalog's three-category convention (NEW METHOD gets fresh ID; ADAPTED METHOD gets variant ID with parent back-pointer; STRAIGHT REUSE warrants no entry — just inline citation). Apply the threshold rule: method warrants entry only if reusable across at least one other context. Most sessions will mark N/A; sessions that introduce novel analytical surprises produce zero or one entry. (Adopted Patch 0449a per OPEN-ORG-016 Phase 3 codification.)
