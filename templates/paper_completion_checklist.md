...
**Parallel-window collision discipline (capture-and-delay, Patch 2107 — CANONICAL, ratified Patch 2111; pending activation).** The same property that makes Phase 7B dropout-prone (it edits shared repo-root files, not the paper's directory) makes it **collision**-prone: two windows shipping two papers the same day clash on those files. Under the Capture-and-Audit Protocol (`templates/capture_and_audit_protocol.md`), the Phase 7B items (C1–C10, C12, D1) **and** the shared-file 7A items (C11 bibliography, D2 INDEX.md, D3 series-README) are NOT hand-applied to canonical in-session: the window writes each **precise delta** to its **own** `Registries_pending/<window-slug>.md`, and the overnight audit merges all windows' pending deltas into canonical and clears them. The genuinely paper-local items (A, B, E, F, and the paper's own subfolder files) are unaffected and run in-session as before. **Before allocating any registry ID, read canonical AND glob `Registries_pending/*.md`** (in-flight claims). See `operating_system.md` §4 "Parallel-window registry discipline". *Until the Step-4 overnight audit is built and scheduled, continue in-session canonical edits (there is no merge yet).*
