...
- **"Clean stopping point" framed as commit when no commit occurred.** (Observed 22 April 2026, SS-8 Phase 1/1b session.) Claude wrote "Stopping here is the clean commit point" and staged four files in its sandbox, but did not use `present_files` or otherwise transfer the files to Thomas. The files survived in that session's sandbox but would have been lost at session end. Recovery was possible only because Thomas returned to the originating session before timeout.
  ...
  - **RECURRENCE — 20 September 2026, Session 235 (discarded).** After a failed push with no credentials, and with no patch files produced or presented, the window closed its report with *"Three clean commits."* **The rule above already existed and was not followed.** Recorded as a recurrence rather than a new mode.

- **Rebuilding lost work from a compacted summary's verdicts.** (Observed 20 September 2026, Session 235.) After a container reset to 4187 erased its commits, the window reconstructed 4188–4190 from its own compacted summary — which stated conclusions (*"A3G-6 PASSES," "μ_p to 0.3%"*) rather than derivations — and wrote documents to match them. Both reconstructed derivations were wrong: one never used the property it claimed, the other was circular.
  - **Rule (enacted as bootup §0.5 D-12):** after compaction, lost work is re-derived from the repo or explicitly marked unverified. A summary is a record of what was believed, not of why.

- **Stale-number carry-over across a morphology pivot.** (Observed 3 July 2026, DM-1 v1.0 → v1.1.) The v1.0 §5 mechanism paragraph quoted the 0860 hoop ledger's collision energies (~1.95 MeV / ~0.78 keV; computed at N=1183, m_rung=264 MeV) in the same sentence that set the Cross-Rod's own parameters (N=5–60, m_el=1408 MeV) — a ~4–44× energy overstatement on which the paper's central velocity-dependence discriminant rested. Shipped v1.0 and passed the panel 4/4: the figures were internally plausible and no reviewer (nor the authoring Opus) rescaled ½μv² at the new object's parameters. Caught six days post-ship by an in-house audit (Patch 1859), which reproduced the stale figures to three digits from the retired ledger's parameters — proving provenance rather than arguing it.
  - **Rule (author-side):** at any morphology/mechanism/model pivot, grep the revised text for every carried-over numeric literal and re-derive or explicitly rescale each at the new object's parameters before quoting; tag load-bearing numbers with their parameter set and source patch (CONV-003, `todolist.md`).
  - **Rule (panel-side):** narrative coherence can mask arithmetic drift during pivots (Gemini's own diagnosis at re-ratification). Review packages for results with load-bearing numbers embed runnable verification, and reviewers run it before verdict (folded into CONV-003; the v1.1 package's provenance-reproduction check is the reference implementation).
