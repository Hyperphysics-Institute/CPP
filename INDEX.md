...
| [`electromagnetism/`](flagship_papers/electromagnetism/) | **SF-6 [v1.0 SHIPPED 21 June 2026, Patches 1600→1607]** — Electromagnetism Unified: classical Maxwell + special-relativistic photon kinematics + QED phenomena from eDP-sea polarization; synthesis/reframing of the relativity companion c06 + the DP-Sea-Polarization model + SR-1 + QM-1..6 + EW-1 (no new derivation; the prior "EW-1 through EW-5" source list was stale). Two-tier rigor (Tier-1 $E=\hbar\nu_C$ ontological *identification*; Tier-2 toy-model constants $\mu_0,\epsilon_0,c,\gamma(v)$ → OPEN-FP-6-CONSTANTS); photon categorically distinct from the SF-2 cage bosons; Michelson–Morley carried as a named open falsifier; OPEN-FP-6-EMHAND (the EM-phenomenology expression of the already-closed substrate chirality). 4-reviewer panel → v0.3 restate → ChatGPT confirm → unanimous advance. Includes `sf-6_electromagnetism.tex`, `code/` (two verifiers), `documentation_suite/` (10 four-tier files), `review/` (package + cycle record), `reasoning/` (1600/1602/1606/1607). Unblocks SF-7 §10 threads SF-6↔SF-2, SF-6↔TP-1 |
