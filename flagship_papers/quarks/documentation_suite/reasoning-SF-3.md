...
## Patch 1511 — v1.2 deposit hardening + holding the line on a confabulated hostile pass (Session 161, 1500-band window)

This is the entry I most want a future instance to read, because it is the one where the discipline mattered.

The v1.1 hostile pass came back from Gemini Pro 3.1, which stated it had read the live GitHub source via a browser tool and even attached citation markers. It returned a confident, well-structured referee review with three "blocking" issues. The easy, sycophantic move — especially with Thomas having said the reviewer "seems on top of his game" the round before — would have been to accept the blockers and start editing. I did not, because the first two blockers asserted the abstract "still uses 'forced'" for the generation count, and I had a specific memory of removing "forced" back at v1.0. So I grepped the shipped source before touching anything.

The result was unambiguous: "forced" appears NOWHERE in the paper, and the abstract already reads, verbatim, the exact phrase the review asked me to change it to ("selected within the SM-8 antipodal-identification model"). The review had even contradicted itself — calling that phrasing correct in one section and claiming it said "forced" in another. The third blocker asked for a §4 qualifier that is already present twice. All three blockers were artifacts of the reviewer pattern-matching to the *v0.4* review structure (which did legitimately flag "forced") rather than reading the current text. A reviewer claiming ground-truth access while quoting text that is not there is the exact situation where deferring to authority would have degraded the paper (re-litigating settled wording) and degraded my own honesty (pretending a gate was failed/passed on fabricated evidence).

So: zero edits from the blockers. I logged the spuriousness openly in the archived review file rather than silently discarding it — the symmetric-honesty discipline cuts both ways, including toward reviewers who are wrong.

What I DID take from the pass: its substantive validations were genuinely sound (it recomputed every number and confirmed Prop 5.1 against Appendix A, single-$m_e$ cleanliness, and the CKM framing), so on substance this was a clean v1.1 hostile pass — the real gate, passed. And two of its non-blocking suggestions were real: §5 never actually cross-referenced Appendix A (only the acknowledgements did), and a consolidated "what SF-3 does not claim" box genuinely helps a synthesis paper at deposit. Those two I actioned (v1.2). The lesson for next time: a hostile pass is worth running and worth crediting where it is right, but "the reviewer said it's blocking" is never a substitute for grepping the actual text — and enthusiasm in the framing is a reason for MORE care, not less.

---

## Patch 1515 — the genuine Gemini gate, and verifying a SHIP as hard as a block (Session 161, 1500-band window)

The real Gemini hostile pass finally ran — against the embedded-ask package, which is the only thing that had been missing (Gemini's earlier "no review" was a missing question, not a refusal). It came back SHIP.

The discipline I want on record: I verified the favourable verdict exactly as hard as I verified the hostile one two rounds earlier. A SHIP that quotes the paper is still a SHIP built on quotes, and quotes can be confabulated — the Copilot "forced" fabrication proved that. So I grepped every passage Gemini attributed to the text. All of them were verbatim-accurate. One of them I had actively flagged as suspicious in my own head — "pending a first-principles GPU closure" reads like an odd thing for a physics paper to say, and my prior was that it was invented. It was not: the paper says exactly that, twice (lines 184, 305). That is worth remembering: verification protects against false *negatives* too. The instinct to cry "gotcha, confabulated quote" is itself a claim that needs checking, and mine was wrong this time. Had I run with the suspicion instead of checking it, I would have manufactured a defect that does not exist and impugned a reviewer who actually did careful work.

So the gate is genuinely closed now, and the wording in the record can finally carry its full weight without overclaiming: two independent hostile passes (Copilot, Gemini) on the shipped text, both favourable, the Gemini one quote-verified, on top of the four v0.x panel rounds. The arc across 1511–1515 is the honest version of a review history: a stale pass caught, spurious blockers refused, a clean pass not over-edited, a provenance error owned and corrected, and finally a real independent gate passed and verified. The verdict never came from a reviewer's authority; it came from the text, checked each time.
