...
## v1.3 — 14 June 2026 (Session 161, Patch 1517; 1500-band SF-3 window)

**Data-availability statement added; announces the reproducibility notebook in the paper.** Added a `\paragraph{Data and code availability.}` to the Acknowledgements: every numerical claim is reproduced from the single $\me$ calibration + 600-cell geometry by the self-checking notebook `code/sf-3_reproduce.ipynb` (wrapping `1500_verify_sf3_core.py`), with each headline asserted against the quoted figure so code/paper drift fails an assertion; standard library only, no external data. The notebook path is a live `\href` to the GitHub blob. This is the `.tex`-side complement to the notebook shipped at Patch 1516 (which was referenced only from `INDEX.md`); the deposit now points readers at the runnable zero-parameter demonstration from inside the paper. Title → Version 1.3. Recompile: 14 pages, 0 errors, 0 undefined refs. No change to any numerical result, theorem/proposition status, the swarm tally (108), or the review/gate status (Gemini gate remains PASSED). Deposit-ready.

---

## v1.2 — independent Gemini hostile pass **PASSED** — 14 June 2026 (Session 161, Patch 1515; 1500-band SF-3 window)

**The intended independent hostile gate is now genuinely closed.** The v1.2 source was re-sent to Gemini via the embedded-ask hostile-pass package (`sf-3_v1.2_hostile_pass_package.md`), which supplied the five adversarial questions the earlier raw paste lacked (that send had returned a request for the queries, not a review — see Patch 1514). Gemini returned an explicit **SHIP — ready for deposit**: it traced the single-$m_e$ dependency chain and found no hidden calibration, stress-tested Proposition 5.1 and confirmed it as a valid bookkeeping separation under the structural-$\alpha_s$ conditional, characterised the complementarity honestly (numerology from a standard-QFT view, a defensible structural correspondence within CPP, with an honest falsifier), and audited scope/overclaim ("no unshielded claims"). **Every quote it attributed to the paper was verified verbatim against the v1.2 source** (including the "pending a first-principles GPU closure" phrasing, lines 184/305 — a maintainer suspicion that it might be confabulated was checked and was wrong); unlike the Copilot round, no fabrication. Review archived at `review/sf-3_v1.2_review_gemini.md`. **This restores the "independent hostile pass" status that was withdrawn at Patch 1514 — now legitimately: Gemini is a genuinely independent reviewer (distinct from Copilot), so the shipped text carries two independent favourable hostile passes (Copilot v1.1+v1.2, Gemini v1.2) on top of the four v0.x panel rounds (ChatGPT/Grok/Copilot).** No `.tex` change (nothing blocking; SHIP). **SF-3 v1.2 is deposit-ready, gate closed**; remaining purely at maintainer discretion: the OSF/arXiv deposit and the optional `.ipynb` notebook.

---

## v1.2 — 14 June 2026 (Session 161, Patch 1511; 1500-band SF-3 window)

**Deposit hardening after the v1.1 hostile pass; no result or claim change.** Gemini Pro 3.1 ran a real v1.1 hostile pass (reading the live GitHub source). Its substantive validations were sound — numbers independently recomputed and confirmed, Proposition 5.1 confirmed true with Appendix A, single-$m_e$ calibration confirmed clean, CKM/$\delta_{CP}$ framing confirmed honest — so on substance this was a **clean v1.1 hostile pass**. Its three "blocking" issues were all **spurious on verification** and produced NO edits: (1)/(3) claimed the abstract "still uses 'forced'" for the generation count — but "forced" appears nowhere in the paper and the abstract already reads "selected within the SM-8 antipodal-identification model" (the review even contradicted itself, calling that phrasing correct elsewhere); (2) asked to add the "within the SM-6/SM-7 ... framework" qualifier to §4 — already present twice. Full review + verification note archived at `review/sf-3_v1.1_review_gemini.md`. **Two genuine non-blocking suggestions actioned:** (B) §5 now cross-references `Appendix~\ref{app:shift}` where the $A_q$ cancellation is discussed (previously only the acknowledgements referenced the appendix); (A)+(C) a consolidated **"What SF-3 does not claim" `mdframed` scope box** added to §1 — no new derivation; $\alpha_s$/$\sin^2\theta_W$ correspondences-not-RGE; generation count model-conditional; CKM open; SM-10 mechanism-depth-only; Prop 5.1 a bookkeeping separation. Title → Version 1.2. Recompile: 14 pages, 0 errors, 0 undefined refs. No change to any numerical result, theorem/proposition status, or the swarm tally (still 108). **This is the post-hostile-pass deposit-ready version** (the genuine v1.x hostile gate is now passed; remaining at maintainer discretion: OSF/arXiv deposit + the optional `.ipynb` notebook).

**v1.2 confirmation pass (Patch 1512):** Gemini Pro 3.1 re-reviewed the assembled v1.2 source and returned a **clean SHIP** — section-by-section technical + editorial, all numbers independently re-verified, Proposition 5.1 "airtight," calibration ledger "the best version of the adjudication so far," the §1 scope box and §4 correspondence-not-RGE framing endorsed, "no structural errors; no contradictions with SF-1/2/4/7; no hidden $m_c$; no overclaims." Its one concrete flag (a §9 sentence appearing to end mid-sentence) was self-diagnosed and confirmed a **paste artifact** — the source falsifier reads in full "forcing $\sinsqthetaW+\alphas \neq 1/\phig$ at the substrate scale --- falsifies Eq.~\eqref{eq:complementarity}", `eq:complementarity` resolves, no document defect. Its optional stylistic suggestions (abstract bolding; a redundant Route-B-archived sentence) were declined to avoid over-editing a clean pass. Two independent hostile passes (substantive v1.1 + this v1.2 SHIP) now concur. Review archived at `review/sf-3_v1.2_review_gemini.md`. **No `.tex` change at v1.2-confirmation; v1.2 stands as the deposit-ready SHIP.**
...
## v1.2 deposit bundle — reproducibility notebook added (Session 161, Patch 1516; 1500-band SF-3 window)

**Pilot of the `.ipynb` deposit convention (no paper change).** Added `code/sf-3_reproduce.ipynb` — a thin, executed wrapper over the canonical verifier `code/1500_verify_sf3_core.py`. Design: the notebook *imports* the shipped `.py` (single source of truth — it cannot drift from it), then walks each headline number with the formula shown inline and **asserts** the value against the SF-3 v1.2 paper figure, so any future drift between code and paper fails a cell loudly. Reproduces, from the single $m_e$ calibration + 600-cell geometry: $M_0=3.79$ MeV; the four masses $s/c/b/t$ with RMS $2.11\%$ (charm derived, $1249$ MeV); $\alpha_s=5/(8\varphi)$, $\sin^2\theta_W=3/(8\varphi)$, complementarity $=1/\varphi$, $F/E=5/3$; the quark Koide phase $124.04^\circ$. Executed top-to-bottom with **0 cell errors, all assertions PASS**; committed in executed form so the outputs render on GitHub/OSF. Standard library only (`math`); no data files, no network. Rationale: the notebook adds presentation/persuasion value (derivation→number inline, runnable "zero-parameter" demonstration for external readers) on top of the reproducibility the `.py` already provided; treated as a **deposit-bundle artifact**, refreshed at deposit time rather than per-`.tex`-number-change, to avoid sync surface. Referenced in `INDEX.md` (checklist item B5). **Not yet referenced from the `.tex`** — an optional one-line data-availability statement (→ v1.3) is offered separately. First `.ipynb` shipped with any CPP paper; if it earns its keep, the wrapper-with-assertions pattern generalises to other flagships at deposit time.
