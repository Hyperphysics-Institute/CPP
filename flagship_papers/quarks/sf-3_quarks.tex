...
{\Large \textbf{Version 1.3 SHIPPED --- 14 June 2026}}}
...
\paragraph{Data and code availability.} Every numerical claim in this paper is reproduced from the single $\me$ calibration and 600-cell geometry --- with zero fitted parameters --- by a self-checking notebook, \href{https://github.com/Hyperphysics-Institute/CPP/blob/main/flagship_papers/quarks/code/sf-3_reproduce.ipynb}{\texttt{flagship\_papers/quarks/code/sf-3\_reproduce.ipynb}}, which wraps the underlying verifier \texttt{1500\_verify\_sf3\_core.py}; each headline value (the four masses and their RMS residual, $\alphas$, $\sinsqthetaW$, the complementarity, and the quark Koide phase) is asserted against the figure quoted here, so any drift between code and paper fails an assertion. Both run on the Python standard library alone, with no external data files.
