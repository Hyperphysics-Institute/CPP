...
\end{equation}
the ratio of the 600-cell face and edge counts. \emph{One spectral trace gives both the electroweak and the strong coupling.} This is a substrate-level electroweak--strong \emph{mode complementarity} from a single spectral trace --- a shared geometric origin for the two couplings, not a dynamical unification --- and the strongest single $\S10$ consistency thread linking SF-3 to SF-1, SF-2, and SF-5: the same adjacency spectrum, partitioned into edge and face modes weighted by $\eta = 1/\phig$, sources both couplings with no independent calibration.
