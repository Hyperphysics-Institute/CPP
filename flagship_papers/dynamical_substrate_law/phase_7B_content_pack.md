...
**Target:** `methods_catalogue.md` (repo root, 213 lines; canonical paper-of-origin programme-level catalogue with METH-CHIR-CONT-N entries from chirality_continuum.tex). NOTE: `methods_catalogue/methods_catalogue.md` (subdirectory, 184 lines) is a SEPARATE document — the layered programme-wide L1/L2/L3 catalogue with 19 entries (7 L1 + 8 L2 + 4 L3). Both files coexist with distinct purposes; no reconciliation needed.

**Landing status:** LANDED at Patch 0578 (Session 143, 25 May 2026) — **AUDIT-ONLY, no METH-DSL-N entries registered**. Audit finding: all 5 pre-staged METH-DSL-N candidates (§4 of this content pack) are workflow / publishing-protocol / reviewer-engagement / paper-composition patterns, NOT substantive-derivation methods. Per the root file's Purpose section ("novel reasoning methods, mathematical techniques, and structural abstractions introduced through CPP **substantive derivation work**") and the subdir README's explicit scope clarification at Patch 0461 ("physics derivation methods only — out of scope: protocol patterns, workflow heuristics, operating-system disciplines, handover discipline, commit-cadence rules, reviewer-interaction protocols, session-management methodology"), all 5 candidates fail the scope test. **Codification carry-forwards** (registered in commit message + Last-updated header but NOT registered as formal OPEN-ORG entries at this surgical patch): METH-DSL-4 + METH-DSL-5 patterns already tracked at OPEN-ORG-019 (Patch 0574) → `templates/operating_system.md` §17 reviewer-protocol codification; METH-DSL-1/2/3 patterns (paper-composition + publishing-protocol; not yet tracked) → propose new OPEN-ORG-021 candidate OR fold into OPEN-ORG-019 expansion → `templates/operating_system.md` §4 Phase 7 paper-completion discipline codification (deferred for handover consideration). **F.1 substantive-derivation contributions to programme-level registries are theorem-level + prediction-level, not method-level**: THEO-DSL-1/2/3 await theorem-registry.md registration at Patch 0581; 5–6 PRED-O-NN entries await predictions.md registration at Patch 0580.
...
**Landing status:** LANDED at Patch 0579 (Session 143, 25 May 2026). Four surgical edits applied: (1) Last-updated header prepend for Patch 0579 audit context (this Patch's update was first since 6 May 2026 — file's Last-updated header had not been touched through the entire F.1 trajectory Patches 0513-0570); (2) Programme-level priority update section header advanced from Chirality Continuum v1.0 SHIPPED (Patch 0509) to F.1 v1.0 SHIPPED (Patch 0570); (3) Intro paragraph reframed: four of five OPEN-SD-CHIR-PRIMITIVE manifestations now closed (i/ii/iii at Layer 4; iv at sketch-document Layer 3 via F.1 Theorem 7.1; v remains open as F.3 territory); (4) F.1 entry REPLACED — patch-by-patch trajectory narrative (Patches 0513-0554 spanning Sessions 137-142 with 7 consecutive Patch 0548-0554 deferrals reaching Patch 0570 v1.0 SHIP) collapsed into clean post-v1.0 SHIPPED status + 6 OPEN-FP-F1-N enumeration + post-v1.0 trajectory queue with 7 priority items led by OPEN-FP-F1-3 G1 publication-grade hardening per ChatGPT R1-R6 convergent priority. The patch-by-patch narrative is preserved verbatim in F.1 paper subfolder artifacts (sketches/, layer3_promotion/, reviewer_pause/, hardened_theorems/) + documentation_suite/ Tier 2/3/4 files + git log entries Patches 0531-0570 — references list embedded in new entry's closing paragraph. (5) F.2 entry reframed: scope SUPERSEDED at F.1 v1.0 SHIP (manifestation (iv) no longer an open closure target at sketch-document Layer 3); three trajectory options registered (a) F.2 as Layer 4 axiomatic derivation of Mechanism A; (b) F.2 as publication-grade hardening companion paper (most actionable given G1 hardening RECOMMENDED first); (c) F.2 retired in favor of F.1 post-v1.0 OPEN-FP-F1-N work. Decision deferred to handover. (6) F.3 entry updated: F.1-dependency CLEARED at v1.0 SHIP; F.3 substantive content trajectory now unblocked per Session 138 Patch 0523 decision gate Scenario A active. Scenarios B + C no longer active. Net file size 378 → 343 lines (-35 lines reflecting narrative consolidation).
...
**Landing status:** LANDED at Patch 0580 (Session 143, 25 May 2026). Three surgical edits applied: (1) Last-updated header prepend for F.1 v1.0 SHIP predictions registration (first touch since Patch 0481L Capotauro v2.0 v1.0 SHIP); existing Patch 0481L header moved to first bullet of Earlier updates per established prepend pattern. (2) **Five paper-body predictions registered** in Section 2 as **PRED-O-28** through **PRED-O-32** (next available PRED-O number was -28; verified at edit time): PRED-O-28 host-to-first-shell uniform projection $-1/(2\phi)$ via Theorem 5.1; PRED-O-29 first-shell unit-vector sum $-6/\phi$ intermediate identity; PRED-O-30 icosahedral rank-1 sum $3/\phi^2$ load-bearing for Theorem 7.1; PRED-O-31 substrate-locality umbrella coefficient $6/\phi^2$ closing manifestation (iv); PRED-O-32 first-shell-to-first-shell edge perpendicularity via Theorem 5.2 (shared with Capotauro v2.0 §5.6 K3-base protection). All 5 entries follow in-place format of existing PRED-O-23-27 entries (5-column markdown table row: ID | Prediction | Description+source | Test/falsifier | Cross-reference). Each entry includes the load-bearing F.1 paper section reference + hardened-theorem .tex file cross-reference + cross-sector consistency match with Capotauro v2.0 where applicable + Falsifier statement + "Not yet a swarm contribution" framing parallel to PRED-O-26/27 substrate-level theorem predictions. (3) F.1 row added to Section 6 Predictions by Paper after Chirality Continuum row with full 5-entry summary + 3-theorem reference + 6-OP enumeration + 3 corpus-establishing conventions + swarm count framing. **Optional PRED-O-N6 (grouped foundations-work predictions: $|\vec{j}(v_i)| = 2 r_0 \delta \sqrt{7-\phi}$ + first-shell-vertex sum $24/\sqrt{7-\phi}$ + discrete curl vanishing) NOT registered at programme level per F.1 v1.0 SHIP convention** of registering only paper-body predictions at programme level (foundations-work identities preserved at `sketches/F1_phase2_foundations_work.md` + `code/verify_b1q4_first_shell_current_sum.py` for cross-trajectory referencing). **Swarm count UNCHANGED at 108** per programme convention — PRED-O-28 through PRED-O-32 are substrate-level structural identities at sketch-document Layer 3 that elevate the structural foundation of OPEN-SD-CHIR-PRIMITIVE manifestation (iv) closure but do not yet bridge to Layer 4 EFT observable scale. Becomes swarm contribution when OPEN-FP-F1-2 Layer 4 axiomatic derivation of Mechanism A from CPP primitive axioms A1-A11 lands (long-term programme target). **PRED-O numbering NOT advanced via Patches 0573-0579 sequence** — confirmed at edit time that no concurrent PRED-O additions occurred since Capotauro v2.0 v1.0 SHIP (Patch 0481L; PRED-O-26/27); F.1's PRED-O-28-32 are clean sequential additions. Net file change: 391 → 399 lines (+8 lines = 5 PRED-O rows + 1 F.1 Section 6 row + 2 net lines from Last-updated paragraph splice).
...
**Landing status:** LANDED at Patch 0581 (Session 143, 25 May 2026). Four substantive edits applied: (1) Last-updated header prepend for F.1 v1.0 SHIP theorem-registry registration + accumulated drift correction (first touch since Patch 0509 Chirality Continuum v1.0 SHIP); existing Patch 0509 header moved to first bullet of Earlier updates per established prepend pattern. (2) SD section header advanced "12 Theorems" → "18 Theorems" (drift correction: +3 accumulated from Patches 0487/0491/0495 THEO-CHIR-CONT-1/2/3 registered at row level but not propagated to section header per Patch 0509 explicit deferral to "future drift-audit patch" + 3 new THEO-DSL-N entries). (3) Three new theorem entries **THEO-DSL-1**, **THEO-DSL-2**, **THEO-DSL-3** appended to SD section after THEO-SD-CHIR-2 (end of table per existing ordering — pre-existing non-chronological row order: SD-CHIR-1, CHIR-CONT-1/2/3, SD-CHIR-2, DSL-1/2/3 — not corrected at this Patch per surgical Phase 7B discipline); each entry follows existing in-place format (5-column markdown table row: ID | Name | Result | Axioms Used | Paper Reference) with substantial detail comparable to THEO-CHIR-CONT-1/2/3 entries; **THEO-DSL-N sub-prefix convention introduced** at this Patch (parallel to THEO-CAP-N + THEO-SD-CHIR-N + THEO-CHIR-CONT-N conventions); each entry includes hardened-theorem artifact reference + Layer rigor classification + cross-sector consistency framing + falsifier + honest scope-limitation framing + four-condition test verification. (4) Summary Statistics SD row 12 → 18 + Total 64 → 70 (+6 net: +3 accumulated drift + 3 new from F.1) + Theorems:Axioms ratio updated to (70 + 9 corollaries) / 6 axiom slots ≈ 13.2× (was 12.2× at stale baseline); explanatory note appended above existing SS row note documenting the +6 advance + corpus-establishing conventions. **THEO-DSL-1** (Perturbation-Theory Propagation Rule + Shell-Locality at $\mathcal{O}(\delta^1)$): publication-grade Layer 3 **unconditional** via hardened artifact `hardened_theorems/perturbation_locality_propagation.tex` (Patch 0550). **THEO-DSL-2** (First-Shell Geometric Identities at Vertex-Aligned Reading C in 600-Cell): publication-grade Layer 3 **conditional on G1** via hardened artifacts `host_first_shell_uniform_projection.tex` (Patch 0552) + `first_shell_perpendicularity.tex` (Patch 0551); combines Theorems 5.1 + 5.2 into single registry entry. **THEO-DSL-3** (Substrate-Locality Umbrella Theorem): **sketch-document Layer 3** (not independently hardened; assembly of THEO-DSL-1 + THEO-DSL-2 + icosahedral rank-1 sum identity); closes OPEN-SD-CHIR-PRIMITIVE manifestation (iv) thermodynamic causal arrow — fourth manifestation closure under the umbrella. **Hardened-theorem artifact convention established at F.1 v1.0 SHIP**: publication-grade Layer 3 theorems supported by dedicated `.tex` artifacts in `hardened_theorems/` subdirectory with explicit hypothesis tracking + five-class exclusion enumeration; three F.1 hardened artifacts at Patches 0550 + 0551 + 0552 (741 lines LaTeX combined; 21 PDF pages combined). **F-line architectural template** for future F.2 / F.3 theorem registration established. **Pre-existing ordering issue surfaced (NOT addressed at Patch 0581)**: THEO-SD-CHIR-2 (Patch 0440) is placed AFTER THEO-CHIR-CONT-1/2/3 (Patches 0487/0491/0495) in the table — non-chronological ordering predates this Patch; chronological re-ordering of SD section rows is a candidate follow-up beautification (surfaced for handover consideration; not registered as formal organizational item at this Patch). Net file size 289 → 298 lines (+9 lines = 3 new theorem rows + 2 blank separator lines + 4 net lines from Last-updated paragraph splice; Summary Statistics note expanded).
...
**Landing status:** LANDED at Patch 0582 (Session 143, 25 May 2026). Two substantive edits applied: (1) Last-updated header prepend for F.1 v1.0 SHIP master_glossary registration (first touch since Capotauro v1.0 SHIP at Patch 0416A; existing 16 May 2026 paragraph continues as compound paragraph since prior Last-updated paragraph was multi-paper concatenated and no truncation needed). (2) NEW section appended at end of file: `## Dynamical Substrate Law (F.1) v1.0 terms (added 25 May 2026, Patch 0582)` with intro paragraph + 15 F.1-specific glossary entries in table format grouped by sub-category — 3 Constants ($-1/(2\phi)$ host-to-first-shell uniform projection constant; $6/\phi^2$ substrate-locality umbrella coefficient; $\sqrt{7-\phi}$ first-shell-vertex current magnitude factor) + 5 Structural terms (DI-bit current; Vertex-aligned Reading C; Host vertex $v_{\text{host}}$; First-shell in the 600-cell; Edge graph of the 600-cell) + 3 Mechanism terms (Mechanism A propagation-rate asymmetry; G1 first-shell inner-product primitive; G2 first-shell edge-direction projection) + 4 Methodology terms (Sketch-document Layer 3; Hardened-theorem artifact; Anti-erasure discipline; Framework axiom). Section follows established CPP corpus convention (per SF-2 / SF-4 / Capotauro v1.0 / Capotauro v2.0 / Chirality Continuum precedent: each flagship paper appends a paper-specific section at the end with table-format entries); convention preserved — no alphabetical interleaving into existing thematic sections (Acronyms / Fundamental Entities / Lattice Geometry / etc.). **Three F.1-specific structural constants** ($-1/(2\phi)$, $6/\phi^2$, $\sqrt{7-\phi}$) introduce a new pattern of pure numerical-constant entries derived from 600-cell first-shell geometry + Mechanism A propagation-rate asymmetry parameter $\delta$; corpus-establishing for future F-line numerical-constants glossary additions. **Four Methodology terms** codify programme-level conventions extended at F.1 v1.0 SHIP: Sketch-document Layer 3 distinct from publication-grade Layer 3; Hardened-theorem artifact convention (three F.1 artifacts at Patches 0550 + 0551 + 0552; 741 lines LaTeX combined; 21 PDF pages combined); Anti-erasure discipline (preserve Layer-distinction + conditionality + explicit open higher-order questions during paper polishing); Framework axiom (substrate-physics commitment at Layer 3 derivation level distinct from CPP primitive axioms A1–A11 at Layer 4). **Cross-sector consistency** highlighted in two entries: $-1/(2\phi)$ shared with Capotauro v2.0 §3 spatial-sector substrate-locality theorem; G2 first-shell edge-direction projection shared with Capotauro v2.0 §5.6 K3-base protection identity. **F.1 v1.0 SHIP is the first F-line flagship v1.0 SHIP in CPP corpus**; first F-line glossary additions; F-line architectural template for future F.2 / F.3 glossary additions established. Each entry includes appropriate cross-references to F.1 paper sections + hardened-theorem artifacts + theorem-registry entries (THEO-DSL-N at Patch 0581) + predictions-registry entries (PRED-O-28/31/32 at Patch 0580) where applicable. **PRED-O-N6 grouped foundations-work entry NOT registered at paper-body predictions registry per F.1 v1.0 SHIP convention** (deferred at Patch 0580); the $\sqrt{7-\phi}$ foundations-work constant is glossary-only at v1.0 SHIP (becomes programme-level prediction when foundations-work content is promoted to a future F.1 follow-up paper). Net file change: 591 → 614 lines (+23 lines = 1 section header + 1 intro paragraph + 1 table header + 1 separator + 15 entry rows + ~4 surrounding blank lines).
...
**Landing status:** LANDED at Patch 0583 (Session 143, 25 May 2026). Three substantive edits applied: (1) Last-updated header prepend for F.1 v1.0 SHIPPED TATWD integration (Patch 0583 frontier paragraph; existing 11 May 2026 paragraph already multi-paper concatenated covering SF-4 v4.4 + SS-9 + earlier work continues as part of compound paragraph; intermediate 20 May 2026 Patch 0514 chirality continuum paragraph at line 775 of file preserved as separate Earlier update entry at file tail per existing convention). (2) Part VI Note on scope two-fold extension — main list extended to include F.1 Dynamical Substrate Law (first F-line flagship v1.0 SHIP; Chapter 22i); parallel sectoral synthesis enumeration extended to describe F.1 as the "parallel sectoral synthesis at the temporal-sector substrate-locality scale (the substrate-locality umbrella theorem characterizing the first-order-in-$\delta$ DI-bit current at the host vertex with closed-form coefficient $6/\phi^2$ at vertex-aligned Reading C, closing the umbrella's manifestation (iv) thermodynamic causal arrow at sketch-document Layer 3)". (3) NEW Chapter 22i added to Part VI — "The Dynamical Substrate Law and Manifestation (iv) Closure (F.1)" — inserted between Chapter 22h close and Part VII transition; structured parallel to Chapter 22g/22h with seven content paragraphs + bibliography footer. Chapter 22i content: opening narrative paragraph contextualizing F.1 as 7th flagship v1.0 SHIP and first F-line flagship; substrate-locality umbrella theorem THEO-DSL-3 closed-form result $\vec{j}_{DI}^{net}(v_{host}) = (6\delta/\phi^2)\hat{n} + \mathcal{O}(\delta^2)$ closing OPEN-SD-CHIR-PRIMITIVE manifestation (iv) thermodynamic causal arrow at sketch-document Layer 3 (fourth manifestation closure under the umbrella); three F.1 theorems THEO-DSL-1/2/3 + THEO-DSL-N sub-prefix convention parallel to THEO-CAP-N + THEO-SD-CHIR-N + THEO-CHIR-CONT-N; hardened-theorem artifact convention establishment at F.1 v1.0 SHIP (publication-grade Layer 3 theorems supported by dedicated `.tex` artifacts in `hardened_theorems/` subdirectory with explicit hypothesis tracking + five-class exclusion enumeration; three F.1 hardened artifacts at Patches 0550 + 0551 + 0552 totaling 741 lines LaTeX + 21 PDF pages); cross-sector consistency at F.1 v1.0 SHIP ($-1/(2\phi)$ shared with Capotauro v2.0 §3 + first-shell-to-first-shell edge perpendicularity shared with Capotauro v2.0 §5.6 K3-base protection); anti-erasure discipline + framework axiom corpus-establishing conventions; forward queue with OPEN-FP-F1-3 G1 publication-grade hardening RECOMMENDED first post-Phase-7 substantive physics Patch per ChatGPT R1-R6 convergent priority + Phase 7C OSF deposit + anthology chapter at Rovelli/SciAm register. **Predictions Scorecard headline count UNCHANGED at 108** per established F.1 convention (Patch 0580 PRED-O-28 through PRED-O-32 parallel to PRED-O-26/27 framing: structural-prediction Layer 3 elevation without new programme-level swarm contributions). Per surgical Phase 7B discipline, accumulated drift in Axiom-to-prediction ratio section (line 740 currently states 111 from Capotauro v1.0 baseline; predictions.md authoritative current count is 108 per Patch 0580) NOT addressed at Patch 0583 — drift correction candidate for future drift-audit Patch. Net file change: 775 → 793 lines (+18 lines = 1 Chapter 22i section header + 7 content paragraphs + 1 bibliography footer line + ~5 surrounding blank lines + 4 lines from Last-updated header prepend integration).
