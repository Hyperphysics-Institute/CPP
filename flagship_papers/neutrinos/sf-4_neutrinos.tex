...
{\normalsize Version 0.5 --- 9 May 2026 (integration polish pass: stale version-number references in body text removed; cross-section consistency verified; equation-reference and theorem-reference checks complete; v1.0 SHIP after AI review passes per SS-9 methodology)}}
...
$\sin^2\theta_{12}$ & $1/3 = 0.333$ & $0.304 \pm 0.012$ (NuFIT) & \S\ref{sec:k3consistency} (TBM 0$^{\mathrm{th}}$) + \S\ref{sec:higher_order} \\
$\sin^2\theta_{23}$ & $1/2 = 0.500$ & $0.570 \pm 0.018$ (NuFIT) & \S\ref{sec:k3consistency} (TBM 0$^{\mathrm{th}}$) + \S\ref{sec:higher_order} \\
$\sin^2\theta_{13}$ & 0 (0$^{\mathrm{th}}$ order) & $0.0224 \pm 0.0007$ & \S\ref{sec:k3consistency} + \S\ref{sec:higher_order} (OP-SM-7d) \\
\midrule
\multicolumn{4}{l}{\emph{Mass ordering (forced by cage-shell assignment)}} \\
Mass ordering & Normal hierarchy & Unresolved (JUNO 2026+) & \S\ref{sec:cageassignment} (forced) \\
\midrule
\multicolumn{4}{l}{\emph{CP phase (deferred)}} \\
$\delta_{CP}$ & Open (route ii) & $\sim 195^\circ \pm 100^\circ$ & \S\ref{sec:deltaCP} (defer to SF-2) \\
\bottomrule
\end{tabular}
\caption{Master predictions table for the eight neutrino sector parameters. Seven of eight parameters at zero free parameters; $\delta_{CP}$ registered as open and deferred to the EW-sector flagship (SF-2) per route (ii). Empirical values from NuFIT 5.3 \cite{nufit53}; cosmological bound from DESI/Planck combined analysis \cite{desi2024,planck2018}.}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{TBM angle corrections} lifting the zeroth-order angles $\sin^2\theta_{12}^{(0)} = 1/3, \sin^2\theta_{23}^{(0)} = 1/2, \sin^2\theta_{13}^{(0)} = 0$ toward the observed $\sin^2\theta_{12} = 0.304, \sin^2\theta_{23} = 0.570, \sin^2\theta_{13} = 0.0224$ via mode-mode mixing corrections at $O(\epsilon)$ where $\epsilon$ is a substrate-derived small parameter from the EW machinery.
...
\subsubsection{Inverted hierarchy (JUNO 2026+)}
...
The Jiangmen Underground Neutrino Observatory (JUNO) \cite{juno_design} is expected to resolve mass ordering at $> 3\sigma$ confidence by 2026--2028 via reactor antineutrino oscillation pattern measurement. \textbf{If JUNO confirms inverted hierarchy, the SF-4 cage-shell assignment is dead}, and the framework's central derivation chain fails. This is a \textbf{clean, named, near-term falsifier} with no escape via parameter adjustment --- the cage-shell assignment is forced by 600-cell topology, not chosen.

\subsubsection{Direct mass measurement above $\sim 5$ meV}
\label{sec:falsifier_direct_mass}

The cage-shell + suppression-factor prediction is $m_{\nu_1} \approx 0.98$ meV (the lightest mass eigenvalue, identified with $m_{\nu_e}$ in normal hierarchy). KATRIN \cite{katrin2022} currently bounds $m_\beta < 0.8$ eV; future direct-measurement programmes (KATRIN++, Project 8 \cite{project8}) target the sub-eV decade with sub-100 meV sensitivity over the coming decade.

A direct mass measurement of $m_{\nu_e} > 5$ meV would force the lightest mass to a value inconsistent with the cage-shell + $\sigmanu$ prediction (which constrains $m_{\nu_1}$ to $\approx 1$ meV with $\sim 2\%$ structural uncertainty from the leading-order $V^2$ approximation). The 5-meV threshold corresponds to $\approx 5\sigma$ deviation from the predicted value; smaller deviations could be accommodated by higher-order corrections from OP-SM-7d (\S\ref{sec:higher_order}).

This falsifier is on a longer timescale than JUNO (decade-class rather than 2026+ near-term) but is an unambiguous direct-measurement test.

\subsubsection{Cosmological tightening to $\Sigma m_\nu < 50$ meV}
\label{sec:falsifier_cosmo}

The current DESI/Planck combined bound is $\Sigma m_\nu \le 72$ meV \cite{desi2024,planck2018}; SF-4 predicts $\Sigma m_\nu = 64.9$ meV (sum of three predicted masses with normal hierarchy). The prediction sits comfortably under the current bound but with limited margin.

Future cosmological surveys (DESI full-survey results, CMB-S4, LSST) are expected to tighten the bound. \textbf{Tightening below $\sim 50$ meV would falsify the SF-4 prediction directly}: $\Sigma m_\nu = 64.9$ meV cannot be reduced to $< 50$ meV by higher-order corrections without breaking the splitting structure. The cage-shell assignment forces a specific $\Sigma m_\nu$ once the absolute scale is fixed by $\sigmanu$.

A bound tightening to $\Sigma m_\nu < 50$ meV is plausible on a 5--10 year timescale.
...
\bibitem{nufit53}
I.~Esteban, M.~C.~Gonzalez-Garcia, M.~Maltoni, T.~Schwetz, and A.~Zhou, \emph{The fate of hints: updated global analysis of three-flavor neutrino oscillations}, J.\ High Energy Phys.\ \textbf{09}, 178 (2020); NuFIT 5.3 (2024) update at \url{http://www.nu-fit.org}.
