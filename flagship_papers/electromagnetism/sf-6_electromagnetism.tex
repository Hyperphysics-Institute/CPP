...
\item It registers \textbf{OPEN-FP-6-EMHAND} (electromagnetic handedness, the substrate-chirality manifestation entering EM phenomenology, has no closure trajectory and lives in the chirality arc), as an inherited-open deferred to the responsible arc (Section~\ref{sec:opens}).
...
% ============================================================
% SECTION 2: The eDP-sea / ZDC substrate
% ============================================================
\section{The $\eDP$-Sea ($\ZDC$) Substrate}\label{sec:substrate}

The dipole sea consists of paired Dipole Particles (DPs) filling space in a random, unpolarized state at rest. The organizational unit of both mass and radiation is the Zero-point Dipole Chain.

\begin{definition}[Zero-point Dipole Chain ($\ZDC$)]
A Zero-point Dipole Chain is a coherent alignment of Dipole Particles along a common direction, sustained by the mutual Coulombic attraction between adjacent DPs and stabilized by the Zitterbewegung (ZBW) oscillation of each constituent DP. Each $\ZDC$ link carries one Planck-scale action quantum $\hbar = E_P t_P$.
\end{definition}

All four DP types (eDP, qDP, hybrid A, hybrid B) participate equally in $\ZDC$ formation; no differential coupling distinguishes the types at the CP charge level, so the $\ZDC$ is a universal organizational structure (\texttt{c06}, $\S2$). The electromagnetic sector is carried by the electron-DP ($\eDP$) component of the sea.

A $\ZDC$ pattern oscillates at the Compton-scale frequency
\begin{equation}
\nuC = \frac{E_{\ZDC}}{\hbar},
\label{eq:nuC}
\end{equation}
where $E_{\ZDC}$ is the energy stored in the pattern. This realizes a two-level frequency hierarchy: Planck-scale DP oscillation (Level 1) aggregates into Compton-scale $\ZDC$ pattern oscillation (Level 2). Equation~\eqref{eq:nuC} is the hinge of the whole paper: it is the same relation whether the pattern stands (mass) or travels (light), and the next section shows that this single fact unifies $E=mc^2$ with $E=h\nu$.
...
\textbf{(Tier 1 --- companion-grade.)} Both inertial mass and electromagnetic radiation are coherent $\ZDC$ patterns of the dipole sea. The single energy relation is
\begin{equation}
\boxed{\,E = \hbar\nuC\,}
\label{eq:unification}
\end{equation}
and the distinction between the two cases is \emph{purely a boundary condition}: the presence or absence of an unpaired CP nucleation center.

\paragraph{Mass = standing $\ZDC$ pattern.} A massive particle's $\ZDC$ pattern is pinned by the unpaired CP at its nucleation center, which emits its type-and-polarity $\SSV$ signal at every Absolute Moment tick, continuously re-nucleating the cloud against Brownian disruption. The result is a driven radial standing wave reflecting between the inner CP-Exclusion boundary and the outer thermal radius $r_{\mathrm{th}} = \hbar/(2mc)$, at which thermal disruption overcomes $\ZDC$ stiffening and the chains break --- defining the particle's effective radius and hence its mass. The standing-wave resonance frequency is $\nuC = mc^2/\hbar$, so
\begin{equation}
mc^2 = \hbar\nuC \qquad (\text{nucleation center present}).
\label{eq:massZDC}
\end{equation}

\paragraph{Photon = traveling $\ZDC$ pattern.} A photon is a coherent $\ZDC$ pattern with \emph{no} unpaired CP nucleation center; it therefore propagates rather than stands. At emission, the instantaneous spatial pattern of DP alignment, density, and phase is imprinted into the outgoing Grid-Point ($\GP$) network and advanced one $\PSR$ shell per Absolute Moment at speed $c$. The $\ZDC$ chains are oriented transverse to propagation, mirroring the transverse electromagnetic field. The Compton frequency of the photon is simply its electromagnetic frequency, $\nuC = \nu$, so
\begin{equation}
h\nu = \hbar\nuC \qquad (\text{no nucleation center}).
\label{eq:photonZDC}
\end{equation}

\paragraph{The unification.} Equations~\eqref{eq:massZDC} and~\eqref{eq:photonZDC} are the \emph{same} Equation~\eqref{eq:unification} with the two boundary conditions, so $E=mc^2$ and $E=h\nu$ are one physical statement. The relativistic energy--momentum relation connects the limits: $E^2 = (mc^2)^2 + (pc)^2$ gives $E=pc=h\nu$ for the photon ($m=0$) and $E=mc^2$ for a massive particle at rest ($p=0$). This is the conceptually broadest and most rigorous result in the paper; it requires no fitted parameter and no new postulate.

\begin{table}[H]
\centering
\caption{The boundary-condition dichotomy. One $\ZDC$ substrate, two patterns, one energy relation.}
\label{tab:unification}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Entity} & \textbf{$\ZDC$ pattern} & \textbf{$E=\hbar\nuC$} \\
\midrule
...
\label{eq:maxwellrel}
\end{equation}
where $\Zzero$ is the impedance of free space, which appears again in Section~\ref{sec:quantum} as the discharge load for photon emission.
...
Electromagnetic handedness --- the substrate-chirality manifestation entering electromagnetic phenomenology --- is not derived in SF-6. It is a manifestation of the substrate-chirality primitive owned by the chirality arc, where it currently has no closure trajectory. SF-6 is a \emph{downstream consumer}: when the chirality arc closes this manifestation, SF-6 can promote OPEN-FP-6-EMHAND from inherited-open to closed. The chirality arc should track SF-6 as a downstream dependency.
\end{openproblem}

\noindent The OPEN-FP-6-EMHAND posture is structurally parallel to SF-4's open neutrino $\delta_{CP}$ and SF-3's open CKM matrix --- a parallel of posture, not an equivalence of difficulty: each fermion/EM flagship derives its sector's core while deferring one mixing-or-handedness piece to the arc responsible for it. Stating the parallel explicitly strengthens the SF-line's uniform ``sector core derived, one piece deferred'' posture.
...
\item \textbf{OPEN-FP-6-EMHAND:} electromagnetic handedness, deferred to the chirality arc (Section~\ref{sec:opens}).
...
\paragraph{The Michelson--Morley tension (handled honestly).} The Tier-2 DP-Sea model yields an effective light speed of the form $c_{\mathrm{eff}}(v)\approx c\,(1-\lambda\,P(v)^2/2)$ with $\lambda\ll1$, i.e.\ an \emph{absolute-frame, velocity-dependent} signature that challenges the strict Michelson--Morley null result. This is a genuine tension with a foundational experiment and is \emph{not} glossed. The candidate resolution route, flagged but not claimed closed, is the Section~\ref{sec:classical} conjecture that $\Zzero$ is an $\SSV$-independent geometric constant: if the variable substrate response moves only the \emph{product} $\muz\epsz=1/c^2$ (a density-dependent channel, giving gravitational light-bending) and not the \emph{velocity}-dependent channel at first order, then the absolute-frame signature is suppressed into the density channel where varying-constants bounds are not yet violated. Whether the velocity-dependent residual survives at observable magnitude is the open question; until OPEN-FP-6-CONSTANTS is closed, the tension stands as stated.

\paragraph{Falsifiers.}
\begin{enumerate}[leftmargin=2em]
\item \textbf{Absolute-frame signature.} A confirmed velocity-dependent $c_{\mathrm{eff}}(v)$ at the magnitude the uncorrected Tier-2 model predicts would either confirm the silly-putty mechanism or, if absent at improved precision below the model's floor, falsify the toy-model form of Section~\ref{sec:relativistic} and force the $\SSV$-independent-$\Zzero$ resolution.
...
Standing $\ZDC$ pattern (nucleation center present) & Inertial mass; $E=mc^2$ \\
Traveling $\ZDC$ pattern (no nucleation center) & The photon; $E=h\nu$ \\
...
\noindent A genuine cross-arc dependency, distinct from the $\S10$ threads, is also surfaced for SF-7's bookkeeping: SF-6's OPEN-FP-6-EMHAND consumes the chirality arc's electromagnetic-handedness manifestation. SF-6 awaits that closure; it does not produce it.
...
\item \textbf{NEW:} OPEN-FP-6-EMHAND registered (electromagnetic handedness; deferred to the chirality arc as a downstream-consumer dependency).
