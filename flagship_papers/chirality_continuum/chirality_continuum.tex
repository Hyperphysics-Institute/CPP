...
Four methodological constructs introduced in this section are catalogued at the programme level in \texttt{methods\_catalogue.md} (initial population at Patch 0498): the sector-agnostic substrate Wigner-Eckart datum (\methref{METH-CHIR-CONT-1}); the continuum-limit projection map $\Phi$ via Wilson-Fisher block-spin renormalization at substrate cutoff (\methref{METH-CHIR-CONT-2}); the topological substrate quantity concept (\methref{METH-CHIR-CONT-3}); and the topological-projection argument (\methref{METH-CHIR-CONT-4}). Each construct is referenced at its first paper use via the corresponding \texttt{[METH-CHIR-CONT-N]} marker.

\subsection{The sector-agnostic substrate Wigner-Eckart datum}
\label{sec:sector_agnostic_datum}

We begin by abstracting the substrate-level Wigner-Eckart structure across the three Capotauro v2.0 sectors \cite{Capotauro} into a single sector-agnostic framework. \methref{METH-CHIR-CONT-1}
...
Under Lemma~\ref{lem:symmetry_preservation} and a valid sector-agnostic substrate Wigner-Eckart datum $\mathcal{D}^{\text{sub}}$ per Definition~\ref{def:datum}, the continuum-limit operator $\Oeff^{\text{sector}} = \Phicont\hat{C}^{\text{sector}}$ satisfies:
\begin{enumerate}[leftmargin=*]
\item \textbf{Irrep content preservation}: $\Oeff^{\text{sector}}$ lives in the continuum-limit projection of the $\zeta^{\text{sector}}$-ODD 1D irrep of $\Gamma$ --- equivalently, $\Oeff^{\text{sector}}$ is $\zeta^{\text{cont,sector}}$-ODD.
\item \textbf{Non-vanishing matrix element}: $\langle\psi^{\text{eff}}_+|\Oeff^{\text{sector}}|\psi^{\text{eff}}_-\rangle \neq 0$ between continuum-limit matter-doublet states.
\item \textbf{Uniqueness}: $\Oeff^{\text{sector}}$ is the unique (up to overall scalar multiple) continuum-limit operator with the parity content of (1) and the non-vanishing matrix-element condition of (2).
...
The matrix-element \emph{magnitude} $|\langle\psi^{\text{eff}}_+|\Oeff^{\text{sector}}|\psi^{\text{eff}}_-\rangle|$ remains to be established. This is the substantive content of Step 4 (Theorem~\ref{thm:magnitude_inheritance} below).
...
\end{enumerate}
The continuum operator $\Oeff^{\text{sector}}$ is identified sector-specifically at §\ref{sec:sector_a} (Sector A; V--A current operator $\bar{\psi}_L\gamma^\mu\psi_L$) and §\ref{sec:sector_b} (Sector B; chirality-asymmetric stabilization-energy operator $\DeltaFqDP$); the bridge theorem establishes existence, magnitude inheritance, and chirality content preservation independent of the sector-specific identification.
\end{theorem}
...
The Sector A instantiation of the bridge theorem's sector-agnostic substrate Wigner-Eckart datum (Definition~\ref{def:datum} \methref{METH-CHIR-CONT-1}) specializes the abstract data to the W-bracelet sector via three sector-specific foundational inputs introduced here:
...
The Sector B instantiation of the bridge theorem's sector-agnostic substrate Wigner-Eckart datum (Definition~\ref{def:datum} \methref{METH-CHIR-CONT-1}) specializes the abstract data to the qDP/eDP sector via three sector-specific foundational inputs:
...
\begin{itemize}[leftmargin=*]
\item \textbf{OPEN-FP-SF-2-CHIR full closure}: Layer 4 closure of OPEN-FP-SF-2-CHIR is achieved at this paper's v1.0 SHIP via Theorem~\ref{thm:sector_a} = THEO-CHIR-CONT-2. Deeper closure --- Layer 1 substrate-dynamics derivation of the substrate primitive 4D direction $\hat{n}$ from CPP primitive axioms --- is registered as future-window work tied to the Q1$'$+Q1$'$.A Layer 3 promotion programme \cite{ReviewerNotes}. The dynamical-substrate-law gate identified by external reviewers as the defining next gate for the Capotauro programme remains the most load-bearing open theorem-level item; its closure would promote FI-CHIR-CONT-1 from Layer 2 status (primitive feature identification at vertex-aligned Reading C) to Layer 1 status (substrate-dynamics derivation from axiom set).
