...
**Session 237 (22 Sep 2026, Patches 4222–4238; SF-2 v1.07 → v1.08).** The session opened with a critic pass that downgraded the v1.06 pure-Gamow–Teller "exact match" to a numerical coincidence (the SU(6) per-quark polarisation 2/3 equals the J=½ Clebsch factor 2/3), then rebuilt the beta-decay correlation problem as a pair-state calculation: with each lepton ejected by the handed rule, a lepton pair created in a spin state fixed by angular momentum, and the two refill senses added as amplitudes, the coefficients a, A, B equal the full V−A amplitude for every g_A/g_V and every β (Patch 4223; v1.07). Pressing on where the pair's spin content is set showed it must be set at birth, not by transit mixing, and that the v1.06 inheritance assignment (the electron takes the quark's orbital) gives A ≈ −1 for every weighting of the refill senses. The founder restated and ratified the assignment that works (G-EW-SWAP-4228): the quark's orbital eDP is released with no core and leaves as the antineutrino; the proton and the electron are the two halves of one counter-pair created from the Sea, a true singlet. Built literally this is the quark-level V−A vertex, (a, A, B) = (0, 0, +1), with the sign of g_A/g_V the singlet's; the nucleon's −1.275 is a 10% cage admixture with which a, A, B and the proton asymmetry C are all returned. §5.7.1 steps 3, 5, 6 and the postdiction paragraph were rewritten (v1.08). The founder then supplied the binding and release picture (G-EW-ZBWRESET-4231): the orbital is held by the core's charge through the ZBW fall-reset-return cycle, and released by the same reset at the W⁰ centroid's zero-gradient pocket; a pocket kick preserves the orbital's angular momentum to 0.4%, and the energy stays with the core as c04's mass mechanism says. Open at close: the bracelet-formation amplitude ∝ (E/m_W)² that sets the lifetime, the cage computation of the 10% admixture (TODO-4234-DELTA), and, for the neutral current, the unnamed act at a bare +qCP core (4236).

**Session 238 (22–23 Sep 2026, Patches 4240–4257; SF-2 v1.08 unchanged, v1.09 owed).** Paper-scoped consequences for SF-2: (i) ONESIGN item 2 now passes in magnitude — the ejection rule's (1−β)/2 with the outgoing states' 2E normalisation equals the SM π → eν/μν ratio identically (4249); (ii) 4231's *"B = +1 by geometry"* is withdrawn — the pocket lever arm bounds the torque about the pole, not the released L; B = +1 rests on G-EW-SWAP-4228's ratified clause and h_ν̄ = 1 (4253; glossary amended; SF-2 never printed the 0.4%); (iii) the neutral-current mechanism (OPEN-EW-NC, which SF-2's Cor. Zinert could not host) is complete as a mechanism: EM part −4Qs² (4235), isospin part from one sign + Fermi statistics + a capacity-one centroid (4246, 4253, 4254) — §5.7 should carry it once the critic on the exchange-antisymmetry step is done; (iv) the bracelet's physical radius is now a working value, 0.346 fm (4251), and Prop. capture/centroid gain a dynamical reading — the trap is ZBW among the ring's +CPs (G-EW-CENTROIDTRAP-4248); (v) Prop. lifetime is in tension with the founder's "stabilised on capture" (4256) — SF-2 must state the bare W⁰ dies within ~ħ/Γ_W if both are kept; (vi) the bracelet-formation probability, which SF-2 does not give, has a candidate (CAND-EW-4DOVERLAP-4255) with the formation ruled host-independent by lepton universality (4256). Not in SF-2's scope but adjacent: g_A = 1.406 with zero parameters from the founder's breathing-core picture (4244), and PRED-O-42's row amended to the 4228 state (4252). **Owed for v1.09:** items (i)–(v) as §5.7/§5.8 text and the changelog.

**Session 239 (23–24 Sep 2026, Patches 4259–4283; SF-2 v1.08 unchanged, v1.09 owed).** Paper-scoped consequences for SF-2:
- **(vii) The "ZBW reset" is now a transit** (R-ZBW-PASS-THROUGH, 4265; restated at 4279). Wherever SF-2 or v1.09 describes the pole stopping at the core or in the W⁰ pocket, it should say the pole passes through at speed and is steered in the one co-location Moment. The founder's 4240 passage-by-inertia already reads this way.
- **(viii) The pole-in-pocket time fraction** (1.24×10⁻³, uniform circulation) must be recomputed for a pass-through swing before Prop. lifetime quotes it (4279).
- **(ix) The orbital's petal loops** (4281) carry the spin and give g = ⟨E_total/E_kin⟩. SF-2 states nothing on g; note for §5 only if the quark-orbital well is derived.

Adjacent, not in SF-2's scope: g_A 1.267–1.270 from the founder's folded-square frame with its size fitted to r_p (4276); μ_n role correction (4271); c04 v2.2 (4277).

**Owed for v1.09:** items (i)–(v) from Session 238, plus (vii) and (viii).
