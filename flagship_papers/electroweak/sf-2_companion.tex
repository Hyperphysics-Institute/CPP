...
%
% Version 1.3 -- 14 May 2026 (Session 83 -- incorporation of Thomas's
%   actual GPU sensitivity-scan results from oblique_parameters_
%   sensitivity_scan.py. The 16x16 grid scan over (Pi_33/Pi_11,
%   Pi_3Q/Pi_11) in [0.85, 1.0] x [0.7, 1.0] showed 214/256 = 83.6%
%   of substrate-symmetry-motivated ratio combinations land within
%   LEP/SLC 3-sigma bounds, with the geometric constraint
%   |r33 - r3q| <~ 0.18 with r33 >= 0.85 clearly visible in the
%   PASS/FAIL pattern. New Section 5.7 'Sensitivity scan: the within-
%   bounds region explicitly' added documenting the result with a
%   PASS/FAIL grid table, corner-case sensitivity table, and
%   interpretation. Patch 0366.)
%
% v1.2 history (Patch 0364, Session 83):
...
{\Large \textbf{Version 1.3 --- 14 May 2026}} \\[0.5em]
{\normalsize Companion paper to SF-2 flagship (sf-2\_electroweak.tex v0.7); issued jointly with SF-2 v1.0 SHIP. Six deliverables corresponding to Copilot and Grok review recommendations: executive overview, cage geometry figures, glossary, cheat-sheet reference tables, parametric scaling heuristics for $W^0$ oblique-parameter sensitivity with GPU-runnable exploratory simulation, and DP-chain composition exploratory Monte Carlo with GPU-runnable proof-of-concept program. Patch 0362 (Session 83): initial Companion kickoff. Patch 0363 (Session 83): v1.0 $\to$ v1.1 review-cycle incorporation (ChatGPT rigor-language tightening, Copilot reading-order box and DP-species table, Grok caption precision and embedded Python listings). Patch 0364 (Session 83): v1.1 $\to$ v1.2 incorporation of actual GPU numerical results --- DP-chain species ratios refined to (40.3 / 29.7 / 29.6 / 0.4)\% qualitatively validating the framework with eDP even rarer than estimated; oblique-parameter $\Delta T \approx 0$ confirms the $m_{W^0} = m_{W^{\pm}}$ mass-degeneracy prediction; $|\Delta S|, |\Delta U|$ at heuristic placeholders outside 3$\sigma$ emerges as a substantive constraint on the continuum-EFT derivation. Patch 0366 (Session 83): v1.2 $\to$ v1.3 incorporation of actual sensitivity-scan results from the supplementary scan program --- 214/256 = 83.6\% of substrate-symmetry-motivated ratio combinations land within LEP/SLC 3$\sigma$ bounds, with the geometric constraint $|r_{33} - r_{3Q}| \lesssim 0.18$ with $r_{33} \geq 0.85$ visible in the PASS/FAIL pattern. New Section 5.7 added.}}
