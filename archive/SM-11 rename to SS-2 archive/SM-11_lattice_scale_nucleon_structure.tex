...
The proton occupies a single tetrahedral cell of the 600-cell lattice. This hybrid-tetrahedral picture reuses the same $K_4$ cell geometry that produced the strange-quark mass in SM-8. Its four vertices are assigned:
\begin{itemize}
...
\end{itemize}
Net charge: $+2/3 + 2/3 - 1/3 = +1$. The open $+$~vertex attracts the open $-$~vertex of a neutron, providing the nuclear binding force. Since each nucleon has exactly \emph{one} open vertex, binding is pairwise --- this naturally explains the saturation property of nuclear forces without additional assumptions.
\end{definition}
