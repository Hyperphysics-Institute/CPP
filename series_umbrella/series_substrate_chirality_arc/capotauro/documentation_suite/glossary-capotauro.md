...
## v2.0-era terms (Session 135 v2.0 v1.0 SHIPPED)

The v2.0 paper introduces a structurally larger architecture than v1.0; the following terms register the v2.0-era concepts not present in v1.0.

### Substrate-Locality Theorem (paper §3 Theorem 3.1)
Under vertex-aligned Reading C with $\hat{n} = v_\text{host}$, the substrate's local geometry at the host 600-cell vertex is preserved as $I_h$-symmetric at first order in the perturbation $\epsilon$, with only an isotropic radial scaling of the first-shell icosahedron by factor $(1 - \epsilon/(2\phi))$. The theorem applies uniformly to any substrate object built from first-shell vertices of the host (K3-base, W-bracelet, qDP antipodal pairs); all such objects inherit substrate-level chirality from the same identification $\chi \equiv \epsilon = \phi^{-3}$, via sector-specific cage-shell averaging on respective stabilizer subgroups of $H_3 = I_h$. The theorem is the substrate-level enabling result for the three-way cross-sector unification. See paper §3 Theorem 3.1.

### Three-way cross-sector unification (at magnitude level)
The substantive content of Capotauro v2.0 SHIP: three structurally distinct sector mechanisms — K3-doublet (mass-mixing under 3D spatial inversion), W-bracelet (electroweak V--A under 4D icosahedral-center inversion), qDP/eDP (electromagnetic handedness under combined $C \times P$) — produce identical numerical magnitude $\chi/6 = \phi^{-3}/6 \approx 0.0394$ through three different cage-shell averaging operations on the same 12-vertex icosahedral cage. Distinct from and stronger than unification-at-mechanism-level (the claim that all sectors inherit from the same primitive $\hat{n}$). See paper §§ 10, 11, 12.

### FI-C-RC-1: Primitive 4D direction $\hat{n}$
Foundational input registered at v2.0 (Reading C extension). Posits the existence of a primitive direction $\hat{n}$ in the substrate's ambient four-space whose presence breaks the 600-cell's full $H_4$ symmetry to a smaller stabilizer subgroup. The substrate's primitive chirality magnitude $|\chi| = \phi^{-3}$ derives from a perturbative-distance-ratio calculation on the broken-symmetry structure produced by $\hat{n}$'s presence. FI-C-RC-1 is one of two new foundational inputs at v2.0 extending the v1.0 set of FI-C-1 through FI-C-10. See paper §§ 2, 3.

### FI-C-RC-2: Vertex-aligned reading $\hat{n} = v_\text{host}$
Foundational input registered at v2.0 (Reading C extension). The primitive 4D direction $\hat{n}$ aligns with a host 600-cell vertex (as opposed to a face direction, an edge direction, or a cell direction). Resolved at Layer 2 via the paper-internal hosting argument (paper §10's "icosahedral cage hosting the K3-doublet" language requires a unique host vertex; face-aligned reading has 2-host ambiguity). Substrate residual symmetry around the host is $H_3 = I_h$, with the K3-doublet's $D_6 = D_{3d}$ stabilizer as one of 10 such sub-stabilizers of $H_3$. See paper §2 + Q1' resolution at Patch 0419 Session 126.

### Reading C
The candidate mechanism for sub-claim (b) of OPEN-SM-4 (the magnitude derivation, why $|\chi| = \phi^{-3}$). Reading C hypothesizes that the substrate's primitive chirality magnitude derives from the perturbative-distance-ratio scale produced by the primitive 4D direction $\hat{n}$'s breaking of the 600-cell's full $H_4$ symmetry. Reading C closure trajectory across Sessions 124-133 (Patches 0417-0441) closed Q1, Q2, Q1', Q3, Q5, Q6 at Layer 3 and SCOPED Q7 (cosmological-timing connection to sub-claim (a)) into four sub-questions for future-window work. Reading C reaches **natural saturation** at the level of theorem-registry closures (THEO-SD-CHIR-1 + THEO-SD-CHIR-2 under OPEN-SD-CHIR-PRIMITIVE umbrella); substantive Q7 work shifts to future-window work.

### Vertex-aligned vs face-aligned reading
The Q1' question (registered Session 125 Patch 0418): does the primitive 4D direction $\hat{n}$ align with a 600-cell vertex direction (stabilizer $H_3 \cong I_h$ of order 120) or with a 600-cell face direction (stabilizer $\mathbb{Z}_2 \times S_3 \cong D_6$ of order 12)? Resolved at Session 126 Patch 0419 in favor of vertex-aligned via three converging arguments: (i) paper-internal hosting argument (§10 requires unique host vertex; face-aligned has 2-host ambiguity); (ii) cross-sector unification with SF-2 W-bracelet at vertex direction per Finding C-W36; (iii) SM-1 first-shell icosahedron preservation. See FI-C-RC-2.

### $\zeta^W$ — 4D icosahedral-center inversion (W-bracelet)
The $\mathbb{Z}_2$ generator of the W-bracelet's dihedral $D_6 = S_3 \times \mathbb{Z}_2$ realization. Geometrically: the operation $p \mapsto \phi\hat{n} - p$ in 4D ambient space, with linear part $-I$ (the 4×4 identity negated), flipping $\hat{n}$ (chirality-flipping) and mapping each first-shell vertex to its first-shell antipode. Stabilizer of the W-bracelet vertex set since the 6 bracelet vertices come in 3 antipodal pairs. Distinct from K3's 3D spatial inversion in 3-space (which is the K3-doublet's $\zeta$); both are inversions through specific points, but in different ambient-dimension settings. See paper §11.

### $\zeta^{qDP}$ — Combined $C \times P$ (qDP/eDP)
The $\mathbb{Z}_2$ generator of the qDP antipodal-pair $D_{5d}$ stabilizer structure. Geometrically: host-CP-centered spatial inversion ($v \to -v$) combined with $\hat{n}$-flip ($\hat{n} \to -\hat{n}$) combined with qCP-sign flip ($\pm \to \mp$). The three flips together preserve the substrate's overall chirality content. Distinct from pure spatial inversion (which alone preserves qCP-sign in CPP, giving vanishing chirality matrix element) and from pure charge conjugation. The combined-$CP$ structure reflects the Capotauro mechanism's three-way coupling (vertex direction × $\hat{n}$ direction × qCP-sign) per SM-2 §10 chiral-polarity-bias mechanism. See paper §12.

### Picture B substrate-orientation field (framework realization at v2.0)
The substrate-orientation-field structure providing the physical content of $\hat{n}$ at Layer 2 substrate-physics. The Capotauro v0.8.1 reviewer concern that Picture B's role in the substrate-vacuum closure was underdeveloped is addressed at v2.0 via the explicit registration of FI-C-RC-1 (the primitive 4D direction) + FI-C-RC-2 (vertex-aligned reading) as foundational inputs grounding Picture B's substrate-orientation-field structure in concrete substrate-geometric content. ChatGPT v0.9 round-2 revised explicitly acknowledged the Picture B internalization at paper §6.3 as one of six v0.9 content-acknowledgments. See paper §§ 2, 6.3.

### OPEN-SD-CHIR-PRIMITIVE umbrella
Programme-level umbrella OPEN-* entry registered Session 127 Patch 0422 scoping the chirality-bias derivation as a programme-wide cross-sector unification target across five observable manifestations: (i) K3-doublet (mass-mixing); (ii) W-bracelet V--A (electroweak); (iii) qDP/eDP (electromagnetic handedness); (iv) thermodynamic causal arrow (PCD-cycle-ordering, pending); (v) cosmological-vacuum asymmetry (Capotauro nucleation event, pending). At v2.0 v1.0 SHIP: three manifestations CLOSED at full Layer 3 (THEO-CAP-1 + THEO-SD-CHIR-1 + THEO-SD-CHIR-2); two manifestations registered for future-window work. Anticipated full-five-manifestation closure prediction: $|M^{(i)}| = |M^{(ii)}| = |M^{(iii)}| = |M^{(iv)}| = |M^{(v)}| = \chi/6$ at substrate level. See `research_frontier.md` OPEN-SD-CHIR-PRIMITIVE.

### Q5-PAIRING (closed Patch 0429) and Q6-PAIRING (closed Patch 0439)
Open structural sub-questions registered during the Reading C closure trajectory. Q5-PAIRING: identification of the W-bracelet analog of K3's perpendicular-wavefunction pairing convention ($\sigma_1\zeta$-EVEN). Resolved via inversion-parity structure on antipodal pairs (Patch 0429 Session 131 — Finding C-W43: $\zeta^W$ = 4D icosahedral-center inversion + $\sigma_1^W \zeta^W$-EVEN pairing). Q6-PAIRING: analog for qDP/eDP sector. Resolved via combined-$CP$ inversion structure (Patch 0439 Session 133 — Finding C-W46: $\zeta^{qDP}$ = combined $C \times P$ + $\zeta^{qDP}$-EVEN pairing). Both resolutions promoted their respective Q5 / Q6 closures from Layer 2 conservative to full Layer 3 with explicit matrix element factorization.

### Constrained closure-and-inheritance research programme
The manuscript identity reframing produced by ChatGPT v0.9 round-2 revised review (Patch 0474 Session 135). The framing: Capotauro v2.0 is best read not as a speculative substrate theory attempting to become physics, but as a constrained closure-and-inheritance research programme — a programme whose individual papers close specific theorems at full Layer 3 rigor under specific foundational inputs that the papers enumerate honestly, with the foundational inputs inherited from either prior corpus closures or explicitly-registered open work. None of the closures claims to be self-contained; all of them claim to be self-aware about what they inherit. Closest historical analog: the lattice-gauge-theory community's relationship to continuum QCD through the 1980s-1990s. The framing is the framework's standing identity going forward. See paper §13 + anthology chapter "A Different Kind of Research Programme" section.

### Layer 1 dynamical engine (paper §13.6)
The structural-physics-vs-dynamical-physics distinction made explicit at the v2.0 paper's §13.6 subsection. The Substrate-Locality Theorem and cage-shell averaging and pairing-convention identifications are all structural physics — physics of the static geometric substrate organized by the primitive direction $\hat{n}$. What the v2.0 paper does NOT derive is the dynamics of the substrate that produces the static geometric structure: the conscious-point-level perceive-compute-displace cycles that organize information-bit propagation through the lattice. §13.6 registers Layer 1 dynamical engine work as explicitly open with programme epistemic methodology articulated (experimental reality as determinant of truth + convergence-of-mathematical-structure-with-experimental-reality as marker + coherence with multi-layer programme architecture as consistency check + primitive-approached-from-above) grounded in corpus track record. See paper §13.6.

### 11-route falsifier set
The expanded falsifier set at v2.0 (5 routes at v1.0 → 11 routes at v2.0). Maps each falsifier route to the theorem(s) it would disconfirm via the §13.4 modular falsification table. Two of the new falsifiers are cross-sector substrate-handle predictions on the SF-2 W-bracelet V--A coupling (conditional on SF-2 v2.0+ Layer 4 EFT closure of OPEN-FP-SF-2-CHIR) and the SM-2 chiral-polarity-bias (conditional on SM-2 v2.0+ chiral-polarity-bias EFT continuum-limit closure). See paper §13.4.
