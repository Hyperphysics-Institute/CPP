...
The matter-doublet basis under $\sigma_1^{qDP} \zeta^{qDP}$-EVEN pairing convention spans a 2D subspace of $A_{1g} \oplus A_{2u}$ in $D_{5d}$, parallel to the K3 and W-bracelet matter-doublet structures. The chirality operator is $\Cchi^{qDP} = \chi \cdot \tfrac{1}{6} \cdot \hat{s}$ with $\hat{s}$ the qCP-sign operator --- geometrically trivial, $\Cchi^{qDP} \in A_{1g}(D_{5d}) \otimes (C\text{-odd})$ under $D_{5d} \times \mathbb{Z}_2^C$, with the pseudoscalar content carried by the coefficient $\chi$ rather than by a $D_{5d}$ irrep (Patch 0937) --- and has non-vanishing matrix element via Wigner-Eckart, factorizing as amplitude factor $|M_{\text{amp}}^{qDP}| = \chi$ (chirality-eigenvalue matching analog of K3 and W-bracelet) times cage-shell factor $|M_\perp^{qDP}| = \dGamma/\Vcage = 2/12 = 1/6$ via cage-shell averaging on the same icosahedral cage shared across all three sectors.
...
$\hat{C}$ irrep & $B_2(\Dsix)$ via $D_{3d} \cong \Sthree \times \mathbb{Z}_2$ & $\zeta^W$-ODD 1D irrep of $\Dsix$ & $A_{1g}(D_{5d}) \otimes (C\text{-odd})$ \\
...
\begin{itemize}[leftmargin=*]
\item \textbf{FI-CHIR-CONT-13} (qDP/eDP sector specialization): the substrate object $\mathcal{S}^{qDP}$ is a Linear-ZBW configuration on a $\pm$qCP center at $v_{\text{host}}$'s first-shell icosahedron with antipodal-pair refinement; stabilizer $\Gamma^{qDP} = D_{5d}$ of order 20 (antipodal-pair $D_{5d}$ refinement of $C_{5v}$); pairing-convention generator $\zeta^{qDP} = $ combined $CP$ (host-CP-centered spatial inversion $v \to -v$ combined with $\hat{n}$-flip combined with qCP-sign flip $\pm \to \mp$); matter-doublet basis $\{|\Psi_-^{qDP,(1)}\rangle, |\Psi_-^{qDP,(2)}\rangle\}$ in the 2D subspace of $A_{1g} \oplus A_{2u}$ in $D_{5d}$ with opposite $\zeta^{qDP}$-parity; chirality operator $\hat{C}^{qDP} = \chi \cdot \tfrac{1}{6} \cdot \hat{s} \in A_{1g}(D_{5d}) \otimes (C\text{-odd})$ under $D_{5d} \times \mathbb{Z}_2^C$, the pseudoscalar content being the coefficient $\chi$ (sign FI-C-9) and not a $D_{5d}$ irrep; $A_u(I_h) \downarrow D_{5d} = A_{1u}$, which has no support on the antipodal pair (Patch 0937). Inheritance: THEO-SD-CHIR-2 Finding C-W46 (theorem \#64) at Capotauro v2.0 v1.0 SHIPPED \cite{Capotauro}.
