...
### Patch 0631 — 29 May 2026 (Session 148): PCD acronym terminology correction (TODO-012 cleared)

**Terminology lexical correction; no substantive content change.** The PCD acronym expansion in this paper was corrected from "Polarize-Capture-Depolarize" (the Session-146-introduced drift, commit `311bc1e`) back to the canonical "Perceive-Compute-Displace" per the original archived definition at `archive/grok-exploratory-SM/SR_companion_papers/c01_absolute_moment_postulate/absolute_moment_postulate.tex`. The correction restores the consciousness-as-fundamental commitment of the Conscious Point name (Perceive → detect local SSV environment; Compute → process response per A1–A11 rules; Displace → emit outgoing DI-bits / move to new position) and removes the conflation with the between-CPs ZBW dynamics that the Polarize/Capture/Depolarize vocabulary risked.

**Affected `.tex` content.** Seven occurrences in `dynamical_substrate_law.tex`: the abstract / §1 introduction (one reference); the §2.3 Scenario A description (Polarize $\to$ Capture $\to$ Depolarize sequence); the §3 framework primitives axiom A6$^{\prime}$ recap; §3.4 substrate-mechanism description; §7.3 proof Step 2 (Mechanism A coupling); §7.4 closing umbrella substrate-mechanism reference. Additionally two phase-label references at §7.3 ("the Compute phase" formerly "the Capture phase") aligned with the canonical phase naming. No theorem statements modified; no proofs modified; no Open Problems modified; no equations modified. Physical-effect verbs (e.g., "polarizes the host's internal state", "depolarization") retained where they describe what happens during the cycle, not its name. Paper compiles to 33 pages unchanged.

**Programme-wide scope.** This Patch is part of a programme-wide cleanup across ~15 active files (~46 canonical references in place after correction) registered as TODO-012 in `todolist.md`. The cleanup precedes Patch 0632 — the THEO-CHIR-AUDIT-1 chirality audit artifact — under Session 148 "Pattern A" sequencing (terminology baseline cleaned first; audit ships from clean baseline). Files touched on this paper's trajectory beyond the main `.tex`: `sketches/F1_subquestion_pcd_orientation_link.md` (substantial drift in §11 cycle-detail descriptive content; phase labels updated, physical-effect verbs retained); `layer3_promotion/F1_layer3_b1q4_algebraic_derivation.md` (one en-dash form); `documentation_suite/keywords-dynamical-substrate-law.md` + `glossary-dynamical-substrate-law.md` (terminology entries).

**No PDF re-commit.** Per `templates/operating_system.md` v1.0 SHIP frozen-PDF discipline, the v1.0 PDF at `dynamical_substrate_law.pdf` (MD5 49e56be92a3ccc126ce09210b5898794, 33 pages, 489 KB, Patch 0570) remains the canonical reference for the v1.0 milestone. The terminology correction is a textual change to the `.tex` source that would produce a slightly different PDF on recompile; the v1.0 PDF is preserved for archival/citation integrity. A future v1.1 polish patch may bundle the terminology correction with other post-SHIP minor edits if/when a v1.1 PDF refresh is warranted.

**Memory and process notes.** The Session-146 drift was introduced by a previous Claude session via a single commit (`311bc1e`) modifying `master_glossary.md` without rationalization. Claude's persistent memory note about this incident is now updated to specify the canonical Perceive-Compute-Displace expansion to prevent future-session re-introduction of the drift. The cleanup pattern (precise sed substitutions for the triple-form acronym; per-instance prose alignment for phase-label PROSE) is recorded as the methodology for future terminology corrections.
