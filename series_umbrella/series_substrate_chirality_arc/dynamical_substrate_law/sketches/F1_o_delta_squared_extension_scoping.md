...
- **Patch 0587** (Session 145, 26 May 2026): **artifact A2 LANDED** — G2 second-shell inner-product primitive. `hardened_theorems/second_shell_inner_product_primitive.tex` (342 lines LaTeX; 10-page PDF; clean three-pass compile). Result: $w_j \cdot v_{\text{host}} = 1/2$ uniformly across 20 second-shell vertices (the dodecahedron at distance 1 from $v_{\text{host}}$). Proof: Lemma 1 (central angle $\pi/3$ via direct coordinate verification on canonical $w_0 = (1/2)(1,1,1,1)$ with explicit 2-hop bridge) + Lemma 2 (icosahedral group acts transitively on 20 dodecahedron vertices with $C_{3v}$ stabilizers). Chord length $|w_j - v_{\text{host}}| = 1$ exactly. Anti-erasure (Remark 1.4): scoping doc's previous candidate chord length $d_2 = \phi^{-1}\sqrt{3-\phi} \approx 0.727$ did NOT match correct value 1 — registered openly per scoping-document Layer 3 discipline rather than silently corrected. Publication-grade Layer 3 **unconditional**.

- **Patch 0588** (Session 145, 26 May 2026): **artifact A3 LANDED** — G2.1 host-to-second-shell uniform projection. `hardened_theorems/host_second_shell_uniform_projection.tex` (312 lines LaTeX; 8-page PDF; clean three-pass compile). Result: $\hat{w}_j \cdot \hat{n} = -1/2$ uniformly across 20 second-shell vertices. Magnitude ratio $|c_2/c_1| = \phi$ (single-step golden-ratio scaling between first and second shells, where $c_1 = \hat{u}_i \cdot \hat{n} = -1/(2\phi)$ from THEO-DSL-2 / Patch 0552). Publication-grade Layer 3 **unconditional**.

- **Patch 0589** (Session 145, 26 May 2026): **artifact A5 LANDED** — G2.4 second-shell-to-second-shell edge-direction perpendicularity. `hardened_theorems/second_shell_perpendicularity.tex` (298 lines LaTeX; 8-page PDF; clean three-pass compile). Result: $\hat{e}_{j_1 j_2} \cdot \hat{n} = 0$ identically across 30 dodecahedron edges. Proof: chord-difference argument — both endpoints have $w_j \cdot v_{\text{host}} = 1/2$ via G2 (Patch 0587), so the difference annihilates the $\hat{n}$-component identically. Publication-grade Layer 3 **unconditional**.

- **Patch 0590** (Session 145, 26 May 2026): **artifact A4 LANDED** — G2.3 first-shell-to-second-shell edge-direction projection. `hardened_theorems/first_shell_second_shell_edge_orbits.tex` (386 lines LaTeX; 10-page PDF; clean three-pass compile). Result: $\hat{e}_{i,j} \cdot \hat{n} = -1/2$ uniformly across 60 cross-shell edges in $E_{S_1, S_2}(v_{\text{host}})$. Proof: chord-length expansion via G1 (Patch 0571) + G2 (Patch 0587) + 600-cell short-edge length $1/\phi$. Transitivity Lemma: $I_h$ acts as a single orbit of size 60 with stabilizer of order 2. Anti-erasure (Remark 1.3): scoping doc's previous "5 orbit classes under $D_{3d}$ first-shell-vertex stabilizer" framing did NOT match — actual stabilizer is $C_{5v}$ of order 10, single orbit of size 60 — registered openly per scoping-document Layer 3 discipline. Publication-grade Layer 3 **unconditional**.

- **Patch 0591** (Session 145, 26 May 2026): **artifact A6 LANDED** — path-class weight enumeration at $\mathcal{O}(\delta^2)$ producing closed-form $\alpha_2 = -9/\phi^2$. `hardened_theorems/o_delta_squared_path_class_weights.tex` (416 lines LaTeX; 10-page PDF; clean four-pass compile — fourth pass required for hyperref forward-references to later-defined labels in the abstract). Four-step proof: (i) 144 directed 2-edge paths in $\mathcal{P}_2(v_{\text{host}})$ partition into four classes B.1–B.4 by endpoint shell; (ii) per-class direction-product weights computed from G1 + G2 + G2.1 + G2.3 + G2.4 + G3 geometric primitives; (iii) per-vertex sum $S(u_i) = -3$ exactly via golden-ratio algebra; (iv) assembly via icosahedral sum $\sum_i \hat{u}_i = -(6/\phi)\hat{n}$ producing $\alpha_2 = (-1/(2\phi)) \cdot (-3) \cdot (-6/\phi) = -9/\phi^2 \approx -3.438$. Path-class contributions: B.1 (12 round-trips) $+3/(2\phi^3) \approx +0.354$ (~$-10\%$ partial cancellation); B.2 (60 first-shell-trans) $0$ exactly via Patch 0551 G1.2 perpendicularity; B.3 (60 cross-shell) $-15/(2\phi^2) \approx -2.865$ (~83% DOMINANT); B.4 (12 third-shell) $-3/(2\phi) \approx -0.927$ (~27% sub-dominant). Closed-form numerical pre-verification via Python script (golden-ratio arithmetic) matched at machine precision before drafting LaTeX. **Sketch-document Layer 3 under (H5)** path-class weight ansatz $W(P) = 1$ for $P \in \mathcal{P}_2(v_{\text{host}})$ per DG-1 sub-option (A) path-integral framework-extension. Anti-erasure (Remark 1.3): (H5) sketch-document Layer 3 status is load-bearing — downstream consumers must include the qualifier when citing the closed-form.

- **Patch 0592** (Session 145, 26 May 2026): **artifact A7 LANDED — Route C sequence 2 FINAL closure (umbrella)**. `hardened_theorems/o_delta_squared_substrate_locality_umbrella.tex` (329 lines LaTeX; 9-page PDF; clean four-pass compile). Umbrella registration of **THEO-DSL-5 (candidate)** at sketch-document Layer 3 under (H5) under hypotheses (H1)–(H5): the closed-form $\alpha_2 = -9/\phi^2$ assembles with the THEO-DSL-4 structural parallel-to-$\hat{n}$ result (publication-grade Layer 3 unconditional; no (H5) dependency) into the combined OPEN-FP-F1-1 closure $\vec{j}_2(v_{\text{host}}) = (-9/\phi^2)\hat{n}$ at $\mathcal{O}(\delta^2)$. Combined closure rigor at weakest-link (H5) i.e. sketch-document Layer 3. Cross-sector consistency framing with Capotauro v2.0 §5–§6 at structural-parallelism level only (§3 of artifact): same closed-form classes (golden-ratio + small integers), same sign patterns, same $1/\phi^2$ order of magnitude, same path-class decomposition structure (cross-shell dominance, within-shell perpendicularity vanishing); strict numerical-coefficient cross-sector matching NOT established (registered as candidate future cross-sector consistency Patch target). Publication-grade-promotion roadmap via DG-1 sub-option A/B/C resolution registered as priority queue item (G) at Session 145 close. Anti-erasure (Remark 4.3): (H5) sketch-document Layer 3 scope qualifier is load-bearing — downstream consumers MUST cite as "THEO-DSL-5 (candidate) at sketch-document Layer 3 under (H5)" per anti-erasure principle. **OPEN-FP-F1-1 CLOSED at sketch-document Layer 3 rigor under (H1)–(H5)**.

- **Patch 0592a** (Session 146, 27 May 2026): registry propagation for the full Session 145 sequence-2 closure. `frontier_sectors/FP.md` OPEN-FP-F1-1 entry updated: Status → "CLOSED at sketch-document Layer 3 rigor under (H1)–(H5)"; history trail entry recording closure at Patch 0592 (Session 145); Current best result updated with closed-form $\alpha_2 = -9/\phi^2$ + path-class B.1–B.4 decomposition + cross-sector parallelism framing; Falsification route updated to publication-grade-promotion falsifiers; Closure trajectory section added describing the two-stage Sessions 144–145 closure; Publication-grade promotion section added pointing to priority queue item (G). Header bold tail extended with Patches 0587–0592 sequence-2 closure announcement. `theorem-registry.md` updated with new **THEO-DSL-5 (candidate)** row in SD section after THEO-DSL-4 ($\mathcal{O}(\delta^2)$ closed-form substrate-current coefficient at vertex-aligned Reading C; sketch-document Layer 3 under (H5); publication-grade Layer 3 unconditional promotion DEFERRED to priority queue item (G); downstream cite-as qualifier mandatory); SD section header advanced 19 → 20; Summary Statistics SD row 19 → 20, Total 71 → 72; Theorems:Axioms ratio updated to (72 + 9 corollaries) / 6 axiom slots ≈ 13.5× (was 13.3×); history note added recording the Session 145 sequence-2 closure as the longest single-trajectory hardening sequence in CPP programme history (11 artifacts across Sessions ~140–145). This scoping document §9.5 patch landings record extended with consolidated entries for Patches 0587 + 0588 + 0589 + 0590 + 0591 + 0592; Route C sequence 2 status advanced from OPEN to CLOSED. **The longest single-trajectory hardening sequence in CPP programme history** at 11 hardened-theorem artifacts (Patches 0550 + 0551 + 0552 + 0571 + 0585 + 0587 + 0588 + 0589 + 0590 + 0591 + 0592; ~3,478 lines LaTeX combined). Single-artifact-per-Patch discipline: 100% compliance across the 11-artifact trajectory. Anti-erasure registration of scoping-doc slips: 3 instances registered openly at Patches 0587 + 0590 + 0591/0592. End-to-end verification: 11/11 passes via fresh codeload tarball clone + git am + MD5 match + 3-4-pass pdflatex compile.
...
**Route C sequence 2 status:** **CLOSED at Patch 0592** (Session 145, 26 May 2026). The sequence-2 trajectory landed 6 single-artifact-per-Patch hardened-theorem deliverables (Patches 0587 + 0588 + 0589 + 0590 + 0591 + 0592; artifacts A2 + A3 + A5 + A4 + A6 + A7 in scoping-doc terminology — A4 and A5 reordered relative to scoping plan per Session 145 execution sequence) within Session 145 at ~2,083 lines LaTeX combined across six `.tex` artifacts in `hardened_theorems/`. Sequence-2 closure produces **THEO-DSL-5 (candidate)** registered at Patch 0592a (Session 146, 27 May 2026) at sketch-document Layer 3 under (H5) — closed-form $\alpha_2 = -9/\phi^2 = -9(2-\phi) \approx -3.438$ at $\mathcal{O}(\delta^2)$ via four-step path-class enumeration. Actual closure cost: **6 Patches in 1 session**, well below the §4.4 + §5.1 estimate of 4–8 sessions for sequence-2 closure. Combined Route C trajectory (sequence 1 + sequence 2) artifact count = 7 (A1 + A2 + A3 + A4 + A5 + A6 + A7); cumulative F.1 Layer 3 hardened-theorem sequence reaches 11 artifacts at Patches 0550 + 0551 + 0552 + 0571 + 0585 + 0587 + 0588 + 0589 + 0590 + 0591 + 0592 (~3,478 lines LaTeX combined) — **the longest single-trajectory hardening sequence in CPP programme history** across Sessions ~140–145.

**Route C trajectory status:** **COMPLETE at Patch 0592**. **OPEN-FP-F1-1 is CLOSED at sketch-document Layer 3 rigor under (H1)–(H5)** — combined closure rigor at the weakest-link of (H5) framework-extension hypothesis. Publication-grade Layer 3 unconditional promotion via DG-1 sub-option A/B/C resolution registered as priority queue item (G) at Session 145 close (DG-1 sub-option A path-integral rigorisation default; 4–8 sessions / sub-option B new framework axiom MA.3; 1–2 sessions / sub-option C Layer 4 derivation OPEN-FP-F1-2; 10–20 sessions). DG-1 sub-option selection is a strategic decision deferred to the (G)-trajectory opening Patch if/when (G) becomes active; not pre-selected at the present Patch 0592a registry propagation.
