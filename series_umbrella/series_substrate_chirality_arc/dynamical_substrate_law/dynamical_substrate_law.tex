...
{\small Version~1.3 --- 17 August 2026 (Patch 3214: added a How-we-review methodology note stating plainly that AI panel review is a self-applied verification method and not journal peer review; twelve chirality papers retitled so the descriptive title leads and the identifier is demoted to a subtitle. No claim, equation, number or section changed.)}} update + CHANGELOG update +
...
The Conscious Point Physics (CPP) programme aims to derive Standard Model physics from the discrete geometry of the 600-cell polytope, with zero free parameters. The framework posits a substrate of Conscious Points (CPs) executing Perceive $\to$ Compute $\to$ Displace (PCD) cycles, with Dipole Points (DPs) as composite intermediate objects, governed by eleven primitive axioms A1--A11 (\textit{Tetrahedrons All the Way Down}, Abshier 2026; central bibliography at \texttt{bibliography/cpp\_references.bib}, to be cited via \texttt{\textbackslash cite} at Patch 0566+).
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Scenario A (positive closure)}: The link is derived via substrate-mechanism. Equation~\eqref{eq:link-constraint} holds with $\sigma$ fixed by a global sign convention; the alignment is derived from the mechanism rather than posited as a separate primitive. A specific substrate-mechanism (e.g., Mechanism~A below) supplies the derivation, with coupling to the PCD cycle's Perceive $\to$ Compute $\to$ Displace sequence and a locality argument extending the derivation from the host vertex to spatially-displaced vertices.
...
The Perceive $\to$ Compute $\to$ Displace cycle at each CP is the elementary discrete-time temporal process per CPP axiom A6$^{\prime}$. The cycle's three phases progress at rates determined by DI-bit propagation: the Perceive phase detects the local SSV environment via incoming DI-bits; the Compute phase processes the SSV-content and computes the response per A1--A11 rules; the Displace phase broadcasts outgoing DI-bits to neighbouring CPs.

Mechanism~A's coupling to the PCD cycle operates at the Compute phase. Under~\eqref{eq:mechanism-a-formal}, DI-bits captured from the $+\hat{n}$ side of the host CP arrive faster than those from the $-\hat{n}$ side. The asymmetric DI-bit arrival induces a definite preference for one cycle progression direction over the other: the "forward" direction (Perceive $\to$ Compute $\to$ Displace) is the one in which the asymmetric DI-bit flow is consistent with energy minimization. The cycle-orientation pseudovector $\omegaPCD$ at the host vertex thereby acquires a definite alignment with $\hat{n}$, satisfying~\eqref{eq:link-constraint}.
...
\end{equation}
where $\ell_0$ is the $H_4$-idealized scale and $\varepsilon$ is the perturbation parameter. \textbf{Relabelling (founder ruling, 14 September 2026, PD-006(a); Patches 0972, 0973, 0979): $\ell(\hat{e})$ is a directed traversal cost --- equivalently a hop time --- not a metric edge length.} The factor $\hat{e}\cdot\hat{n}$ is reversal-odd, so the same edge acquires two different values by traversal direction; a metric is a symmetric bilinear form, so every length built from an inner product is reversal-even on any lattice whatever (checked over 200 arbitrary positive-definite metrics: forward and backward differ by exactly zero). \emph{The equation is unchanged; only its description was wrong.} \textbf{The counterfactual is the point}: were $\ell$ a genuine reversal-even length, depending on the midpoint $\hat{m}\cdot\hat{n}$, then $r = c/\ell$ would be reversal-even and would generate the $A$-term with $\delta = 0$ exactly --- so MA.1's reversal-odd first harmonic cannot arise from a metric length perturbation at all, and its form \emph{requires} non-reciprocity. The 1-form that permits a directed cost is a Randers/Finsler $\beta = \varepsilon\hat{n}$; \textbf{this is not a new field}, since $\hat{n}$ is already a substrate primitive (FI-C-RC-1) --- what changes is whether $\hat{n}$ enters as a direction, giving even structure only, or as a 1-form, which makes odd structure available. \textbf{Physically the non-reciprocity is the arrow of time}: detailed-balance violation comes entirely from the reversal-odd part (the even term $A(\hat{m}\cdot\hat{n})$ gives maximum cycle affinity exactly zero at $A = 0.3$, $0.5$ and $1.0$, while the odd term violates on 420 of 1200 faces), and THEO-CHIR-MERGE-2 establishes $\mathrm{sign}(\delta)$ as P-even and T-odd. So the substrate's geometry is reciprocal and its dynamics are not, which is what an arrow of time means; the closest physical analogy is light in a flowing medium, where the distance is identical both ways and the traversal time is not. Equations~\eqref{eq:mechanism-a-formal} and~\eqref{eq:edge-length-perturbation} share the same first-order linear structure, which motivates Mechanism~A as the leading candidate over mechanisms B (substrate-orientation field with gauge-like coupling) and C (position-dependent clock-skew), both of which would introduce additional structure beyond the Reading~C pattern.
...
The substrate primitive's magnitude $|\chi| = \phig^{-3} \approx 0.236$ controls the Reading~C edge-length perturbation parameter $\varepsilon$ in Equation~\eqref{eq:edge-length-perturbation}. The relationship between $\varepsilon$ and the Mechanism~A propagation-rate parameter $\delta$ in Equation~\eqref{eq:mechanism-a-formal} \textbf{is pinned}: constant-speed traversal of a perturbed edge gives $\delta = -\varepsilon$ at first order, with second-order coefficient $+\varepsilon^2(\ehat\cdot\nhat)^2$ (Patch 0972, \texttt{l4e\_delta\_epsilon.md}, verify 5/5). The two are therefore \emph{not} independent framework inputs. \textbf{Pinning the relation exposes a physical commitment the equations have always made and the prose has never stated}: $\ehat\cdot\nhat$ is reversal-\emph{odd}, so Equation~\eqref{eq:edge-length-perturbation} assigns the same edge two different values by traversal direction, which no metric length can do. The quantity is a directed traversal cost, and the substrate is non-reciprocal --- \emph{in its dynamics, not in its geometry} (\S\ref{sec:scenario-a-mechanism-a}).
...
\]
where $\mathcal{E}_1(v)$ is the set of first-shell unit-direction edges at $v$ (i.e., the 12 unit-direction edges from $v$ to its first-shell neighbours in the 600-cell). \textbf{Here $r(-\hat{e}; v)$ denotes the reverse traversal of the same edge}, governed by the rate law at the far end: $-\hat{e}$ is not itself an edge direction at $v$, since the first-shell edge set is \emph{not} centrally symmetric, so the literal reading has no referent and the reverse-traversal reading is the one the construction requires (which in turn requires the vertex-uniformity above). The antisymmetric form is forced by A6$^{\prime}$'s Perceive/Displace split together with vertex-uniformity, and recovers $\alpha_1 = 6/\phig^2$ as a check (Patch 0941, \texttt{l4c\_antisymmetric\_current.md}, verify 8/8). Further, and $\bigO{\delta^2}$ collects contributions from higher shells per the perturbation-locality propagation rule (Theorem~\ref{thm:perturbation-locality} + Corollary~\ref{cor:shell-locality}). At $v = \vhost$ specifically, $\mathcal{E}_1(\vhost) = \{\hat{u}_i : i = 1, \ldots, 12\}$ and the construction reduces to Equation~\eqref{eq:di-bit-current-construction}.
\end{quote}

Axiom MA.2 makes the framework-local \emph{construction} explicit for any host vertex, not just the chosen $\vhost$ of vertex-aligned Reading~C. \textbf{It is the construction that is vertex-independent, not its value}: the net current's magnitude tracks the $\hat{n}$-shell of the vertex, of which there are nine as graded by $v \cdot \hat{n}$. Vertex-uniformity of the rate law is \emph{derived}, not assumed: it follows from A11 via the transitivity of $\mathrm{Stab}(\hat{n})$ on each shell --- not via vertex-transitivity of the 600-cell, which is invalid once $\hat{n}$ is fixed (Patch 0940, \texttt{l4b\_vertex\_uniformity.md}, verify 7/7). The construction's vertex-independence is operationally important for the substrate-locality theorem of \S\ref{sec:substrate-locality-theorem}, which applies it at any vertex of the 600-cell substrate that satisfies the Reading~C alignment.
...
\item At \emph{Layer~4 axiomatic derivation} (the long-term programme target), Mechanism~A's derivation from CPP A1--A11 alone is \emph{partially discharged}, and exclusion class E1 must be narrowed accordingly. \textbf{Two of Mechanism~A's four framework commitments are now derived}: vertex-uniformity from A11 (Patch 0940) and the antisymmetric current construction from A6$^{\prime}$ + vertex-uniformity (Patch 0941). \textbf{A third is derived at its leading harmonic}: the reversal-odd first harmonic of the rate law is unique up to scale and is exactly MA.1's form (Patch 0949), so Equation~\eqref{eq:mechanism-a-formal}'s functional form is \emph{forced} at that order rather than posited. \textbf{What remains open} is that the first-harmonic family is two-parameter, $A(\hat{m}\cdot\nhat) + B(\ehat\cdot\nhat)$: neither $A = 0$ nor truncation at first harmonic is forced, and the $A$-term cancels identically in the antisymmetric current, so all $\bigO{\delta^1}$ content sees only $B$. E1 therefore now reads \emph{``MA.1 beyond its reversal-odd first harmonic''}, not ``Mechanism~A as framework axiom, not derived''. The residual is registered as Open Problem~\ref{op:layer4-mechanism-a}.
\end{itemize}
...
\begin{description}
  ...
  \item[\texttt{OPEN-SS-B1}] \hfill \\ Layer-4 axiomatic derivation and second-order extension of the Mechanism-A minimal-local framework.
