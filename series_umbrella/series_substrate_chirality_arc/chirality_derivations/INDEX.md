...
**Current arc state (from CHIR.md):** spatial verdict **V3 confirmed / V1 excluded, conditional on Mechanism A** (THEO-CHIR-CAPACITY-1 enacted Patch 0927, 3/3 review-closed; V2 still excluded at axiom level, reopener = cross-sector OPEN-SM-4) · temporal verdict **W3→W1 conditional on Mechanism A** (TARROW-2, Patch 0692). Both deep-capacity axes now review-closed conditional on Mechanism A. The 1d-β-ii capacity half is resolved (negative-conditional); what stays open is the V2 reopener (OPEN-SM-4) and the residual (derive pointwise non-degeneracy of the dynamical η — the narrow PCD door), unconditionalized by discharging Mechanism A (OPEN-FP-F1-2 / F.1 §14.17).

**Cross-band development trail:** CAPACITY-1 was developed across two patch-bands (08xx closure computation, F.1/DSL window; 09xx review/adjudication/enactment, chirality lane). The full ordered trail across both bands is the problem-keyed history `problem_histories/PH-OPEN-CHIR-1d-beta.md` (per the `parallel_dev/multi_window_protocol.md` cross-band development-trail convention).
