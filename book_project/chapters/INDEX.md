...
0a. **`spin-I_kepler_in_the_electron.md`** — Spin I v2 16 March 2026 (v2.1 math-audit correction Aug 2026, Patch 3210); chapter written retroactively 19 Aug 2026 at the spin arc's completion (the paper predates the anthology cadence of 26 April 2026). Electron spin $\hbar/2$ exactly, from the orbital angular momentum of a captured Dipole Particle: Kepler's third law turning the $r_{\rm out} = 2r_{\rm in}$ condition into the $2\sqrt{2}$ frequency ratio; $r_{\rm in} = a_0/[4(1+\sqrt2)^2] \approx 2.27$ pm at zero free parameters. Dramatic centerpiece: the half emerging exactly from geometry that Lorentz's 1925 superluminal-surface objection never touched — the objection killed the spinning ball, not the possibility of motion.
0b. **`spin-II_the_integer_in_the_pipe.md`** — Spin II v2 16 March 2026; chapter written retroactively 19 Aug 2026 (same cadence note). The $r_{\rm out} = 2r_{\rm in}$ debt paid in integers: the ZBW cloud as a stopped pipe (node at center by CP Exclusion, free end at $r_{\rm th} = \hbar/2m_ec$), Mode 2 as the lowest mode with both an interior antinode and node, and the ratio $= (2r_{\rm th}/3)/(r_{\rm th}/3) = 2$ — independent of every physical constant. Dramatic centerpiece: the factor of two revealed as the arithmetic of sine zeros, the oldest exact move in physics.
...
14. **`spin-III_the_twelve_sided_room.md`** — spin-III v1.0 SHIPPED 19 Aug 2026 (Patch 3248). Completion of the spin arc: the continuum resonator of Spin I/II checked — and enforced — on the lattice. The true Voronoi cell (founder ruling A1: regular dodecahedron, 600-cell dual = 120-cell); the corrected instrument (two semantic traps diagnosed and closed) returning MODE2-RECOVERED under frozen verdicts; the character-arithmetic Lemma ($m_l = 0$, $l = 1..5$), Selection Theorem ($k(6,1)R = 8.211 > 3\pi/2$), and Protection Theorem ($O(\epsilon_6^2)$, predicted 0.0026 vs measured 0.0031); OPEN-QM-8 panel-ratified SUBSTANTIALLY RESOLVED (CONV-026). Dramatic centerpiece: selection-by-exclusion — the room cannot offer the electron any spin but one half — sealed by the eight-interloper fingerprint (radial Mode 2 at global index ten, predicted and found).
...
*Spin trio (0a, 0b, 14): primary outline mapping pending — the revised outline has no dedicated spin/QM-sector chapter row yet; assign at the next outline revision.*
