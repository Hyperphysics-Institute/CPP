...
### TODO-4133-F3CALC — F3 as a calculation on the real cages under the GP-summation picture (registered Patch 4133, EW/SS lane)

R-F3 is closed as a founder question (below). F3 itself — the strong sector stays P-even — is still open, and is now a bounded computation: under the working extension (per-CP, per-Moment declaration; all channels incl. A_i summed at the receiving GP over its PSR shell), does the summed **A·V** a confined quark reads vanish on the corpus's actual nucleon cage? **Step 1 (D-3, before any computing):** locate in SF-2/SF-3 the type and polarity assignment on the icosahedral 12- and dodecahedral 20-shells, and establish what spin (A) assignment the amendment implies — none was ever made (founder, 4107). **Step 2:** test inversion symmetry of that configuration *with spins*; if it holds, b(−r) = −b(r) and the shell sum cancels pairwise (4101's result in the amendment's language). **Step 3 — required, or the argument proves too much:** the W bracelet's D6 ring is also inversion-symmetric in position; show on the same footing why the bracelet's response is handed (the corpus answer is that it reads the *incoming free particle's* bit, 4097) and the cage's is not. Run on the real shells and the real bracelet, not generic configurations (Session 233 method warning). Doc: `axiom_maturation/4133_emergent_handedness_assessed.md` §3. **Lane: EW/SS.**
