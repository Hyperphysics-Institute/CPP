...
**STAGES 2–4 OPEN, in order:** (2) axiom text — A1′/A3′, `templates/` — and the **43 verify scripts**, which must agree with the glossary or the gates drift; (3) the three `.tex` papers (GR-1b, GR-2, QM-1) **with recompiles** — Thomas's mechanical action; (4) remaining live `.md`, heaviest series first (series_phenomena 108, series_gravitation 60, series_relativity 49). **Never:** `handovers/`, `session_logs/`, `archive/` (33 files, append-only). Classification is a **reading** job — regex was tried and discarded at 4145.
