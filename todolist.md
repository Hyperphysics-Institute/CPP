...
### TODO-4162-EDGEINDEX — `e_i · V_i` is an index collision; re-letter the EDGE index (registered Patch 4162, GR/WORKFLOW lane)

The 12-edge selection rule is written `e_i · SSV_net` (c03, GR-1b). Under the retirement that becomes **`e_i · V_i`**, where the two `i` subscripts mean different things — the **edge index** and the vector's **Cartesian component**. Unacceptable in a corpus that has just spent a session on one-token-two-referents. **Recommendation: re-letter the EDGE index** (`e_k`, `ê_n`) — far smaller footprint than re-lettering the vector, which appears at 100+ sites. Flagged in the glossary at 4162, **not executed**; the edit pass should carry it. **Lane: GR/WORKFLOW.**
