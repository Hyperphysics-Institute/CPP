...
The NC constituent weights of 4198/4216 split exactly (`code/4235_nc_weight_split.py`): EM part −4Qs² per constituent with the derived s² = 3/(8φ) — no mechanism needed; isospin part +1 (bare +qCP core), −2 (linear −eCP). The elastic W⁰ capture-and-re-eject event must give a unit throw one way at a bare core and a double throw the other way where a linear −eCP occupies the centroid. First test: −2 = (−1 reversed event) + (−1 core event cancelled by occupancy). Not computed. Route (b) not needed for anything in the split. **Lane: EW.**
