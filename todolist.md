...
- **SM lane — SM-2 composition-level corrigendum (owed since 3531; CONV-038).** Its sign is now derived, not asserted: the + host is the stabilised class in the FI-C-9 convention fixed by the corrected §10. Items: cage assignment (down = +qCP core + linear −eCP + orbital eDP + cloud), §10 sentence, the down mass fit. Cheap now; the SD corrigendum above should land with it or before it so §10 and C-W46 quote the same operator.
- **Chirality lane — OPEN-FP-F1-2, the unconditionalizer.** **Corrected at Patch 0940 (D-2):** L4-D is NOT the next item — its chirality half was resolved at 0647 (THEO-CHIR-MERGE-2, verdict M1-χ: `sign(δ)` is the P-even/T-odd arrow, not a chirality) and its residual is OPEN-CHIR-2a, STATUS-ANSWERED at W3 (0661) with the upgrade pinned to the capacity engine — not workable without F.2. The 0933 flagship scope and the 0936 review both still say "L4-D carved as first sub-target, UNEXECUTED"; **both predate 0647 and should not be acted on** (wording fix owed to `flagship_assembly_scope.md`). What actually lifts the V3/W3 conditionality is the F.2-free remainder:
  ...
  - **L4-A — OPEN, next.** Derive MA.1's rate-law FORM (linear in ê·n̂, single scalar δ, no O(δ⁰) tangent term) from A1 + A6′ by representation-theoretic uniqueness. L2.5, no F.2. The hard half of the remainder.
  - **L4-C — OPEN.** Derive MA.2's antisymmetric first-shell current construction from A1 + A6′ Displace-phase dynamics; the *locality* half is already THEO-DSL-1. No F.2.
