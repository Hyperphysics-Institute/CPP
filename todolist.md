...
**STAGE 1 DONE at Patch 4146 — names settled by founder decision (19 Sep): `V_i` (the LSP′ l=1 slot, the gravitomagnetic **potential** g_ti) and `SSV_disp` (the **field** a CP displaces by); `SSV_net` RETIRED.** `master_glossary.md` rewritten: two entries plus a retired-token row, the explanatory note, and the **bare-`V` prohibition** (V alone already carries V(r), vertex counts V ∈ {4,12,20,30} and more at ~800 sites — this quantity is always `V_i`). Two corroborations found while executing: AP-4d **already** listed them separately (*"beyond the three payload slots — Φ, V_i, Q_ij, SSV_net, SSV_abs"*), and **all 11 remaining glossary occurrences classified by reading as the displacement field, none as the potential**.

**STAGE 2 DONE at Patch 4147.** Live normative text fixed: **`axiom-registry.md` — 6 occurrences** (A1′'s AP-4d clause, A3′'s definitional clause, the A3′/AP-4 history note), each read and classified as the **displacement field** → `SSV_disp`. `templates/` and `bootup.md` carried none. **POLICY SET (and it narrows the 4145 plan):** the retirement governs **live normative text only**; **frozen provenance is NOT retro-edited** — `handovers/`, `session_logs/`, `archive/`, **and the 43 verify scripts** (103 occurrences). A verify script is the evidence for a registered patch, fixed when it ran; editing it alters provenance for a cosmetic gain and risks breaking a script that still runs. **All 103 were scanned for potential/metric-component markers: 6 matched, all inside the two audit scripts (`4141_bivector_partner_absent.py`, `4142_ssvrole_resolved.py`) which discuss the distinction and legitimately quote the retired token; the other 97 are uniformly `SSV_disp`.** Recorded in the glossary so no per-file annotation is needed.

### TODO-4161-STARVED — is any physical regime ARRIVAL-STARVED enough for b to see the lattice? (registered Patch 4161, EW lane — the only place the anisotropy could still live)

4161 showed the spread of ⟨b⟩ over spin-axis directions collapses with the number of DI-bit arrivals summed into V: **0.5034 at N = 1, 0.1127 at N = 2, 0.0106 at N = 4, and at the sampling floor by N = 12** (the generic case, one per PSR-shell direction). **At N = 1 the lattice shows through completely** — that is 4160's staircase, now understood as the N = 1 row of a table whose physical case is N ≥ 12. **The open question:** is any regime arrival-starved? Candidates: very low DP-sea density, or the first Moment after a creation event. **Not examined.** If one exists it would carry the anisotropy signature — and would be the amendment's distinctive prediction after all. **Lane: EW.**

### TODO-4160-ANISOTROPY — **CLOSED AS WITHDRAWN at Patch 4161 (my modelling error)** — **b can see the lattice's global orientation, and the effect is not observed** (registered Patch 4160, EW lane — LIVE FALSIFIER-CLASS, ahead of F5)

**WITHDRAWN at 4161.** 4159 and 4160 both computed ⟨b⟩ by substituting **the twelve edge directions** for û — but **the 12-edge rule quantises the DISPLACEMENT, not V**: c03 says *"the 12-edge selection rule chooses the lattice edge i\* that maximizes e_i · SSV_net"*, so **V is prior and continuous** and the edge is chosen *from* it, while the amendment evaluates b on V (4120: *"b ~ A·V is derived, not postulated"*). **b never sees the twelve directions; it sees their SUM**, and the sum is isotropic at second order **exactly** — (1/12)Σ e_i e_iᵀ = (1/3)I, off-diagonals 2.5e−18. So escape (b) was not "cheap and contradicts c03"; it was simply **correct**, and calling it a contradiction was part of the same mistake. **Falls with it: 4159's "first distinctive empirical signature"** — same error, opposite sign. **Stands: 4158's ⟨b⟩ = (v/c)cos θ**, now on a third and finally sound premise (the 5-design covariance of the summed register). Residual → [[TODO-4161-STARVED]] above. *(original entry:)* **Link 2 of CRYSTAL turned the 4159 "signature" into a contradiction.** The 600-cell needs no material crystal to orient it — **it already has a global orientation**, the same twelve neighbour directions at every vertex — so 4159's orientation-averaging, which rescued ⟨b⟩ = (v/c)cos θ, **is not available; there is nothing to average over.** The corpus's own argument (*founders_vision*: "the same in every direction at every vertex… no built-in bias") proves **vertex-transitivity**, not **directional isotropy**. **Why it went unnoticed:** the 12-set is a spherical **5-design**, so polynomial moments of degree 2 and 4 are direction-independent **to 1e-16** — every observable CPP has tested is low-degree polynomial, which is why SR-1's emergent Lorentz invariance has held. **But b is SIGN-valued and sign() is not a polynomial:** the spread of ⟨b⟩ over spin-axis directions is **0.33 at β = 0.1, 0.50 at β = 0.2 and 0.5, 0.17 at β = 0.9** — 17–50% of full polarisation. **b is the first observable in CPP that can see the lattice's anisotropy, and the amendment introduced it.** **Predicts** a sidereal, icosahedrally-structured modulation of beta-decay polarisation at tens of percent — **not observed since 1957**, and bounded at ~1e−33 GeV by the same comagnetometer class used at 4139. **Also reopens Patch 4143**, whose "a covariant theory has no fixed cosmic direction" holds for polynomial observables but not for b. **Four candidate escapes, none derived:** (a) cage orientation decorrelates from the lattice — hard, the cage *occupies* vertices; **(b) the 12-edge selection and the b-evaluation happen at different stages — cheapest, but contradicts c03/GR-1b as written**; (c) b enters only through low-degree polynomials — contradicts B3's linearity; (d) lattice domains smaller than a lab — unproposed anywhere in the corpus. Doc: `series_standard_model/axiom_maturation/4160_lattice_anisotropy.md`. **Lane: EW.**

### TODO-4159-CRYSTAL — **link 2 answered at Patch 4160; see TODO-4160-ANISOTROPY above** — can an ORIENTED-lattice source test the predicted deviation from the v/c law? (registered Patch 4159, EW lane — the amendment's first candidate distinctive signature)

Patch 4159 found that a **fixed** lattice orientation gives a **staircase**, not β: vertex-aligned ⟨b⟩ − β = **−0.200 at β = 0.2, +0.333 at β = 0.5**; face-aligned **+0.300, 0.000, +0.200**. **An O(10–40%) deviation from the standard polarisation law — large, not marginal** — and no other account of V−A predicts *any* lattice-orientation dependence, because no other account has a lattice. **This answers Patch 4155 §4's "A_i currently has no independent empirical signature."** **But the chain has three unverified links:** whether any real experiment emits polarised particles from an oriented lattice (a single crystal); whether the substrate lattice's orientation is tied to a material crystal's at all; and whether an emitted particle retains its emitter's cage orientation. **The prediction is only as good as that chain.** **Lane: EW.**

### TODO-4158-ZBWMEASURE — **DISCHARGED at Patch 4159, but NOT by the route it named** — show the ZBW double rotation is EQUIDISTRIBUTED on the sphere (registered Patch 4158, EW lane — the one premise the magnitude result rests on)

**Result at 4159: the premise is justified by the ENSEMBLE, not by the orbit.** **The χ draft's double rotation does NOT sweep the sphere uniformly** — Weyl gives equidistribution on the **Clifford torus**, which the Hopf map sends to **one latitude circle**, and the naive 3-projection gives ⟨cos²θ⟩ = 0.500/0.293/0.100 against the uniform 0.333. **So the Weyl route filed at 4158 would not have worked.** What does work: û is **discrete** (the 12 icosahedral edges, c03), a *fixed* orientation gives a **staircase**, but **orientation-averaging restores ⟨b⟩ = β exactly** (0.0501, 0.2005, 0.5001, 0.7998, 0.9500) — and orientation-averaging is physically compulsory, since a beam's particles share no cage orientation. **4158's result stands on a different and better premise than the one it stated.** Side effect: the χ draft loses one thing it was thought to supply. New signature → [[TODO-4159-CRYSTAL]] above. *(original entry:)* Patch 4158 derived **⟨b⟩ = (v/c)·cos θ**, the measured V−A law, from b being a sign (A1′) + the response being linear in b (B3) + **one geometric input: that the ZBW measure is isotropic on the sphere**. The χ draft axiom says the ZBW is a **double** rotation — circulation in two orthogonal planes — and two orthogonal circulations at incommensurate frequencies do sweep the sphere, but **uniform sweeping is not shown**. Likely route: a **Weyl equidistribution** argument on the torus of the two phases. **The gap cuts in the corpus's favour:** both single-plane alternatives are *excluded by the measured law* (⊥ gives sign(β), maximal at all speeds; ∋ gives the arcsine law), so the result is **evidence for** the double rotation — but evidence is not proof, and until equidistribution is shown the magnitude result rests on an assumption. **Lane: EW.**

### TODO-4157-CHIRMAG — **DISCHARGED as to form at Patch 4158; open as to the isotropy premise** — the magnitude half of the detectability theorem (registered Patch 4157, EW lane)

**Result at 4158: ⟨b⟩ = (v/c)·cos θ, the measured V−A law, no fitted parameter.** b is a **bit** and a CP's SSV_disp is **ZBW circulation (~c) + drift (v)**, so b flips over a cycle and the drift biases the fraction spent on each sign. For û uniform on the sphere, A·û is uniform on [−1,1] (Archimedes), giving ⟨b⟩ = (1+β)/2 − (1−β)/2 = **β** exactly at every speed; the angular law follows. **Maximality answered:** the coupling is 100% because **b takes only ±1** and B3 makes the response linear — there is no intermediate value to couple to; **the coupling is maximal and the observation is diluted**, ⟨b⟩ → 1 as v → c (0.9990 at β = 0.999), the same massive/massless split as 4143 arriving as a magnitude. **Premise:** isotropy → [[TODO-4158-ZBWMEASURE]] above. **Not derived:** the sign (left vs right), set by the polarity-to-duality assignment in the χ draft. *(original entry:)* **THEO-CHIR-1 (Patch 4157) completes only the allowed/forbidden half.** It is a **selection rule**: a pseudoscalar observable vanishes identically if any improper symmetry fixes the full configuration (positions + charges + spins), and is *allowed* otherwise. **It cannot yield a magnitude** — not 100% V−A, not any number. The founder asked for the derivations completing "the theorem on manifesting axial vector effects as empirical detections"; this is half of it, and **the magnitude half is a different argument that nothing in this session has begun.** Overlaps OPEN-FP-SF-2-CHIR (theorem-level closure of 100% V−A at the massless helicity limit), which stays open. **Lane: EW.**

### TODO-4156-TRAVERSAL — **CLOSED AS REFUTED at Patch 4157** — is A_i's 3D realisation the TRAVERSAL SENSE of a closed CP chain? (registered Patch 4156, EW lane — Claude's proposal, partly corpus)

**REFUTED at 4157.** A traversal sense is a physical degree of freedom only if **no symmetry reverses it** — and of the bracelet's elements preserving *both* positions and Cor. Wcp's alternating polarity, **six reverse the cyclic order** (m0, m2, m4 and the C₂′ axes from them). The two senses are **the same physical state**; the static bracelet is **achiral**. **What replaced it — THEO-CHIR-1, and it is a better result:** a configuration is parity-odd-detectable **iff positions + charges + spins admit NO improper symmetry**. Applied: the bracelet with spins along its own ring normal is **0/64 chiral — symmetry-forced to zero**, because σ_h fixes any spin along the normal. **So the bracelet alone can NEVER show parity violation, and its handedness must be imported — which DERIVES the filter table's bracelet row** (the founder's 4097 ruling that it reads the *incoming* particle's bit), a row [[TODO-4137-FILTERTABLE]] had as a conditional worker proposal. *(original entry:)* **Corpus part:** the χ draft axiom already says an axial vector is a **circulation** — *"the ZBW of a CP is a double rotation… whose relative sense (self-dual or anti-self-dual) is fixed by the CP's polarity."* So A_i's realisation is **not a spatial arrangement but a traversal sense**, whose mirror image is the same points gone around the other way — which is why no 3D snapshot of positions shows it. **Proposal part (mine):** the W is SF-2's 6-cycle **bracelet**; a 6-cycle has **two traversal senses**; on decay the bracelet **opens at a vertex** and which vertex is next depends on that sense; the opening vertex sets the emission direction relative to the ring axis → **the V−A angular asymmetry**. No force anywhere — a successor rule in a cyclic order, the same kind of object as the 12-edge rule. **Consilience:** the proton's three occupied tetra vertices also carry two senses, equally populated in a ground state, which is *why* 4134's orientation average vanishes; the W's sense is set by the incoming particle, which is 4135's free/confined distinction from a third direction. **NOT settled:** steps 2–4 are mine, not corpus; the **magnitude** is untouched (why 100% V−A, not slightly chiral — OPEN-FP-SF-2-CHIR stays open); and it may be the same mechanism as [[TODO-4155-EDGETIE]] at composite scale (edge tie-break for one CP, traversal sense for a chain) — whether one rule or two is unestablished. Doc: `series_standard_model/axiom_maturation/4156_what_configuration_carries_a_i.md`. **Lane: EW.**
...
**Three times in this session a correction already recorded in the corpus was read past:** 4138 (a computed table whose prose inverted it), 4161 (c03's own sentence, which I had quoted myself six patches earlier), 4168 (the founder's Patch 0733 correction, argued across several rounds and written into `founders_vision.md`). **In each case the corpus contained the answer and the failure was in SEARCH, not in reasoning.** [[TODO-4138-TABLEPROSE]] was scoped to script tables; on this evidence the discipline is broader and belongs at bootup. **Rule to adopt:** before asserting that the corpus does not supply a premise — especially when contradicting the founder — grep `founders_vision.md` and the `reasoning/` records for a prior correction on that premise. **Three for three, the answer was there.** **Lane: WORKFLOW.**

### TODO-4167-PSRSCALE — **CLOSED at Patch 4168: yes, and it was already settled at 0733** — can the PSR ever exceed l_P? (registered Patch 4167, GR lane — several counts scale with the answer)

**ANSWERED — and the question was already closed in the corpus.** From the Patch 0733 record: *"if l_P is the **baseline** PSR rather than a hard ceiling, **PSR can exceed it** (the variable-speed-of-light route)"*, and l_P is **environment-dependent** — smaller near a nucleus, larger in flat space. **And 4167's premise was wrong outright:** c01 says *"the true grid is sub-Planck (~ l_P/10³⁰) by nesting"*, so the ±10% PSR band holds **~1.3e90 GPs, not 12**. 4167 is withdrawn entirely at 4168. *(original entry:)* 4167 computed that a ±10% band around **PSR = l_P** contains **exactly 12** GPs: the 600-cell's next shell is a factor **φ = 1.618034** out, so the band would have to widen to **±24%** before a single second-shell vertex entered. That rests entirely on the glossary's *"in the rest frame, PSR = l_P … the Voronoi cell volume **shrinks**"* — PSR equals one lattice spacing and never grows. **The founder's reading (N ≫ 12) is right whenever PSR ≫ l_P**, where N ≈ 1.26(R/l_P)³ — 1.3e3 at 10 l_P, 1.3e9 at 1e3 l_P. **So the question is physical, not geometric.** If any regime has PSR > l_P, the shell population changes by many orders and several results scale with it. **I have not verified that the glossary's definition is complete.** **Lane: GR.**

### TODO-4166-MIXING — **DISCHARGED at Patch 4170: ε ≤ 5e−47, and the signature is unreachable**

**Result at 4170.** In a polarised medium the pull is **ballistic** (θ ≈ εPn) rather than diffusive (√(ε²n)), so the bound tightens by **√n/P**: **ε ≤ 5.4e−37** (neutron through a polarised foil), **5.4e−42** (μSR, magnetised sample), **1.1e−45** (nuclear spin in a ferromagnet), **5.4e−47** (muon g−2) — **2e22 times tighter than the diffusive bound.** **The rule survives; its magnitude does not.** At ε ≲ 5e−47 the environmental term is unobservable everywhere except where the bound was set, which **sharpens [[TODO-4165-CHANNELJOB]] rather than answering it**: a channel whose two jobs are reproducing magnetism the corpus already had and an effect below 1e−46 per Moment is still looking for work. **The signature that would exist:** ε enters as ε·P·n, so the CPP term is the part of a relaxation rate **linear in the medium's polarisation** — genuine, and the first this session that is not already excluded, but **unreachable in practice** at 1e−46. *(original entry:)* Patch 4166 bounded the founder's spin-mixing proposal at **ε ≤ ~1e−24 per Moment** (muon g−2 7e−25, neutron polarimetry 2e−23, Penning-trap electron 7e−27) — but that is the **loose** bound: it assumes A_env is random and **uncorrelated Moment to Moment**. **If environmental axials are correlated over many Moments — which a nearby polarised medium makes them — the walk is ballistic rather than diffusive and the bound tightens by a further √n, up to nineteen orders.** **The prediction that would live here:** a spin in a **polarised medium** should relax faster than one in vacuum by a calculable factor. That is falsifiable and it is the tight version of the test, not the permissive one. **Lane: EW.**

### TODO-4174-VECTORGW — **DISCHARGED at Patch 4175: κ_A/κ_grav ≤ 1.6e18, the channel's first numerical bound**

**Result at 4175: κ_A/κ_grav ≤ 1.6e18** — weak, because A_i is sourced by **intrinsic** spin density and bulk neutron matter is unpolarised, giving a suppression of **6.3e−20 × P** relative to the mass quadrupole (a magnetar's P ~ 1e−3 gives 6e−23) against a ≲10% non-tensor amplitude bound. **First numerical bound the channel has ever carried:** TODO-4172-SOURCEEQ moves from *"coefficient unknown"* to *"coefficient bounded above"*. **THE CHOICE IT FORCES, and it depends on no proposed identification:** if κ_A is the gravitational value (the zero-parameter choice) A_i sits **1e18 below the LIGO bound and is unobservable**; if κ_A is large enough to be observable it is **a new parameter**. **The amendment cannot have both: zero parameters, or observability.** **Also:** the *static* spin–spin test — the tighter route — **cannot be run at all until A_i has a field equation**, so [[TODO-4172-SOURCEEQ]] is now the binding constraint on every other test. *(original entry:)* 4174 withdrew the Einstein–Cartan identification: **EC torsion is NON-propagating** (algebraic field equation, tied point-by-point to local spin density, no independent degrees of freedom — which is exactly why its effects are confined to the contact term), while **A3′ has every channel *"propagating at c = l_P/t_P … all channels obeying the same icosahedral shell-sum"*** and the amendment adds A_i as a fourth channel of that packet. **A propagating axial field is not torsion.** **What that opens:** a propagating channel carries energy and radiates; A_i is **T₁ (l=1) axial**, so its radiation is a **VECTOR mode**; and the standard six-polarisation framework's non-tensor modes are **directly bounded by LIGO/Virgo/KAGRA** antenna-pattern tests. **Compute A_i's radiated vector amplitude from its source c(∇×s) and confront those bounds.** **Second exposure:** propagating-torsion theories are notoriously delicate — dynamics on an axial channel generically brings ghosts or gradient instabilities — and **A3′ gives A_i flat per-hop transport and no kinetic term at all**, so whether its propagation is even well-posed has never been asked. **Lane: EW/GR.**

### TODO-4173-EARLYUNIVERSE — **WITHDRAWN at Patch 4174** (it followed from the Cartan density, an EC quantity; the EC identification failed)

4173 found the Einstein–Cartan contact term is ~55 orders too small **everywhere accessible** — polarised iron 8e−59 of the field energy, nuclear matter 3e−55 of the binding, neutron-star core 3e−56 of the pressure. **Torsion becomes significant only at the Cartan density, n ~ mc²/(κħ²) ≈ 6.5e100 m⁻³, which is 3.8e56 times nuclear** — so **not the lab and not neutron stars; only the early universe**, exactly where the EC literature puts torsion (singularity avoidance, bouncing cosmologies). **CPP has an early-universe lane and none of this session's work looked at it.** That is where the channel should be tested. **Lane: EW/COSMO.**
...
**4222:** the C/P/T table of 4193 §3 re-derived by running the argmax selection on twelve icosahedral neighbours under explicit P, T, C transformations — identical, cell for cell; hop = T-odd enters only as e → −e and holds because the register travels with the CP. Table stands; the filter tests (self-propulsion, F2, F4, F7) are still not run.
...
**4220 — SESSION 236 CLOSED.** Item 4 reverted to untested (wake mechanism unknown, founder). Handover: boot card `handovers/2026-09-22_session_236_p4220_BOOTCARD.md`, detail in `handovers/detail/`. Next window: critic pass, then two-GP coherent ejection → V−A; do not start λ magnitude.

**4213 — neutral-current contact scalar: SEARCH ENDS AT A GAP.** SF-2's Z is inert by Cor. Zinert (no activated Z), so it cannot host the measured electron–quark neutral current; no CPP mechanism produces atomic parity violation / Qweak. Registered **OPEN-EW-NC** in `frontier_sectors/EW.md`. Routes: (a) elastic W⁰ capture-and-re-ejection near a quark core (first); (b) interacting Z (second). Side finding: SF-2 §5.9 has the lepton hierarchy the founder gave at 4212 (e bare; μ + hybrid tetrahedron; τ + hybrid icosahedron) — the glossary's "Lepton Cage" entry is stale; quark side still owed under [[TODO-4212-CAGETABLE]].

**4210 — SF-2 AMENDED to v1.06** (§5.7.1 inheritance mechanism; §5.7.3 V−A attribution; PRED-O-23 re-registered; changelog-sf-2.md created). **Founder owes:** the SF-2 PDF recompile (the 23rd on his list). Still open on this vertex: the neutral-current contact scalar; λ from the DP-arc bias (persistence); the two-GP coherent ejection → Fermi limit; CKM as a different-generation refill (TODO-4188).

**4209 — SF-2 §5 read.** Range (charged current) is in SF-2 already: Prop 5.2 captures only non-zero-gradient host CPs, Prop 5.4 disintegration is where the ejection rule fires → bracelet range by construction. **CONFLICT:** SF-2's shipped β⁻ walkthrough (§5.7.1) is P1 — no refill, Sea capture at the electron — which is Fermi-only (4202) against the 83% GT fraction; the founder's G-EW-INHERIT-4205 supersedes it. SF-2's EC criterion (freshly induced = anti-type) supplies the ν/ν̄ label owed since 4199. **Owed: one SF-2 amendment patch** — §5.7.1 steps 3/5/6 + Postdiction to inheritance, PRED-O-23 re-registration (4208), v-bump, founder PDF recompile. Wording drafted at 4209 §3; execute unless the founder objects. Neutral-current contact scalar still open.

**4208 — founder: the refill DP is emergent from summed DI-bits of every GP in the area** (answers 4207's critic — several senses are present in the sum). Program restated; the W⁰ supplies site and catalysis, no chirality of its own (= THEO-CHIR-1); the handedness is imported by the released CP's own A via the ejection rule. **Registry action owed (PD-006, not a founder question):** re-register PRED-O-23 / OPEN-FP-SF-2-CHIR — SF-2 §5's "V−A from D₆ phase bias" was refuted (4046, THEO-CHIR-1) and SF-2 is unamended; new wording: V−A from the released charge's own handedness at ejection, bracelet = site. Do when SF-2 is next opened.

**4207 — c03 checked:** its two-quadrature Born-rule conjecture makes outcome contributions **add as vectors before the 12-edge selection**, so a refill with two candidate senses at one GP has the cross term built in; λ = g_A/g_V maps onto the amplitude ratio (1.275, anti-phased) and 4203's f = 0.24 is the same statement (`code/4207_*.py`). Not given by c03: the singlet / Fermi limit, which needs a two-GP coherent ejection — that is V−A itself, a window's work. **Recommended next-window arc:** (i) compute λ from the DP-arc bias at the refill under c03's sum (blocked on persistence, 4203); (ii) two-GP coherent ejection → singlet → full a, A, B; read with [[TODO-4188-F5SCOPE]]. Critic: are both senses simultaneously present at the core's GP, or does the Sea supply one DP with one sense at a time?

**4206 — averaging done (`code/4206_*.py`): the model reproduces the SM pure-GT limit EXACTLY (a=−1/3, A=−2/3, B=+2/3) with SU(6) P=5/6, nothing fitted.** It fails the Fermi limit in kind (assigns definite spins inside a singlet) and A does not depend on the refill bias f, so the persistence question does not bear on A (4205's "one item" claim withdrawn). The measured A needs the Fermi–GT interference term (+0.434): the two refill senses must be **amplitudes of one event**. **Owed:** does c03's Born-rule mechanism (12-edge selection under ZBW fluctuation) make the refill sense a superposition resolved at ejection? If yes, derive the cross term and A; if no, the model is stuck at the pure-GT column. Founder picture wanted only if c03 does not settle it. **Lane: EW.**

**4205 — FORK RESOLVED: founder ratified (b), G-EW-INHERIT-4205; the inheritance model is the working picture.** First test: signs of A and B right; magnitude of A 3.5–5.5× too large on one axis (measured −0.118). **Owed, and now one item:** the full triplet-orientation average with the refill bias f — which needs the founder's persistence picture (4203). Do the average with f as a parameter first and read off what f the measured A demands; compare with 4203's 0.24/0.66. **Lane: EW.**

**4204 — THE FORK, awaiting the founder:** does the nascent electron capture **(a)** a Sea DP (P1: core keeps its orbital, no refill, **Fermi-only — no GT decay**, dead against the 83%) or **(b)** the down quark's own orbital DP (P2: inherited spin, core refills, GT = refill sense)? His 4204 picture reads as (a) but does not say. Everything downstream (g_A, persistence, ejection rule) is conditional on (b). Energy check passes either way: ν̄ takes 61% of Q on average but 0–100% continuously; it must be nearly massless (`code/4204_*.py`).

**4203 — g_A target fixed, calculation BLOCKED on a missing object.** Needs the Sea at the core radius counter-circulating (relative to the departed DP) by f ≈ 0.24 (per-state) or 0.66 (overall) at refill (`code/4203_ga_target.py`); sign is already in the picture, number is not. DP-arcs exist only as CONJ-FP-1 (inertia sketch) with no circulation, decay length or lifetime, and no glossary entry. **Unblock:** founder picture — how long does an induced partner sense persist in a Sea DP after the inducing orbital leaves (a Moment / a ZBW period / until re-paired)? Then f = fraction of captureable neighbours still carrying it: one-line check. Alternatively promote CONJ-FP-1 to a rule (larger). Also owed: a glossary entry for DP-arc once either exists.

**4202 — count argument:** every fresh Sea capture owes one counter-spinning partner; two captures = two antineutrinos, over by one; so the electron must take the quark's DP (P2, 4201) — P1 (4199) gives one neutrino but no GT mechanism. **Discriminating test filed:** P2 predicts the beta electron's spin = the departed d quark's sense, parallel to the neutron spin 5/6 of the time (SU(6)) before ejection; compare with the measured electron-spin/neutron-spin correlation in polarised neutron decay (parameter N) with −v/c and the ejection rule folded in. **Refill-sense (g_A) restated as a computation** (4202 §4). Critic: does every fresh capture owe a partner, or can DP-arcs take the angular momentum (SF-6)?

**4201 — inheritance picture (founder, verbatim): the electron TAKES the down quark's orbital DP; the bare core refills from the Sea; the refill's counter-spinning Sea partner is the antineutrino.** One neutrino per decay, angular momentum conserved, both channels = the sense of the refill (same → Fermi, opposite → GT). Supersedes 4200's transfer/activation. **The handed rule relocates to the EJECTION direction** (4199 §2 bannered). **Now owed, in order:** (1) **g_A**: why the refill takes the opposite sense ~4.9× as often — try the anisotropy the departing electron's DP-arcs leave in the local Sea; (2) the GT correlation a = −1/3 from the triplet average (asserted at 4201 §3); (3) ν vs ν̄ as partner-of-a-+qCP-refill vs −qCP-refill — does the Sea DP carry it; (4) the ejection rule as a 12-neighbour choice, checked against c03; (5) **range** (SF-2 W⁰ catalyst), still unread.

**4200 — Gamow–Teller.** Founder postulate filed verbatim: the unit comes from the **quark's orbital ZBW DP** and goes to the Sea DP that becomes the antineutrino (pair → triplet); Fermi = no transfer (4199's singlet). Source and destination accepted as working picture. **His switch (collisional activation) fails:** a free neutron at rest decays GT 83% of the time (`code/4200_*.py`), so the ground-state orbital must hand over the unit unaided. **Now owed:** what decides transfer vs no transfer in the ground state; **compute g_A** (measured 1.275) as the with/without-unit capture ratio — near 1 kills the picture; the ν/ν̄ label in a Sea DP; the −v/c form; **range** (SF-2 W⁰ catalyst) still unread.

**4199 — gap (b) addressed in kind; item (4) opened.** Founder's neutrino-formation picture filed verbatim: the escaping −eCP captures an orbital DP; the antineutrino is the anti-spin left on a nearby Sea DP. With ONE trial rule — **the weak vertex selects q (v̂·A) = +1**, the same term as 4193's gate read the other way — it gives handedness that vanishes at rest, no coupling for right-handed states, and **a = +1 for Fermi decays** (`code/4199_*.py`). **Now owed:** Gamow–Teller (parallel lepton spins; the core must flip — how?); what distinguishes ν from ν̄ in a spinning Sea DP; the linear form −v/c; (c) **range**, still untouched — read SF-2's W⁰ catalyst framework; next-window critic to attack 4199 §2 item 2, which is an argument, not a computation.

**4198 — item (1) advanced again.** SF-4: the neutrino is *"an unbound 3D orbital ZBW configuration … with no central CP anchor"* — fits "no anchor", not "linear". Assembly that fits: **2T₃ = Σ polarity signs of unpaired CPs + mode term (0 orbital, −1 linear-captured, +1 unanchored)**; u and e are genuine checks, d and ν are fitted, polarity sum and mode term are separately conserved in beta decay (`code/4198_*.py`). **Now owed:** (a) how a register distinguishes linear from orbital ZBW; (b) **right-handed states have T₃ = 0 — the weight must also depend on the mover's b; unaddressed**; (c) **range** — why the gate reads only contact-range partners, not the rangeless a/r far field (look at SF-2's Z icosahedron); (d) antiparticles and heavier generations.

**4197 — item (1) advanced.** Founder's pointer: the down quark's **linear −eCP oscillator**. Sorted by CPP constituent with sin²θ_W = 3/(8φ): **+0.382 (= 1/φ² exactly) per +qCP core, −1.073 per linear oscillator**; oscillators alone fail (p:n = +0.5 vs −0.07), cores + oscillators reproduce p = +0.073, n = −1. Captured −eCP minus free electron = **exactly −1** = the antineutrino's share. **Now owed under (1):** find a register quantity (from AP-4's E and S content at a GP) that evaluates to those two numbers; look up what a neutrino is in CPP (SF-4) and whether "one unit of linear ZBW set free" fits it; decide whether 1/φ² is structure or arithmetic.
...
### TODO-4192-COHERENT — **CLOSED at Patch 4250: the exclusion stands; the three critic points supply ≤ O(10) against a 10⁶–10¹² gap; accumulation is the founder's own 4182 loop; the physics conclusion (mixing cannot organise an electron) holds on either reading; the 1/r² falloff is the only lever and 4186 rules it out** (registered Patch 4192, EW lane — PD-008 SUBMISSION)
...
### TODO-4191-DETAILDIR — **CLOSED at 4220**: folder created with README; session-234 full handovers moved in; boot card names its detail file so the fallback is unneeded

bootup Step 2 says full handovers live there; git does not track empty folders, so 4189's `mkdir` never landed. **Hazard before fixing:** a placeholder file added under `handovers/detail/` becomes the *newest added file* in bootup's fallback command (`git log --diff-filter=A -- handovers/`), so that command must exclude `detail/` in the same patch. Do at the next handover. **Lane: WORKFLOW.**

### TODO-4223-PAIRSTATE — build the two-GP coherent ejection that produces the pair's joint spin state (registered Patch 4223, EW lane — THE NAMED TARGET replacing "derive V−A")

4223 (`code/4223_pair_state_correlations.py`): (ejection rule, h_e = β, h_ν̄ = 1) + (pair spin state: same-sense refill → singlet; opposite-sense → the triplet Σ_i (σ_i|n⟩)⊗|T_i⟩, all three components, weight 3λ²) + (senses added as amplitudes, c03) = the full V−A a, A, B for every λ and β, to 1e−15, interference included. **The middle term is imported from angular-momentum conservation, not derived.** Owed: show from the 12-edge selection and c03's amplitude sum that the released −eCP (with inherited orbital, at one GP) and the refill's counter-spinning partner (at the core's GP) leave as ONE event whose joint spin state is the singlet or the σ-written triplet — not two independently spinning leptons. Consequences already fixed by 4223: the electron does not carry the quark's spin as a definite value in the Fermi channel (SF-2 v1.07 §5.7.1 amended; founder may rule on whether this is within G-EW-INHERIT-4205); the "3" in 1+3λ² is the number of flip directions; the quark polarisation Δd enters only λ. Blocked with λ on the DP-arc/wake mechanism (4220) for the magnitude; the spin-state question itself is NOT blocked and is the next thing to build. **4224:** founder — *probably true, cannot validate until the mechanism is seen*; SF-2 v1.07 wording stands PROVISIONALLY. The requirement translated into his bookkeeping (`code/4224_spin_flow_from_pair_state.py`): the antineutrino carries the neutron's spin out in 99.4% of decays; the electron keeps the quark's sense in 56%, reverses in 44%; the naive inheritance event (p unchanged, e keeps s, ν̄ counter) is 0.64%, its amplitude (1+λ)/√2 nearly cancelling — that cancellation is what λ<0 means. The 44% configuration (p unchanged, e reversed, ν̄ parallel to the refill) is unreachable by naive bookkeeping: the refill sense fixes only the EXCHANGE SYMMETRY of the two-lepton spin state, not either lepton's sense. **Founder question outstanding (4224 §4):** can the departing electron DP and antineutrino DP induce sense on each other through DP-arcs while within reach, so that neither sense is settled until they separate? **4225: founder answered YES and gave a mechanism (W⁰-centroid capture; random tangential refill with antispin partner; W⁰ collision breaks the pair off and frees the electron in one act; spins mix until separation) — G-EW-PAIRSTATE-4225, provisional: build on it, reconsider at a roadblock.** Watch-list (4225 §3): (1) the SYMMETRY RULE — why same-sense refill ⇒ antisymmetric pair, opposite-sense ⇒ symmetric; next computation; (2) the W⁰-collision asymmetry = CONJ-FP-1 arc reach (λ magnitude); (3) rate (neutron lifetime), filed only. Already resolved: 'spins fixed at separation' must mean fixed at EJECTION, not as a product configuration at separation — decohered separation gives a = −0.037 vs measured ≈ −0.105 (`code/4225_coherent_vs_decohered_separation.py`); 'go off in the same direction' must mean one event, not collinear (that would be a = +1). **4226 (correction accepted by founder):** watch-list item 1 DISSOLVED — any rotationally invariant transit mixing commutes with total spin, so the singlet/triplet content is set at BIRTH (`code/4226_birth_state_and_swapped_inheritance.py`). Birth states tested: (a) inheritance as ruled at 4205 (electron keeps the quark's orbital; ν̄ = antispin of the refill) gives A ≈ −1 for EVERY refill-sense ratio — excluded; (b) SWAPPED — the ν̄ carries the quark's orbital angular momentum (an unanchored orbital: SF-4's neutrino), the proton and electron born as one counter-pair (singlet) — gives (a,A,B) = (0,0,+1) = quark-level V−A at λ = −1 exactly; the measured nucleon state is (b) + a 10% σ-triplet admixture δ = λ+1 = −0.275 (cage/nucleon structure, the SU(6)-to-1.27 object). **FORK HANDED UP (4226 §5):** at the centroid, does the stabilised −eCP take the quark's orbital as it passes (4205), or a Sea DP while the quark's orbital is knocked free (b)? Pending the ruling: ONESIGN item 4's 4219 mechanism superseded (sign = the singlet's), CONJ-FP-1 released from blocking λ (|λ|=1 at the vertex; 1.27 is nucleon structure), SF-2 §5.7.1 steps 3/5/6 + postdiction would change under (b) — not amended. Unchecked: can the substrate release the orbital without its core (4181/4184 binding)? **4227:** founder restated branch (b) in his objects and asked if it is right — built literally it IS the vertex, (0,0,+1), and with the δ admixture returns C = −0.2385 vs measured −0.2377±0.0026 (`code/4227_founders_restatement_literal.py`). Two precisions filed: the created pair must be a true singlet, and 'then' is not a later step for the spins (at the vertex the proton and electron are the entangled pair, not e–ν̄). **RATIFIED at 4228 — G-EW-SWAP-4228** (`founders_voice/4228`): 4205 superseded in the one clause; ONESIGN item 4 passes (checksum **7/7, 0 untested, 0 failed**); TODO-4203 re-scoped — |λ| = 1 at the vertex, the nucleon's 1.275 is a CAGE computation (δ = λ+1 = −0.275: the SU(6) 5/3 and its relativistic reduction), CONJ-FP-1 released from blocking it; SF-2 → v1.08 (steps 3, 5, 6, postdiction). **Still owed on this item (substrate, not registers):** (i) the release of the orbital eDP from its core at the W⁰ centroid — **4229: the 4181/4184 citation was a misattribution (withdrawn); the binding is the constituent-mass identification r_ZBW = ħc/m_const (4178) and the question was posed at 4204.** Ledger (`code/4229_release_ledger.py`): the release carries out L = ħ/2 unchanged but NOT the ~313 MeV of circulation — it is an EXPANSION by 10⁹–10¹⁰ in radius at fixed L (to SF-4's r = ħc/m_ν, microns), the energy handed to the refill half; the centroid's zero-SSV pocket (0.0025 fm) is 250× smaller than the orbital (0.630 fm), so the orbital DP transits it, once per circulation when aligned (a rate factor). **4230: branch (i) BLOCKED on a missing rule** — the corpus has no statement of what holds an orbital eDP to its core or how many a core holds (searched SF-2/3/4/6, maturation series; only r_ZBW = ħc/m_const, 4178). Under the default electrostatic binding, a neutral refill eDP cannot screen the +2/3 monopole (needs d = 0.21 fm vs the grid-scale DP separation; `code/4230_refill_screening_test.py`), so the pair creation does not release the old orbital. **4231 — ANSWERED: charge, through the ZBW reset (G-EW-ZBWRESET-4231, provisional):** the −eCP pole falls to the core, superimposes (zero-gradient Moment = reset), returns; the release is the same reset at the W⁰ centroid pocket, where the strongest gradient (the stabilised linear −eCP) sends the pole outward — branch (ii), needs no screening. Computed: a kick inside the pocket changes L about the core by ≤ 0.4% (B = +1 by geometry, `code/4231_pocket_kick_L_bound_and_rate_budget.py`); lifetime budget 6.7e25 cycles, pole-in-pocket 1.2e−3, so (W⁰ present, aligned) × (outward branch) ~ 1e−23 per cycle — the rate item's shape. **4232 — energy test PASSED on reading (α):** the 313 MeV is the energy of the bound oscillation in the core's field (configuration energy), not carried by the DP — implied by 4231's own binding statement (an oscillation that exists only because of the core has its energy in the pole–core system); reading (β), DP carries it, is refuted by the endpoint (313 MeV vs Q = 0.782, factor 400; `code/4232_energy_locus_test.py`). Nothing is shed; the refill half re-inhabits the core's configuration; 4229's 'expansion' is a description of the free state, not a process. (α) is c04's mass mechanism — rest mass is the energy stored in the Compton-scale oscillation of the polarization cloud — applied to the quark orbital; my first draft claimed the sentence was absent, the absence gate failed it, and the unscoped search found c04. No confirmation needed. **4233 — lifetime in cycle units:** P = 1.4e−26 per ZBW cycle; pocket geometry (1.2e−3, fixed by m_const/m_W) divided out; the substrate must supply (W⁰ present, aligned) × (outward) = 1.2e−23 = (m_e/m_W)⁴ (m_e/m_const) × 4.4 — the (E/m_W)⁴ is the squared W propagator read as bracelet-presence probability, the O(4) is alignment × branch (`code/4233_lifetime_in_cycle_units.py`). **TARGET, sector:** a bracelet-formation amplitude scaling as (available energy/m_W)² — no rule on file. Remaining: that, and δ from the cage — registered as [[TODO-4234-DELTA]]; (ii) one counter-pair landing on two cores as one event; (iii) the cage computation of δ. These are the next computations; the register work of the arc is closed. **Lane: EW.**

### TODO-4235-NC-ISOSPIN — route (a) for OPEN-EW-NC must produce +1 per bare core, −2 per linear oscillator (registered Patch 4235, EW lane)

The NC constituent weights of 4198/4216 split exactly (`code/4235_nc_weight_split.py`): EM part −4Qs² per constituent with the derived s² = 3/(8φ) — no mechanism needed; isospin part +1 (bare +qCP core), −2 (linear −eCP). The elastic W⁰ capture-and-re-eject event must give a unit throw one way at a bare core and a double throw the other way where a linear −eCP occupies the centroid. **4236 — the signs decide first:** the bare-core push is ALONG the electron's spin (+1), the linear-oscillator push AGAINST it (−2); the ejection rule throws against spin always, so route (a)'s capture-and-re-eject fits the linear oscillator (sign, plausibly ×2 by occupancy) and CANNOT be the bare-core event. The corpus has no act for a passing electron at a centroid beside a bare +qCP. **FOUNDER QUESTION (4236 §3):** what does a transient W⁰ centroid beside an up quark do with a passing electron's −eCP, such that it is sent along its spin one unit? Route (b) still not needed. **Lane: EW.** **4240 — founder offered ideas, not a ruling (`founders_voice/4240_*`):** the centroid is zero-gradient only from the W⁰ itself and not shielded; the W⁰ holds the DP-arcs' KE for one Moment, vibrates, relaxes and propels the −eCPs forward in a mixed state; passage is carried by inertia (SF-6 DP-arc build-up in front, collapse behind); **direction is selected by the CP's A_I and the path GPs' A_i**. It supplies a direction-selecting agent for the along-spin push; it does not yet supply the unit magnitude, and its scene is the β-decay one (linear −eCP + old orbital −eCP), not 4236's bare core with a free electron. **Next:** apply it to the bare-core scene and test +1 (bare core) / −2 (linear) before handing up anything further. **4246 — done (`code/4246_*`): capacity-one centroid + fermion exchange give core +1, linear −2 exactly (4235's reversed + cancelled: cancelled = direct transit blocked by occupancy; reversed = exchange sign); signs follow under 4216's reading, open under the amplitude reading → [[TODO-4246-NCPHASE]].**

### TODO-4240-CORE-MOTION — does the quark's +qCP core itself move in the cage? R = 0.765 depends on it (registered Patch 4240, EW lane → strong; founder question outstanding)

TODO-4234-DELTA's reduction R must be spin–orbit sharing at the constituent scale (4240 §3). In that family R is set by the quark's motion in the cage: a moving spin-½ quark's axis, as the vertex sees it, is rotated by its motion; R = 0.765 needs k = 368 MeV (1.18 m_const) for a single |k|. The corpus statement found is SS-2's *ZBW smearing* — the quark's charge smeared *"by its ZBW orbital motion around its cage vertex"* at r_ZBW — which gives g_A 1.341 (p = ħ/r_ZBW) or 1.549 (p = ħ/2r_ZBW). **Founder question (4240 §5):** does each +qCP core itself travel (circling its vertex at ~r_ZBW, or wandering in the cell) while its orbital eDP circulates, or does the core sit on its vertex so the r_ZBW circulation is the orbital eDP's alone? **Then:** if the core travels, build its momentum distribution from his picture and compute R as the average spin projection (not one |k|); check the moment in the same distribution. If it sits, B2 needs another carrier — reopen. Do not report any single-scale g_A as a result (4240 §4). **Lane: EW → strong.** **4242 — founder answered (`founders_voice/4242_*`): the frame is the bonding hTetra (not a cage); the quarks sit on its vertices and move by ZBW breathing against them and hops between them on the colour force.** Computed (`code/4242_breathing_core_gA_and_moment.py`): (i) 4240's (1+R)/2 moment is identically the spin magnetisation of a moving core; (ii) 4240's B1/B2 split relabelled — the exclusion is of a magnetisation that includes the convection current of the motion (full Dirac current: μ_p = 1.63, −42%, for any κ = −1 spinor), not of 'the cavity'; (iii) a breath with no circulation fits: μ_p/μ_N = (M_N/m_q)(1 + 3g_A/5)/2 → 2.648/−1.765 (−5.2%/−7.7%), no free parameter, conditional on the Gordon spin factor for the carried dipole; (iv) R's value needs the breath's size and shape: SS-2's r_ZBW gives 1.24 (3D) to 1.57 (1D); measured needs 1D σ_x = 0.18 fm or 3D rms 0.70 fm. **Open — founder question (4242 §5):** how far out does the core breathe (fraction of the 0.62-fm edge), and along one line or in changing directions? **Then:** compute R from his breath (distribution, not one scale) and report g_A with the tied μ. Do not adopt the 3D reading (1.2375) without his answer (4242 §4). **4243 — founder: the breath is the ZBW oscillation of opposite charges, radial and sideways between opposite-charge quarks; modes follow the colour combinations; size to be calculated (`founders_voice/4243_*`).** Calculated (`code/4243_zbw_breath_modes_gA.py`): each mode in its ground state at the Compton frequency (c04) — σ_x = r_ZBW/√2; the u's two modes reproduce SS-2's ⟨r²⟩ = r_ZBW² exactly (check); the classical Coulomb breath at the same period would be 0.78–0.94 fm, larger than the edge (rejected). **g_A = 1.380 / 1.431 (anchored / u–d both moving), +8 / +12%, zero parameters** — 70–73% of the reduction from 5/3; μ_p −1.5 / 0.0%, μ_n −5.6 / −3.6%. Tie inverted: measured g_A + μ_p fix m_q = 296.7 MeV. **Open — founder question (4243 §5):** do the two u's breathe sideways along u–u (colour attraction, same electric sign)? Yes → g_A 1.312 (+2.9%) anchored. **Also owed (critic, next window):** anchored-vs-moving partner; relativistic anharmonicity of a mode with ⟨T⟩ ~ mc²/4; non-orthogonal mode directions in the real frame geometry; α_geom's two corpus values (SS-2 1/√5 vs SS-1 0.559). **4244 — founder: the two u's do not breathe sideways (same charge, they repel); mode rule fixed. Critic items (G) geometry (≤0.8%) and (K) relativistic kinematics (−2.5%) CLOSED; anchored-vs-moving settled by the founder's words (both quarks move). RESULT: g_A = 1.406 (+10.2%), μ_p = 2.772 (−0.8%), μ_n = −1.829 (−4.4%), zero parameters (`code/4244_*`). This TODO is DONE as a mechanism-and-size item; the ~10% residual lives in TODO-4234-DELTA.** Still open, SS lane: α_geom's two corpus values (SS-2 1/√5, SS-1 0.559).

### TODO-4240-F3STATE — **CLOSED at Patch 4245: true-state |W| is exactly half (W = (s_d − s_u)V_d); isotropy unchanged; 4136 bounds loosen ×2 (ε ≤ 1.7e−7, g ≤ 5.1e−7); no verdict moves** (registered Patch 4240, EW lane)
...
### TODO-4251-LKICK — **CLOSED at Patch 4253: 4231's bound was about the torque about the pole, not about the core or the eDP's centre — withdrawn at any size; B = +1 rests on G-EW-SWAP-4228's 'unchanged' clause + 4223's h_ν̄ = 1; glossary amended** (registered Patch 4251, EW lane)
...
Pole-in-ring per cycle is 0.175, not 1.2e−3; (W⁰ present, aligned) × (outward) must be 8.3e−26 = 0.031 × (m_e/m_W)⁴(m_e/m_const). The (E/m_W)² bracelet-formation target (4233, TODO-4234-era item 2 on the boot card) stands in form; its coefficient changes 4.4 → 0.03. Compute with the target, not before it. **Lane: EW.** **4255 — candidate registered (CAND-EW-4DOVERLAP-4255): the propagator squared as a 4D overlap (E/m_W)⁴ of the released wave with the bracelet's Compton pocket; shape right, coefficient 5e−3 (the SM's g⁴f/64π³) not supplied; needs a substrate statement on 4-volume coherence. Founder answered 4256: **by chance** (G-EW-W0CHANCE-4256), with lepton universality as the evidence (bias < 0.1%); E enters at the capture, where the candidate places it; coefficient = the Sea's universal presence rate × CPP phase-space measure — owed as one number. Tension recorded: 'stabilised on capture' vs SF-2 Prop. lifetime (captured charge is the decay channel) — SF-2 must state the bare W⁰ dies within ~ħ/Γ_W if both are kept. Not adopted.**
...
4246 derives the NC isospin integers (core +1, linear −2) from capacity one (SF-2 Def. activated W⁰) + fermion exchange at an occupied centroid, in units of the bare-core event. The bare-core SIGN follows under 4216's displacement reading (R1: exchange = the CC release, against; direct = its exchange partner, along) but under the amplitude reading (R2) β-decay helicity (|T|²) does not fix the interference sign 2Re(A_γ* T_dir). **Owed:** (i) critic — is applying the exchange sign to a displacement weight (R1) legitimate, or is R2 the right frame? (ii) under R2, compute the phase of the direct transit from the founder's one-Moment hold (4240 element 3) and check it gives along-spin. **4247:** the founder asked who is in the centroid (captured linear −eCP per 4225, or both −eCPs by chance, or the core). The NC integers decide: d_iso = 1 − 2f needs f = 1; a transit-by-chance linear −eCP gives f ≈ 0.015 and d ≈ +0.97 — excluded. Answered: linear −eCP held (4225 as written), orbital pole by chance (4231), core outside; **ruling handed up (4247 §5).** If he rules the linear −eCP is not held, 4246's −2 fails and OPEN-EW-NC reopens at 4236. **4248 — RULED: the picture stands (G-EW-CENTROIDTRAP-4248): the W⁰ holds the linear −eCP by ZBW among the bracelet's +CPs; orbital pole by chance; core outside.** 4246's occupancy premise is now ruled, not assumed. **4253 — founder: switch and pass-through are indistinguishable, so a mixing (G-EW-NCMIX-4253): 4246's exchange premise supported; 'releases both' is the inelastic case, not Q_W's.** **Lane: EW.**

### TODO-4284-RPFLOOR — with one quark mass, no frame size reaches r_p (registered Patch 4284, EW lane → strong; physics question for the founder)
4273/4274/4276 converted r_p to fm at m_q = m_p/3 while fitting m_q ≈ 296 MeV to μ_p; r_ZBW = ħc/m_const is one symbol (4240, 4268, 4272). At one m_q the r_p floor is **+1.0%** (collapsed flat frame) and 4276's geometries sit at +5.8%; the owed vertex swing and shorter inner swing (4275) both *raise* it. Closing it needs smaller quark swings than route (H) gives, a derivation of m_q (4268 item (iii)), or a changed r_p formula (4274 §PD-008 list). **Founder question (physical picture):** is the mass that sets a quark's swing radius the same mass that sets its magnetic moment? If the picture separates them, that is an axiom-level distinction and reopens 4276. Script `series_standard_model/code/4284_critic_one_mq_rp_floor_pauli.py`.

### TODO-4284-PAULIFRAME — the u–u Pauli energy as what holds the minus diagonal open (registered Patch 4284, EW lane → strong; lead)
With +ħc/r gone (4274; reason corrected at 4284: route (H) widths do not depend on r), the one r-dependent kinetic term left is the antisymmetrised u–u pair's exchange energy: repulsive, ≈ 0.19 m_q c²/r_ZBW (≈ 84 MeV/fm), nearly constant for u–u 0.5–1.8 r_ZBW (4284 rows (E)). Balance it against the hTetra edge bonds once a bond law exists; none is on file. Would fix t and so pin g_A (which depends on t alone).

### TODO-4284-MODECRITIC — **CLOSED at Patch 4285** (registered Patch 4284, EW lane)
Both implementations faithful. 4262: geometry verified (30.3°, sin 0.505 from SS-2 frame); equal-width rule is route-(H) mechanics; open question (u_e’s strong-force diagonal) unchanged, alternative row given. 4271: neutron frame and μ_n role-correction solid; free-nucleon diagonal mode is a conditional inference from 4270’s "may be able to" — PD-008 framing endorsed; g_A values are at SS-2’s frame (r_p +11.6% at one m_q per 4284).

### TODO-4286-GMOMENT — which charge makes the electron’s magnet? (registered Patch 4286, EW lane → QM/SPIN; founder question)
**[4287 — FOUNDER ANSWERED: a modified (a): the magnet is the outer −eCP’s circle MINUS the inner +eCP’s lily-pad pass-through of the core (founders_voice/4287); the ZBW spin account was Grok’s, never re-derived. WORKED: μ/μ_B = [⟨vb⟩_out − ⟨vb⟩_in]/(c r_ZBW); the lily-pad removes SPIN-1’s 71% cancellation; but any MASSIVE charge on a circle has g = 1, so g = 2 needs massless speed-c poles, then g = 2(R − ⟨b_in⟩)/r_ZBW (pure pass-through: outer at ħ/mc → 2 exactly). OPEN: (A) founder confirmation — do CPs have rest mass? (4287 §6); (B) derive R − ⟨b_in⟩ = ħ/mc (4286 §3’s well condition, now on the outer CP), ideally from SPIN-2/3’s standing wave; (C) redo SPIN-1’s L = ħ/2 with massless speed-c poles — does r_out = 2r_in survive; (D) is the lily-pad’s ⟨b⟩ ≈ 0 (row 1: L = 0.05ħ already gives ⟨b⟩ = 0.22). Original owed items (i)–(iii) below stand, (i) now superseded by (A)/(B).]**
**[4288 — (A) ANSWERED (founders_voice/4288): CPs have no rest mass but have DP-arc inertia (SF-6 m₀γ) and move at V_i. Then g = −(2m_e/eS)Σq_iL_i/(2m_i): speed drops out; with a pass-through inner g = m_e/m_out, so g = 2 needs the circulating −eCP’s inertia = m_e/2 (at any speed; light speed only at R = ħ/mc). Founder’s whole-ħ difference tested: no whole-ħ sum/difference is ħ/2; ħ and ħ/2 counter-rotating needs 1.5 m_e per CP. (B) REPLACED by (B′): derive the circulating CP’s DP-arc inertia (SF-6’s m₀ is tuned) — is it m_e/2? (C) REPLACED by (C′): redo SPIN-1 with DP-arc inertia and a pass-through inner. (D) stands. Also open: can a bound CP’s inertia be defined apart from the core’s and the field’s (4288 §7).]**
**[4289 — (C′) first step: SPIN-1/SPIN-2 take ħ/2 as INPUT (r_in is solved from L = ħ/2; Coulomb circles exist at every r_in with L ∝ √r_in). Parameter-free (both radii at SPIN-2’s anchors r_th/3, 2r_th/3), L = (1+√2)√(α/6)ħ = 0.168 × ħ/2. So there is no derivation of ħ/2 to redo. (C′) is REPLACED by (E): derive why the outer −eCP’s circulation carries ħ/2 (the founder’s minimum-action idea, 4288); whole-ħ amounts cannot give ħ/2.]**
**[4290 — founder (founders_voice/4290): ħ/2 is the fundamental unit; outer carries ħ, inner lily-pad ħ/2 the opposite way. That resolves the arithmetic. It also exposes a choice: opposite charges circulating OPPOSITE ways make magnets that ADD, so (ħ, −ħ/2) needs 1/m_out + 1/(2m_in) = 1/m_e (moving inertia ≥ 2.91 m_e; the mass would be a near-cancellation); circulating the SAME way the magnets oppose, but then spin = ħ/2 forces (ħ/2, 0) with m_out = m_e/2. **FOUNDER QUESTION (4290 §6):** which way does the inner lily-pad go around compared with the outer? (E) now reads: once decided, derive why a circulation carries whole units of ħ/2 (and, on the opposite-way branch, the −1.9 m_e c² binding).]**
**[4291 — founder (founders_voice/4291): SAME way; inner +eCP traces a spirograph through the core (a rose). Spins add, magnets oppose. Exact: the inner’s contribution comes only from the turning of its line, ⟨r×v⟩ = W⟨r²⟩; the turning must be driven by the partner (a central pull keeps a line through the core fixed). g = m_e(1 − x/2)/(m_o + m_i x/2), x = W/ω_o (sinusoidal swing): tips at 2ω_o give g = 0 (the remembered “twice the rate”); fixed diameter W = 0 gives g = m_e/m_o. Meeting-the-partner solutions W = ω_o + (2j−1)ω_r. **FOUNDER QUESTION (4291 §8):** per outer revolution, how many times does the inner touch the circle, and is its partner there each time? Owed after the answer: the pair dynamics that set W and the inertias; (E) why the total is ħ/2.]**
**[4292 — founder delegated (founders_voice/4292): compute what works; the outer orbit’s shape is also open. TARGET (equal inertias m_e/3, flagged): g = 3(1 − 2f), f = L_inner/S, so g = 2 ⇔ the +eCP carries 1/6 of the spin; on a circle with a still core, the inner’s line turns at 0.4ω_o (6 touches per 5 circuits if it meets its partner). DYNAMICS: Coulomb forces between three point CPs hold NO orbit (54/54 negative-energy starts break into a neutral pair plus a free −eCP; the Euler line breaks at a 10⁻⁴ nudge), so they select no rate or shape. OWED: (F) a binding model; start from the pass-through ZBW lock (the inner +eCP as the core’s ZBW partner, 4265, route H), then the c04/SF-6 cloud; (F-check) 3-D runs, other softenings, longer runs of the breakup; (G) the equal-inertia assumption.]**
**[4293 — (F) first model: HUB-AND-RIM. Hub = core −eCP ZBW-locked to the inner +eCP; rim = outer −eCP on the DP bond; the dumbbell spins about its CM (the core circles at R/3). The hub’s charges move together, so its magnet cancels: g = M_hub/m_rim = 2 exactly, for any R, ω, speed or bond law; the inner carries 1/6 of the spin (4292’s target). The hub’s ZBW swing shifts g only at second order (g < 2 in plane; 2 along z). Recorded, not claimed: mass-energy outside the CPs carrying no spin gives g = 2(1+ε). OWED: (F1) the DP-bond law and the dumbbell’s dynamic stability; (F2) R from ħ/2; (G) equal inertia is now load-bearing. **FOUNDER QUESTION (4293 §8):** does the hub-and-rim picture (the inner stays with the core; the core circles the centre) match his conception, given that his 4291 spirograph reached the rim?]**
**[4294 — founder: what force holds the rim? ANSWER: not charge. To carry ħ/2 below c the dumbbell must be ≥ 1.5 r_C; there a full unscreened Coulomb pull gives < 1% of the needed force (short by α), and the neutral hub’s polarisation 10⁻³ or less. Charge could hold spin ħ/2 only at ~1.1 a₀ with an unscreened charge (why SPIN-1 landed at Bohr scale). The ZBW itself needs a force ~137× stronger than charge at the Compton range; with route-(H) stiffness between hub and rim, ħ/2 puts the rim on route (H)’s circle at c and the hub on r_th at c/2 (convenient; flagged). BUT the orbital energy is m_e c²/2, so 4293’s accounting fails and g = 2 is provisional. OWED: (F1′) g with self-consistent relativistic energy and inertia (no CP rest mass; the rim at c); (F1″) identify the ZBW restoring force. **FOUNDER QUESTION (4294 §7):** what pulls a ZBW CP back toward its partner: sea tension (SSV), DP-arcs, or other?]**
**[4295 — founder (founders_voice/4295): the electron is a spinning rod −eCP · +eCP · −eCP, the +eCP shuttling through each pole to just outside it and back at one PSR per Moment. WORKED: close passes pull hard (inward = k e²(1/2L)(1/s − 1/(2d+s)), ~d/2s times plain Coulomb; formula = softened numerics). With uniform shuttle speed, g = 3(1−2f), f = ⟨r²⟩/(2d²+⟨r²⟩): g = 2 at an overshoot s = 9.5% of d (15/7 with none). The balance with ħ/2 gives d = 4.1 r_C, poles at 0.15c, ~9.5 shuttles per turn, small energies: **charge alone holds it** (4294’s “charge cannot” corrected to the dumbbell only). A 1-PSR turnaround binds absurdly (poles 10¹⁰ c). OWED: (H) the self-consistent shuttle with the +eCP’s inertia (non-uniform speed), does it exist and stay stable while rotating, and what overshoot and g follow; (H2) the +eCP-at-c versus SF-6 m₀γ inertia tension. **FOUNDER QUESTION (4295 §7):** after passing through a pole, what turns the +eCP back, and how far out does it go first?]**
**[4296 — founder: the pull of the pole it just passed. WORKED with inertia: energy fixes the overshoot; at the crossing threshold s = (√5−1)/2 d = 0.618 d (golden ratio). g passes 2 only at an energy 0.004% above threshold (δ = 7.4e-5), logarithmically sensitive; there the rod is 77 r_C, poles 0.016c, and the shuttle is slower than the spin (0.71). Full planar 3-body runs break up (equal inertia at the g = 2 energy; +eCP 1/100, robust crossing). 4295’s “holds on charge alone” withdrawn (erratum). CONCLUSION: every Newton-plus-Coulomb version of the electron so far comes apart; CPP’s motion rule (one PSR per Moment along the SSV-net, with DP-arc inertia) is not F = ma. OWED: (I) write down the CP motion rule and simulate the rod with it (a coded motion rule was not located in the searches made); (I-check) 3-D, longer, relativistic-cap versions. **FOUNDER QUESTION (4296 §6):** each Moment, how does a CP choose its step: straight along its SSV-net, or keeping some previous heading through its DP-arcs, and how much?]**
**[4297 — founder’s energy picture (KE stored as DP-arcs, returned as SSV gradient) is conservative inertia, the class already simulated; it changes the verdict only if the arc store adds memory, loss or quantised amounts. With the +eCP between two poles it is slowest at the centre, fastest at the poles (“max at the centre” needs the swing ≫ the pole spacing). Four classical electrons have failed; electrons are identical, so the electron is a ground state. The shuttle’s action at g = 2 is 4.6ħ (0.73h), so quantisation shapes the motion. DIRECTION CHANGE: stop classical orbit search; (J) derive the lattice quantisation rule (why a circulation carries whole units of ħ/2: PSR, Moment, step momentum), then solve the −eCP · +eCP · −eCP ground state. **FOUNDER QUESTION (4297 §6):** on the lattice, what makes ħ/2 the smallest turning a CP can carry?]**
**[4298 — founder proposed: ħ/2 is set by the weakest centripetal acceleration at a PSR radius. COMPUTED: at one PSR and one PSR per Moment, a = c²/l_P = 5.6e51 m/s² is the lattice’s MAXIMUM; weaker needs a slower CP; landing is GP-quantised at a sub-Planck spacing, so a minimum step, speed and acceleration exist (value not evaluated). Action needs momentum: an electron-scale CP at one PSR carries 1.4e-23 ħ; ħ/2 at one PSR needs half the Planck momentum; at electron energy ħ/2 needs ~1.5 r_C (3.6e22 PSR). ħ = p_P l_P is partly unit bookkeeping (the Planck units contain ħ). The 600-cell’s loops give 0.289, 0.433 (rhombus) and 0.688 PSR, not ½ (only a regular square would). Size-independence of spin rules out any fixed-radius mechanism; the rule must be a fixed action lump per exchange. OWED: (J1) is there a fixed action per DI-bit (V_i change quantum)? (J2) test the pass-through ZBW ‘two crossings to return’ as the source of the ½. **FOUNDER QUESTION (4298 §7):** what does a single DI-bit carry: a fixed smallest nudge of V_i, a fixed energy or momentum?]**
**[4299 — founder: a DI-bit carries the message to move a CP one GP; a CP’s emission must reach every GP on the sphere at the smallest PSR (l_P/2). COMPUTED: the covering count is 3.1e60 (s = l_P/1e30) to 3.2e64 (s = 9.9e-33 l_P) DI-bits per Moment. “One GP” is a LENGTH quantum (velocity step 1e-30–1e-32 c); the angular momentum per DI-bit is m·dv·r, 3e-31 ħ for an electron-scale CP versus 3e12 ħ for 1 kg at 1 m, so it cannot give size-independent ħ/2. Coincidence recorded, not claimed: p_P × (l_P/2) = ħ/2 identically (bookkeeping plus the underived ½ of the floor). OWED: (J3) the GP-spacing values disagree (~100×: GR-FE-1 vs EU budget); (J4) decide whether ħ is input (via PSR = l_P, Moment = t_P) or derived. **FOUNDER QUESTION (4299 §6):** is Planck’s constant given, like c, or to be derived? If given, only the ½ remains (PSR floor l_P/2; the pass-through’s two crossings).]**
**[4300 — FOUNDER ANSWERED: ħ is to be DERIVED; the l_P/2 PSR floor has no conceptual standing (withdrawn as a ½ candidate). AUDIT: the corpus identifies ħ (c04: action per Planck-ZBW half-cycle, ħ = E_P t_P, with E_P = ħ/t_P) but does not derive it (c03: “by construction”); GR-1/1a’s G = ħc/m_P² is an identity. SI fixes h exactly (2019), so the derivable content is dimensionless: α = 1/137.036 (and (m_e/m_P)² = 1.75e-45). TARGET: derive α (already open, QM-6 capstone l.307); a_e ≈ α/2π links it to this arc. **FOUNDER QUESTION (4300 §5):** between two CPs, what fraction of one’s DI-bits does the other intercept, and what push does each deliver?]**
**[4301 — founder (founders_voice/4301): N DI-bits per GP per Moment (same for all GPs), travelling one GP per Moment to the PSR, landing in a 10% band; one GP of push per DI-bit; delivery diffuse via re-radiation; ACTION QUANTUM = one GP of displacement against the least stress over one Moment (ħ_CPP = f₁ s t_M); conversion numbers not yet articulated. WORKED: the counting gives Coulomb’s 1/r, and **α = Nσ/(4π s PSR) = N s/(4π PSR)** (σ = s²): the conversion f₁ CANCELS. α = 1/137 needs N ≈ 9e28 (PSR/s = 1e30) or 9e30 (EU spacing) per GP per Moment; “cover every GP of the sphere” needs 1e61–1e65 (band: 1e90–1e96), i.e. charge 1e32 too strong: not all of {cover every GP, one-GP push, one-push-per-Moment action} can hold. OWED: (K) read SF-4_alpha_exponent_closure.md against this; (K2) whether re-radiation conserves push; (K3) any independent fix of N. **FOUNDER QUESTION (4301 §6):** is “every GP on the sphere gets a DI-bit” a requirement or a hope? With N ~ 1e29–1e31, direct reach is one GP in 1e32 and the rest is re-radiation.]**
**[4302 — founder: the covering was one DI-bit per CP on the PSR sphere (not per GP). WORKED: the landing shell holds 0.025 CPs (today’s Sea), 0.3 (end of inflation), 1.3 (BH packing): covering CPs needs N ~ 1, so 4301’s 10³² conflict is withdrawn (erratum); the conception is consistent but does not set N. In GP layers, α’s N = 4πα(PSR/s) = 0.0917 PSR/s = one DI-bit per GP layer of a band 9.2% of a PSR deep; if a GP emits one DI-bit per layer of a band of depth f, **α = f/4π**, independent of s; the founder’s “about 10%” gives 1/126 (8% from 1/137). RECORDED WITH CAUTION (one number vs one number; the per-layer rule is Claude’s construction). **FOUNDER QUESTION (4302 §4):** does a GP emit one DI-bit per GP layer of its landing band, and why is the band ~a tenth of a PSR deep (exactly, or a little less)? OWED: (L) if yes, derive f from the lattice.]**
**[4303 — founder: 4302 was a slip; it IS one DI-bit per GP. 4301’s tension reinstated: α allows N = 0.092 R (one DI-bit per GP LAYER of the band; R = GPs per PSR); every GP on the surface needs R² ≈ 10³²–10³⁴ × that; every GP in the 10% band ≈ 10⁶¹–10⁶⁵ × (α would be 10⁵⁹–10⁶³). Not all of {every GP covered, action = one least-work per Moment, α = 1/137} can hold. **FOUNDER QUESTION (4303 §5):** does a GP’s emission paint the band’s depth (one per layer, ~10³⁰) or fill it (one per GP, ~10⁹⁰)? If the latter, the action quantum is ~R² least-works.]**
**[4304 — founder: N = one DI-bit per GP on a one-GP-deep surface at the smallest (black-hole) PSR, spread through the 10% shell. WORKED: α = PSR_min²/(s·PSR); neither floor on file fits (l_P/2 → 10²⁹, one GP → 10⁻³⁰); α = 1/137 PREDICTS PSR_min ≈ 10⁻¹⁶ PSR (~10¹⁴ GP spacings). FANOUT (R-OUTWARD-FANOUT, 3135): shares split, not multiply, so N thins as 1/r²; the 10% band is DERIVED (pass 3: σ_r/⟨r⟩ = 0.076–0.096) and **4πα = 0.0917 lies inside it** (1.4% from 0.093). OWED: (M1) recompute pass 3’s band at the physical hop count and with an agreed width definition, compare f/4π to α; (M2) GR lane: can the registered SR-1 PSR(SSV_abs) reach 10⁻¹⁶ PSR at a black hole? **FOUNDER QUESTIONS (4304 §5):** is “one DI-bit per GP layer” his picture; how small is the black-hole PSR?]**
On a circle a single charge carrying ħ/2 at c with E = mc² has g = 2r/r_ZBW: route (H) (r = ħ/mc) gives 2, c04’s r_th gives 1. SPIN-1’s captured neutral DP (+eCP at r_in, −eCP at 2r_in, mass m_e each) gives, on its own equations, g = 3 − 2√2 = 0.172, μ = 0.0858 μ_B (1/11.7 of measured; sign right). **Founder question (4286 §6):** does the magnet come from (a) the dipole’s two ends, (b) the electron’s own −eCP on its ZBW circle at ħ/mc, or (c) something else? **Computations owed (either answer):** (i) can the core −eCP’s own ZBW (SPIN-1 fixes it at r = 0) supply reading (b) without breaking SPIN-1’s L = ħ/2; (ii) the massless-pole version of SPIN-1’s two-pole carrier (4281 inertia, SPIN-1 geometry) — does 3 − 2√2 survive; (iii) SPIN-1 v2.2: add the moment result, or a scope note that SPIN-1 derives spin but not the moment. **Also:** SPIN-1’s premises (CP rest mass m_e, slow Coulomb orbits) against SF-6 l.170, 4265 and 4281 — flagged, axiom-level, not resolved. phenomena-SM-1 §C4 corrected in print at 4286. Script `series_standard_model/code/4286_g_is_orbit_radius_route_H_gives_2.py`.

### TODO-4289-SPINREV — SPIN-1 v2.2 and SPIN-2 revision; both HELD from deposit (registered Patch 4289, QM/SPIN lane)
Hold markers (`%%TODO:` comments above \documentclass, so the PDF is unaffected) withhold both via `code/build_osf_queue.py` (“unfinished”). Text owed: SPIN-1: thm:spin restated as “the radius at which L = ħ/2”, not a derivation of ħ/2; the abstract’s and the Zenodo description’s “No free parameters” and the verification table’s “exact match” corrected; add the forward reading (L = 0.168 × ħ/2 at SPIN-2’s anchors) and the magnetic-moment result g = 3 − 2√2 (4286); note the founder’s CP-inertia ruling (4288). SPIN-2: §Scale Connection states that the pair leaves its anchors at 35.27×; the “complete derivation … no free parameters” conclusion is withdrawn. Then Thomas recompiles both PDFs (mechanical). Remove the hold markers only after the revision. Source: `series_standard_model/axiom_maturation/4289_spin_papers_hbar_half_is_input.md`.

### TODO-4289-SPINSWEEP — find every citation of “spin ħ/2 derived with no free parameter” (registered Patch 4289)
Unscoped sweep (SPIN-1, Spin I, c20, captured DP, “spin is emergent”, 2√2) across papers, companions, FAQs, glossary, programme_orientation and the SR companions; each gets a pointer to 4289. Dev notes (GR-1c l.1388; the spin-arc notes l.491/505/526) are history: annotate, do not rewrite.

### TODO-4300-HBARSWEEP — align “ħ, m_P fixed by the 600-cell lattice / no free parameters” with c03’s accurate wording (registered Patch 4300)
GR-1 (l.265), GR-1a (l.322), FAQ-GR-1a Q3, phenomena-GR-1a and phenomena-GR-1 state that m_P (and ħ, l_P, t_P) are fixed by the 600-cell lattice, with G = ħc/m_P² “no free parameters”. That is an identity; c03 l.264 gives the accurate statement (a physical identification, not a numerical derivation). Add scope notes; neither GR paper is deposited. Also sweep any other “ħ derived” wording. Source: `series_standard_model/axiom_maturation/4300_what_a_derivation_of_hbar_needs.md`.

### TODO-4284-SF2V109 — SF-2 v1.09 had no queue entry (registered Patch 4284, EW lane; found at Session 240 boot)
`development-SF-2.md` lines 281–290 list v1.09 as owed: items (i)–(v) (Session 238) plus (vii) the ZBW reset is a transit (R-ZBW-PASS-THROUGH), and (viii) the recomputed pocket time fraction (also under TODO-4264-PASSTHROUGH (a)). 4283's `NOTHING-DEFERRED` said all were filed; (vii) and the v1.09 edit itself were not. Any g_A figure SF-2 quotes must follow 4284 (no r_p-selected value).

### TODO-4234-DELTA — the nucleon's g_A/g_V from the cage (registered Patch 4234, EW lane; owned by the STRONG sector)

**Critic status (Patch 4284):** items 4274, 4266 and 4276 done. 4276's r_p selection **withdrawn** (two quark masses in one row → TODO-4284-RPFLOOR); 4274 §1.2's conclusion kept, reason corrected; 4266 (iv) rejection stands on reason 1 alone. g_A now **1.242–1.282, unpinned**; the zero-parameter exchange-off value is 1.312. TODO-4284-MODECRITIC **CLOSED at 4285** (both implementations faithful; see 4285 fragment).

δ = λ+1 = −0.275 ⟺ g_A = 1.275 = (5/3) × 0.765. The vertex is λ = −1 (G-EW-SWAP-4228); the cage must supply BOTH the SU(6) spin-flavor content 5/3 (Δd_n − Δu_n) and the reduction R = 0.765 — SU(6) alone gives a = −0.190, A = −0.238, excluded by the correlations (4222). Unscoped search (4234): no axial-charge or SU(6) computation exists in the tree. R in the Dirac-quark account is the lower-component fraction ⟨v²⟩ = 0.176; the CPP-native candidate is the fraction of the quark's resolved 0.63-fm circulation the vertex sees as spin along the nucleon axis. Bar: SF-3's — m_e, the 600-cell and SU(3) alone. `code/4234_delta_decomposition.py`. **Lane: EW → strong.** **4240 (first computation, `code/4240_gA_five_thirds_and_the_reduction_family.py`):** (i) the **5/3 is derived** from the SS-2 cage — colour antisymmetric over V₁–V₃ (SS-1c) + L = 0 ⇒ spin state symmetric under the u–u exchange V₁↔V₂; the unique such J = ½ state gives Δu − Δd = 5/3 — conditional on quantum spin-½ (OPEN-QM-3) and Fermi statistics; (ii) **R is not a lower-component (cavity) reduction** — g_A = 1.2754 needs a 0.757-fm cavity where μ_p = 1.17 vs 2.79 (−58%); (iii) R must be **spin–orbit sharing at the constituent scale** (μ reduced only by (1+R)/2: μ_p 2.648, μ_n −1.765, inside the naive ±7%); (iv) its value needs the quark's motion in the cage: k = 368 MeV = 1.18 m_const; corpus scales give 1.31–1.51, none a derivation. **Open:** R — blocked on [[TODO-4240-CORE-MOTION]] (4242: mechanism = the core's breath, founder; μ tied to g_A with no free parameter). **4243–4244: computed — g_A = 1.406 (+10.2%) under the founder's modes at the ZBW frequency, relativistic, real geometry, zero parameters; μ_p −0.8%, μ_n −4.4%. OPEN: the ~10% residual — each mode needs ~20% more momentum spread than its Compton ground state. Untried: Dirac-oscillator spin–orbit terms; the d's internal linear −eCP oscillator's effect on its core; excited-mode admixture. Do not attribute the residual without a computation.** Also found: `mc_su3_algebra.py`'s hard-coded g_A_cpp = 1.27 is a PDG-matched table entry, annotated at 4240; SS-2's m_const is assigned (m_p/3), not derived — SI-1 M9 corrected. **4259 — lever 1 CLOSED** (`series_standard_model/code/4259_gA_lever1_dirac_spin_orbit.py`): the breath's well solved as a Dirac equation, lower component included — scalar 1.487 (+16.5%, wrong way), S = V 1.416 (+11.0%), vector no bound state (Klein), Moshinsky oscillator 5/3 or 1.100 (−13.7%) by sign; nothing lands at zero parameters, and a partial oscillator coupling would be a fit. Kinematic baseline in the joint quadratic form 1.398 (4244's independent-mode sum 1.406). **Still untried:** the d's internal linear −eCP oscillator; excited-mode admixture. **Critic owed:** solve the no-pair-projected vector well (the electric breath's natural coupling) and confirm it stays within ~1% of the 1.398 kinematic row. **4260 — lever 2 CLOSED** (`series_standard_model/code/4260_gA_lever2_linear_oscillator_recoil.py`): the d's linear −eCP oscillator carries no angular momentum and acts only through core recoil on the d's 1/5 share of g_A (floor 1.245 at infinite recoil). Closing needs ~6 GeV of recoil, a d core at ~5.4 M_N, and μ_n −18%. The scale on file (u→d 2.5–4 MeV) moves g_A by < 1e−4; eCP trading ≤ 0.028. The residual must come from the u's own modes or from something acting on both quarks alike. **Remaining:** lever 3, excited-mode admixture; the vector no-pair critic item above. **4261 — lever 3 CLOSED as a mechanism search** (`series_standard_model/code/4261_gA_lever3_admixture_and_the_u_third_axis.py`): excitation would need every mode ×1.49 (~0.6 quanta), and no source is on file. **Found:** (B) 4244's u carries zero momentum along its third axis (in plane, normal to its u–d edge), so it is unlocalised there, contradicting 4243's ⟨r²⟩ check; held at width w = r_ZBW/√2 it gives g_A 1.332. (C) Colour = vertex + SS-1c + 4240's symmetric spin–flavour ⇒ the two u's breaths form a Slater determinant; the u–u exchange gives 1.308 (+2.6%) at that width. **Every mechanism lowers μ_p with g_A** (4242 tie): closing g_A at m_q = m_p/3 costs μ_p ~5%, and keeping μ_p needs m_q ≈ 297 MeV (= 4243 (T)). **Open, owed:** (i) ~~founder question (4261 §5)~~ **ANSWERED 4262** (`founders_voice/4262_…`): the u is bound to BOTH minus vertices (own: strong + charge; other: charge only) and ZBW-oscillates with each, so its third mode runs along the u–u line. g_A 1.333 (+4.5%); with the u–u exchange (overlap 0.35), **1.310 (+2.7%)**, μ_p −4.2% (m_q 299.6 MeV would restore μ_p); **clarified same day:** the seats are inequivalent (u_q on the −qCP: charge + strong; u_e on the −eCP: charge only). Under route (H) the widths do not depend on binding strength, so the numbers are unchanged. **Founder question (4262 §4):** u_e's diagonal pull toward the −qCP has its charge cancelled by u_q's +2/3 — is it carried by the strong force? If not, u_e is unheld (g_A 1.369 before exchange); **ANSWERED 4263** (`founders_voice/4263_…`): no bound and no loose quarks, only oscillating ones. A quark falls into superposition with its vertex, reverses, and heads toward the strongest gradient (Sea, sideways vertex, or the diagonal vertex when open); CP charges are integer, and fractional charges are effective. Sequential excursions: if each is its own ground state, g_A 1.49–1.56 (fails); if the quark carries its whole breath energy through the bounce, 1.316–1.383, classical bounce **1.322** before exchange. Partner choice drops out of g_A. **Owed (4263):** ~~the u–u exchange recomputed for sequential excursions~~ **CLOSED 4267**: the sequential picture is the classical (L = 0 line-orbit) reading of the same ground state; line orbits reproduce the quantum g_A to 0.5%, so 4262's 1.310 with exchange is the ruled-picture number. Still owed: the per-quark breath energy (it is part of m_q, item (iii)); (ii) u–d exchange under colour = vertex — **4269, bounded by two limits**: (L1) d pinned to the +qCP seat → u–d exchange zero, g_A 1.310, μ_n −1.6%; (L2) colour antisymmetry over all three seats (3-orbital Slater, R_u = R_d) → g_A 1.271 (−0.3%), μ_n −2.7%. **Founder question (4269 §4):** does the down quark ever take a turn on a minus vertex, or does the vertex charge keep it on the +qCP seat always? If partial, the answer lies between. **ANSWERED 4270** (`founders_voice/4270_…`): the up quark bonds to 2 of 4 vertices (minus only), the down to 4 of 4. For the proton, the seat count excludes the u–d-exchanged configuration → **L1, g_A 1.310**; L2 closed for the proton. **Owed (4270) → 4271:** (a) the d's bond to the open +eCP, computed as the odd quark's diagonal mode (mirror of the u's): free-nucleon **g_A 1.294–1.297 (+1.4 to +1.7%)**, m_q ≈ 299 MeV, μ_n ≈ −1.861 (−2.7%). **Still owed:** the open vertex's position (off the quark plane), to pin the direction and replace the bracket. In a bound nucleon the mode is lost and g_A rises toward 1.310, the opposite of nuclear quenching (recorded negative). (b) ~~neutron in its own frame~~ **CLOSED 4271**: it is the proton's mirror (u on −qCP, d's on +qCP/+eCP, −eCP open; dd edge −0.2%), so d–d exchange = u–u exchange. **Correction:** μ_n in the 4244–4269 scripts was assigned by flavour; by role, the ruled-state μ_n = −1.871 (−2.2%), not −1.5% (errata on 4268, 4270). **SS-2 paper correction owed:** its neutron assignment (V₁(−) d, V₂(−) d, V₃(+) u, V₄(−) open) has three minus vertices and a u on a plus vertex, contrary to hTetra composition and founder 4270; replace with the mirror assignment; (iii) **the m_q tension** — derive m_q or show that g_A and μ_p jointly require ≈ 297 MeV, SS lane with EW; SS-2's m_p/3 is assigned (SI-1 M9). **4268 audit:** no derivation is on file. Under route (H) g_A is m_q-independent, so the +2.7% g_A residual belongs to the motion (erratum on 4261 §3 and 4262 §3(iv)). m_q fitted to μ_p = 299.6 MeV; μ_n is then predicted at −1.884 (−1.5%), and μ_n/μ_p −1.5% whatever m_q is. **Still owed:** derive m_q from the up quark's structure (+qCP in a polarised eDP sphere, founders_voice/4263; c04 cloud resonance), SS lane.

### TODO-4264-PASSTHROUGH — ZBW pass-through (**RULED 4265: R-ZBW-PASS-THROUGH**, supersedes 3134 §3)
Founder proposal (`founders_voice/4264_proposal_zbw_is_pass_through_shm.md`): ZBW is a conservative pass-through oscillation, not superposition-stop-reverse; would supersede R-ARC-CANCEL-TURNAROUND (3134 §3). Assessment: `series_standard_model/axiom_maturation/4264_zbw_pass_through_assessment.md` (recommend adoption on energy conservation and 3134's own asymmetric-pair logic). **RULED 4265** (`founders_voice/4265_ruling_zbw_is_pass_through.md`); keep the name "ZBW" (4265 §2). Sweep campaign (4264 §4 table): (1) ~~supersession banner on 3134 §3~~ **DONE 4265**, plus the glossary ZBW-cycle entry and CONV-023 row A; (2) ~~c04 scope banner rewrite~~ **DONE 4277** (v2.2; the proof and convention were fixed too); (3) ~~re-sweep F-SW-10~~ **DONE 4280** (only c04 carried arc-cancel wording in `.tex`, fixed at 4277; banners on subpsr_pass2_radial_resolution.md and 3201); (4) ~~sea-gravitation R-DWELL-1~~ **DONE 4278**: the dwell is kept as transit; FQ-12 is answered in substance (exit momentum = carried-through crossing momentum; calibration unmoved per 3131 §4); 3132's "mutual requirement" is superseded; R-ARC-BULK-NULL survives (both arc polarities per full cycle, turnarounds at the extremes). Banners are on sigma_n_derivation.md and the 3132 examination. darcforce_*/selfconsist_* scripts: no change needed while σ_n carries the kick; (5) ~~EW 4231/4232 reset restated as transit~~ **DONE 4279** (banner on 4231; glossary restated). **Owed (4279):** (a) recompute the pole-in-pocket time fraction for a pass-through swing (time density ∝ 1/speed); 4233's "pocket factor fixed at m_const/m_W" does not survive unexamined; (b) ~~founder question (4279 §4)~~ **ANSWERED 4281: petal loops** (beside the core). **4281:** a massless pole on a rosette with L = ħ/2 and E = mc² has **g = ⟨E_total/E_kin⟩**: 1 for a free circle (radius ħ/2mc = r_th), 1.54 at route (H)'s stiffness, 2 for a well 6× softer (petals out to ~3.2 r_ZBW); the pole never comes closer than ħ/2mc. **Owed:** (i) derive the well of the quark's own orbital (depth and shape), with the +eCP pole and the cloud included, and see whether g = 2 follows; (ii) if the quark's g ≠ 2, the nucleon moments scale by g/2 (at 1.54, fitted m_q ≈ 230 MeV) — hold until (i) **[4286: 1.54 mis-resolved route (H) (D-7); at route (H)’s own orbit g = 2 exactly (g = 2r/r_ZBW on a circle), so the 0.77 scaling is WITHDRAWN and (ii) is closed. (i) is sharpened: show the orbital stores half its energy at a circular equilibrium at r_ZBW (V = rV′ there). Carrier question → TODO-4286-GMOMENT.]**; (6) DM 2454/2455/conv001: **4280 — yes, it did** (home-anchored rebound, founder 2433). **Owed, DM lane:** dance v5 with pass-through legs (through the partner, turning on the far side); recompute ring−straight (−68.8 ± 6.5 MeV at 2455) and the ring-survival verdict (~75%, unpromoted, so nothing downstream rests on it); (7) ~~read QM-1/2/6, SPIN-1, GR-1j~~ **DONE 4280**: every "reset" there is the DI-bit or phase sense, so nothing to change. (8) ~~ZBW frequency factor~~ **DONE 4266** (`series_standard_model/code/4266_zbw_frequency_convention_and_gA.py`): three conventions are in the corpus, and g_A depends on them more than on any mechanism (ruled set 1.333 / 1.191 / 0.952). Route (H)'s ω = mc²/ħ (full swing = Compton period h/mc²) stands. Schrödinger's 2mc²/ħ is the ±E beat, which the pass-through shows as speed peaking twice per swing. **Added to step 2 (c04):** (i) c04's T_C = 2r_th/c = ħ/mc² is 2π short of the Compton period — the proof of mc² = ħν_C pairs ħ with 1/T; correct it; (ii) reword c04's Schrödinger remark as "speed peaks twice per pass-through swing". **Critic owed (4266 §3(iv), PD-008):** with 4244's pre-4262 mode set, the ω that closes g_A is 2.007 mc²/ħ, Schrödinger's to 0.4%. Rejected (it uses the superseded mode set; it is a beat, not a swing frequency; μ_p −4.9%). Next window: critique the rejection. (9) First use in each paper: *pass-through ZBW cycle*, with one sentence on the supersession. **4272 (step 2 opened):** (i) **c04 mass proof broken**: eq. (E-cloud) gives ħν_P²/ν_C, not ħν_C; the displayed intermediate is not an energy; mc² = ħν_C is asserted, not derived. (ii) Pass-through with a speed-c crossing ⇒ ω × amplitude = c: r_ZBW = ħ/mc ↔ ω = mc²/ħ (route H); c04's r_th = ħ/2mc ↔ 2mc²/ħ (Schrödinger's triad). (iii) **Second observable:** r_p with the founder's modes: route H 0.920 fm (+9.4%), Schrödinger 0.784 (−6.8%); ω closing r_p ≈ 1.43, closing g_A ≈ 1.12 — a tension, recorded. **4273:** relativistic widths (−) and u–u exchange on positions (+) roughly cancel; in the consistent state r_p is +11.6% at route (H) and closes near ω ≈ 1.35–1.4, while g_A asks ≈ 1.12. The tension survives. **Owed:** (a) ~~re-derive the frame size~~ **4274 — CONVENIENT RESULT, CRITIC OWED:** fractional charges are 0.4% of SS-2's frame balance (4273's pointer corrected); the dominant +ħc/r term double-counts the breath kinetic energy now carried explicitly. Without it the frame sits at lattice size (u–d = l_edge 0.364 fm, u–u between l_edge and 2 l_edge). Across that whole range r_p is −2.1% to +1.7% and g_A −1.1% to +0.8%; at SS-2's own 120° shape r_p −0.2%, g_A 0.0%; m_q(μ_p) ≈ 296, μ_n −2.6%. **Critic (next window):** the double-counting argument; Gaussian exchange at overlaps 0.6–0.8; the r_p model; l_edge is Λ_QCD-anchored; g_A's weak frame sensitivity. ~~Founder question (4274 §6)~~ **ANSWERED 4275:** the lattice is far finer than a proton, so "one lattice cell" has no proton-scale meaning. The proton's effective diameter is the oscillation length of the four hTetra CPs plus the quarks' oscillation; the inner swing is shorter (two repulsive CPs and one attractive inside). **Consequence:** 4274's anchor u–d = l_edge (0.364 fm, SS-2's Λ_QCD-matched unit) is **withdrawn**; its rows stand only as a diagnostic (the observables need a small static triangle). **Owed (4275):** build the frame from oscillations — (i) the vertex CPs' own swing amplitude (bond-held, smaller than the quark's, 4264), which adds to r_p and moves the breath's reference point; (ii) the asymmetric radial well (stiffer inside), needing the inner/outer stiffness ratio (direction: r_p up, g_A slightly down); (iii) only then compare with g_A and r_p. The double-counting point (4274 §1.2) stands. **4276 — founder: the hTetra is a 2eCP–2qCP square** (attractive pairs are the edges, like-sign pairs the diagonals, flat until quarks bond to its corners). Proton: u–d = edge a; u–u = minus diagonal ∈ [a, √2a]; the d's diagonal mode points across the plus diagonal; SS-2's 120° shape is outside the range. **Size a fitted to r_p** → u–u ≈ 0.60–0.62 fm and **g_A predicted 1.267–1.270 (−0.4 to −0.7%)** across the whole shape range; m_q ≈ 296; μ_n −2.5%. [**4284: withdrawn as a prediction** — the r_p fit used m_q = m_p/3 for length; at one m_q no size fits r_p, and g_A spans 1.242–1.282 unpinned.] **Owed:** (i) the vertex swing amplitude (not given; it shrinks the fitted a); (ii) the asymmetric inner swing; (iii) a better-than-Gaussian exchange at overlaps ≈ 0.7; (iv) a derivation of a itself (the attractive-edge length of the 2eCP–2qCP element); (v) founder to confirm the diagonals are the like-sign pairs (his first bullet lists (e−,q+),(e+,q−); read as (e−,q−),(e+,q+)). **HELD pending critique:** SS-2 (ε, r_p, r_eq formula factor-2 typo), SS-5, SS-6, SS-9 (use 1.07/0.62 fm); (b) one consistent non-Gaussian treatment of the exchange in position and momentum (overlaps 0.45 vs 0.35 now); (c) then decide the swing size and write the c04 correction (proof, ħ/h pairing, radius). The c04 text edit is held until then. No joint (ω, frame) fit to (g_A, r_p) — two parameters for two data points (PD-008). **c04 DONE 4277 (Version 2.2):** pass-through banner; proof restated (the proposition is equivalent to r_th = ħ/2mc; the v2 chain gave ħν_P²/ν_C); ν_C declared angular; Schrödinger remark reworded. r_th is the particle's own cloud radius, not the quark's swing, so no radius change. **Founder action:** recompile the c04 PDF. **Independent of the ruling, owed:** route (C) coupling under integer CP charges (4263) — SS-2's colour coefficients no longer apply; derive a from the founder's charge picture and re-run 4264's pass-through rows, EW → strong.

### TODO-4222-CRITIC — **CLOSED at Patch 4252: (i) 4250, (ii) 4252, (iii) 4249 all done** (registered Patch 4222, EW lane, under PD-006)

Done at 4222 (`code/4222_critic_*.py`): **4193 CONFIRMED** (table by selection); **4216 CONFIRMED** (sign from couplings + Dzuba–Flambaum / Sahoo forms matched across γ₅ conventions; my own first draft had the L_eff sign wrong and would have withdrawn a sound result — recorded); **4206 §2 DOWNGRADED** — a = −1/3 is a structural match, A = −2/3 and B = +2/3 match only because the SU(6) per-quark polarisation 2/3 equals the J=½ Clebsch factor 2/3 (no shared input; the SM at the SU(6) point gives A = −0.238). **Owed, deferred:** (i) critic on 4192's coherent-mixing exclusion — **DONE at 4250, stands;** (ii) critic on 4219/4220 — **DONE at 4252: no load-bearing residue; PRED-O-42 row amended to the 4228 state;** (iii) 4215's open question, (1−β) or (1−β²) for the e/μ ratio — **DECIDED at 4249: (1−β); with the 2E_l·2E_ν normalisation the model equals the SM tree ratio exactly; the 0.70 was the omitted normalisation.** SF-2 v1.06's postdiction sentence carrying the 4206 wording: **replaced at 4223** (v1.07). **Lane: EW.**

### TODO-4214-ONESIGN — the absolute sign of the ejection rule is the primitive; derive the RELATIVE signs (registered Patch 4214, EW lane — NEXT ARC)

Sequencing ruling (PD-006, 22 Sep 2026): **do not attempt to derive why the throw goes against the spin.** THEO-CHIR-STATUS-1/2 (0653/0654): chirality is V3, an irreducible primitive; no axiom-level pseudoscalar fixes the sign; the 600-cell is achiral. "Against rather than along" is the CP-conjugate choice of universe and is not derivable from an achiral substrate — a derivation would contradict two proved theorems. **What IS derivable and falsifiable:** that ONE primitive sign, q(v̂·A) = +1, gives the right relative sign of every handed observable. Check list (each is a sign, not a magnitude): (1) electron vs antineutrino helicity in β⁻ (done, 4199); (2) π⁺ → μ⁺ν: **PASSED at 4215** — the μ⁺ is *left*-handed (wrong for an antiparticle), forced by the left-handed ν; the item's original wording 'right-handed' was an error. Magnitude: **4249 — (1−β) confirmed; with 2E_l·2E_ν normalisation the e/μ ratio equals the SM tree exactly (magnitude passes too);** (3) atomic PV: **PASSED at 4216** — electron drifts against its spin in neutron-rich matter (Q_W < 0), same direction as the W throw; one rule with positive weight −Q_W(partner): +1.073 per linear oscillator (the object the W captures), −0.382 per core; 4197's table re-signed; **critic re-derived the γ₅ → σ·p/mc sign at 4222 from the Z-exchange couplings and two literature conventions: CONFIRMED**; (4) λ < 0: **PASSED at 4228 (G-EW-SWAP-4228):** the sign is the minus sign of the created proton–electron singlet — no bias, no imprint, no mechanism beyond a counter-pair born with zero net angular momentum; 4219's counter-imprint mechanism retired; 4220's reversion stands as history. Earlier framing kept for the record — the refill's anti-phase (4207) as a sign the model must reproduce, not fit; **criterion fixed at 4223:** the same-sense (singlet) amplitude and the m=0 component of the opposite-sense (σ-written triplet) amplitude must be ANTI-phased — in-phase gives (A, B) = (−0.987, +0.119), the measured pair swapped; a relative sign, so derivable under this ruling, and what the DP-arc/wake mechanism must deliver with |λ|; (5) **PASSED at 4217** — the β → 1 limit of 4215's suppression, wrong-helicity fraction (1−β)/2 ~ 1e−13; and the rule answers SF-4's open Majorana question: Dirac, inert right-handed states at ~m²/4E², **no 0νββ** — a detection kills the model. Register in predictions.md and SF-4 §openwork only after items 4, 6, 7; critic: are ν and ν̄ different objects or only different histories?; (6) **PASSED at 4218** — the electron's own orbital is shed, q = −1, left-handed; (7) **PASSED at 4218** — one q serves the ejection rule and the polarity clause, so C flips helicity by construction. **(4) REVERTED at 4220 — UNTESTED.** The founder's "indefinitely" was about the new orbital and the antineutrino, not the wake; on the wake (DP-arcs seeding the replacement's sense) he has no idea: **unknown mechanism, explore later.** Six passed, one untested, none failed. PRED-O-42 stands on items 5–7 (row amended). Magnitude of λ still blocked, now on the DP-arc geometry alone (CONJ-FP-1), not on persistence. Owed: SF-4 §openwork amendment citing PRED-O-42 (after the SF-2 recompile lands); critic to ask the founder whether neighbours' counter-sense can be re-paired before the refill. **A single wrong relative sign kills the model.** Do this before any magnitude work (λ, Fermi limit). **Lane: EW.**

### TODO-4212-CAGETABLE — reconcile cage-per-particle assignments (registered Patch 4212, SM lane; founder owes the table)

Founder (4212, verbatim): only the **muon** and the **strange quark** have a tetrahedral cage. Conflicts with `master_glossary.md` "Lepton Cage"/"Quark Cage" (all charged leptons; c, b, t) and with SM-3's K3 Koide premise (K3 face of a tetrahedral cage for all three charged leptons). SF-2 has the tau icosahedral. **Founder owes** one line each for u, d, c, b, t (e and τ settled by SF-2 §5.9 at 4213: e bare −eCP, τ −eCP + hybrid icosahedron). **SM lane owes** the SM-3 premise reconciliation and the two glossary entries. 4211's step-2 correction withdrawn; F5 disposition unchanged. **Lane: SM.**

### TODO-4188-F5SCOPE — **CLOSED at Patch 4211: conclusion holds, premise corrected.** Quarks DO share the tetrahedral K3 graph (SM-3 line 123); what breaks is the spectrum (strong terms, SM-3 §quarks). CKM = misalignment of two near-diagonal (strong-dominated) eigenbases at the refill; PMNS = pure K3 → TBM. F5 to panel on the inherited-open framing with step 2 corrected. First computable question for OPEN-SM-11: sin θ_C from SF-3's masses (target √(m_d/m_s) = 0.224).

From a discarded window whose context was carrying wrong verdicts (see `4188_session235_incident.md`), but the one piece the assessment judged sound: **K3 → TBM works for neutrinos because all three generations share the tetrahedral cage type, so the K3 base graph is common and its adjacency eigenvectors give U_TBM** — SM-3 §4: *"the theorem applies to leptons only"*; **quarks share no cage base graph across generations**, since strong-sector mass dominates; so **CKM mixing is a W-vertex mismatch between up- and down-type cage eigenstructures**, not a base-graph problem, routed to **OPEN-SM-11** as a multi-paper SF-2 extension. **If it holds, F5 goes to panel on an inherited-open framing parallel to SF-4's δ_CP** (χ₄ conserves CP; the CP-violation source is sign(δ) at W3/CHIR; the CKM magnitude needs OPEN-SM-11), **pending a compatibility check that χ₄ does not suppress sign(δ).** **Verify each step against SM-3 §4 and SF-2 directly — D-12.** **Lane: EW.**

### TODO-4185-LINEARITY — **CLOSED at Patch 4186: YES, and it was already proved in GR-1j**

**Answered at 4186 from GR-1j's own lemmas.** *"Fixed per-GP emission + static-snapshot payload + reset-at-delivery ⇒ the per-Moment computed state at a GP is a **linear shell-sum** of origin-GP registers one hop away"*; the kernel is the **shell mean**; and *"static self-consistency … is, for ANY PSR profile, **exactly flat-lattice Laplace**: ∇²u = 0 … verified numerically at 1e−9."* **All three premises hold for A_i** — A3′'s *"all channels obeying the same icosahedral shell-sum"*, AP-4c's *"imprint invariant in transit"* and *"reset at delivery"*. **Exactly Laplacian statics ⇒ a 1/r Green's function**, which is precisely the falloff [[TODO-4184]]'s ε = a/r assumed — **now grounded in an existing theorem rather than asserted.** **BONUS: GR-1j's Homogeneous-cancellation lemma closes SEASPIN outright** — *"the **uniform Sea** … **cancels identically**"* — so a uniform Sea contributes **zero**, whether or not its spins are paired. *(original entry:)* The founder's source/relay wording (4185) makes an empty GP a **faithful relay**: it *"integrates its input DI-bits and adds nothing more."* **That is what makes amplitude falloff purely geometric — 1/r for a point source spreading over a shell of area 4πr² — rather than caused by relay loss, and it is the assumption Patch 4184's ε = a/r ratio rests on.** **But "adds nothing more" is neutral only if the integration is LINEAR.** A nonlinear shell-sum would let empty GPs distort the relayed field even while adding no source term, and then the falloff is not 1/r and 4184's ratio does not follow. A3′ says all channels obey *"the same icosahedral shell-sum"* but I have not found it stated whether that sum is linear. **Small, and load-bearing for a number already reported.** **Lane: GR.**

### TODO-4183-SEASPIN — **CLOSED at Patch 4186: a uniform Sea cancels identically (GR-1j)**

**Superseded at 4184.** The founder corrected the premise: **every GP receives, integrates and rebroadcasts regardless of occupancy** — only CP-bearing GPs *source*, but the shell is full of **carriers**. So the register is **A_env(x) + a self-term from the resident CP**, and on a lattice the self-term is cut off at **one grid spacing a = 1.62e−65 m**. **The ratio is r/a:** own:environment = **6.2e49** (nearest spin 1 fermi), **6.2e54** (1 ångström), **6.2e64** (1 metre) — so **ε = 1.6e−50 / 1.6e−55 / 1.6e−65**, all **below the 5.4e−47 observational cap** by 3e3 to 3e18. **The local CP dominates by fifty to sixty-five orders — the founder's suspicion confirmed by a margin nobody guessed — and the ratio is DERIVED (a/r), not calibrated, so NO NEW PARAMETER.** Sea pairing changes A_env by at most √N and cannot close a 1e50 gap. *(original entry:)* 4183 withdrew 4182's 10⁴⁵ domination figure: **it counted GPs, but axial content comes only from GPs that CARRY a CP.** The PSR shell (R = l_P, 10% band, 5.31e−105 m³) holds **1.26e90 GPs** but, in solid matter at n = 1e29 m⁻³, **5.3e−76 matter CPs** — and 2.7e−60 even in a neutron-star core. **In ordinary matter the shell contains NO other particle's CP at all, by seventy-six orders.** So the only possible environmental axial source is the **DP Sea**, and the ratio the founder asks about is set entirely by the Sea's spin structure: **if Sea DPs are spin-paired with anti-aligned partners**, each pair contributes zero, the residual is whatever the pairing fails to cancel, and **the local CP could dominate outright**; **if Sea spins are independent**, the residual is √N over the occupied count. **The corpus defines the DP by CHARGE polarity (+/−) and does not state whether its two CPs' AXIAL attributes are correlated** — searched, not found. **Order of work: Sea spin structure first, ratio second, calibration only if both fail** — because a *calibrated* ratio is a free parameter (the cost flagged at 4172 §5 and 4175 §4), while a ratio *derived from the Sea's pairing* is not. **Lane: EW.**

### TODO-4182-LOOP — **is the spin-mixing loop part of the theory?** (registered Patch 4182, EW lane — FOUNDER QUESTION, PD-006(a); decides A3′)

**The founder corrected 4181 §1(a) and he is right:** the loop he describes — GP computes a register → stamps outgoing DI-bits → they arrive at the next GP → that GP sums them into its register → stamps its resident CP → the CP hops carrying it — **needs the GP-HELD REGISTER, which is A3′'s content, not merely AP-4's transport.** A payload alone cannot close the loop; something must compute and hold the sum between arrival and stamping. **But the conditional carries the weight.** The PSR band holds **N ≈ 1.3e90** GPs (4168), whose axial contributions sum to **~1.1e45** against the resident CP's single unit — **so raw stamping would replace a CP's spin with its neighbourhood's average in ONE Moment**, exceeding the measured cap **ε ≤ 5.4e−47** (4170, muon g−2) by **~2e46**. **The loop is consistent only at one setting:** stamp = (1 − 5e−47)×own prior A + 5e−47×register — a real architecture, the founder's, and **not "mixing" in any ordinary sense**. **SO THE QUESTION IS NO LONGER "is A3′ useless"** but **"is the loop part of the theory?"** — **if yes** A3′ is **required machinery** supporting one 1e−47 effect, which is a legitimate reason to keep an axiom; **if no**, the GP computes a register nothing reads and A3′ retires. **No third position:** a register computed and never read is idle by definition. **[[TODO-4181-RETIREMENT]]'s six requirements stand if retirement is chosen**, and 4179 stands regardless — keeping A3′ would be **for the loop, not for the completeness argument**, which 4178/4179 refuted. **Lane: EW.**

### TODO-4181-RETIREMENT — the six things retiring A3′'s A_i channel requires (registered Patch 4181, EW lane — **pending the founder's decision**)

**Steelman attempted and failed** (4181 §1): the GP-must-hold-A argument reaches **AP-4**, not A3′; no consumer of a *remote* CP's spin exists (b uses the CP's own A locally, magnetism comes from ZBW charge circulation per SM-2, quantum phase uses the V_i register); angular-momentum conservation does not need it because CPP's spin is **resolved circulation** and V_i carries it; and 4120's own reason — *"b must be derived, not postulated"* — is satisfied by **A1′ + the pre-existing V_i**. **Nothing else is needed physically**, but six things must be done: **(1)** make **A1′ stand alone** — 4120 says "all three required together"; this one matters most, A1′ carries χ₄'s physics; **(2)** restate **b at source** as A1′'s attribute contracted with the *pre-existing* V_i register; **(3)** decide **AP-4's A-clause explicitly** — not automatically retired, and **retiring it forecloses the founder's own spin-mixing proposal** (bounded at 5e−47, so no observable loss, but his choice not a side effect); **(4)** audit the suite — A3G-2 and A3G-3 survive, **A3G-1 (vacuum magnetisation) may become moot** since with no register there is nothing to sum; **(5)** **VERIFY, not assume**, that THEO-CHIR-1 and ⟨b⟩ = (v/c)cos θ use A1′ only (D-4); **(6)** record **why 4113's observation is true but unused** — T₁u ≠ T₁g under I_h stays correct, but the axial content is **∇ × V_i, derived not independent** (4179), so no future window rediscovers 4113 and re-proposes the channel. **Costs nothing:** χ₄'s physics, the zero-parameter claim (it *removes* TODO-4172-SOURCEEQ), and the falsifier record all survive. **Lane: EW.**

### TODO-4178-INDEPENDENCE — **RESOLVED at Patch 4179: NOT independent**

**Resolved at 4179.** **Helmholtz settles it:** any axial field is **∇ × (something) + ∇φ_A**, the first piece being exactly what **∇ × V_i** already supplies and the second longitudinal with a **PSEUDOSCALAR** potential (A axial, ∇ polar). **So A_i's only independent content is a pseudoscalar — precisely what 4172 excluded**, since T_μν contains none and that is why A_u is absent on principle. **DOF count on CPP's own shell:** (∇×V)(k) = i k × V(k) has rank **2 of 3** (2000 random k, min 2 max 2), and **the discrete 12-neighbour icosahedral curl is the same — rank > 2 in 0 of 2000.** So ∇ × V_i supplies two of A_i's three components at every wavevector; the missing one is the unsourceable longitudinal mode. **One loophole, unattractive:** a longitudinal mode can carry *free* radiation with no source — but a longitudinal mode of a propagating field is the classic **ghost/non-dynamical** case, which is exactly [[TODO-4174-VECTORGW]]'s unexamined second exposure. **So the channel is either redundant or carrying a probable ghost.** **EVERY SUPPORT A3′ HAS BEEN GIVEN IS NOW TESTED AND NONE HOLDS** (empirical: 4155/4165/4170/4175; structural: 4178; independence: here) — a recommendation to **restate or retire A3′'s amendment**, and **the founder's call**. **NOT "χ₄ is wrong":** A1′ carries χ₄'s physics intact, its attribute being a **sign, not a field**. *(original entry:)* 4178 found the structural argument fails on CPP's own numbers. **In Einstein–Cartan spin is an independent source because Dirac spin is NOT resolvable circulation. In CPP it is:** glossary, *"ZBW smearing … due to its **ZBW orbital motion** … r_ZBW = ħc/m_const ≈ **0.631 fm**"* — **6.31e−16 m against a grid spacing of ~1.6e−65 m, so the orbit spans ~4e49 grid spacings.** A resolved circulating current's angular momentum is **orbital**, living in T_0i, which A3′ already broadcasts as **V_i**. **And the packet already contains an axial field:** B^G = **∇ × V_i** (named at 4141 §2). **So A_i adds an INDEPENDENT axial field where a DERIVED one exists — a new degree of freedom, not a completed enumeration.** **What could still save it:** if the lattice's axial content is genuinely *not* fixed by the polar channel's curl, A_i is a new fact rather than a duplicate. **Checkable and unchecked** — it needs the shell-sum's action on both channels compared. **Does not touch A1′**, whose carried attribute is a *sense* (a sign), not obtainable from |∇ × V_i| and needed by b. **Lane: EW/GR.**

### TODO-4176-BUNDLE — **FOUNDER RULED at Patch 4178: keep, on realism — and the condition is under test**

**Last gap closed at 4177, positively.** SM-2 *Mass Generation*, verbatim: *"**Orbital ZBW DP**: All fermions; produces **spin/magnetic moment**."* **The corpus derives spin AND magnetic moment from orbital ZBW charge circulation**, in the EM/mass sector, long before 4120 — so A_i's magnetism job is redundant not merely with SF-6 but with the mechanism that produces spin itself. **And that clarifies A1′ modestly:** if spin is already emergent, *"CPs carry the attribute"* means **the ZBW circulation's sense is a definite carried property** (χ draft: *"a double rotation whose relative sense … is fixed by the CP's polarity"*) — compatible with emergence, and exactly what b needs. **A1′ is load-bearing and cheap; A3′ and AP-4 are structural only.** **Genuinely unevaluated:** the completeness argument (4172 §§1–3) stands on its own and owes nothing to observability; broadcast consistency may make transport follow from holding; 4120 may rest on a coupling argument not located. **THE QUESTION FOR THE FOUNDER:** does A3′'s channel stay on the structural argument alone, accepting it is unobservable on the zero-parameter commitment — or was the bundle meant to rest on something not found? **Not an argument for withdrawal** — unobservable structure is ordinary — **but for saying so explicitly in A3′'s text**, so no future window reads an empirical promise the amendment never made. *(original entry:)* Patch 4120 bundled three amendments as **"all three required together"** — A1′ (CPs carry the attribute), A3′ (LSP′ gains the channel), AP-4 (DI-bits transport it). **Against what this session established, every observable consequence traces to A1′ alone:** **b = A·V_i** — hence V−A, F3, THEO-CHIR-1 and ⟨b⟩ = (v/c)cos θ — contracts the **CP's own** A with the **V_i** register and **never reads the A_i channel** (4165); **magnetism** needed nothing new, since *SF-6 Electromagnetism Unified* was drafted **21 June 2026**, three months before the amendment, and **contains no reference to A_i or any axial channel**; and the only job left to A3′+AP-4 is the founder's **spin-evolution mixing rule**, bounded at **ε ≤ 5e−47** (4170). **NOT a proposal to withdraw A3′** — the completeness argument (4172 §§1–3) is structural and stands on its own, a channel may be needed for broadcast consistency without being measurable, and 4120 may rest on a coupling argument I have not found. **The claim is that the bundle's justification should be re-examined:** if A3′ and AP-4 are kept it should be on the structural argument, stated as such, not on an implied promise that they earn their place empirically. **Lane: EW.**

### TODO-4172-SOURCEEQ — **RESOLVED BY FORCING at Patch 4176: κ_A = 16πG/c⁴, or a new parameter**

**Answered at 4173, conditionally on 4172's torsion identification.** Dimensions: T_ij is an energy density while spin density carries **one factor of time more**, so the same coefficient cannot act on s_i directly — the natural bridge is **source_A = c(∇ × s)**, the construction magnetisation already uses to source B from M. **No new constant.** And in Einstein–Cartan **spin sources torsion with the same κ = 8πG/c⁴ that sources curvature** — torsion introduces no new coupling. **So the missing coefficient is G, which A3′ already carries, and the zero-parameter claim survives.** **CONDITIONAL:** all of this rests on the Einstein–Cartan identification, which is *Claude's proposal and is not in the corpus* — attractive precisely because it resolves four problems at once, which is when a proposal deserves more scrutiny, not less. *(original entry:)* **Resolved at 4176 by FORCING, not derivation.** 4173 §1's dimensional bridge survives 4174 (only §§2–5 fell): the energy-density source is **c(∇ × s)**. The coefficient is then forced by the corpus's own commitments with no analogy needed — A3′ has **all channels sharing transport and shell-sum**, fixes Q_ij at −16πG/c⁴ with *"zero new parameters"*, and **G/c⁴ is the only constant in the corpus with the right dimensions**. **So κ_A = 16πG/c⁴, or the amendment introduces a new parameter; there is no third option.** **Combined with 4175's fork:** on the zero-parameter branch A_i sits **1e18 below the LIGO bound**, so **on the programme's own commitments the A_i broadcast channel is unobservable** — not unmeasured but unobservable, by every route examined (4155, 4165, 4170, 4175). A theory may hold unobservable structure, but **χ₄ can never be tested through A_i's own effects**, only through what is built on it → [[TODO-4176-BUNDLE]] above. *(original entry:)* A3′ gives Q_ij a source **with a coefficient**: *"Q_ij sourced by **−(16πG/c⁴)T_ij^{TF}** (λ fixed by the scalar sector's G — zero new parameters)."* **The amendment gives A_i none.** Its text says what A_i *is* (the CP's ZBW spin) and which irrep it fills, and stops. **That is why the channel has no empirical signature** — 4155 §4, 4165 §6, 4170 §3 and 4171 §1 each found the same emptiness from a different direction and none named the cause. **A channel with no coupling constant cannot predict a magnitude, only a sign structure**, and everything this session derived about A_i *was* sign structure (THEO-CHIR-1 is a selection rule; ⟨b⟩ = (v/c)cos θ got its magnitude from kinematics). **Owed:** write A_i's source clause in A3′'s own form — A_i sourced by (coefficient)×(spin density) — with the coefficient fixed by an existing constant **or declared as a new parameter**. **If it needs a new parameter, the programme's zero-parameter claim has to absorb that cost and it should be stated as such.** **Lane: EW.**

### TODO-4172-TORSION — is A_i CPP's torsion, and do Einstein–Cartan bounds apply? (registered Patch 4172, EW/GR lane)

4172's principle — energy–momentum sources the three T_μν channels, **spin sources A_i** — is exactly **Einstein–Cartan**'s structure, where spin sources torsion independently because spin is not in T_μν. **So A_i is plausibly CPP's torsion analogue.** **Not corpus:** every occurrence of "torsion" in the corpus is *mechanical* torsion of dark-matter rods; Einstein–Cartan appears nowhere. **The bill:** Einstein–Cartan torsion effects are bounded experimentally, and if A_i is the torsion channel **those bounds apply to it directly** — which would be the first real constraint the channel has faced from outside the corpus. **Lane: EW/GR.**

### TODO-4171-PARITYPARTNERS — **ANSWERED at Patch 4172: the principle is SOURCES, not representations**

**Answered at 4172, and the principle was in A3′'s own text.** A3′ sources Q_ij by **−(16πG/c⁴)T_ij^{TF}** — so the channels are **the pieces of the matter source, decomposed**, not an abstract representation catalogue. T_μν decomposes exactly and only as **T_00 → A_g (Φ), T_0i → T₁u (V_i), T_ij^TF → H_g (Q_ij)**; the 1+3+5 is a rank-2 source decomposition, not a point-group fact. **There is no pseudoscalar in T_μν and no parity-odd rank-2, so A_u and H_u have nothing to source them** — absent on principle. **And the same principle is the amendment's real argument:** spin density is matter's *second* conserved current, an axial vector, absorbed in the Belinfante tensor and not independently represented — **A_i exists because spin is a source and had no channel**, which is a better argument than 4120 gave. **A3′ is complete over its sources.** Two items owed → [[TODO-4172-SOURCEEQ]] and [[TODO-4172-TORSION]] above. *(original entry:)* 4171 made LSP′'s parities explicit — Φ = **A_g** (l=0), V_i = **T₁u** (l=1), Q_ij = **H_g** (l=2), A_i = **T₁g** (l=1) — and asked how far 4113's argument goes. **If the defect was that a rotation-only enumeration cannot see parity, the fix is to enumerate l ≤ 2 with BOTH parities — and two partners remain missing: A_u (dim 1, a PSEUDOSCALAR broadcast) and H_u (dim 5, an ODD RANK-2).** Neither is added; neither is discussed; 4113's text addresses T₁u vs T₁g only. **So either a principle selects T₁g alone — and that principle, not "completeness", is the argument — or A3′ is still incomplete by two channels.** **And the missing two are not idle:** **A_u would give a P-odd term needing no axial vector at all, a direct RIVAL to b as the source of chirality**, arising from inside the amendment's own argument; **H_u would be a parity-odd gravitational-wave polarisation, already tightly bounded by LIGO/Virgo birefringence searches.** **Recommendation: state the selecting principle, or add both and face the H_u bound.** **Lane: EW.**

### TODO-4165-CHANNELJOB — **ANSWERED at Patch 4171: both supports are weaker than they looked** — if b does not read the A_i register, what does the register do? (registered Patch 4165, EW lane)

**Answered at 4171.** **Empirical support near-empty:** the channel's identified jobs are sourcing A₁·A₂ (ordinary magnetism the corpus already had) and the spin-evolution term bounded at **ε ≤ 5e−47** (4170) — and **b does not read the register at all** (4165/4166/4168). **Structural support sound in form, incomplete in application** → [[TODO-4171-PARITYPARTNERS]] above. **Not a refutation of χ₄**, but both its supports are now honestly sized. *(original entry:)* **Second job found at 4166.** The founder's proposal — *"the environmental axial vector and the CP axial vector mix in some proportion, and that is the axial vector carried by the CP to its next location"* — makes the register **the environmental term in a spin-evolution rule**, A_next = normalise((1−ε)A + ε·A_env). Thin, but real, and better than "magnetism only"; it also makes **spin relaxation a CPP observable with a number attached**, so the rule is falsifiable rather than merely permitted. Tight version → [[TODO-4166-MIXING]] above. *(original entry:)* 4165 decided that b contracts the **CP's own** A, not the GP's A_i register. **So what is the register for?** It still exists — A3′ broadcasts it, AP-4 transports it. On Patch 4155's coupling census it sources the **A₁·A₂ spin–spin term, i.e. ordinary magnetism**, and nothing else identified. **That is a thin job for a broadcast channel.** The amendment's structural case at 4120 — that A_i *completes* A3′ over the full point group I_h — stands; but its **dynamical** content, now that b is known not to read it, reduces to reproducing magnetism the corpus already had. **Worth asking whether the channel earns its place.** **Lane: EW.**

### TODO-4154-WHICHA — **CLOSED at Patch 4165: the CP's OWN A** — which A enters b = A·V_i, the CP's own spin or the GP's A_i register? (registered Patch 4154, EW lane)

**Decided at 4165, on the cycle's structure rather than on convenience.** The founder's 4152 ruling: the GP populates its register by summing *"the DI-bits **arriving from** GPs at a PSR distance"* — **built from arrivals, and a GP's resident CP does not send itself a DI-bit**, so the CP's own A is **not in its own GP's register**. Under the register reading, b would be built from the *neighbourhood's* axial field: computed over an unpolarised sea, the correlation with the particle's own spin is **0.011 / 0.002 / 0.002 / 0.003 at N = 12 / 60 / 600 / 6000** — zero at every N. **That reading does not weaken V−A; it abolishes it.** The DI-bit-payload reading is the same defect at N = 1. **Structural point kept:** b contracts a **CP-local attribute** with a **GP-held register** — two different objects in the cycle, which is exactly why both are present at the CP when it displaces. Cost → [[TODO-4165-CHANNELJOB]] above. *(original entry:)* They are different objects: 4120 calls A_i *"the CP's ZBW spin"* **broadcast**, so the GP's A_i register is the **summed** axial field from arrivals, while the CP carries **its own** A as an A1′ attribute. For a helicity bit the CP's **own** spin must be what is contracted — otherwise b measures the neighbourhood's average handedness rather than the particle's, and the free/confined contrast carrying F3 (4134–4135) fails. **The corpus does not say which.** This is the twin of the ambiguity 4142 raised about V, on the A side, and it was not examined. Derivable from the F3 argument's requirements, so mine under PD-008. Doc: `series_gravitation/audits/4154_register_is_a_packet.md` §3. **Lane: EW.**

### TODO-4153-REGISTERIRREP — **CLOSED at Patch 4154: one register PER IRREP (Reading 2)** — the one-register ruling and the χ₄ amendment are in tension (registered Patch 4153, EW/GR lane — FOUNDER QUESTION, PD-006(a))

**Founder ruling (`founders_voice/4154_ruling_register_is_a_packet.md`): the GP holds a PACKET** — *"a vector of scalars, tensors, and vectors"* — with the amendment adding *"a 4th vector element, and that 4th element is a vector."* So **"one vector" at 4152 meant one POLAR vector**, the one stamped on the CP. The GP holds one component per irrep of I_h: **Φ (A_g, 1) + V_i (T₁_u, 3) + Q_ij (H_g, 5) + A_i (T₁_g, 3)**. Corroborated by the glossary's own Grid Point entry, which already names **two** held registers. **Everything 4152 established is preserved** (V_i uniquely stamped on the CP; b = A·V_i well-posed; the retirement stands) **and the amendment survives intact** — exactly Patch 4113's completion argument. **Terminology pushback recorded:** the packet must NOT be called a "vector" — "a vector of vectors" would reintroduce, in the sentence that resolves it, the one-token-two-referents defect this session spent twelve patches unwinding; proposed instead **GP state packet** with **components/registers**, `V_i` reserved for the T₁_u vector alone. Residual: [[TODO-4154-WHICHA]] above. *(original entry:)* Founder (4152): *"each GP only stores **one** vector in its register."* A3′ as amended (4120): *"LSP′ gains a fourth channel **A_i**, an axial vector"* — and LSP′ is what the GP **broadcasts**, with AP-3 having the GP imprint its register on outgoing DI-bits. **To broadcast A_i the GP must hold A_i — a second stored vector.** Two ways out, not equivalent: **(1)** A_i is never GP-stored — A1′'s matched amendment says *"CPs carry the attribute"*, so the CP could imprint its own A at emission — which keeps the ruling exact but means **A_i is not an LSP′ channel** and weakens the amendment's claim to *complete* A3′ over I_h; **(2)** the GP holds one vector **per irrep** (T₁_u and T₁_g being inequivalent under the full group — exactly Patch 4113's argument), which keeps the amendment but requires restating the ruling. **Reading 2 is the likelier one, but the ruling as given says "one vector" and a qualifier should not be read into the founder's words to save the amendment.** Doc: `series_standard_model/axiom_maturation/4153_what_the_audit_settled.md` §4. **Lane: EW/GR.**

### TODO-4162-EDGEINDEX — **DONE at Patch 4163: edge index re-lettered to `k`**

**Executed at 4163.** The rule now reads **`k* = argmax_k (e_k · V_i)`**, displacement `d = l_P e_{k*}`. Footprint was **three live files** — GR-1b, c03, c01 — plus two frozen duplicates left alone under the 4147 policy; c03's passage also used `i` for the *outcome* index, re-lettered with it. **Recompiles deliberately NOT requested yet:** these three files are also in the 142-site edit pass, so holding the recompile until that lands costs one compile each instead of two. *(original entry:)* The 12-edge selection rule is written `e_i · SSV_net` (c03, GR-1b). Under the retirement that becomes **`e_i · V_i`**, where the two `i` subscripts mean different things — the **edge index** and the vector's **Cartesian component**. Unacceptable in a corpus that has just spent a session on one-token-two-referents. **Recommendation: re-letter the EDGE index** (`e_k`, `ê_n`) — far smaller footprint than re-lettering the vector, which appears at 100+ sites. Flagged in the glossary at 4162, **not executed**; the edit pass should carry it. **Lane: GR/WORKFLOW.**

### TODO-4152-UNWIND — **DONE at Patch 4162 (glossary + axiom registry); the edit pass remains** — correct the glossary and axiom registry, then run the mechanical edit pass (registered Patch 4152, GR/QM lane)

**Parts (a) and (b) DONE at Patch 4162.** `master_glossary.md`: the two entries collapsed into **one** — `V_i`, "the polar vector of the GP state packet", **one object with three uses** (GP sums arrivals → imprints on outgoing DI-bits → stamps on its CP); broadcast it is g_ti, read directly it is the displacement instruction, differentiated it sources g_ij. `SSV_disp` **retired alongside `SSV_net`**, with the note rewritten to record that **the defect was two names for one object, not one name for two**, and that `SSV_disp` was my own coinage on a mistaken premise. 17 downstream uses in the glossary and **6 in `axiom-registry.md`** rewritten to `V_i`. **New problem surfaced: [[TODO-4162-EDGEINDEX]]** above. **(c) EDIT PASS EXECUTED at Patch 4164.** **642 sites across 193 live files** (170 `.md`, 23 `.tex`) rewritten to **`V_i`** — more than the 142 `.tex` sites scoped at 4149, because that census counted only `.tex`. **Frozen and untouched, per the 4147 policy extended at 4164:** `handovers/`, `session_logs/`, `archive/`, `*/code/`, `*/scripts/`, `duplicates/`, `development/`, **`founders_voice/` (the founder's verbatim words)**, `reviews/`, **every per-patch `NNNN_*.md` reasoning document**, prereg documents, `founders_vision.md`, and `founder_registration_*`. Three passes were needed: the plain token, then `\SSV_{{\rm net},\perp}` / `**SSV**_net` / `SSV-net`, then `\mathrm{}` / `\text{}` / `$..$` wrappers. **Final real residual: 4** — two prose uses of "net" and two *filenames* under `founders_voice/`, all correct to leave. **19 papers need recompiling** (see 4164). *(original entry:)* The founder's one-register ruling overturns Patch 4142's "two objects" premise, so the artifacts built on it must be undone before any paper is touched: **(a)** `master_glossary.md` — collapse the two entries (4146) into **one**, retire **`SSV_disp`** alongside `SSV_net`, and rewrite the explanatory note (the potential/field framing is wrong in the vector channel); keep the bare-`V` prohibition unchanged. **(b)** `axiom-registry.md` — the 6 sites changed at 4147 read `SSV_disp` and should read **`V_i`**. **(c)** then the edit pass itself, now **mechanical**: `SSV_net` → `V_i` across the **142 live sites**, against the fixed inventory. **The 31 contested sites are no longer contested** — QM-1, QM-6, GR-1j, c06, SF-2, QM-4, QM-5 all take `V_i`. **Lane: GR/QM.**

### TODO-4150-REGISTERCOUNT — **ANSWERED by founder 19 Sep 2026: ONE register** — **how many vector registers does a GP hold?** (registered Patch 4150, GR/QM lane — FOUNDER QUESTION, PD-006(a); blocks QM-1 and re-opens one 4147 edit)

**Founder ruling (`founders_voice/4152_ruling_one_register.md`): ONE vector register.** *"Each GP computes one register vector, stores it, and imprints it on the DI-bits and, if present, on the CP… each GP only stores one vector in its register."* **Checked against the corpus at 4152 and consistent** — including GR-1b's *"|∇SSV_net|_ij is the spatial gradient tensor"*, which looked like a counterexample and is not: **one stored vector read two ways**, directly to direct the CP and by its gradient to source g_ij. GR-1a's *force = k∇(ΔSSV)* is the **scalar** channel, which needs a gradient because a scalar carries no direction; the vector channel does not. **This OVERTURNS Patch 4142's "two objects, one name"** — the defect was always the reverse, **two names for one object**, A3′'s rename never propagated. See [[TODO-4152-UNWIND]] above. *(original entry:)* QM-1 fuses both roles in one object: it maps `SSV_net` to the **l=1 slot** (→ `V_i`) *and* calls it the **GP-held register refreshed every Moment** (→ `SSV_disp` per Patch 4147). The whole quantum-phase identification (φ = that register's orientation, FI-QMRG-1) rests on it. **The difficulty is real:** A3′'s definitional clause says the GP *"computes, holds and per-Moment refreshes … and **imprints on every outgoing DI-bit**"* — so the held register is **what gets broadcast**, which makes 4147's classification of that clause as `SSV_disp` questionable. **Two readings:** (1) the GP holds **one** vector register, broadcast and handed to its CP — then `V_i` and `SSV_disp` are one object in two roles; (2) the GP holds **two** — the summed arrivals it imprints (potential) and the displacement it computes for its CP (field, the gradient) — which is what the audit has assumed and what GR-1a's *force = k∇(ΔSSV)* implies. Reading 1 would **not** overturn the retirement but **would** change QM-1's name and make the 4147 A3′ edit wrong. **QM-1's 20 sites and the A3′ clause wait on this; nothing edited meanwhile.** Doc: `series_gravitation/audits/4149_ssv_tex_site_inventory.md` §3. **Lane: GR/QM.**

**STAGE 3 CLASSIFICATION COMPLETE at Patch 4151 — all 162 sites dispositioned; editing blocked on one question.** **Totals: 102 [V] → `V_i`, 26 [D] → `SSV_disp`, 31 CONTESTED, 3 probable false positives.** **The pattern is clean by series:** gravitation/relativity *metric* papers use it for the broadcast vector; electromagnetism/substrate papers (SF-6, SF-8, c01, c03, c04) for the displacement field — SF-8 states it outright (*"the running vector bookkeeping of DI-bit arrivals… each Conscious Point **displaces by** d = …"*). **Two communities of usage, each internally coherent, that never reconciled.** **Scope reductions from existing policy:** 20 sites sit in `duplicates/` and `development/` folders and are **not live** under Patch 4147's frozen-provenance rule → **142 live sites, not 162**; and 3 SR-1 hits look like false positives of the permissive pattern, marked **[?] re-check**, not asserted either way. **THE CONTESTED SET IS BIGGER THAN QM-1: 31 sites across 7 files** (QM-1 20, QM-6 3, GR-1j 3, c06 2, SF-2 1, QM-4 1, QM-5 1) — **19% of all sites** — every one the *"Grid Point holds two dynamical registers"* formulation on which FI-QMRG-1's phase identification rests, and every one blocked on [[TODO-4150-REGISTERCOUNT]]. **No editing until that is answered**, since a wrong answer means re-editing seven files and re-opening the 4147 A3′ edit.

**STAGE 3 CLASSIFICATION round 2 at Patch 4150 — 45 more settled, 20 contested.** GR-1f **[V] 13/13**, GR-1c **[V] 11 / [D] 1**, SR-2 **[V] 12/12**, GR-1 **[V] 8/8**. **SR-2 is documentary proof of the rename history:** it writes *"V_i ≡ SSVnet is the vector"* and tables *"V_i = SSVnet (irrep T₁) → g_ij"* — the identity is stated in a published paper, so §1's reconstruction is confirmed outright rather than inferred. QM-1's 20 sites → [[TODO-4150-REGISTERCOUNT]] above. **Running totals: 74 of 162 classified (71 [V], 3 [D]), 20 contested, 68 unread.**

**STAGE 3 GROUNDWORK at Patch 4149 — inventory built, GR-1b classified, and the history explained.** **Census corrected AGAIN: 162 sites in 28 `.tex` files** (was 3, then 138; 138 undercounted because the grep patterns still missed `\mathbf{SSV}_{\rm net}` and comment/label forms). A **fixed, auditable inventory** of all 162 with context now lives at `series_gravitation/audits/4149_ssv_tex_site_inventory.md`, so classification proceeds against a list rather than a regex that keeps changing its answer. **GR-1b read and classified: 26 of 29 sites are the BROADCAST VECTOR → `V_i`**, only 3 are `SSV_disp`. **THE HISTORY, which reframes the audit:** GR-1b's LSP is the two-component pre-LSP′ form `(x, t, |SSV|_abs, SSV_net)` where *"the vector SSV_net sources g_ij"* — so `SSV_net` **was** the broadcast vector; **A3′ (Patch 1129) renamed it `V_i`** when LSP′ replaced the packet, and never propagated the rename; then **A1′/AP-3 (Patch 2982)** started using `SSV_net` for the GP-computed displacement register. **Two names for one thing, one name for two things.** The retirement finishes a job left half-done. **Consequence:** the gravitation series maps predominantly to `V_i`, the OPPOSITE of the glossary and axiom text — a uniform substitution would have got 26 of GR-1b's 29 sites wrong. **Remaining: 133 sites in 27 files to classify.**

**⚠ STAGE 3 HALTED at Patch 4148 — THE 4145 SURVEY UNDERCOUNTED THE PAPERS BY 9×.** 4145 reported "only 3 `.tex` files… the published papers are almost untouched." **Wrong.** The survey pattern matched the literal string `SSV_net` and missed the LaTeX macro form `\SSV_{\rm net}`, which is how the papers actually write it: **26 `.tex` files, 138 occurrences**, plus 26 more in `.md`. The GR series (GR-1, GR-1b/c/e/f/g/h/i/j), QM-1/4/5/6, SR-2, the SR companions c01/c03/c04, and SF-2/SF-6/SF-8 all carry it. **And the usage is genuinely MIXED within a single series:** GR-1 §315–319 presents it as an **LSP component** that *"sources g_ij"* (potential sense) while GR-1j §176–182 has *"CPs… displace per their GP's computed SSV_net"* and *"computes SSV_net and SSV_abs from arrivals"* (field sense). **No paper is edited and none should be until the 138 sites are read.** Revised Stage 3: read and classify all 138, then patch series by series with recompiles. (4) remaining live `.md`, heaviest series first (series_phenomena 108, series_gravitation 60, series_relativity 49). **Never:** `handovers/`, `session_logs/`, `archive/` (33 files, append-only). Classification is a **reading** job — regex was tried and discarded at 4145.

**SCOPED at Patch 4145 (survey only, nothing renamed):** **1,662 occurrences across 410 files** — 364 .md, 43 .py, **only 3 .tex** (GR-1b, GR-2, QM-1, so the published papers are nearly untouched); 33 of the 410 are append-only history (`handovers/`, `session_logs/`, `archive/`) and are **never** rewritten. **Root cause located:** `master_glossary.md` line 29 defines SSV_net as *"the vector sum of all SSV contributions at a point"* — **neutral between the two roles**, which is where every downstream ambiguity came from; that one line is the highest-value edit in the audit. **Regex role-classification was attempted and DISCARDED as untrustworthy** (file count disagreed with grep; sample contexts did not contain the token) — classification requires reading the surrounding physics, so this is a **reading job across ~377 live files, not a find-and-replace**. **Staged:** 0 naming decision (BLOCKING — tokens are the founder's call), 1 glossary, 2 axiom text + 43 verify scripts, 3 the three .tex papers with recompiles, 4 remaining live .md heaviest-series-first. Doc: `axiom_maturation/4145_ssv_audit_survey.md`. *(original entry:)* Patch 4142 established that **SSV_net names two objects**: **SSV_net^pot** (the LSP′ l=1 broadcast component V_i = g_ti — potential-like, what GPs broadcast) and **SSV_net^disp** (the vector a CP displaces by in one Moment — field-like, built from derivatives of the broadcast potentials, what the A1′ Displace clause means). Every use in the corpus should resolve to one. Touches SR-1, GR-1 and companions, `master_glossary.md`, the A1′/A3′ axiom text, and the χ₄ patch series. **Mechanical but wide; no physics moves.** 4142 deliberately did NOT rewrite existing text. **Lane: GR/WORKFLOW.**
...
### TODO-4138-TABLEPROSE — **ENACTED at Patch 4169 as bootup §0.5 D-11**
...
- **[4289: REVERTS to unmet by owned structure.** SPIN-1’s 2√2 follows from r_out = 2r_in, which SPIN-2 anchors at r_th/3 and 2r_th/3 but SPIN-1 uses at 35.27× those radii, beyond r_th, where the anchoring condition does not apply. 4090 found the ratio “does no work”, so no number moves.]
