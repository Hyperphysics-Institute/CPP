...
**Last updated:** 5 May 2026 (Session 17 Phase 5: cluster-geometry shift channels R3 (uniform $R_\alpha(N)$ shift) and R4 (cluster shape distortion) **PASS SCOPING** under all three falsifier criteria — F1 (sign) passes universally and analytically by Gaussian symmetry: $\Delta V_{\rm edge} = B_{\rm pair} \cdot [1 - \exp(-\delta r^2/(2\sigma^2))] > 0$ for any $\delta r \neq 0$, regardless of sign of shift; F2 (magnitude capacity) passes with substantial overhead — R3 max binding loss capacity $5.62$ MeV/α at $N=10$ vs empirical $\sim 1$ MeV/α scale; F3 (pattern) passes for any monotonic $\delta R(N)$ or constant $\epsilon_{\rm rms}$. **First non-rule-out outcome in five sequential phases of OPEN-SS-32 ↔ U-shape investigation.** Sign-orthogonal complement to Phase 4 (Session 16) closure: Phase 4 had $\Delta E < 0$ universally by Wick + negative Taylor coefficient on $\xi^4$; Phase 5 has $\Delta E > 0$ universally by Gaussian symmetry. Same Gaussian function generates both signs in different framings — closure of one motivates opening of the other; effectively partitions OPEN-SS-32 candidate space by sign of predicted ΔE. R3-lin calibration: $\alpha = 0.175$ fm/(N-4 unit), $\delta R(N=10) = 1.05$ fm = $44.4\%$ of $R_{\rm canon}$ as natural target $\delta R$ scale — substantially larger than $\hbar\omega^*$-derived $R_{\rm pct}$ values ($\delta R(10) = 0.54$ fm), highlights that binding-deficit signal corresponds to different $\delta R(N)$ than $\hbar\omega^*$-deficit signal. R3 and R4 advance to **multi-session derivation status**; next Priority 1 = identify $\delta R(N)$ functional form from CPP first principles (R3-Coulomb cluster repulsion; R3-Pauli internal contact blocking; R3-surface-density alternatives; R4-shape spin-orbit cluster contributions). Phase 5 does NOT claim to derive U-shape mechanism, only that R3/R4 are channel-compatible. Methodology lesson from Phase 4 reinforced: F1 sign analytical check applied first, computation only for F2/F3. Programme negative-result count UNCHANGED at 10 (Phase 5 is positive scoping). Six programme-level OPEN-SS-35 stages preserved (stage (vi) refined further: now "R3/R4 cluster-geometry shift channels under active scoping with positive F1/F2/F3 result; multi-session $\delta R(N)$ derivation from CPP physics in progress"). Earlier 5 May 2026 Session 16 Phase 4: anharmonic K$_3$ $\xi^4$ + all-orders Gaussian RULED OUT via sign theorem; **programme-level closure of Gaussian-K$_3$ framework at fixed cluster geometry**; tenth programme-level negative result. Earlier 5 May 2026 Session 15 Phase 3B-B: full $C_n$ IRREP decomposition RULED OUT via n-vs-N structural argument; **R2 FORMALLY CLOSED**; ninth programme-level negative result.
