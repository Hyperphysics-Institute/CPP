...
**Last updated:** 5 May 2026 (Session 15 Phase 3B-B: full C$_n$ IRREP decomposition with three belt-IRREP variants (B-B1 all $m \neq 0$; B-B2 $m=2$ only; B-B3 radial $m \neq 0$) RULED OUT via **n-vs-N structural argument** — empirical magnitude monotonically increasing in $N$ across J-solid range while cyclic symmetry order $n$ that drives any IRREP decomposition is non-monotonic in $N$ ($n = 3, 5, 2, 3, 4$ for $N = 5, 7, 8, 9, 10$); class-level argument extends ruling to entire family of belt-IRREP-projection mechanisms within K$_3$-Gaussian-Hessian framework including full point group with improper rotations, energy-weighted IRREP filtering, higher-$m$ harmonics. **Ninth programme-level negative result** in OPEN-SS-35 closure programme. **R2 FORMALLY CLOSED — RULED OUT**: all four plausible model-(b) realizations failed (Phase 2 uniform-only, Phase 3A all-modes, Phase 3B-A fixed-dim belt subspace, Phase 3B-B C$_n$ IRREP decomposition). Unification hypothesis at canonical $\sigma_{K3}$ falsified. U-shape mechanism investigation redirected outside K$_3$-Gaussian-Hessian framework — Session 16 Priority 1 = anharmonic K$_3$ corrections at order $\xi^4$ in Gaussian expansion (scales monotonically with edge count, single-session-tractable). Both registered candidates for OPEN-SS-35 sub-question (a) A-scaling closure (R1 Session 12 + R2 Session 15) now ruled out)
