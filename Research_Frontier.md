...
**Cross-sector connections:** Connects to the broader question of cluster-shell topology in CPP nuclear physics: which alpha-cluster geometries are actually realized by the alpha cluster physics, and which are forbidden? A non-realizable FvdW centroid-arrangement would correspond to a contradiction within the SS-9 conditional theorem at the relevant $\Nalpha$, requiring restructuring of clause (iv). The empirical match between SS-7 Table 1 and FvdW-deltahedron edge counts at $\Nalpha \in \{4, 5, 6, 7, 8, 9, 10, 12\}$ provides empirical support for C8 (twelve zero-parameter binding-energy predictions match within $\pm 1.5\%$), though not a derivation.
**Current best lead:** Route (a) facet (b) sufficiency derivation has the most natural closure path. Facet (b) was introduced in SS-7 v1.3 \S 2.1 with three candidate mechanisms (face-edge hybrid, K$_3$ delocalization, partial-overlap); identifying the actual mechanism likely requires AMD or Brink--Bloch cluster-model calculations of contact-distance distributions at degree-5 sites in observed nuclei (e.g., ${}^{28}$Si at $\Nalpha = 7$ pentagonal bipyramid). May fold into OPEN-SS-32 closure if facets (b) and (c) share Layer-3 ancestry as the same K$_3$ scale-recurrence at different geometric venues.
...
**Paper(s):** SS-9 v0.5 (Hyperphysics Institute, 6 May 2026 Session 28 v1.0 polish sub-task (d.1) ChatGPT review incorporation): \texttt{series\_strong/papers/SS-9/SS-9\_simplicial\_alpha\_polytope\_connectivity.tex} \S 2.4 (C8 paper-level hypothesis with physical motivation, NEW at v0.5), \S 2.7 (Sub-Lemma 2.3 corrections per ChatGPT Points 1, 2), Lemma B$'$ Step 3 (rework per ChatGPT Point 3), Theorem clause (iv) (rework per ChatGPT Point 4), Remark on facet (b) (reframe per ChatGPT Point 5), \S 9 (new entry for Steinitz-to-centroid realization gap CLOSED via C8 registration). The first-principles derivation will share the SS-9 paper with the OPEN-SS-24 closure (C8 registered at v0.5 polish; closure routes registered as candidates for follow-up SS-paper or v1.0+ polish).
**Registered:** 6 May 2026 Session 28 v1.0 polish sub-task (d.1) at SS-9 v0.5 ship. Surfaced by ChatGPT v0.4 review as the previously implicit Steinitz-to-centroid realization gap that escaped detection through the §9 v0.1 6-gap list AND the three sub-lemma reviews of Sessions 25/26/27.
