...
**Last updated:** 5 May 2026 (Session 14 Phase 3B-A: minimal fixed-dim belt-subspace projection RULED OUT — magnitude × 3 short at J-solid peak, **pattern-shape anti-correlated** within axial polytopes (belt fraction monotonically decreases N$_\alpha$=5→10 while empirical magnitude monotonically increases), targets (b) and (c) met by symmetry-structural-identity rather than as differential tests. **Eighth programme-level negative result** in OPEN-SS-35 closure programme. R2 reduced to one untested realization — three of four model-(b) realizations now failed (Phase 2 uniform-only, Phase 3A all-modes, Phase 3B-A fixed-dim belt subspace). Phase 3B-B (full character-theory IRREP decomposition with belt-IRREP dimension scaling) is sole untested R2 realization with sharpened constraint: must produce non-monotonic-in-belt-size pattern; small δ at N$_\alpha$=5, large at N$_\alpha$=10. N$_\alpha$=5 overshoot is structurally hardest constraint — any belt-IRREP capturing 3-vertex belt's full radial-displacement subspace overshoots empirical −12%)
