...
**Status:** OPEN — registered as candidate, pending ratification (registered 2 May 2026 Session 4 follow-up 3rd sub-arc as the deepest dependency surfaced by OPEN-SS-34 Level-1 closure)
**Sector(s):** SS (nuclear physics, Pattern-6 cross-paradigm bridge)
**Priority:** HIGH (deepest dependency in OPEN-SS-34 closure; cross-paradigm consilience claim if closed)
...
**What a solution looks like:** A first-principles derivation that shows under A1–A11 + bound-nucleon assumptions, the nucleon shell-organization recovers the magic-number sequence exactly. Most plausible closure route: (a) identify the CPP analog of spin-orbit coupling (likely a ZBW-phase-correlation effect at 600-cell coordination); (b) compute the j-shell splitting from this CPP-internal mechanism; (c) verify magic numbers emerge at the predicted shell-closure points. Alternative route: (d) bypass spin-orbit by deriving the magic numbers directly from CPP-lattice-geometry constraints on bound-nucleon configurations (e.g., 600-cell orbit-organization producing closed shells at specific nucleon counts).
**Dependencies:** Inherits from SS-1/SS-2/SS-3 (lattice geometry, nucleon structure) and SS-5 (light-nuclei alpha-cascade). Closure of OPEN-SS-35 would unlock OPEN-SS-34 from "Level-1 derived under H3" to "Level-2 derived from CPP primitives." Methodologically parallel to OPEN-SS-32 and OPEN-SS-34 in the sense of "explain an empirically-clean structure from CPP machinery"; here the empirical structure is the magic-number sequence itself, established for nearly a century in standard nuclear physics.
**Cross-sector connections:** This is the deepest cross-paradigm consilience target the programme has identified. The shell-model magic-number sequence is a load-bearing structural feature of all standard nuclear physics; deriving it from CPP primitives would be a cross-paradigm consilience claim of the same magnitude as deriving the QCD $\beta_0 = 7$ from cage geometry (PRED-C-10) but at the nuclear-shell scale. The Pattern-6 K$_3$ scale-recurrence would extend to yet another scale (nucleon-shell-organization), making the recurrence count 8+ instead of 7.
**Current best lead:** SS-1 establishes the SU(3) gauge group from tetrahedral hopping; SS-2 establishes the nucleon mass and magnetic moment from ZBW; SS-3 establishes color uniqueness. The shell-organization scale would build on these foundations. The 600-cell coordination $z = 12$ at the lattice scale and the ZBW phase structure at the nucleon scale provide candidate machinery; the open question is how nucleon configurations in CPP's lattice geometry produce shell closures at the standard magic numbers.
**Falsification route:** A direct CPP derivation that produces a *different* magic-number sequence (e.g., 30, 52, 84 instead of 28, 50, 82) would falsify both the OPEN-SS-34 derivation and CPP's cross-paradigm consistency. Empirical: the standard magic numbers are extensively confirmed in nuclear data (binding-energy peaks, separation-energy discontinuities, etc.), so a CPP derivation must produce these exactly.
**Paper(s):** SS-9 sketch `SS-9_OPEN-SS-34_derivation_attempt.md` (where OPEN-SS-35 is registered as the deepest dependency); SS-10+ candidate (closure attempt likely a separate paper given scope).
**Registered:** 2 May 2026 Session 4 follow-up 3rd sub-arc in OPEN-SS-34 derivation §8 (formal registry entry pending ratification).

---

### OPEN-SS-36: Programme-Level Closure of the $B_{\rm slip} = \sqrt{3} \cdot B_{\rm pair}$ Exact Form
**Status:** OPEN — registered as candidate, pending ratification (registered 2 May 2026 Session 4 follow-up 3rd sub-arc; numerical agreement at 1.4% to ${}^{56}$Ni calibration)
**Sector(s):** SS (nuclear physics, K$_3$ scale-recurrence)
**Priority:** MEDIUM (Pattern-6 closure refinement; promotes satellite-regime formula to fully zero-parameter)
**One-line statement:** Derive from CPP primitives (A1–A11) + refined-C1 + SS-5 K$_3$ closure-bonus mechanism the exact form $B_{\rm slip} = \sqrt{3} \cdot B_{\rm pair}$ via three-K$_3$-mode symmetric coupling at the satellite-attachment face. The candidate exact form $\sqrt{3} \cdot B_{\rm pair} = 4.056$ MeV agrees with the ${}^{56}$Ni-calibrated value of $\approx +4.0$ MeV to 1.4%, and re-running the cumulative satellite-regime fit with this form gives RMS 0.30 MeV at 0.052% relative accuracy across 9 nuclei (better than the 0.32 MeV calibrated fit).
**What a solution looks like:** A rigorous derivation that the $\sqrt{3}$ factor in $B_{\rm slip}$ comes from SU(2) coupling of three K$_3$ symmetric modes at the face where a satellite alpha attaches to the deltahedron core (three core-alphas at the corners of the shared face couple symmetrically to the satellite's K$_3$ contribution, producing the $\sqrt{3}$ enhancement). Verify the SU(2)-coupling argument rigorously and identify whether the $\sqrt{3}$ factor is exact or an approximation to a related (slightly different) value (e.g., $\sqrt{\pi^2/3}$, $\sqrt{8/3}$, or some specific Pattern-6 form).
**Dependencies:** Inherits from SS-5 (K$_3$ closure-bonus mechanism), SS-7 (alpha-alpha edge K$_3$ contact via C3), and OPEN-SS-34 Level-1 derivation. Closure of OPEN-SS-36 would promote the satellite-regime formula from "1-parameter zero-input" to "fully zero-parameter" — eliminating the only calibration in the entire alpha-chain swarm.
**Cross-sector connections:** Pattern-6 K$_3$ scale-recurrence at the alpha-cluster scale would now have a precise quantitative form ($\sqrt{3}$-coupling specifically). This may inform OPEN-SS-32 (slip-plane mechanism at $N_\alpha = 7$–$14$) which has its own provisional attenuation factors ($+0.55 B_{\rm pair}$ at J-solids, $+0.30 B_{\rm pair}$ at icosahedron). If the $\sqrt{3}$ form generalizes, the OPEN-SS-32 attenuation factors should also derive from K$_3$-mode coupling at the slip-plane geometry.
**Current best lead:** Three-K$_3$-mode symmetric coupling at a triangular-face attachment site produces an SU(2) eigenvalue of $\sqrt{3}$ for the symmetric superposition. This is standard SU(2) algebra. The challenge is mapping this generic eigenvalue to the specific structure of the deltahedron-core / satellite face-coincidence configuration. Candidate paths: (a) explicit Hamiltonian for the 4-alpha sub-cluster (3 core + 1 satellite) at the attachment face; (b) symmetric-mode analysis of the three-K$_3$-couples; (c) reduction to standard SU(2) Clebsch-Gordan to identify the $\sqrt{3}$ eigenvalue.
**Falsification route:** A rigorous derivation that produces a different exact form (e.g., $\varphi$, $7/4$, or some other Pattern-6-natural value) would falsify the $\sqrt{3}$ candidate. Empirical: better-precision binding-energy data on the 9 satellite-regime nuclei would distinguish between candidates; the current calibration uncertainty $\pm 0.2$ MeV at ${}^{56}$Ni admits multiple closed forms within $\pm 5\%$.
**Paper(s):** SS-9 sketch `SS-9_OPEN-SS-34_derivation_attempt.md` §6; SS-10+ candidate or SS-9 v1.0 if Level-2 closure is achieved before paper-text formalization.
**Registered:** 2 May 2026 Session 4 follow-up 3rd sub-arc in OPEN-SS-34 derivation §8 (formal registry entry pending ratification).
