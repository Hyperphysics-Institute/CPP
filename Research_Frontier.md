...
**Last updated:** 4 May 2026 (Session 13 Phase 3A: full-Hessian naive sum-over-all-modes RULED OUT — flat pattern across all 8 polytopes, factor 2.5 magnitude overshoot at empirical peak, no $N_\alpha=6$ selection. **Seventh programme-level negative result** in OPEN-SS-35 closure programme. But constructive: establishes upper-bound benchmark — empirical $-33.6\%$ at peak is ~40% of full-mode-space upper bound $-85\%$; mode space is rich enough, selection is the bottleneck. Phase 3 Phase B (IRREP-selective decomposition with belt-mode projection) sharply constrained as Session 14 Priority 1)
