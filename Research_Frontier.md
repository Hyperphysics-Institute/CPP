...
**Last updated:** 5 May 2026 (Session 18 Phase 6: R3-Coulomb scoping using simplified CPP charge model (point-charge alphas $+2e$ at J-solid vertices, force balance against K$_3$ Gaussian restoring) **PASSES SCOPING with 5% magnitude bullseye at $N = 10$** — $\delta R_C(10) = 1.104$ fm vs Phase 5 R3-lin target $1.052$ fm, ratio 1.05. **First quantitative agreement at the 5% level for a zero-parameter prediction in the OPEN-SS-32 ↔ U-shape thread; second positive scoping outcome (Phase 5 was first).** F1 sign passes analytically by composition (Coulomb repulsive → $\delta R > 0$; Phase 5 sign theorem → $\Delta E > 0$); F2 magnitude bullseye at $N = 10$; F3 pattern monotonic across J-solid range (5→10): 0.821, 0.940, 0.995, 1.051, 1.104 fm. Functional shape differs from R3-lin's linear assumption — Coulomb gives floor $\sim 0.78$ fm at $N = 4$ + slow growth, a meaningful physics prediction. SEMF cross-check: point-charge $V_C(0)$ matches $0.711 \cdot Z^2/A^{1/3}$ within $\sim 10\%$ across J-solid range, validating simplified CPP charge model. Sign-theorem composition workflow codified — classical-physics sign argument for candidate $\delta R$ + Phase 5 §2 sign theorem → F1 decision in one paragraph; default F1 check for any R3-channel mechanism going forward. Calculation is **zero-parameter**: simplest charge model, canonical $R_\alpha = 2.37$ fm and $\sigma_{K3} = 1.68$ fm, no Pauli/surface/spin-orbit, no parameter tuning. Pauli scoping qualitatively assessed but deferred — Pauli sign passes by composition, magnitude could overshoot if Coulomb already at target; Pauli's role at most small correction *or* Phase 5 R3-lin target underestimates empirical scale. **R3-Coulomb advances to multi-session full-derivation status.** Phase 6 does NOT yet derive U-shape mechanism; establishes natural scale correct to within 5% at $N = 10$ for simplest model. Programme negative-result count UNCHANGED at 10. Six programme-level OPEN-SS-35 stages preserved; stage (vi) refined further: now reads "R3-Coulomb under active multi-session full derivation; 5% quantitative agreement at $N = 10$ for zero-parameter calculation; refinement and full pattern-match in progress." Earlier 5 May 2026 Session 17 Phase 5: cluster-geometry shift channels R3 (uniform $R_\alpha(N)$ shift) and R4 (cluster shape distortion) PASSED SCOPING under all three falsifier criteria — F1 universal pass by Gaussian symmetry; F2 magnitude capacity well above empirical; F3 pattern monotonic; first non-rule-out outcome in five sequential phases. Earlier 5 May 2026 Session 16 Phase 4: anharmonic K$_3$ $\xi^4$ + all-orders Gaussian RULED OUT via sign theorem; programme-level closure of Gaussian-K$_3$ framework at fixed cluster geometry; tenth programme-level negative result.
