...
*Note: SF-line papers are flagship synthesis papers covering Sector Foundations questions. SF-4 is the first SF-line paper to ship at v1.0 (Session 54) and the first to reach conditional-theorem-closure SHIP-ready state at v4.3 (Session 77, four ChatGPT review cycles after v4.0 closure). The cage-shell mass formula achieves conditional theorem closure at v3.0 via THEO-SF-4-4 ($\alpha$-Exponent Reduction; resolves OPEN-FP-SF-4-1). The K3-Cage-Shell Consistency Theorem (THEO-SF-4-1) closes all three clauses at v4.0 via the Composite K3-Cage-Shell Coupling Theorem (THEO-SF-4-5; resolves OPEN-FP-SF-4-2 and SM-5 op:nu_id cross-sector — first cross-sector closure in CPP). All v4.x closures are conditional theorem closures within the current CPP theorem stack; see rem:conditional_closure in the paper for the FI-accounting framing.*
