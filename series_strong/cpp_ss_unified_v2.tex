...
Prior numerical work in \texttt{nested\_cage\_masses.ipynb}
(CPP/series\_strong, v8.0) establishes the following partial
results that any complete formula must reproduce:

\begin{enumerate}
\item \textbf{The SSV integral formula (established):}
  $M_q \sim \int_0^{r_n} S(r)\,\gamma(r)\,r^2\,dr$,
  where $S(r)$ is the SSV field energy density and
  $r_n = \phig^{n/2}r_0$ is the cage radius at depth $n$.
  This is the correct \emph{type} of formula; the difficulty
  is finding the kernel $S(r)$ that reproduces the observed
  five-orders-of-magnitude span $m_u : m_t \approx 1 : 78{,}000$.
  The kernel $S = 1/r^4$ (SSV electric density) gives
  ratios of only $1:1.2$ across four cages --- far too mild.

\item \textbf{The inner SSV mechanism for $m_u < m_d$ (established):}
  The bare up quark carries a central $+$qCP with no outer cage.
  Its ZBW orbital radius is small, where the local SSV stress is
  strongest.  This stronger stress polarises the surrounding eCPs
  outward, reducing the hDP overlap fraction to
  $\delta_{\rm up} \approx 0.95 \times \frac{1}{3}$,
  while the down quark's outer hDP structure gives
  $\delta_{\rm down} \approx \frac{1}{3}$ (unmodified).
  The direction $m_u < m_d$ follows from this geometry without
  free parameters.  The precise magnitudes $m_u = 2.2$~MeV and
  $m_d = 4.7$~MeV are not yet derived from first principles.

\item \textbf{The open kernel problem:}
  The ZBW frequency $\omega_{\rm ZBW} \propto 1/r_{\rm cage}$
  is the most promising candidate kernel, since it directly
  connects cage size to mass through $E = \hbar\omega_{\rm ZBW}$.
  If $S(r) \propto \omega_{\rm ZBW}(r) \propto 1/r$, the integral
  $\int_0^{r_n} r^{-1} r^2\,dr = r_n^2/2 \propto \phig^n$
  gives mass ratios $\propto \phig^n$, yielding the spectrum
  $1 : \phig : \phig^2 : \phig^3 : \phig^4 \approx
  1 : 1.6 : 2.6 : 4.2 : 6.9$.
  This still falls short of the observed $1 : 78{,}000$ span,
  but is closer in structure and motivates replacing the radial
  integral with a ZBW-frequency-weighted sum over cage layers.
\end{enumerate}

The complete formula for OP-SS-1 must: (a)~produce the correct
mass ratios across all six quarks from a single geometric
expression; (b)~derive $m_u < m_d$ from the inner SSV mechanism;
(c)~require no free parameters beyond sea\_strength and the
600-cell geometry.
\end{openprob}
