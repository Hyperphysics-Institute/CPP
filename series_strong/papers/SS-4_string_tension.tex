...
%   template. Numbering note: this paper is SS-4 (sequential in
%   Strong Sector) and resolves OPEN-SS-5 (Research_Frontier.md §1).
%
...
\item \textbf{OPEN-SS-5 (string tension $\sigma$ from $\sea$):}
  OPEN $\to$ \textbf{CONJ-SS-4-1} (Conjecture~\ref{conj:main}),
  subject to the open step on $z^2$ multiplicity
  (OPEN-SS-4-new-1, to be registered).
\item \textbf{SS-2 $\sigma = 243~\MeVfm$:} superseded by
  $\sigma = 926.5~\MeVfm$; SS-2's proton radius calculation
  inherits a quantitatively correct string tension (though
  $\varepsilon$ and $r_p$ will need re-derivation with the new
  $\sigma$, yielding a revised estimate that will be reported in
  the next SS-2 revision).
\item \textbf{OPEN-SS-4-new-1 (rigorous $z^2$ multiplicity):}
