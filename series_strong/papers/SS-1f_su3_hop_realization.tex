...
{\large Strong Sector Series --- SS-1f --- Version 0.2 (mechanism note, panel-revised)}}
...
\begin{itemize}
\item supply a \emph{candidate}, corpus-consistent realisation of the SU(3)
colour hop --- colour as vertex occupancy, the gluon generator as a
charge-driven $\ZBW$/$\SSV$ transition, and the non-commutativity as a
consequence of the coupling between vertices
(Sections~\ref{sec:occupancy}--\ref{sec:coupling}) --- pending the frame map of
...
\medskip
\noindent\textbf{Three open items are registered honestly} and collected in
Section~\ref{sec:status}: (i) candidate-carrier vs forcing; (ii) the
per-quark-cage / baryon-$\hDP$-tetrahedron seam; (iii) the absence of an
in-baryon test.
...
construction~\citep{ss1c}: a baryon places one quark at each of the three base
vertices, the colour index is the vertex, and the colour singlet is the totally
antisymmetric combination over the three vertices.

\begin{equation}
\text{colour of a quark} \;\equiv\; \text{the cage vertex its central }\qCP
\text{ is bonded to.}
\end{equation}
...
\section{The unreconciled frames (registered)}
\label{sec:seam}

Two geometric frames for ``the three colour vertices'' coexist in the corpus and
are not yet reconciled; the reconciliation is the open core of
\emph{op:strong\_primitive} on the mechanism side.
\begin{itemize}
\item \textbf{SS-1b per-quark colour cage.} Each quark sits in a tetrahedral
cage whose apex $V_4$ holds that quark's central $\qCP$ and whose three base
vertices are that quark's three colour states $\{r,g,b\}$. Colour is internal to
one quark; the hop moves a single quark's colour amplitude among its own base.
\item \textbf{Baryon $\hDP$-tetrahedron (founder's account).} The baryon core is
a hybrid tetrahedron built from type-A and type-B $\hDP$s; three quarks occupy
three of its four vertices and the fourth vertex is an open $\eCP$; up and down
quarks are themselves cageless ($\qCP$ core $+$ radial $\ZBW$ $\eCP$ $+$ cloud).
Colour is which baryon vertex a quark is bonded to; the hop moves a quark
between baryon vertices.
\end{itemize}
Both place colour on three vertices and both yield the SS-1b algebra by the
three-ness; for a colour-singlet baryon (one quark per colour) the two
descriptions may coincide, since ``which colour'' and ``which vertex'' carry the
same information. Establishing that coincidence --- or the precise map between
the frames --- is required before the realisation can be said to be derived
rather than pictured. The sub-problem to be answered is explicit: \emph{what
does the hop operator $E_{ij}$ physically act on?} --- a quark's internal colour
coordinate (per-quark cage), the quark's baryon-vertex occupancy (baryon
$\hDP$-tetrahedron), or a quotient identifying the two for a colour singlet. The
two aspects must be kept separate. As a \emph{labelling}, ``which colour'' and
``which vertex'' may coincide for a colour-singlet baryon (one quark per
colour), and this is the only aspect on which the algebra-level consistency of
this note depends. As a \emph{dynamics}, the two frames carry different physical
distance and charge-distribution scales --- a per-quark cage and a baryon-scale
tetrahedron are not the same geometry --- so the $\SSV$-gradient mechanism of
Section~\ref{sec:coupling} acquires definite physical content only once the
frame, and hence the geometry, is fixed. This note relies only on the labelling
coincidence; the dynamical (geometric) fixing is part of the open forcing
problem (Section~\ref{sec:status}). We register the
operator-domain question as the principal open sub-problem.
...
\begin{openprob}[Frame reconciliation]
Establish the explicit map between the SS-1b per-quark colour cage and the
baryon $\hDP$-tetrahedron of Section~\ref{sec:seam}, or show they coincide for a
colour-singlet baryon. \textbf{Open.}
\end{openprob}
...
prior SS-1b/SS-1c/SS-3 work. It is issued at v0.1 as a mechanism note for
review; shared-registry registration is deferred to ship-time integration.
