...
{\large Strong Sector Series --- SS-1f --- Version 0.1 (mechanism note)}}
...
\begin{itemize}
\item supply a physical, corpus-consistent realisation of the SU(3) colour
hop: colour as vertex occupancy, the gluon generator as a charge-driven
$\ZBW$/$\SSV$ transition, and the non-commutativity as a consequence of the
coupling between vertices
(Sections~\ref{sec:occupancy}--\ref{sec:coupling});
...
\begin{proposition}[Uncoupled modes form a torus]
\label{prop:torus}
Let the eight degrees of freedom be taken as uncoupled single-bond oscillation
amplitudes (number-like operators, each diagonal in the occupancy basis,
commuting pairwise). Then they generate the abelian algebra
$\mathfrak{u}(1)^{\oplus 8}$, not $\mathfrak{su}(3)$. The $\mathfrak{su}(3)$
structure constants of Equation~\eqref{eq:su3} appear if and only if the
generators are realised as inter-vertex transposition (hop) operators on
$\mathcal{H}_{\rm colour}=\mathbb{C}^3$.
\end{proposition}
...
rather than pictured. We register it as the principal open sub-problem.
