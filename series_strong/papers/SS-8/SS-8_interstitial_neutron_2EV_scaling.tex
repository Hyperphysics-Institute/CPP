...
%
% Version 0.1 — 23 April 2026 (initial draft)
%
% FILENAME NOTE: This paper was initially written to disk as
%   SS-8_paper.tex and renamed to SS-8_interstitial_neutron_2EV_scaling.tex
%   on 24 April 2026 via `git mv` to conform to the CPP filename convention
%   [series]-N_descriptive_snake_case.tex (codified in
%   templates/paper-formatting.md §11, added in the same patch as the rename).
%   Git history preserves the rename; LaTeX compilation is unaffected.
%
% CHANGELOG:
...
\date{23 April 2026 \\[2pt] \small Version 0.1 (initial draft)}
...
\begin{enumerate}
  ...
  \item Level-3 physical-principle independence for D1. Both sufficient premises of Theorem~\ref{thm:d1-cond} share a proximity-binding ancestor principle; a D1 derivation from a non-proximity mechanism (topological, entropic, or geometric-phase) would be needed and is flagged as a candidate programme-level problem in \S\ref{sec:level-3-gap}. Registration as a cross-series OPEN is deferred to a dedicated programme-level review and is not part of v0.1's scope.
...
\begin{itemize}
  \item \textbf{OPEN-SS-23} (inherited from SS-7 v1.2): neutron-excess binding beyond the strict $N = Z$ alpha-chain. \emph{Partially addressed.} SS-8 v0.1 covers the interstitial regime $\Nex \in \{0, \ldots, 8\}$ on-chain; full OPEN-SS-23 scope (off-chain and dripline-asymptotic regimes) remains open.
...
\paragraph{Status.} D3 is a paper-level structural hypothesis at a tier analogous to SS-5's uniform nucleon-distribution assumption: supported empirically by the \S\ref{sec:numerical-predictions} fit, but not derived from first principles. First-principles derivation of D3 plus a proof that the observed residuals decompose exactly as H3$'$ (pairing) $+$ H5$'$ (small-polytope) $+$ H4$'$ (Pauli) without hidden absorption is the content of OPEN-SS-28 (\S\ref{sec:open-ss-28}).
...
\begin{table}[h]
\centering
\caption{Empirical $k_{\text{eff}}$ vs.\ Theorem~\ref{thm:h2prime} prediction for the strict $N = Z$ alpha-chain at $\Nex = 2$, from the Phase 1b empirical map. Data source: AME~2020 binding energies. Predicted values computed with $\Bpair = 2.342$~MeV from equation~\eqref{eq:Bpair}; no parameters fitted.}
\label{tab:k-eff}
\begin{tabular}{@{}rrrrr@{}}
\toprule
...
\textbf{(AUTHOR NOTE: this subsection requires Thomas's eye before Round 1 submission. The approach here mirrors SS-7 v1.2 §6.5 but the specific alternative geometries to test for SS-8 need your judgment. Draft content follows for structural completeness.)}
...
\subsection{Residual decomposition: provisional H3$'$ transport of the opposite-polarity pair bonus}
\label{sec:residual-decomposition}

The mean residual across the bulk regime ($\Nalpha \in \{6, \ldots, 14\}$) is $+0.38$ in units of $k_{\text{eff}}$, or $+0.9$~MeV per interstitial neutron. This residual is \emph{predicted, not absorbed}, and decomposes into three separately registered mechanisms:
...
The mediated coupling is expected to be weaker than the direct SS-5 pair bonus by a geometric factor reflecting the inter-vertex separation. A natural candidate is the $1/\phig^2$ factor, which has two independent motivations from the SS-5 machinery:
...
H3$'$ is \emph{provisional}, not derived. The specific attenuation $1/\phig^2$ in equation~\eqref{eq:eps-pair-refined} is motivated by the Pattern 6 geometric recurrence and by the numerical coincidence with SS-5's Pauli-penalty ratio, but not rigorously established; alternative attenuations within a factor of 2 of equation~\eqref{eq:eps-pair-refined} (e.g., $1/\phig$ giving $0.618\Bpair$, or $1/\phig^{3/2}$ giving $0.486\Bpair$) would also fit the empirical pair-bonus magnitude within a factor of 2. A first-principles derivation of the pair-bonus magnitude at the interstitial scale --- which would fix the attenuation factor and distinguish H3$'$ from its plausible competitors --- is content of OPEN-SS-28.
...
\begin{table}[h]
\centering
\caption{Extension residuals $(k_{\text{eff}}^{\text{obs}} - k_{\text{eff}}^{\text{pred-H4'}})/k_{\text{eff}}^{\text{pred-H4'}}$ as fraction of prediction. \textbf{Placeholder values pending local re-run of \texttt{ss8\_empirical\_map\_extended.py}} (v0.1 sandbox did not contain AME~2020 data file; entries below are the author's best read from Phase~1b empirical-map sketches and are to be regenerated before v0.2). Parenthesized rows indicate polytope-identity-ambiguity points where multiple simplicial deltahedra could realize the observed $\Nalpha$; residuals in those rows carry additional uncertainty attributable to OPEN-SS-24.}
\label{tab:ext-nex}
...
\textbf{(AUTHOR NOTE: this section is a v0.1 draft awaiting Thomas's review. It attempts to render the interstitial-neutron mechanism in CPP-native language --- CPs, DPs, chains, cages, SSV fields, DI-bit propagation. The founders-voice content of this section is load-bearing for the paper's accessibility to CPP-internal readers; revision with Thomas's direct input is expected before Round 1 submission.)}
...
\textbf{(AUTHOR NOTE: this section is a v0.1 draft awaiting Thomas's review. The structural correspondences below are first-pass; Thomas's judgment on the right granularity and whether particular mappings are too literal or not literal enough is needed.)}
...
$\Bpair = \Mzero/\phig$ from SS-5 \Kthree\ eigenvalue & Pairing-interaction strength $\sim 12/\sqrt{A}$~MeV (roughly matches at $A \sim 30$) \\
Pattern 6 scale recurrence (nucleon-nucleon, alpha-alpha, interstitial-alpha) & --- (no conventional analog; CPP-distinctive) \\
...
\subsection{What SS-8 v0.1 covers}
\label{sec:coverage}
...
\subsection{What SS-8 v0.1 defers: OPEN-SS-23 (inherited from SS-7)}
...
A first-principles derivation of D2 (\Kthree-edge coupling at interstitial-alpha contact scale) via extension of A6$'$ from the 600-cell cage scale to the nucleon-interstitial scale. Closure would automatically deliver the residual D1 content as a corollary (via the D1--D2 coupling of \S\ref{sec:d1-d2-coupling}). Target paper: SS-8 v0.x if tractable, or a dedicated A6$'$-extension paper otherwise.
...
\textbf{(AUTHOR NOTE: this subsection requires Thomas's review. The specific predictions below are drafted as structural examples; their precise framing and the priority ordering are founders-voice decisions.)}
...
\textbf{(AUTHOR NOTE: see also \S\ref{sec:hostile-geometry}. Thomas's review should determine whether to consolidate both discussions or keep them separated.)}
...
Axiom count: 9 (unchanged).\quad Conditional theorems added by this paper: 3.\quad Zero-parameter predictions added: 42 (12 primary + 30 secondary).
