% ============================================================
% SS-1: The Strong Sector from the 600-Cell Lattice
...
% ==================================================================
% SS-1: The Strong Sector from the 600-Cell Lattice
% 600-Cell Standard Model Emergence Series — Version 3
% Claude v1 series (SS #1–5) + Grok v1 merge + v3 harmonization
% GitHub: CPP/series_strong/cpp_ss_unified_v3.tex
% Monte Carlo: CPP/series_strong/mc_su3_algebra.py
%
% Grok v1 contributions (merged in v2):
...
%
% v3 harmonization (29 March 2026):
%   H1: Title to series standard (SS-1 prefix, \and, \date block)
%   H2: Keywords added
%   H3: Table 1 — linear ZBW DP explicit for all down-type quarks
%   H4: CPP primitives — tetrahedral cage model-postulate clarification
%   H5: W bracelet locally-linear coupling face added to masslessness remark
%   H6: sea_strength language softened: "derived to within 3.8%"
%   H7: \raggedright after \end{abstract}

\medskip\noindent
\textbf{Keywords:} SU(3) colour algebra, strong force, 600-cell lattice, gluon masslessness, asymptotic freedom, beta function, tetrahedral cage, Gell-Mann matrices, quark confinement

\bigskip\noindent
\textbf{Plain Language Summary:}
The strong nuclear force, which binds quarks inside protons and neutrons, is governed by the SU(3) colour symmetry. In CPP, this symmetry emerges exactly from the tetrahedral cage faces of the 600-cell lattice. Each triangular face has three vertices that map to three colour charges, and the face permutations generate the eight Gell-Mann matrices of SU(3). Gluon masslessness follows from the topological closure of cage faces, and asymptotic freedom follows from the lattice mode counting. Nine theorems are proved, all verified numerically.
...
\usepackage{graphicx}
% \usepackage{svg}  % no SVG figures in this document
...
{\normalsize Version 2, 29 March 2026}}
...
\noindent\textbf{Keywords:} Conscious Point Physics, 600-cell lattice,
SU(3) colour symmetry, strong interaction, quark confinement, asymptotic
freedom, gluon masslessness, Gell-Mann matrices, tetrahedral cage,
Voronoi geometry, Standard Model emergence, Koide formula
...
\item \textbf{Tetrahedral cage:} For quarks more massive than $u$ and $d$,
  the qCP sits at the apex $V_4$ of a regular tetrahedron embedded in
  the 600-cell.  Base vertices $\{V_1, V_2, V_3\}$ encode the three
  color states.  \textit{The assignment of qCPs to tetrahedral cage
  structures is a CPP model postulate: quarks are identified with
  qCP-plus-cage configurations, not derived from geometry alone.
  What geometry derives is the SU(3) algebra and its consequences,
  given this identification.}  The $u$ and $d$ quarks have no
  polyhedral cage (see Table~\ref{tab:quarks}); the cage is additional
  mass-generating structure first appearing at the strange quark.
...
\emph{derived to within 3.8\%} from 600-cell Voronoi geometry (see
Section~\ref{sec:alpha_geom} and Theorem~\ref{thm:sea_strength}).
The residual 3.8\% is identified as the stereographic $S^3\to\mathbb{R}^3$
projection correction; its full analytic derivation is deferred to
the Stiffness~C companion.
...
\caption{Six quark flavours: cage structure and masses.  The mass
  ...
  All down-type quarks ($d$, $s$, $b$) carry a linear ZBW DP
  in addition to the inner orbital ZBW; this is the structural
  origin of the additional $+1/3$ charge screening that brings
  all three to charge $-1/3$.  Up-type quarks ($u$, $c$, $t$)
  carry only the inner orbital ZBW (no linear ZBW DP) and
  reach charge $+2/3$ via a single $1/3$ screening.}
\label{tab:quarks}
\begin{tabular}{@{}lccp{5.2cm}cc@{}}
\toprule
Flavour & $Q$ & Layers & Cage structure & $m_q^{\rm curr}$ & $M_q^{\rm const}$ \\
\midrule
$u$ & $+2/3$ & 0 & Bare $+$qCP; inner orbital ZBW (2:1 freq.\ ratio);
    qDP cloud to thermal zone; no polyhedral cage
  & $2.2$~MeV & $336$~MeV \\
$d$ & $-1/3$ & 0 & $-$qCP; inner orbital ZBW; \textbf{linear ZBW DP};
    no polyhedral cage
  & $4.8$~MeV & $340$~MeV \\
$s$ & $-1/3$ & 1 & $-$qCP; inner orbital ZBW; \textbf{linear ZBW DP};
    tetrahedral cage
  & $93.5$~MeV & $486$~MeV \\
$c$ & $+2/3$ & 2 & $+$qCP; inner orbital ZBW;
    tetrahedral + icosahedral cages
  & $1.27$~GeV & $1.55$~GeV \\
$b$ & $-1/3$ & 3 & $-$qCP; inner orbital ZBW; \textbf{linear ZBW DP};
    tetrahedral + icosahedral + dodecahedral cages
  & $4.18$~GeV & $4.73$~GeV \\
$t$ & $+2/3$ & 4 & $+$qCP; inner orbital ZBW;
    all four cages; fourth = 30-vertex shell ($d^2$=2, candidate)
...
$[T^a,T^b]=if^{abc}T^c$).  All massive bosons are closed structures:
the W (bracelet hDP chain), Z (icosahedral cage), H (dodecahedral
cage).  Mass arises from topological closure; masslessness from
topological openness.

The W bracelet is a closed ring and therefore massive, but it
presents a \emph{locally linear coupling face} to the qCP during
flavour transitions: at the point of contact, the relevant
interaction geometry is a linear segment of the ring rather than
the full closed structure.  This directional asymmetry is the
geometric origin of the W's polarity-inverting coupling ---
the bracelet's linear segment couples to the central $\pm$qCP and
exchanges polarity (up-type $\leftrightarrow$ down-type),
acting as a catalyst in the charge-exchange transformation.
The Z's full icosahedral symmetry, by contrast, engages the cage
symmetrically and does not invert qCP polarity.
\end{remark}
...
sea\_strength $= 0.185$
  & $0.1781$ (derived to $3.8\%$) & --- & \textbf{Derived to 3.8\%} (\S\ref{sec:alpha_geom}) \\
$\as(M_Z) \approx 0.118$
...
\emph{derived to within 3.8\%} from the 600-cell Voronoi stiffness
integral (Theorem~\ref{thm:sea_strength}):
...
contains \textbf{effectively zero free parameters}: $\phig$ is fixed by
600-cell geometry; sea\_strength is derived to within 3.8\% from the same
Voronoi integral $\alpha_{\rm geom}$ that appears in the CPP
electromagnetic stiffness and SR coupling constants.  The remaining
3.8\% gap has a known single geometric origin and will be closed
analytically in the Stiffness~C companion paper.
...
\bibliography{cpp_strong_series}
