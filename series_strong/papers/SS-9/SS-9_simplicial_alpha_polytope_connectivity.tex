...
%
% Version 0.4 -- 6 May 2026 (Session 27 OPEN-ORG-012-followup
...
\[
\mathrm{Conf}(\Nalpha) \;=\; \big\{(c_1, R_1, \ldots, c_{\Nalpha}, R_{\Nalpha}) \in (\mathbb{R}^3 \times \mathrm{SO}(3))^{\Nalpha} \,:\, \text{rigid packing; } |E(\mathcal{C})| \geq \Nalpha - 1\big\} \big/ \mathrm{SE}(3),
\]
...
$G$-connectedness requires the contact graph to be connected, equivalently $|E(\mathcal{C})| \geq \Nalpha - 1$ (at least a spanning tree). For $\mathcal{C}$ connected through $G$, any two centroids $c_i, c_j$ are linked by a path in $G$ of length at most $\Nalpha - 1$, with each edge contributing centroid-distance $\Raa$. Hence $\mathrm{diam}(\{c_i\}) \leq (\Nalpha - 1) \Raa$. Modulo $\mathrm{SE}(3)$ (translation $+$ rotation), the centroids fit inside a closed ball $\overline{B(0, (\Nalpha-1)\Raa)} \subset \mathbb{R}^3$ after centering on $c_1 = 0$.
...
The supremum is finite: $|E| \leq 3\Nalpha - 6$ by Euler's formula on $S^2$ for any planar realization (a generic upper bound, possibly not achieved for non-FvdW $\Nalpha$), so $B(\mathcal{C}) \leq \Nalpha \Balpha + (3\Nalpha - 6) \Bpair < \infty$.
...
A simple linear chain of $\Nalpha$ alphas with $\Nalpha - 1$ consecutive face-to-face contacts is physically realizable (rigid packing satisfied; $G$-connected via the chain). Call this $\mathcal{C}_{\mathrm{chain}}$. Then $B(\mathcal{C}_{\mathrm{chain}}) = \Nalpha \Balpha + (\Nalpha - 1) \Bpair$, so $\sup B \geq \Nalpha \Balpha + (\Nalpha - 1) \Bpair$. Hence $B(\mathcal{C}^*) \geq \Nalpha \Balpha + (\Nalpha - 1) \Bpair$, which forces $|E(\mathcal{C}^*)| \geq \Nalpha - 1$ (since $\Bpair > 0$). Therefore $\mathcal{C}^*$ is $G$-connected, i.e., $\mathcal{C}^* \in \mathrm{Conf}(\Nalpha)$.
