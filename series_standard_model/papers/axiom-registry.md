...
| **A7** | Face-Bond Circulation | Tensioning any one qDP bond in a triangular K₃ loop generates a displacement pulse that propagates through shared vertices into the other two bonds, forming a literal circulating pulse. The 3-bond closed loop supports 8 independent standing-wave patterns (3 diagonal bond-tension asymmetries + 4 off-diagonal oscillation modes + 1 symmetric breathing mode). Because each vertex response depends on the current SSV_abs (modified by preceding pulses), successive displacements are order-dependent: U(γ₁)U(γ₂) ≠ U(γ₂)U(γ₁). This realises an effective non-Abelian (SU(3)) gauge structure. Energy remains confined in the loop until transferred or the triangle breaks. | SM-7 | SM-7; SS-1 |
...
*Face-Bond Circulation (Non-Abelianity):* A triangular K₃ face has 3 CPs connected by 3 qDP bonds. When any bond is tensioned (by ZBW oscillation, SSV_abs shift, partner switching, or collision), the displacement propagates through the shared vertices into the other two bonds. Because the bonds form a closed loop, the displacement pulse literally circulates: AB → BC → CA → AB. The 8 independent standing-wave patterns on this 3-bond ring (3 diagonal DC asymmetries + 4 off-diagonal AC oscillations + 1 symmetric breathing mode) map exactly onto the 8 Gell-Mann generators of SU(3). The circulation is non-Abelian because each vertex's response depends on its current SSV_abs, which was just modified by the preceding pulse — so the order of displacements matters. The energy remains trapped in the closed loop until the pattern is transferred to an adjacent face or the triangle breaks (confinement → hadronisation).
