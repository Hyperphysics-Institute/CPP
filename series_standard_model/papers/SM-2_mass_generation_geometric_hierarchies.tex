...
topologically from C3 cage symmetry in SM-1~\cite{abshier_sm1},
superseding the approximate golden-ratio derivation
($1/\phi^2 \approx 0.382$) used in Appendices~G and~H here.
\textbf{The Koide ratio} $K = 2/3$ for charged leptons is derived from
the K3 spectral theorem in SM-3~\cite{abshier_sm3}.
...
(PS-1~\cite{ps1}) confirms that no 60-vertex distance shell exists, and
the 30-vertex shell at $d^2 = 2$ (degree-4, vertex-transitive) is the
leading candidate.
...
(cage type, vertex count, ZBW mode, DP composition) and iterative
refinement.  The effective occupancy parameters ($N_k$) that drive
the mass hierarchy are motivated by 600-cell cage geometry but have
not been derived from first principles; this is the primary open
problem addressed in OP-SS-1.
...
This paper was written before SM-1, SM-3, SM-4, SM-5, and SS-1 were
completed. Several claims have been superseded.
...
--- remains the physical picture underlying the later, more rigorous
derivations.
...
SM-1~\cite{abshier_sm1}, SM-3~\cite{abshier_sm3}, SM-4~\cite{abshier_sm4},
SM-5~\cite{abshier_sm5}, and SS-1~\cite{abshier_ss1} (all 2026) provide
rigorous derivations that supersede several results here:

\begin{itemize}
\item \textbf{SM-1} proves $\delta = 1/3$ exactly from C3 symmetry
...
\item \textbf{SM-3} derives the Koide ratio $K = 2/3$ for charged
...
\item \textbf{SS-1~\cite{abshier_ss1} / PS-1~\cite{ps1}} establishes
  that the C$_{60}$ fullerene (60 vertices) does not appear as a
  distance shell of the 600-cell.  The fourth cage candidate is now
  the 30-vertex shell at $d^2 = 2$ (degree-4, vertex-transitive, all
  30 vertices equidistant from the reference vertex).  All references
  to C$_{60}$ in this paper should be read as referring to this
...
\item Linear $-$eCP ZBW extra for down-type quarks ($d=1$,
...
has been falsified by exact 600-cell shell computation (PS-1,
2026~\cite{ps1}).  No 60-vertex distance shell exists in the 600-cell.
...
\item Down: Central $+$qCP, linearly oscillating $-$eCP extra, orbital eDP, polarised CP cloud ($N_k = 2.5$ eff.\ occupancy). \emph{(Composition corrected per FORK-DM-COMPOSITION-1, founder ruling 12 Sep 2026, extended to all three down-type quarks 13 Sep 2026; the earlier entries' central $-$qCP with a neutral extra DP is inconsistent with the charge arithmetic of \S``Charge Quantisation''. The $-1/3$ charge is cage-independent, so no cage-size argument distinguishes $d$, $s$ and $b$ --- Patches 0942, 0943.)}
...
\item $W^0$: neutral 12-member ring --- three qDPs and three eDPs alternating
  (qDP--eDP--qDP--eDP--qDP--eDP), one per vertex of the Petrie hexagon of the
  first-shell icosahedron (the W-bracelet, SF-2 v1.0 Thm~4.2); the alternating
  order is a structural assignment pending an observable, on the same footing as
  the $N_k$ values (Patch 0948); $W^\pm$: a $W^0$ carrying a $\pm$eCP on the
  enzymatic structure, the charge residing on the unpaired CP; Z: icosahedral
  cage; Higgs: dodecahedral cage. \emph{(Corrected in topology and species, not
  member count: the former ``linear hDP chain'' and the $W^0$ ring are both 12
  CPs. The ring is the Weak Sector lane's W-bracelet, so the correction brings
  SM-2 into agreement with SF-2 rather than away from it; the $W^\pm$
  construction is the partnerless-third structure of Patch 3513. The order must
  be achiral: with $\chi \ne 0$ the lattice reflections are not symmetries of the
  physical substrate, and a chiral order would split into two inequivalent
  neutral ring states against the Standard Model's single $W^3$ --- Patches
  0944, 0945, 0947, 0948.)}
\end{itemize}
...
\emph{W row amended, Patch 4002 (EW lane, \texttt{OPEN-EW-7}).}  The cage
hypothesis is corrected from ``Linear 6-hDP chain'' to the $W^0$ ring per
Patches 0944--0948, and two cells move with it: $E_{\text{inter}}$ from
$0$ to $8038$ and the residual from $22774$ to $14736$.  \textbf{The
$80380$~MeV total is unchanged, and no other row is touched.}  Reason:
across all twelve rows of this table $E_{\text{inter}}$ is either exactly
$0.10$ of the total or exactly zero, and the switch is set, $12/12$, by
whether the object has a closed polyhedral cage --- it is zero for the
electron, the up, the down and (formerly) the linear-chain W, and $0.10$
for every tetrahedral, icosahedral and dodecahedral object.  The ring
sits on the first-shell icosahedron (the Z's own cage; SF-2 v1.0
Thm~4.2), so the correction moves the W across that switch.  The W row is
now term-for-term identical to the Z and Higgs rows.  \emph{The switch
rule is read off this table's own consistency, not stated in prose
elsewhere in this paper; verify script
\texttt{series\_standard\_model/code/4002\_ew7\_w\_breakdown.py}.}
...
W & $W^0$ ring: 3 qDP $+$ 3 eDP on the Petrie hexagon of the first-shell icosahedron & 40190 & 13397 & 8038 & 4019 & 0.0 & 14736 & 80380 \\
...
%=====================================================================
\section{Charge Quantisation: Relationship to SM-1}
%=====================================================================
...
has a 14.6\% error and is \textbf{superseded} by Theorem~1 of
SM-1~\cite{abshier_sm1}, which gives $\delta = 1/3$ exactly from a
...
(SM-3~\cite{abshier_sm3}, SM-4~\cite{abshier_sm4}, and
SS-1~\cite{abshier_ss1}) identifies the ZBW thermal mechanism as the
likely correct approach for heavy quarks: for $(c, b, t)$,
$K(c,b,t) = 2/3$ to $0.42\%$, consistent with the K3 spectral theorem.
Light quark masses ($u, d, s$) require the non-perturbative ZBW
Schrödinger equation in $V(r) = -\text{sea} \times \hbar c/r$
...
\section{Charge Quantisation (Approximate; Superseded by SM-1)}
...
\textbf{The exact result} $\delta = 1/3$ is proved in SM-1~\cite{abshier_sm1}
(Theorem~1) from C3 symmetry and cage completeness, without reference
to $\phi$.  All quantitative charge calculations in the CPP series
...
The exact statement is that $\delta = 1/3$ exactly
(SM-1~\cite{abshier_sm1}), and $1/\phi^2$ is an approximation to this.

\section{Quark Charge Asymmetry and Capotauro}

Down-type quarks carry a linear $-$eCP ZBW extra that
...
stabilises linear ZBW extras on \textbf{positive} ($+$qCP) centres.
\emph{(Sign corrected: the earlier statement is the anti-quark's, identified at Patch 3524. The bias
operator is now derived rather than asserted --- $\hat{C}^{qDP} = \chi\,(1/6)\,\hat{s}$, coupling to
the sign of the central charge, Finding C-W46 as corrected at Patch 0937; the substrate's handedness
supplies no preference among first-shell placements, only among centre signs.)}
...
The exact charge values follow from $\delta = 1/3$
(SM-1~\cite{abshier_sm1}, Theorem~1).
...
(SM-1~\cite{abshier_sm1}).  The $\phi$-based expressions are retained
as geometric motivation but carry ${\sim}15\%$ systematic error relative
to the exact topological result.
...
\item \textbf{Charge quantisation:} $\delta = 1/3$ exactly
  (SM-1~\cite{abshier_sm1}, Theorem~1); $+2/3$ and $-1/3$ quark
  charges follow exactly.
\item \textbf{Koide ratio:} $K = 2/3$ exactly
  (SM-3~\cite{abshier_sm3}, K3 spectral theorem).
...
\item \textbf{Cage}: Stable cluster of CPs surrounding the central CP.
  ...
  been ruled out as a 600-cell distance shell by PS-1~\cite{ps1}).
...
\item \textbf{Up quark}: Central $+$qCP, bare; $+2/3$ charge from
  $\delta = 1/3$ (exact, SM-1~\cite{abshier_sm1}).
...
\item \textbf{Neutrinos}: Unbound ZBW with $\sigma = 120^{-3}$ suppression;
  nearly massless.
\item \textbf{W boson}: $W^0$ neutral 12-member ring of three qDPs and three
  eDPs alternating on the Petrie hexagon of the first-shell icosahedron, with
  $W^\pm$ carrying a $\pm$eCP on the enzymatic structure (Patches 0944--0948);
  Z: icosahedral cage;
...
\bibitem{abshier_sm5}
T.~L.~Abshier, Grok (xAI), Claude Sonnet (Anthropic),
Tribimaximal Neutrino Mixing from the K3 Cage Base Graph (SM-5),
\textit{600-Cell Standard Model Emergence Series}, 2026.\\
\url{https://github.com/Hyperphysics-Institute/CPP/blob/main/series_standard_model/papers/SM-5_tribimaximal_neutrino_mixing_from_k3.tex}
