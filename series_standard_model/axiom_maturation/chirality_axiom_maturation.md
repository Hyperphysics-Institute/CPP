...
- **4072** — document opened; 2a (n̂ vs −n̂ is not a chirality in ℝ⁴) and 2b (a double rotation carries a
  ...
  readiness criteria and falsifiers written.| **F2** | **HOLDS under DP-CAL-1 (4107).** Founder ruling: no CP spin has ever been assigned; he assumes antiparallel and directs a calibration turn. **One axis of the 4106 table was never free:** SF-6 derives EM from *eDP-Sea Polarization*, polarizing a dipole displaces its +CP and −CP in OPPOSITE directions by definition, and 4097 makes the arc cohort the product of that displacement ⇒ **arcs opposed, forced**. So F2 holds iff spins are antiparallel — the assumed value. Under DP-CAL-1: b₊ = b₋ and R = q(b₊−b₋) = 0 **exactly and pointwise** (100,000 random (ω,v), max = 0; mismatched control returns 2). Empirics sweep: 4 SUPPORT (bosonic sea; **no ferromagnetic vacuum**; R-F3 wants the same principle; ), 1 no-conflict, 1 silent — no contradiction. **Calibration, not derivation** — the spin half is the founder's assumption carried as first evidence. Doc: `axiom_maturation/4107_dp_cal1_calibration_turn.md`. |
