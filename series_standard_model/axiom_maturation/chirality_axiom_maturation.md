...
| **F5** | **OPEN — and NOT independent (4102).** χ₄ conserves CP exactly (§2s), so F5 needs a second source: an O(1) phase near 65.5°. **Patch 4102 premise correction:** that phase does not come from any "SF-2 generation-transition structure" (no such thing — SF-2 is explicitly generation-BLIND by load-bearing Proposition, and takes CKM as INPUT). SF-2's actual route is δ_CP^(CKM) = arg(Capotauro phase factor) ≈ 65°, self-labelled "Conjectural (Phase 7 OPTIONAL)" and never computed anywhere in the corpus. **F5 therefore reduces to the Capotauro/H1 blocker**, which the June 2026 panel adjudicated RESTATEMENT-NEEDED / long-horizon 2028+ (Patch 1202, 3/3 no-derivation-chain on the PMNS signpost). H1 has not delivered sign(μ²) (4056/4057 refuted, 4062 reopened, 0983 neither, 0993 VW-a-4 refuted on the single-walker measure). **χ₄'s panel-readiness is blocked by H1, not by an unattempted SF-2 derivation.** |
