...
- **4072** — document opened; 2a (n̂ vs −n̂ is not a chirality in ℝ⁴) and 2b (a double rotation carries a
  ...
  readiness criteria and falsifiers written.| **F2** | **REDUCED to a binary founder question (4106) — and it is now a FALSIFIER, not a soft conditional.** A DP is a bound pair of OPPOSITE-polarity CPs; the response to b must be LINEAR (B3, 4076) with a C-ODD (polarity) coefficient FORCED by CPT (F6, 4085). So one sea DP's net response is R = q(b₊ − b₋), and **sea isotropy is irrelevant** — cancellation is per-DP and pointwise, superseding the 4101 framing. Decision table over the two free structural facts (spins parallel/antiparallel; arc direction common/internal): **F2 holds ⟺ the two relative orientations MATCH in parity**; on a mismatch R/q = 2 for EVERY sea DP against an intrinsic-EM-PV bound of ~1e−10 — **overshoot ≈ 2e10, χ₄ REFUTED outright.** No suppression is available (linearity required, polarity clause CPT-forced). **The naive reading of both facts — antiparallel spins (spin-paired ground state) with a common drift direction — is one of the two FATAL cells.** Founder question filed TODO-4106-F2DP. Doc: `axiom_maturation/4106_f2_dp_parity_match.md`. |
