# Problem History: OPEN-FP-SF-2-η — Substrate-derivation of cage-stability dilution factor η
...
**Research_Frontier.md entry:** OPEN-FP-SF-2-eta
...
Derive from CPP primitives the cage-stability dilution factors η_W, η_Z, η_H currently calibrated per cage in the SF-2 mass-formula machinery.

### Mechanism context (SF-2 v1.0)

The SF-2 v1.0 mass-formula architecture reproduces the four electroweak boson masses ($m_W, m_Z, m_H$, plus the structural prediction $m_{W^0} = m_{W^\pm}$) via three calibrated dilution factors $\eta_W, \eta_Z, \eta_H$ acting on the cage-stability mass-formula machinery (inherited from SM-7/8/9). The dilution factors capture the cage-specific binding-energy reduction relative to the cage-cooperative SSV reinforcement baseline. At v1.0, each $\eta$ is calibrated to one observed mass; substrate-derivation of $\eta$ from CPP primitives would close the calibration to a zero-parameter prediction.

### Closure route

Likely via the substrate-thermodynamic framework that also governs OPEN-FP-SF-2-chaincomp (DP-chain composition) and OPEN-FP-SF-2-shelldens (shell-density factor). The cage-stability dilution may decompose into geometric factors (cage vertex count V, stabilizer symmetry order |G|) plus dynamical factors (substrate temperature, equilibrium ensemble measure). First-principles closure registered as Layer 4 work per PD-004 publication-pathway.

### Open remarks

Proof-outline-level closure may be tractable for the W bracelet via the $D_6$-symmetric centroid SSV structure (PROP-SF-2-2); Z icosahedron and H dodecahedron closure are likely tied to the more general substrate-thermodynamic framework. The mass-degeneracy structural prediction $m_{W^0} = m_{W^\pm}$ (PROP-SF-2-1, PRED-O-21) is robust against η-calibration choice (depends only on the SSV-gradient depth at the bracelet centroid, not on the absolute mass scale).
...
`OPEN-FP-SF-2-EWSB`, `OPEN-FP-SF-2-loopfactor`, `OPEN-FP-SF-2-shelldens`, `OPEN-FP-SF-2-chaincomp`, `OPEN-FP-SF-2-CHIR`
...
The closure of OPEN-FP-SF-2-eta from CPP primitives plus the appropriate foundational inputs would be falsified by:
