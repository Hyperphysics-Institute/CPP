...
- **Unconditionalize:** ~~discharge Mechanism A~~ **DONE, Patch 0960** (narrowed to the derived reversal-odd first harmonic; residual named). ~~And/or derive the residual (pointwise non-degeneracy) from the PCD layer~~ **DONE, Patch 0968** (discharged and replaced by a structural premise + the 3-plane lemma). **TARROW-2 RE-READ DONE, Patch 0971** (`chirality_derivations/review/0971_tarrow2_reread.md`, verify 6/6). The PH's prediction is **half right, and the half that matters is right**: TARROW-2's **conclusion** (`C_T = Yes`, non-reversibility, the W3 → W1 candidate) is **robust to the residual and strengthened by it** — the same 420 of 1200 faces violate detailed balance at every `A` tested, and violation appears *earlier*, at O(δ¹) rather than O(δ³). Its **order-counting claim (i)** — "no O(δ²) term, first violation at O(δ³)" — **consumes `A = 0`**, exactly as 0949 predicted, because a constant reversal-even `A` makes the effective tilt `B/(1 + A m_e)` position-dependent and breaks the per-face `a+b+c = 0` cancellation. **So today's result does extend from the spatial verdict to the temporal one.** Owed: a scope clause on claim (i) — a wording correction, not a re-derivation, and not a panel.
